...
func TestFloat64BinarySearchSignedZeroAndNaN(t *testing.T) {
	negZero := math.Copysign(0, -1)
	posZero := 0.0
	nan := math.NaN()

	// Total order: -0.0 < +0.0 < 1.0 < NaN.
	l := Float64Of(nan, 1.0, posZero, negZero)
	l.Sort()

	get := func(i int) float64 { v := l.Get(i); return v }
...
func TestFloat32BinarySearchSignedZeroAndNaN(t *testing.T) {
	negZero := float32(math.Copysign(0, -1))
	posZero := float32(0.0)
	nan := float32(math.NaN())

	l := Float32Of(nan, 1.0, posZero, negZero)
	l.Sort()

	get := func(i int) float32 { v := l.Get(i); return v }
