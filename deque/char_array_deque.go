...
// Char is a double-ended queue of uint16 values, backed by a
...
// caller-supplied slice in CharOf.
...
func (d *Char) AddFirst(value uint16) {
...
func (d *Char) AddLast(value uint16) {
	if d.size == len(d.items) {
		d.grow()
	}
	mask := len(d.items) - 1
...
func (d *Char) RemoveFirst() (uint16, bool) {
	if d.size == 0 {
		return 0, false
	}
	mask := len(d.items) - 1
...
func (d *Char) RemoveLast() (uint16, bool) {
	if d.size == 0 {
		return 0, false
	}
	mask := len(d.items) - 1
...
func (d *Char) PeekFirst() (uint16, bool) {
	if d.size == 0 {
		return 0, false
	}
...
func (d *Char) PeekLast() (uint16, bool) {
	if d.size == 0 {
		return 0, false
	}
	mask := len(d.items) - 1
...
// for emptiness.
func (d *Char) Len() int { return d.size }
...
func (d *Char) Contains(value uint16) bool {
	mask := len(d.items) - 1
	for i := 0; i < d.size; i++ {
...
func (d *Char) ForEach(f func(uint16)) {
	mask := len(d.items) - 1
	for i := 0; i < d.size; i++ {
...
func (d *Char) All() iter.Seq[uint16] {
...
// whose concatenation reproduces All in order, so a *Char satisfies
// par.Segmenter[uint16] and feeds par.From directly. Each view maps its
...
func (d *Char) Segments(n int) []iter.Seq[uint16] {
	return segment.SplitIndex(d.size, n, d.at)
}

// AnySatisfy returns true if any element satisfies the predicate.
func (d *Char) AnySatisfy(predicate func(uint16) bool) bool {
	mask := len(d.items) - 1
	for i := 0; i < d.size; i++ {
...
func (d *Char) AllSatisfy(predicate func(uint16) bool) bool {
	mask := len(d.items) - 1
	for i := 0; i < d.size; i++ {
		if !predicate(d.items[(d.head+i)&mask]) {
			return false
...
func (d *Char) Equals(other *Char) bool {
