...
// CharArrayDeque is a double-ended queue of uint16 values, backed by a
...
type CharArrayDeque struct {
	items []uint16 // len == capacity, always a power of two; indexed modulo cap
...
const initialCharDequeCap = 16

// ceilPow2 rounds n up to the next power of two, with a floor of
// initialCharDequeCap. Used when sizing the buffer to fit a
// caller-supplied slice in CharArrayDequeOf.
func ceilPow2CharDeque(n int) int {
	cap := initialCharDequeCap
	for cap < n {
		cap <<= 1
	}
	return cap
...
// NewCharArrayDeque creates a new empty CharArrayDeque.
func NewCharArrayDeque() *CharArrayDeque {
	return &CharArrayDeque{items: make([]uint16, initialCharDequeCap)}
}

// CharArrayDequeOf creates a new CharArrayDeque from the given values in
// front-to-back order.
func CharArrayDequeOf(values ...uint16) *CharArrayDeque {
	d := &CharArrayDeque{
		items: make([]uint16, ceilPow2CharDeque(len(values))),
		size:  len(values),
	}
	copy(d.items, values)
	return d
...
func (d *CharArrayDeque) grow() {
	newCap := len(d.items) * 2
	if newCap == 0 {
		newCap = initialCharDequeCap
	}
...
func (d *CharArrayDeque) AddFirst(value uint16) {
...
func (d *CharArrayDeque) AddLast(value uint16) {
	if d.size == len(d.items) {
		d.grow()
	}
	mask := len(d.items) - 1
...
func (d *CharArrayDeque) RemoveFirst() (uint16, error) {
	if d.size == 0 {
		return 0, fmt.Errorf("CharArrayDeque: RemoveFirst on empty deque")
	}
	mask := len(d.items) - 1
	v := d.items[d.head]
	d.items[d.head] = 0 // let GC reclaim references if uint16 ever carries them
	d.head = (d.head + 1) & mask
	d.size--
	return v, nil
}
...
func (d *CharArrayDeque) RemoveLast() (uint16, error) {
	if d.size == 0 {
		return 0, fmt.Errorf("CharArrayDeque: RemoveLast on empty deque")
	}
	mask := len(d.items) - 1
	d.size--
...
func (d *CharArrayDeque) PeekFirst() (uint16, error) {
	if d.size == 0 {
		return 0, fmt.Errorf("CharArrayDeque: PeekFirst on empty deque")
	}
	return d.items[d.head], nil
}

// PeekLast returns the back element without removing it, or an error if empty.
func (d *CharArrayDeque) PeekLast() (uint16, error) {
	if d.size == 0 {
		return 0, fmt.Errorf("CharArrayDeque: PeekLast on empty deque")
	}
	mask := len(d.items) - 1
...
func (d *CharArrayDeque) Size() int { return d.size }

// IsEmpty returns true if the deque contains no elements.
func (d *CharArrayDeque) IsEmpty() bool { return d.size == 0 }

// Clear removes all elements. The backing buffer is retained.
func (d *CharArrayDeque) Clear() {
...
func (d *CharArrayDeque) Contains(value uint16) bool {
	mask := len(d.items) - 1
	for i := 0; i < d.size; i++ {
...
func (d *CharArrayDeque) ForEach(f func(uint16)) {
	mask := len(d.items) - 1
	for i := 0; i < d.size; i++ {
...
func (d *CharArrayDeque) AnySatisfy(predicate func(uint16) bool) bool {
	mask := len(d.items) - 1
	for i := 0; i < d.size; i++ {
...
func (d *CharArrayDeque) AllSatisfy(predicate func(uint16) bool) bool {
	mask := len(d.items) - 1
	for i := 0; i < d.size; i++ {
		if !predicate(d.items[(d.head+i)&mask]) {
			return false
...
func (d *CharArrayDeque) ToSlice() []uint16 {
	out := make([]uint16, d.size)
	if d.size == 0 {
		return out
	}
	cap := len(d.items)
...
func (d *CharArrayDeque) Equals(other *CharArrayDeque) bool {
...
func (d *CharArrayDeque) String() string {
	if d.size == 0 {
		return "[]"
	}
