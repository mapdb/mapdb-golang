...
func TestFloat32_Generated_AddLastRemoveFirst(t *testing.T) {
	d := NewFloat32()
	d.AddLast(1.0)
	d.AddLast(2.0)
	d.AddLast(3.0)
	...
	v0, ok := d.RemoveFirst()
	if !ok || !(math.Float32bits(v0) == math.Float32bits(1.0)) {
		t.Errorf("RemoveFirst = %v, ok=%v, want 1.0", v0, ok)
	}
	v1, ok := d.RemoveFirst()
	if !ok || !(math.Float32bits(v1) == math.Float32bits(2.0)) {
		t.Errorf("RemoveFirst = %v, ok=%v, want 2.0", v1, ok)
	}
	v2, ok := d.RemoveFirst()
	if !ok || !(math.Float32bits(v2) == math.Float32bits(3.0)) {
...
func TestFloat32_Generated_AddFirstRemoveLast(t *testing.T) {
	d := NewFloat32()
	d.AddFirst(1.0)
	d.AddFirst(2.0)
	d.AddFirst(3.0)
	got, ok := d.PeekFirst()
	if !ok || !(math.Float32bits(got) == math.Float32bits(3.0)) {
		t.Errorf("PeekFirst = %v, ok=%v, want 3.0", got, ok)
	}
	got, ok = d.PeekLast()
	if !ok || !(math.Float32bits(got) == math.Float32bits(1.0)) {
...
func TestFloat32_Generated_RemoveEmpty(t *testing.T) {
	d := NewFloat32()
...
func TestFloat32_Generated_MixedOps(t *testing.T) {
	d := NewFloat32()
	d.AddLast(2.0)
	d.AddFirst(1.0)
	d.AddLast(3.0)
...
func TestFloat32_Generated_ContainsAndClear(t *testing.T) {
	d := NewFloat32()
	d.AddLast(1.0)
	d.AddLast(2.0)
	if !d.Contains(1.0) {
...
// TestFloat32_Generated_RingBufferWrapAround exercises the ring buffer's
...
func TestFloat32_Generated_RingBufferWrapAround(t *testing.T) {
	d := NewFloat32()
...
// TestFloat32_Generated_AddFirstIsO1 is a complexity sanity check.
...
func TestFloat32_Generated_AddFirstIsO1(t *testing.T) {
	const N = 10000
	d := NewFloat32()
	for i := 0; i < N; i++ {
		d.AddFirst(1.0)
...
// TestFloat32_Generated_AlternatingEndsGrow exercises the grow path while
...
func TestFloat32_Generated_AlternatingEndsGrow(t *testing.T) {
	d := NewFloat32()
