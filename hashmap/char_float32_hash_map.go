...
const (
	charFloat32HashMapDefaultCapacity = 16
	// Load factor 3/4 = 0.75, using integer math to avoid float conversion per insert.
)

// charFloat32HashMapEntry holds a single slot in the hash map for cache locality.
type charFloat32HashMapEntry struct {
	key      uint16
	value    float32
	occupied bool
}

// CharFloat32HashMap is an open-addressing hash map with uint16 keys and float32 values.
type CharFloat32HashMap struct {
	entries []charFloat32HashMapEntry
	size    int
}

// NewCharFloat32HashMap creates a new empty CharFloat32HashMap with default capacity.
func NewCharFloat32HashMap() *CharFloat32HashMap {
	return NewCharFloat32HashMapWithCapacity(charFloat32HashMapDefaultCapacity)
}

// NewCharFloat32HashMapWithCapacity creates a new empty CharFloat32HashMap with the given initial capacity.
func NewCharFloat32HashMapWithCapacity(capacity int) *CharFloat32HashMap {
	cap := nextPowerOfTwoCharFloat32HashMap(capacity)
	return &CharFloat32HashMap{
		entries: make([]charFloat32HashMapEntry, cap),
		size:    0,
	}
}

// CharFloat32HashMapOf creates a new CharFloat32HashMap from key-value pairs.
func CharFloat32HashMapOf(pairs ...struct {
	Key   uint16
	Value float32
}) *CharFloat32HashMap {
...
func (m *CharFloat32HashMap) Put(key uint16, value float32) (float32, bool) {
	if m.needsResize() {
		m.resize()
	}
	cap := len(m.entries)
...
func (m *CharFloat32HashMap) Get(key uint16) (float32, bool) {
	cap := len(m.entries)
	if cap == 0 {
		return 0.0, false
	}
...
func (m *CharFloat32HashMap) GetOrDefault(key uint16, defaultValue float32) float32 {
...
func (m *CharFloat32HashMap) Remove(key uint16) (float32, bool) {
	cap := len(m.entries)
	if cap == 0 {
		return 0.0, false
	}
...
func (m *CharFloat32HashMap) ContainsKey(key uint16) bool {
	_, ok := m.Get(key)
	return ok
}

// ContainsValue returns true if the map contains the given value.
func (m *CharFloat32HashMap) ContainsValue(value float32) bool {
...
func (m *CharFloat32HashMap) Size() int {
	return m.size
}

// IsEmpty returns true if the map contains no entries.
func (m *CharFloat32HashMap) IsEmpty() bool {
	return m.size == 0
}

// Clear removes all entries from the map.
func (m *CharFloat32HashMap) Clear() {
	for i := range m.entries {
		m.entries[i] = charFloat32HashMapEntry{}
	}
	m.size = 0
}

// All returns an iter.Seq2 that yields all key-value pairs.
func (m *CharFloat32HashMap) All() iter.Seq2[uint16, float32] {
...
func (m *CharFloat32HashMap) Keys() iter.Seq[uint16] {
...
func (m *CharFloat32HashMap) Values() iter.Seq[float32] {
...
func (m *CharFloat32HashMap) ForEach(f func(uint16, float32)) {
	for i := range m.entries {
		if m.entries[i].occupied {
...
func (m *CharFloat32HashMap) ForEachKey(f func(uint16)) {
	for i := range m.entries {
		if m.entries[i].occupied {
			f(m.entries[i].key)
		}
...
func (m *CharFloat32HashMap) ForEachValue(f func(float32)) {
	for i := range m.entries {
		if m.entries[i].occupied {
			f(m.entries[i].value)
		}
...
func (m *CharFloat32HashMap) Select(predicate func(uint16, float32) bool) *CharFloat32HashMap {
...
func (m *CharFloat32HashMap) Reject(predicate func(uint16, float32) bool) *CharFloat32HashMap {
	result := NewCharFloat32HashMap()
	for i := range m.entries {
		if m.entries[i].occupied && !predicate(m.entries[i].key, m.entries[i].value) {
			result.Put(m.entries[i].key, m.entries[i].value)
...
func (m *CharFloat32HashMap) Detect(predicate func(uint16, float32) bool) (uint16, float32, bool) {
	for i := range m.entries {
		if m.entries[i].occupied && predicate(m.entries[i].key, m.entries[i].value) {
...
func (m *CharFloat32HashMap) AnySatisfy(predicate func(uint16, float32) bool) bool {
	for i := range m.entries {
		if m.entries[i].occupied && predicate(m.entries[i].key, m.entries[i].value) {
			return true
		}
...
func (m *CharFloat32HashMap) AllSatisfy(predicate func(uint16, float32) bool) bool {
	for i := range m.entries {
		if m.entries[i].occupied && !predicate(m.entries[i].key, m.entries[i].value) {
			return false
		}
...
func (m *CharFloat32HashMap) NoneSatisfy(predicate func(uint16, float32) bool) bool {
	for i := range m.entries {
		if m.entries[i].occupied && predicate(m.entries[i].key, m.entries[i].value) {
			return false
		}
...
func (m *CharFloat32HashMap) Count(predicate func(uint16, float32) bool) int {
	count := 0
	for i := range m.entries {
		if m.entries[i].occupied && predicate(m.entries[i].key, m.entries[i].value) {
			count++
...
func (m *CharFloat32HashMap) String() string {
	if m.size == 0 {
		return "{}"
	}
...
func (m *CharFloat32HashMap) Equals(other *CharFloat32HashMap) bool {
...
func (m *CharFloat32HashMap) KeysToSlice() []uint16 {
...
func (m *CharFloat32HashMap) ValuesToSlice() []float32 {
...
func (m *CharFloat32HashMap) ToImmutable() *ImmutableCharFloat32HashMap {
	return ImmutableCharFloat32HashMapFrom(m)
}

// InjectInto performs a left fold over all key-value pairs.
func (m *CharFloat32HashMap) InjectInto(initial float32, f func(float32, uint16, float32) float32) float32 {
	result := initial
	for i := range m.entries {
		if m.entries[i].occupied {
...
func (m *CharFloat32HashMap) AddToValue(key uint16, amount float32) float32 {
	if v, ok := m.Get(key); ok {
		newVal := v + amount
		m.Put(key, newVal)
		return newVal
...
// Returns the new value.
func (m *CharFloat32HashMap) UpdateValue(key uint16, initialValue float32, f func(float32) float32) float32 {
	if v, ok := m.Get(key); ok {
		newVal := f(v)
		m.Put(key, newVal)
		return newVal
...
func (m *CharFloat32HashMap) WithKeyValue(key uint16, value float32) *CharFloat32HashMap {
	m.Put(key, value)
	return m
}

// WithoutKey returns the map after removing the key (fluent API).
func (m *CharFloat32HashMap) WithoutKey(key uint16) *CharFloat32HashMap {
	m.Remove(key)
	return m
}

// WithoutAllKeys removes all given keys (fluent API).
func (m *CharFloat32HashMap) WithoutAllKeys(keys []uint16) *CharFloat32HashMap {
	for _, k := range keys {
		m.Remove(k)
	}
	return m
...
func (m *CharFloat32HashMap) SumOfValues() float32 {
	var sum float32
	for i := range m.entries {
		if m.entries[i].occupied {
...
// SynchronizedCharFloat32HashMap wrapper's Lock / RLock, or your own mutex) is required
...
func (m *CharFloat32HashMap) hashKey(key uint16) uint64 {
	return func() uint64 { h := uint64(key) * 0x9E3779B97F4A7C15; return h ^ (h >> 32) }()
}

func (m *CharFloat32HashMap) needsResize() bool {
	return (m.size+1)*4 > len(m.entries)*3 // 0.75 load factor, integer math
}

func (m *CharFloat32HashMap) resize() {
	oldEntries := m.entries
	newCap := len(oldEntries) * 2
	if newCap == 0 {
		newCap = charFloat32HashMapDefaultCapacity
	}
	m.entries = make([]charFloat32HashMapEntry, newCap)
	m.size = 0
...
func (m *CharFloat32HashMap) rehashFrom(deleted int, mask int) {
	c := len(m.entries)
	idx := (deleted + 1) & mask
	for m.entries[idx].occupied {
		...
		if distCurrent > distGap {
			...
			m.entries[idx] = charFloat32HashMapEntry{}
			deleted = idx
		}
		idx = (idx + 1) & mask
		if idx == deleted {
...
func nextPowerOfTwoCharFloat32HashMap(n int) int {
	if n <= 0 {
		return 16
	}
	n--
