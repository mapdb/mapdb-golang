...
// NewCharFloat64WithCapacity creates a new empty CharFloat64 sized to hold at
...
func (m *CharFloat64) findIndex(key uint16) (int, bool) {
	cap := len(m.entries)
	mask := cap - 1
	hash := m.hashKey(key)
	tag := uint8(hash & 0x7F)
...
func (m *CharFloat64) rehashForInsert() {
	cap := len(m.entries)
...
func (m *CharFloat64) rehashTo(newCap int) {
...
func (m *CharFloat64) insertNoGrow(key uint16, value float64) {
	cap := len(m.entries)
	mask := cap - 1
	hash := m.hashKey(key)
	tag := uint8(hash & 0x7F)
