...
// NewCharInt16WithCapacity creates a new empty CharInt16 sized to hold at
...
func (m *CharInt16) findIndex(key uint16) (int, bool) {
	cap := len(m.entries)
	mask := cap - 1
	hash := m.hashKey(key)
	tag := uint8(hash & 0x7F)
...
func (m *CharInt16) rehashForInsert() {
	cap := len(m.entries)
...
func (m *CharInt16) rehashTo(newCap int) {
...
func (m *CharInt16) insertNoGrow(key uint16, value int16) {
	cap := len(m.entries)
	mask := cap - 1
	hash := m.hashKey(key)
	tag := uint8(hash & 0x7F)
