...
const (
	charInt32HashMapDefaultCapacity = 16
	// Load factor 3/4 = 0.75, using integer math to avoid float conversion per insert.
)

// charInt32HashMapEntry holds a single slot in the hash map for cache locality.
type charInt32HashMapEntry struct {
	key      uint16
	value    int32
	occupied bool
}

// CharInt32HashMap is an open-addressing hash map with uint16 keys and int32 values.
type CharInt32HashMap struct {
	entries []charInt32HashMapEntry
	size    int
}

// NewCharInt32HashMap creates a new empty CharInt32HashMap with default capacity.
func NewCharInt32HashMap() *CharInt32HashMap {
	return NewCharInt32HashMapWithCapacity(charInt32HashMapDefaultCapacity)
}

// NewCharInt32HashMapWithCapacity creates a new empty CharInt32HashMap with the given initial capacity.
func NewCharInt32HashMapWithCapacity(capacity int) *CharInt32HashMap {
	cap := nextPowerOfTwoCharInt32HashMap(capacity)
	return &CharInt32HashMap{
		entries: make([]charInt32HashMapEntry, cap),
		size:    0,
	}
}

// CharInt32HashMapOf creates a new CharInt32HashMap from key-value pairs.
func CharInt32HashMapOf(pairs ...struct {
	Key   uint16
	Value int32
}) *CharInt32HashMap {
...
func (m *CharInt32HashMap) Put(key uint16, value int32) (int32, bool) {
	if m.needsResize() {
		m.resize()
	}
	cap := len(m.entries)
...
func (m *CharInt32HashMap) Get(key uint16) (int32, bool) {
	cap := len(m.entries)
	if cap == 0 {
		return 0, false
	}
...
func (m *CharInt32HashMap) GetOrDefault(key uint16, defaultValue int32) int32 {
...
func (m *CharInt32HashMap) Remove(key uint16) (int32, bool) {
	cap := len(m.entries)
	if cap == 0 {
		return 0, false
	}
...
func (m *CharInt32HashMap) ContainsKey(key uint16) bool {
	_, ok := m.Get(key)
	return ok
}

// ContainsValue returns true if the map contains the given value.
func (m *CharInt32HashMap) ContainsValue(value int32) bool {
...
func (m *CharInt32HashMap) Size() int {
	return m.size
}

// IsEmpty returns true if the map contains no entries.
func (m *CharInt32HashMap) IsEmpty() bool {
	return m.size == 0
}

// Clear removes all entries from the map.
func (m *CharInt32HashMap) Clear() {
	for i := range m.entries {
		m.entries[i] = charInt32HashMapEntry{}
	}
	m.size = 0
}

// All returns an iter.Seq2 that yields all key-value pairs.
func (m *CharInt32HashMap) All() iter.Seq2[uint16, int32] {
...
func (m *CharInt32HashMap) Keys() iter.Seq[uint16] {
...
func (m *CharInt32HashMap) Values() iter.Seq[int32] {
...
func (m *CharInt32HashMap) ForEach(f func(uint16, int32)) {
	for i := range m.entries {
		if m.entries[i].occupied {
...
func (m *CharInt32HashMap) ForEachKey(f func(uint16)) {
	for i := range m.entries {
		if m.entries[i].occupied {
			f(m.entries[i].key)
		}
...
func (m *CharInt32HashMap) ForEachValue(f func(int32)) {
	for i := range m.entries {
		if m.entries[i].occupied {
			f(m.entries[i].value)
		}
...
func (m *CharInt32HashMap) Select(predicate func(uint16, int32) bool) *CharInt32HashMap {
	result := NewCharInt32HashMap()
	for i := range m.entries {
...
func (m *CharInt32HashMap) Reject(predicate func(uint16, int32) bool) *CharInt32HashMap {
	result := NewCharInt32HashMap()
	for i := range m.entries {
		if m.entries[i].occupied && !predicate(m.entries[i].key, m.entries[i].value) {
			result.Put(m.entries[i].key, m.entries[i].value)
...
func (m *CharInt32HashMap) Detect(predicate func(uint16, int32) bool) (uint16, int32, bool) {
	for i := range m.entries {
		if m.entries[i].occupied && predicate(m.entries[i].key, m.entries[i].value) {
...
func (m *CharInt32HashMap) AnySatisfy(predicate func(uint16, int32) bool) bool {
	for i := range m.entries {
		if m.entries[i].occupied && predicate(m.entries[i].key, m.entries[i].value) {
			return true
		}
...
func (m *CharInt32HashMap) AllSatisfy(predicate func(uint16, int32) bool) bool {
	for i := range m.entries {
		if m.entries[i].occupied && !predicate(m.entries[i].key, m.entries[i].value) {
			return false
		}
...
func (m *CharInt32HashMap) NoneSatisfy(predicate func(uint16, int32) bool) bool {
	for i := range m.entries {
		if m.entries[i].occupied && predicate(m.entries[i].key, m.entries[i].value) {
			return false
		}
...
func (m *CharInt32HashMap) Count(predicate func(uint16, int32) bool) int {
	count := 0
	for i := range m.entries {
		if m.entries[i].occupied && predicate(m.entries[i].key, m.entries[i].value) {
			count++
...
func (m *CharInt32HashMap) String() string {
	if m.size == 0 {
		return "{}"
	}
...
func (m *CharInt32HashMap) Equals(other *CharInt32HashMap) bool {
...
func (m *CharInt32HashMap) KeysToSlice() []uint16 {
...
func (m *CharInt32HashMap) ValuesToSlice() []int32 {
...
func (m *CharInt32HashMap) ToImmutable() *ImmutableCharInt32HashMap {
	return ImmutableCharInt32HashMapFrom(m)
}

// InjectInto performs a left fold over all key-value pairs.
func (m *CharInt32HashMap) InjectInto(initial int32, f func(int32, uint16, int32) int32) int32 {
	result := initial
	for i := range m.entries {
		if m.entries[i].occupied {
...
func (m *CharInt32HashMap) AddToValue(key uint16, amount int32) int32 {
	if v, ok := m.Get(key); ok {
		newVal := v + amount
		m.Put(key, newVal)
		return newVal
...
// Returns the new value.
func (m *CharInt32HashMap) UpdateValue(key uint16, initialValue int32, f func(int32) int32) int32 {
	if v, ok := m.Get(key); ok {
		newVal := f(v)
		m.Put(key, newVal)
		return newVal
...
func (m *CharInt32HashMap) WithKeyValue(key uint16, value int32) *CharInt32HashMap {
	m.Put(key, value)
	return m
}

// WithoutKey returns the map after removing the key (fluent API).
func (m *CharInt32HashMap) WithoutKey(key uint16) *CharInt32HashMap {
	m.Remove(key)
	return m
}

// WithoutAllKeys removes all given keys (fluent API).
func (m *CharInt32HashMap) WithoutAllKeys(keys []uint16) *CharInt32HashMap {
	for _, k := range keys {
		m.Remove(k)
	}
	return m
...
func (m *CharInt32HashMap) SumOfValues() int32 {
	var sum int32
	for i := range m.entries {
		if m.entries[i].occupied {
...
// SynchronizedCharInt32HashMap wrapper's Lock / RLock, or your own mutex) is required
...
func (m *CharInt32HashMap) hashKey(key uint16) uint64 {
	return func() uint64 { h := uint64(key) * 0x9E3779B97F4A7C15; return h ^ (h >> 32) }()
}

func (m *CharInt32HashMap) needsResize() bool {
	return (m.size+1)*4 > len(m.entries)*3 // 0.75 load factor, integer math
}

func (m *CharInt32HashMap) resize() {
	oldEntries := m.entries
	newCap := len(oldEntries) * 2
	if newCap == 0 {
		newCap = charInt32HashMapDefaultCapacity
	}
	m.entries = make([]charInt32HashMapEntry, newCap)
	m.size = 0
...
func (m *CharInt32HashMap) rehashFrom(deleted int, mask int) {
	c := len(m.entries)
	idx := (deleted + 1) & mask
	for m.entries[idx].occupied {
		...
		if distCurrent > distGap {
			...
			m.entries[idx] = charInt32HashMapEntry{}
			deleted = idx
		}
		idx = (idx + 1) & mask
		if idx == deleted {
...
func nextPowerOfTwoCharInt32HashMap(n int) int {
	if n <= 0 {
		return 16
	}
	n--
