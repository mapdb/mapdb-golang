...
// charInt8Entry holds a single slot in the hash map for cache locality.
...
// CharInt8 is an open-addressing hash map with uint16 keys and int8 values.
...
// CharInt8Of creates a new CharInt8 from key-value pairs.
func CharInt8Of(pairs ...struct {
	Key   uint16
	Value int8
}) *CharInt8 {
...
// has more distinct keys than expected — use CharInt8BulkLoadExact for the
...
func CharInt8BulkLoad(keys []uint16, values []int8, policy pump.DuplicatePolicy) (*CharInt8, error) {
	...
	m := &CharInt8{entries: make([]charInt8Entry, CharInt8bulkCap(len(keys)))}
...
func CharInt8BulkLoadExact(keys []uint16, values []int8, n int, policy pump.DuplicatePolicy) (*CharInt8, error) {
	...
	m := &CharInt8{entries: make([]charInt8Entry, CharInt8bulkCap(n))}
	if len(keys) > n {
...
func (m *CharInt8) bulkInsert(keys []uint16, values []int8, policy pump.DuplicatePolicy) error {
	for i := range keys {
		if m.needsResize() {
			m.resize()
		}
...
func (m *CharInt8) ForEachKey(f func(uint16)) {
	for i := range m.entries {
		if m.entries[i].occupied {
			f(m.entries[i].key)
		}
...
func (m *CharInt8) ForEachValue(f func(int8)) {
	for i := range m.entries {
		if m.entries[i].occupied {
			f(m.entries[i].value)
		}
...
func (m *CharInt8) Equals(other *CharInt8) bool {
...
func (m *CharInt8) KeysToSlice() []uint16 {
...
func (m *CharInt8) ValuesToSlice() []int8 {
...
func (m *CharInt8) ToImmutable() *ImmutableCharInt8 {
	return ImmutableCharInt8From(m)
}

// InjectInto performs a left fold over all key-value pairs.
func (m *CharInt8) InjectInto(initial int8, f func(int8, uint16, int8) int8) int8 {
	result := initial
	for i := range m.entries {
		if m.entries[i].occupied {
...
func (m *CharInt8) AddToValue(key uint16, amount int8) int8 {
	if v, ok := m.Get(key); ok {
		newVal := v + amount
		m.Put(key, newVal)
		return newVal
...
// Returns the new value.
func (m *CharInt8) UpdateValue(key uint16, initialValue int8, f func(int8) int8) int8 {
	if v, ok := m.Get(key); ok {
		newVal := f(v)
		m.Put(key, newVal)
		return newVal
...
func (m *CharInt8) PutReturning(key uint16, value int8) *CharInt8 {
	m.Put(key, value)
	return m
}

// RemoveKeyReturning returns the map after removing the key (fluent API).
func (m *CharInt8) RemoveKeyReturning(key uint16) *CharInt8 {
	m.Remove(key)
	return m
}

// WithoutAllKeys removes all given keys (fluent API).
func (m *CharInt8) WithoutAllKeys(keys []uint16) *CharInt8 {
	for _, k := range keys {
		m.Remove(k)
	}
	return m
...
func (m *CharInt8) SumOfValues() int8 {
	var sum int8
	for i := range m.entries {
		if m.entries[i].occupied {
...
// SynchronizedCharInt8 wrapper's Lock / RLock, or your own mutex) is required
...
func (m *CharInt8) rehashFrom(deleted int, mask int) {
	c := len(m.entries)
	idx := (deleted + 1) & mask
...
func nextPowerOfTwoCharInt8(n int) int {
	if n <= 0 {
		return 16
	}
	n--
