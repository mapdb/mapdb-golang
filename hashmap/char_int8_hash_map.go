...
// SynchronizedCharInt8HashMap wrapper's Lock / RLock, or your own mutex) is required
...
// CharInt8Entry provides in-place check-and-modify operations for a single
// key. Not thread-safe — see CharInt8HashMap.Entry.
	...
	for {
		...
		if e.m.entries[idx].key == e.key {
			...
			if prevLen != len(e.m.entries) || prevPtr != &e.m.entries[0] {
				panic("CharInt8Entry.AndModify: map was resized during callback — do not mutate the map from within AndModify")
			}
			return e
		}
		idx = (idx + 1) & mask
