...
// NewCharInt8WithCapacity creates a new empty CharInt8 sized to hold at
...
func (m *CharInt8) findIndex(key uint16) (int, bool) {
	cap := len(m.entries)
	mask := cap - 1
	hash := m.hashKey(key)
	tag := uint8(hash & 0x7F)
...
func (m *CharInt8) rehashForInsert() {
	cap := len(m.entries)
...
func (m *CharInt8) rehashTo(newCap int) {
...
func (m *CharInt8) insertNoGrow(key uint16, value int8) {
	cap := len(m.entries)
	mask := cap - 1
	hash := m.hashKey(key)
	tag := uint8(hash & 0x7F)
