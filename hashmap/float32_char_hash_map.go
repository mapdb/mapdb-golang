...
// NewFloat32CharWithCapacity creates a new empty Float32Char sized to hold at
...
func (m *Float32Char) findIndex(key float32) (int, bool) {
	cap := len(m.entries)
	mask := cap - 1
	hash := m.hashKey(key)
	tag := uint8(hash & 0x7F)
...
func (m *Float32Char) rehashForInsert() {
	cap := len(m.entries)
...
func (m *Float32Char) rehashTo(newCap int) {
...
func (m *Float32Char) insertNoGrow(key float32, value uint16) {
	cap := len(m.entries)
	mask := cap - 1
	hash := m.hashKey(key)
	tag := uint8(hash & 0x7F)
