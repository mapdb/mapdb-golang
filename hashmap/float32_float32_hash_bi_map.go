...
type Float32Float32BiMap struct {
	forward *Float32Float32
	reverse *Float32Float32
}

// NewFloat32Float32BiMap creates a new empty Float32Float32BiMap with default capacity.
func NewFloat32Float32BiMap() *Float32Float32BiMap {
	return &Float32Float32BiMap{
		forward: NewFloat32Float32(),
		reverse: NewFloat32Float32(),
	}
}

// NewFloat32Float32BiMapWithCapacity creates a new empty Float32Float32BiMap with the given initial capacity.
func NewFloat32Float32BiMapWithCapacity(capacity int) *Float32Float32BiMap {
	return &Float32Float32BiMap{
...
func Float32Float32BiMapBulkLoad(keys []float32, values []float32, policy pump.DuplicatePolicy) (*Float32Float32BiMap, error) {
	...
	m := &Float32Float32BiMap{
		forward: NewFloat32Float32WithCapacity(Float32Float32bulkCap(len(keys))),
		reverse: NewFloat32Float32WithCapacity(Float32Float32bulkCap(len(values))),
	}
...
func (m *Float32Float32BiMap) Put(key float32, value float32) (float32, bool) {
...
func (m *Float32Float32BiMap) RemoveValue(value float32) (float32, bool) {
...
func (m *Float32Float32BiMap) ContainsKey(key float32) bool {
	return m.forward.ContainsKey(key)
}

// ContainsValue returns true if the map contains the given value.
func (m *Float32Float32BiMap) ContainsValue(value float32) bool {
	return m.reverse.ContainsKey(value)
}

// Len returns the number of elements. Use m.Len() == 0 to test for emptiness.
func (m *Float32Float32BiMap) Len() int {
	return m.forward.Len()
}

// Clear removes all entries from both directions.
func (m *Float32Float32BiMap) Clear() {
	m.forward.Clear()
	m.reverse.Clear()
}

// ForEach calls the given function for each key-value pair.
func (m *Float32Float32BiMap) ForEach(f func(float32, float32)) {
	m.forward.ForEach(f)
}
...
func (m *Float32Float32BiMap) All() iter.Seq2[float32, float32] {
	return m.forward.All()
}

// Keys returns an iter.Seq that yields all keys.
func (m *Float32Float32BiMap) Keys() iter.Seq[float32] {
	return m.forward.Keys()
}

// Values returns an iter.Seq that yields all values.
func (m *Float32Float32BiMap) Values() iter.Seq[float32] {
	return m.forward.Values()
}

// Inverse returns a new Float32Float32BiMap with keys and values swapped.
func (m *Float32Float32BiMap) Inverse() *Float32Float32BiMap {
	result := NewFloat32Float32BiMap()
...
func (m *Float32Float32BiMap) String() string {
	if m.forward.Len() == 0 {
		return "{}"
	}
...
func (m *Float32Float32BiMap) Equals(other *Float32Float32BiMap) bool {
