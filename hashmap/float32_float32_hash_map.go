...
// NewFloat32Float32WithCapacity creates a new empty Float32Float32 sized to hold at
...
func (m *Float32Float32) findIndex(key float32) (int, bool) {
	cap := len(m.entries)
	mask := cap - 1
	hash := m.hashKey(key)
	tag := uint8(hash & 0x7F)
...
func (m *Float32Float32) rehashForInsert() {
	cap := len(m.entries)
...
func (m *Float32Float32) rehashTo(newCap int) {
...
func (m *Float32Float32) insertNoGrow(key float32, value float32) {
	cap := len(m.entries)
	mask := cap - 1
	hash := m.hashKey(key)
	tag := uint8(hash & 0x7F)
