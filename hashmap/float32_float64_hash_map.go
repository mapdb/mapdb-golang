...
// Float32Float64BulkLoad builds a Float32Float64 from keys/values in a single pass,
...
// has more distinct keys than expected — use Float32Float64BulkLoadExact for the
// zero-rehash guarantee.
func Float32Float64BulkLoad(keys []float32, values []float64, policy pump.DuplicatePolicy) (*Float32Float64, error) {
	if len(keys) != len(values) {
		panic("mapdb: Float32Float64BulkLoad: len(keys) != len(values)")
	}
	m := &Float32Float64{entries: make([]float32Float64Entry, Float32Float64bulkCap(len(keys)))}
...
// Float32Float64BulkLoadExact is like Float32Float64BulkLoad but guarantees zero
...
func Float32Float64BulkLoadExact(keys []float32, values []float64, n int, policy pump.DuplicatePolicy) (*Float32Float64, error) {
	if len(keys) != len(values) {
		panic("mapdb: Float32Float64BulkLoadExact: len(keys) != len(values)")
	}
	if n < 0 {
		panic("mapdb: Float32Float64BulkLoadExact: negative n")
	}
	m := &Float32Float64{entries: make([]float32Float64Entry, Float32Float64bulkCap(n))}
	if len(keys) > n {
...
func (m *Float32Float64) bulkInsert(keys []float32, values []float64, policy pump.DuplicatePolicy) error {
	for i := range keys {
		if m.needsResize() {
			m.resize()
		}
...
func (m *Float32Float64) bulkPut(key float32, value float64, policy pump.DuplicatePolicy) (bool, error) {
...
// Float32Float64bulkCap returns the presized table capacity for n entries that
...
func Float32Float64bulkCap(n int) int {
	c := pump.HashCapacityFor(n)
	if c < float32Float64DefaultCapacity {
		return float32Float64DefaultCapacity
	}
	return c
}
