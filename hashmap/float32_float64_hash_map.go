...
// NewFloat32Float64WithCapacity creates a new empty Float32Float64 sized to hold at
...
func (m *Float32Float64) findIndex(key float32) (int, bool) {
	cap := len(m.entries)
	mask := cap - 1
	hash := m.hashKey(key)
	tag := uint8(hash & 0x7F)
...
func (m *Float32Float64) rehashForInsert() {
	cap := len(m.entries)
...
func (m *Float32Float64) rehashTo(newCap int) {
...
func (m *Float32Float64) insertNoGrow(key float32, value float64) {
	cap := len(m.entries)
	mask := cap - 1
	hash := m.hashKey(key)
	tag := uint8(hash & 0x7F)
