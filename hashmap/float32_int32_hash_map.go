...
// Float32Int32BulkLoad builds a Float32Int32 from keys/values in a single pass,
...
// has more distinct keys than expected — use Float32Int32BulkLoadExact for the
// zero-rehash guarantee.
func Float32Int32BulkLoad(keys []float32, values []int32, policy pump.DuplicatePolicy) (*Float32Int32, error) {
	if len(keys) != len(values) {
		panic("mapdb: Float32Int32BulkLoad: len(keys) != len(values)")
	}
	m := &Float32Int32{entries: make([]float32Int32Entry, Float32Int32bulkCap(len(keys)))}
...
func Float32Int32BulkLoadExact(keys []float32, values []int32, n int, policy pump.DuplicatePolicy) (*Float32Int32, error) {
	...
	m := &Float32Int32{entries: make([]float32Int32Entry, Float32Int32bulkCap(n))}
	for i := range keys {
		if m.size >= n {
...
func (m *Float32Int32) bulkInsert(keys []float32, values []int32, policy pump.DuplicatePolicy) error {
	for i := range keys {
		if m.needsResize() {
			m.resize()
		}
...
func (m *Float32Int32) bulkPut(key float32, value int32, policy pump.DuplicatePolicy) (bool, error) {
...
// Float32Int32bulkCap returns the presized table capacity for n entries that
...
func Float32Int32bulkCap(n int) int {
	c := pump.HashCapacityFor(n)
	if c < float32Int32DefaultCapacity {
		return float32Int32DefaultCapacity
	}
	return c
}
