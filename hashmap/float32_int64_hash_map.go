...
// has more distinct keys than expected — use Float32Int64BulkLoadExact for the
...
func Float32Int64BulkLoad(keys []float32, values []int64, policy pump.DuplicatePolicy) (*Float32Int64, error) {
	...
	m := &Float32Int64{entries: make([]float32Int64Entry, Float32Int64bulkCap(len(keys)))}
...
func Float32Int64BulkLoadExact(keys []float32, values []int64, n int, policy pump.DuplicatePolicy) (*Float32Int64, error) {
	...
	m := &Float32Int64{entries: make([]float32Int64Entry, Float32Int64bulkCap(n))}
	if len(keys) > n {
...
func (m *Float32Int64) bulkInsert(keys []float32, values []int64, policy pump.DuplicatePolicy) error {
	for i := range keys {
		if m.needsResize() {
			m.resize()
		}
...
func (m *Float32Int64) bulkPut(key float32, value int64, policy pump.DuplicatePolicy) (bool, error) {
...
func (m *Float32Int64) rehashFrom(deleted int, mask int) {
	c := len(m.entries)
	idx := (deleted + 1) & mask
