...
// NewFloat32Int8WithCapacity creates a new empty Float32Int8 sized to hold at
...
func (m *Float32Int8) findIndex(key float32) (int, bool) {
	cap := len(m.entries)
	mask := cap - 1
	hash := m.hashKey(key)
	tag := uint8(hash & 0x7F)
...
func (m *Float32Int8) rehashForInsert() {
	cap := len(m.entries)
...
func (m *Float32Int8) rehashTo(newCap int) {
...
func (m *Float32Int8) insertNoGrow(key float32, value int8) {
	cap := len(m.entries)
	mask := cap - 1
	hash := m.hashKey(key)
	tag := uint8(hash & 0x7F)
