...
// Float32Int8BulkLoad builds a Float32Int8 from keys/values in a single pass,
...
// has more distinct keys than expected — use Float32Int8BulkLoadExact for the
// zero-rehash guarantee.
func Float32Int8BulkLoad(keys []float32, values []int8, policy pump.DuplicatePolicy) (*Float32Int8, error) {
	if len(keys) != len(values) {
		panic("mapdb: Float32Int8BulkLoad: len(keys) != len(values)")
	}
	m := &Float32Int8{entries: make([]float32Int8Entry, Float32Int8bulkCap(len(keys)))}
...
// Float32Int8BulkLoadExact is like Float32Int8BulkLoad but guarantees zero
...
func Float32Int8BulkLoadExact(keys []float32, values []int8, n int, policy pump.DuplicatePolicy) (*Float32Int8, error) {
	if len(keys) != len(values) {
		panic("mapdb: Float32Int8BulkLoadExact: len(keys) != len(values)")
	}
	if n < 0 {
		panic("mapdb: Float32Int8BulkLoadExact: negative n")
	}
	m := &Float32Int8{entries: make([]float32Int8Entry, Float32Int8bulkCap(n))}
	for i := range keys {
		if m.size >= n {
...
func (m *Float32Int8) bulkInsert(keys []float32, values []int8, policy pump.DuplicatePolicy) error {
	for i := range keys {
		if m.needsResize() {
			m.resize()
		}
...
// Float32Int8bulkCap returns the presized table capacity for n entries that
...
func Float32Int8bulkCap(n int) int {
	c := pump.HashCapacityFor(n)
	if c < float32Int8DefaultCapacity {
		return float32Int8DefaultCapacity
	}
	return c
}
