...
// NewFloat64CharWithCapacity creates a new empty Float64Char sized to hold at
...
func (m *Float64Char) findIndex(key float64) (int, bool) {
	cap := len(m.entries)
	mask := cap - 1
	hash := m.hashKey(key)
	tag := uint8(hash & 0x7F)
...
func (m *Float64Char) rehashForInsert() {
	cap := len(m.entries)
...
func (m *Float64Char) rehashTo(newCap int) {
...
func (m *Float64Char) insertNoGrow(key float64, value uint16) {
	cap := len(m.entries)
	mask := cap - 1
	hash := m.hashKey(key)
	tag := uint8(hash & 0x7F)
