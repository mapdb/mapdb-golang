...
// NewFloat64Float32WithCapacity creates a new empty Float64Float32 sized to hold at
...
func (m *Float64Float32) findIndex(key float64) (int, bool) {
	cap := len(m.entries)
	mask := cap - 1
	hash := m.hashKey(key)
	tag := uint8(hash & 0x7F)
...
func (m *Float64Float32) rehashForInsert() {
	cap := len(m.entries)
...
func (m *Float64Float32) rehashTo(newCap int) {
...
func (m *Float64Float32) insertNoGrow(key float64, value float32) {
	cap := len(m.entries)
	mask := cap - 1
	hash := m.hashKey(key)
	tag := uint8(hash & 0x7F)
