...
const (
	float64Float64DefaultCapacity = 16
	// Load factor 3/4 = 0.75, using integer math to avoid float conversion per insert.
)

// float64Float64Entry holds a single slot in the hash map for cache locality.
...
// Float64Float64 is an open-addressing hash map with float64 keys and float64 values.
type Float64Float64 struct {
	entries []float64Float64Entry
	size    int
}

// NewFloat64Float64 creates a new empty Float64Float64 with default capacity.
func NewFloat64Float64() *Float64Float64 {
	return NewFloat64Float64WithCapacity(float64Float64DefaultCapacity)
}

// NewFloat64Float64WithCapacity creates a new empty Float64Float64 with the given initial capacity.
func NewFloat64Float64WithCapacity(capacity int) *Float64Float64 {
	cap := nextPowerOfTwoFloat64Float64(capacity)
	return &Float64Float64{
		entries: make([]float64Float64Entry, cap),
		size:    0,
	}
}

// Float64Float64Of creates a new Float64Float64 from key-value pairs.
func Float64Float64Of(pairs ...struct {
	Key   float64
	Value float64
}) *Float64Float64 {
	m := NewFloat64Float64WithCapacity(len(pairs) * 2)
	for _, p := range pairs {
		m.Put(p.Key, p.Value)
	}
	return m
...
// Float64Float64BulkLoad builds a Float64Float64 from keys/values in a single pass,
...
// has more distinct keys than expected — use Float64Float64BulkLoadExact for the
// zero-rehash guarantee.
func Float64Float64BulkLoad(keys []float64, values []float64, policy pump.DuplicatePolicy) (*Float64Float64, error) {
	if len(keys) != len(values) {
		panic("mapdb: Float64Float64BulkLoad: len(keys) != len(values)")
	}
	m := &Float64Float64{entries: make([]float64Float64Entry, Float64Float64bulkCap(len(keys)))}
...
// Float64Float64BulkLoadExact is like Float64Float64BulkLoad but guarantees zero
...
func Float64Float64BulkLoadExact(keys []float64, values []float64, n int, policy pump.DuplicatePolicy) (*Float64Float64, error) {
	if len(keys) != len(values) {
		panic("mapdb: Float64Float64BulkLoadExact: len(keys) != len(values)")
	}
	if n < 0 {
		panic("mapdb: Float64Float64BulkLoadExact: negative n")
	}
	m := &Float64Float64{entries: make([]float64Float64Entry, Float64Float64bulkCap(n))}
	for i := range keys {
		if m.size >= n {
...
func (m *Float64Float64) bulkInsert(keys []float64, values []float64, policy pump.DuplicatePolicy) error {
	for i := range keys {
		if m.needsResize() {
			m.resize()
		}
...
func (m *Float64Float64) bulkPut(key float64, value float64, policy pump.DuplicatePolicy) (bool, error) {
...
// Float64Float64bulkCap returns the presized table capacity for n entries that
...
func Float64Float64bulkCap(n int) int {
	c := pump.HashCapacityFor(n)
	if c < float64Float64DefaultCapacity {
		return float64Float64DefaultCapacity
	}
	return c
}
...
func (m *Float64Float64) ContainsValue(value float64) bool {
...
func (m *Float64Float64) Clear() {
	for i := range m.entries {
		m.entries[i] = float64Float64Entry{}
	}
	m.size = 0
}
...
func (m *Float64Float64) ForEachKey(f func(float64)) {
	for i := range m.entries {
		if m.entries[i].occupied {
			f(m.entries[i].key)
		}
...
func (m *Float64Float64) ForEachValue(f func(float64)) {
	for i := range m.entries {
		if m.entries[i].occupied {
			f(m.entries[i].value)
		}
...
func (m *Float64Float64) Equals(other *Float64Float64) bool {
...
func (m *Float64Float64) KeysToSlice() []float64 {
...
func (m *Float64Float64) ValuesToSlice() []float64 {
	result := make([]float64, 0, m.size)
	for i := range m.entries {
		if m.entries[i].occupied {
...
func (m *Float64Float64) ToImmutable() *ImmutableFloat64Float64 {
	return ImmutableFloat64Float64From(m)
}

// InjectInto performs a left fold over all key-value pairs.
func (m *Float64Float64) InjectInto(initial float64, f func(float64, float64, float64) float64) float64 {
	result := initial
	for i := range m.entries {
		if m.entries[i].occupied {
...
func (m *Float64Float64) AddToValue(key float64, amount float64) float64 {
	if v, ok := m.Get(key); ok {
		newVal := v + amount
		m.Put(key, newVal)
		return newVal
...
// Returns the new value.
func (m *Float64Float64) UpdateValue(key float64, initialValue float64, f func(float64) float64) float64 {
	if v, ok := m.Get(key); ok {
		newVal := f(v)
		m.Put(key, newVal)
		return newVal
...
func (m *Float64Float64) PutReturning(key float64, value float64) *Float64Float64 {
	m.Put(key, value)
	return m
}

// RemoveKeyReturning returns the map after removing the key (fluent API).
func (m *Float64Float64) RemoveKeyReturning(key float64) *Float64Float64 {
	m.Remove(key)
	return m
}

// WithoutAllKeys removes all given keys (fluent API).
func (m *Float64Float64) WithoutAllKeys(keys []float64) *Float64Float64 {
	for _, k := range keys {
		m.Remove(k)
	}
	return m
...
func (m *Float64Float64) SumOfValues() float64 {
	var sum float64
	for i := range m.entries {
		if m.entries[i].occupied {
...
// SynchronizedFloat64Float64 wrapper's Lock / RLock, or your own mutex) is required
...
func (m *Float64Float64) hashKey(key float64) uint64 {
	h := math.Float64bits(key) * 0x9E3779B97F4A7C15
	return h ^ (h >> 32)
}

func (m *Float64Float64) needsResize() bool {
	return (m.size+1)*4 >= len(m.entries)*3 // 0.75 load factor, integer math
}

func (m *Float64Float64) resize() {
	oldEntries := m.entries
	newCap := len(oldEntries) * 2
	if newCap == 0 {
		newCap = float64Float64DefaultCapacity
	}
	m.entries = make([]float64Float64Entry, newCap)
	m.size = 0
...
func (m *Float64Float64) rehashFrom(deleted int, mask int) {
	c := len(m.entries)
	idx := (deleted + 1) & mask
	for m.entries[idx].occupied {
		...
		if distCurrent > distGap {
			...
			m.entries[idx] = float64Float64Entry{}
			deleted = idx
		}
		idx = (idx + 1) & mask
		if idx == deleted {
...
func nextPowerOfTwoFloat64Float64(n int) int {
	if n <= 0 {
		return 16
	}
	n--
