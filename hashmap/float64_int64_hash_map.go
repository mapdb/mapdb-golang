...
// Float64Int64Of creates a new Float64Int64 from key-value pairs.
func Float64Int64Of(pairs ...struct {
	Key   float64
	Value int64
}) *Float64Int64 {
	m := NewFloat64Int64WithCapacity(len(pairs) * 2)
	for _, p := range pairs {
		m.Put(p.Key, p.Value)
	}
	return m
...
// Float64Int64BulkLoad builds a Float64Int64 from keys/values in a single pass,
...
// has more distinct keys than expected — use Float64Int64BulkLoadExact for the
// zero-rehash guarantee.
func Float64Int64BulkLoad(keys []float64, values []int64, policy pump.DuplicatePolicy) (*Float64Int64, error) {
	if len(keys) != len(values) {
		panic("mapdb: Float64Int64BulkLoad: len(keys) != len(values)")
	}
	m := &Float64Int64{entries: make([]float64Int64Entry, Float64Int64bulkCap(len(keys)))}
...
// Float64Int64BulkLoadExact is like Float64Int64BulkLoad but guarantees zero
...
func Float64Int64BulkLoadExact(keys []float64, values []int64, n int, policy pump.DuplicatePolicy) (*Float64Int64, error) {
	if len(keys) != len(values) {
		panic("mapdb: Float64Int64BulkLoadExact: len(keys) != len(values)")
	}
	if n < 0 {
		panic("mapdb: Float64Int64BulkLoadExact: negative n")
	}
	m := &Float64Int64{entries: make([]float64Int64Entry, Float64Int64bulkCap(n))}
	if len(keys) > n {
...
func (m *Float64Int64) bulkInsert(keys []float64, values []int64, policy pump.DuplicatePolicy) error {
	for i := range keys {
		if m.needsResize() {
			m.resize()
		}
...
func (m *Float64Int64) bulkPut(key float64, value int64, policy pump.DuplicatePolicy) (bool, error) {
...
// Float64Int64bulkCap returns the presized table capacity for n entries that
...
func Float64Int64bulkCap(n int) int {
	c := pump.HashCapacityFor(n)
	if c < float64Int64DefaultCapacity {
		return float64Int64DefaultCapacity
	}
	return c
}
...
func (m *Float64Int64) Equals(other *Float64Int64) bool {
...
func (m *Float64Int64) ToImmutable() *ImmutableFloat64Int64 {
	return ImmutableFloat64Int64From(m)
}
...
func (m *Float64Int64) AddToValue(key float64, amount int64) int64 {
	if v, ok := m.Get(key); ok {
		newVal := v + amount
		m.Put(key, newVal)
		return newVal
...
// Returns the new value.
func (m *Float64Int64) UpdateValue(key float64, initialValue int64, f func(int64) int64) int64 {
	if v, ok := m.Get(key); ok {
		newVal := f(v)
		m.Put(key, newVal)
		return newVal
...
func (m *Float64Int64) PutReturning(key float64, value int64) *Float64Int64 {
	m.Put(key, value)
	return m
}

// RemoveKeyReturning returns the map after removing the key (fluent API).
func (m *Float64Int64) RemoveKeyReturning(key float64) *Float64Int64 {
	m.Remove(key)
	return m
}

// WithoutAllKeys removes all given keys (fluent API).
func (m *Float64Int64) WithoutAllKeys(keys []float64) *Float64Int64 {
	for _, k := range keys {
		m.Remove(k)
	}
	return m
...
// SynchronizedFloat64Int64 wrapper's Lock / RLock, or your own mutex) is required
