...
// Float64Int8BiMap is a bidirectional map with float64 keys and int8 values.
// Both key-to-value and value-to-key lookups are O(1).
type Float64Int8BiMap struct {
	forward *Float64Int8
	reverse *Int8Float64
}

// NewFloat64Int8BiMap creates a new empty Float64Int8BiMap with default capacity.
func NewFloat64Int8BiMap() *Float64Int8BiMap {
	return &Float64Int8BiMap{
		forward: NewFloat64Int8(),
		reverse: NewInt8Float64(),
	}
}

// NewFloat64Int8BiMapWithCapacity creates a new empty Float64Int8BiMap with the given initial capacity.
func NewFloat64Int8BiMapWithCapacity(capacity int) *Float64Int8BiMap {
	return &Float64Int8BiMap{
		forward: NewFloat64Int8WithCapacity(capacity),
		reverse: NewInt8Float64WithCapacity(capacity),
	}
}

// Float64Int8BiMapBulkLoad builds a Float64Int8BiMap from keys/values in a
...
func Float64Int8BiMapBulkLoad(keys []float64, values []int8, policy pump.DuplicatePolicy) (*Float64Int8BiMap, error) {
	if len(keys) != len(values) {
		panic("mapdb: Float64Int8BiMapBulkLoad: len(keys) != len(values)")
	}
	m := &Float64Int8BiMap{
		forward: NewFloat64Int8WithCapacity(Float64Int8bulkCap(len(keys))),
		reverse: NewInt8Float64WithCapacity(Int8Float64bulkCap(len(values))),
	}
...
func (m *Float64Int8BiMap) Put(key float64, value int8) (int8, bool) {
...
func (m *Float64Int8BiMap) RemoveValue(value int8) (float64, bool) {
...
func (m *Float64Int8BiMap) ContainsKey(key float64) bool {
	return m.forward.ContainsKey(key)
}

// ContainsValue returns true if the map contains the given value.
func (m *Float64Int8BiMap) ContainsValue(value int8) bool {
	return m.reverse.ContainsKey(value)
}

// Len returns the number of elements. Use m.Len() == 0 to test for emptiness.
func (m *Float64Int8BiMap) Len() int {
	return m.forward.Len()
}

// Clear removes all entries from both directions.
func (m *Float64Int8BiMap) Clear() {
	m.forward.Clear()
	m.reverse.Clear()
}

// ForEach calls the given function for each key-value pair.
func (m *Float64Int8BiMap) ForEach(f func(float64, int8)) {
	m.forward.ForEach(f)
}
...
func (m *Float64Int8BiMap) All() iter.Seq2[float64, int8] {
	return m.forward.All()
}

// Keys returns an iter.Seq that yields all keys.
func (m *Float64Int8BiMap) Keys() iter.Seq[float64] {
	return m.forward.Keys()
}

// Values returns an iter.Seq that yields all values.
func (m *Float64Int8BiMap) Values() iter.Seq[int8] {
	return m.forward.Values()
}

// Inverse returns a new Int8Float64BiMap with keys and values swapped.
func (m *Float64Int8BiMap) Inverse() *Int8Float64BiMap {
	result := NewInt8Float64BiMap()
...
func (m *Float64Int8BiMap) String() string {
	if m.forward.Len() == 0 {
		return "{}"
	}
...
func (m *Float64Int8BiMap) Equals(other *Float64Int8BiMap) bool {
