...
// has more distinct keys than expected — use Float64Int8BulkLoadExact for the
...
func Float64Int8BulkLoad(keys []float64, values []int8, policy pump.DuplicatePolicy) (*Float64Int8, error) {
	...
	m := &Float64Int8{entries: make([]float64Int8Entry, Float64Int8bulkCap(len(keys)))}
...
func Float64Int8BulkLoadExact(keys []float64, values []int8, n int, policy pump.DuplicatePolicy) (*Float64Int8, error) {
	...
	m := &Float64Int8{entries: make([]float64Int8Entry, Float64Int8bulkCap(n))}
	for i := range keys {
		if m.size >= n {
...
func (m *Float64Int8) bulkInsert(keys []float64, values []int8, policy pump.DuplicatePolicy) error {
	for i := range keys {
		if m.needsResize() {
			m.resize()
		}
...
func (m *Float64Int8) bulkPut(key float64, value int8, policy pump.DuplicatePolicy) (bool, error) {
