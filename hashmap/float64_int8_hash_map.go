...
// NewFloat64Int8WithCapacity creates a new empty Float64Int8 sized to hold at
...
func (m *Float64Int8) findIndex(key float64) (int, bool) {
	cap := len(m.entries)
	mask := cap - 1
	hash := m.hashKey(key)
	tag := uint8(hash & 0x7F)
...
func (m *Float64Int8) rehashForInsert() {
	cap := len(m.entries)
...
func (m *Float64Int8) rehashTo(newCap int) {
...
func (m *Float64Int8) insertNoGrow(key float64, value int8) {
	cap := len(m.entries)
	mask := cap - 1
	hash := m.hashKey(key)
	tag := uint8(hash & 0x7F)
