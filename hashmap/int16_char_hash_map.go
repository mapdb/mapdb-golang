...
// NewInt16CharWithCapacity creates a new empty Int16Char with the given initial capacity.
func NewInt16CharWithCapacity(capacity int) *Int16Char {
	cap := nextPowerOfTwoInt16Char(capacity)
	return &Int16Char{
...
// Int16CharBulkLoad builds a Int16Char from keys/values in a single pass,
...
// has more distinct keys than expected — use Int16CharBulkLoadExact for the
// zero-rehash guarantee.
func Int16CharBulkLoad(keys []int16, values []uint16, policy pump.DuplicatePolicy) (*Int16Char, error) {
	if len(keys) != len(values) {
		panic("mapdb: Int16CharBulkLoad: len(keys) != len(values)")
	}
	m := &Int16Char{entries: make([]int16CharEntry, Int16CharbulkCap(len(keys)))}
...
// Int16CharBulkLoadExact is like Int16CharBulkLoad but guarantees zero
...
func Int16CharBulkLoadExact(keys []int16, values []uint16, n int, policy pump.DuplicatePolicy) (*Int16Char, error) {
	if len(keys) != len(values) {
		panic("mapdb: Int16CharBulkLoadExact: len(keys) != len(values)")
	}
	if n < 0 {
		panic("mapdb: Int16CharBulkLoadExact: negative n")
	}
	m := &Int16Char{entries: make([]int16CharEntry, Int16CharbulkCap(n))}
	for i := range keys {
		if m.size >= n {
...
func (m *Int16Char) bulkInsert(keys []int16, values []uint16, policy pump.DuplicatePolicy) error {
	for i := range keys {
		if m.needsResize() {
			m.resize()
		}
...
// Int16CharbulkCap returns the presized table capacity for n entries that
...
func Int16CharbulkCap(n int) int {
	c := pump.HashCapacityFor(n)
	if c < int16CharDefaultCapacity {
		return int16CharDefaultCapacity
	}
	return c
}
...
func (m *Int16Char) rehashFrom(deleted int, mask int) {
	c := len(m.entries)
	idx := (deleted + 1) & mask
...
func nextPowerOfTwoInt16Char(n int) int {
	if n <= 0 {
		return 16
	}
	n--
