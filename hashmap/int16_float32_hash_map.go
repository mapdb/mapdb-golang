...
func (e Int16Float32Entry) OrInsert(defaultValue float32) float32 {
...
func (e Int16Float32Entry) OrInsertWith(f func() float32) float32 {
	if v, ok := e.m.Get(e.key); ok {
		return v
	}
	val := f()
...
func (e Int16Float32Entry) AndModify(f func(*float32)) Int16Float32Entry {
	cap := len(e.m.entries)
	if cap == 0 {
		return e
	}
	...
	for {
		...
		if e.m.entries[idx].key == e.key {
			...
			if prevLen != len(e.m.entries) || prevPtr != &e.m.entries[0] {
				panic("Int16Float32Entry.AndModify: map was resized during callback — do not mutate the map from within AndModify")
			}
			return e
		}
		idx = (idx + 1) & mask
