...
// has more distinct keys than expected — use Int16Float32BulkLoadExact for the
...
func Int16Float32BulkLoad(keys []int16, values []float32, policy pump.DuplicatePolicy) (*Int16Float32, error) {
	...
	m := &Int16Float32{entries: make([]int16Float32Entry, Int16Float32bulkCap(len(keys)))}
...
func Int16Float32BulkLoadExact(keys []int16, values []float32, n int, policy pump.DuplicatePolicy) (*Int16Float32, error) {
	...
	m := &Int16Float32{entries: make([]int16Float32Entry, Int16Float32bulkCap(n))}
	for i := range keys {
		if m.size >= n {
...
func (m *Int16Float32) bulkInsert(keys []int16, values []float32, policy pump.DuplicatePolicy) error {
	for i := range keys {
		if m.needsResize() {
			m.resize()
		}
...
func (m *Int16Float32) bulkPut(key int16, value float32, policy pump.DuplicatePolicy) (bool, error) {
