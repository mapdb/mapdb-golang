...
// NewInt16Float32WithCapacity creates a new empty Int16Float32 sized to hold at
...
func (m *Int16Float32) findIndex(key int16) (int, bool) {
	cap := len(m.entries)
	mask := cap - 1
	hash := m.hashKey(key)
	tag := uint8(hash & 0x7F)
...
func (m *Int16Float32) rehashForInsert() {
	cap := len(m.entries)
...
func (m *Int16Float32) rehashTo(newCap int) {
...
func (m *Int16Float32) insertNoGrow(key int16, value float32) {
	cap := len(m.entries)
	mask := cap - 1
	hash := m.hashKey(key)
	tag := uint8(hash & 0x7F)
