...
const (
	int16Float64HashMapDefaultCapacity = 16
	// Load factor 3/4 = 0.75, using integer math to avoid float conversion per insert.
)

// int16Float64HashMapEntry holds a single slot in the hash map for cache locality.
type int16Float64HashMapEntry struct {
	key      int16
	value    float64
	occupied bool
}

// Int16Float64HashMap is an open-addressing hash map with int16 keys and float64 values.
type Int16Float64HashMap struct {
	entries []int16Float64HashMapEntry
	size    int
}

// NewInt16Float64HashMap creates a new empty Int16Float64HashMap with default capacity.
func NewInt16Float64HashMap() *Int16Float64HashMap {
	return NewInt16Float64HashMapWithCapacity(int16Float64HashMapDefaultCapacity)
}

// NewInt16Float64HashMapWithCapacity creates a new empty Int16Float64HashMap with the given initial capacity.
func NewInt16Float64HashMapWithCapacity(capacity int) *Int16Float64HashMap {
	cap := nextPowerOfTwoInt16Float64HashMap(capacity)
	return &Int16Float64HashMap{
		entries: make([]int16Float64HashMapEntry, cap),
		size:    0,
	}
}

// Int16Float64HashMapOf creates a new Int16Float64HashMap from key-value pairs.
func Int16Float64HashMapOf(pairs ...struct {
	Key   int16
	Value float64
}) *Int16Float64HashMap {
...
func (m *Int16Float64HashMap) Put(key int16, value float64) (float64, bool) {
	if m.needsResize() {
		m.resize()
	}
	cap := len(m.entries)
...
func (m *Int16Float64HashMap) Get(key int16) (float64, bool) {
	cap := len(m.entries)
	if cap == 0 {
		return 0.0, false
	}
...
func (m *Int16Float64HashMap) GetOrDefault(key int16, defaultValue float64) float64 {
...
func (m *Int16Float64HashMap) Remove(key int16) (float64, bool) {
	cap := len(m.entries)
	if cap == 0 {
		return 0.0, false
	}
...
func (m *Int16Float64HashMap) ContainsKey(key int16) bool {
	_, ok := m.Get(key)
	return ok
}

// ContainsValue returns true if the map contains the given value.
func (m *Int16Float64HashMap) ContainsValue(value float64) bool {
...
func (m *Int16Float64HashMap) Size() int {
	return m.size
}

// IsEmpty returns true if the map contains no entries.
func (m *Int16Float64HashMap) IsEmpty() bool {
	return m.size == 0
}

// Clear removes all entries from the map.
func (m *Int16Float64HashMap) Clear() {
	for i := range m.entries {
		m.entries[i] = int16Float64HashMapEntry{}
	}
	m.size = 0
}

// All returns an iter.Seq2 that yields all key-value pairs.
func (m *Int16Float64HashMap) All() iter.Seq2[int16, float64] {
...
func (m *Int16Float64HashMap) Keys() iter.Seq[int16] {
...
func (m *Int16Float64HashMap) Values() iter.Seq[float64] {
...
func (m *Int16Float64HashMap) ForEach(f func(int16, float64)) {
	for i := range m.entries {
		if m.entries[i].occupied {
...
func (m *Int16Float64HashMap) ForEachKey(f func(int16)) {
	for i := range m.entries {
		if m.entries[i].occupied {
			f(m.entries[i].key)
		}
...
func (m *Int16Float64HashMap) ForEachValue(f func(float64)) {
	for i := range m.entries {
		if m.entries[i].occupied {
			f(m.entries[i].value)
		}
...
func (m *Int16Float64HashMap) Select(predicate func(int16, float64) bool) *Int16Float64HashMap {
	result := NewInt16Float64HashMap()
	for i := range m.entries {
...
func (m *Int16Float64HashMap) Reject(predicate func(int16, float64) bool) *Int16Float64HashMap {
	result := NewInt16Float64HashMap()
	for i := range m.entries {
		if m.entries[i].occupied && !predicate(m.entries[i].key, m.entries[i].value) {
			result.Put(m.entries[i].key, m.entries[i].value)
...
func (m *Int16Float64HashMap) Detect(predicate func(int16, float64) bool) (int16, float64, bool) {
	for i := range m.entries {
		if m.entries[i].occupied && predicate(m.entries[i].key, m.entries[i].value) {
...
func (m *Int16Float64HashMap) AnySatisfy(predicate func(int16, float64) bool) bool {
	for i := range m.entries {
		if m.entries[i].occupied && predicate(m.entries[i].key, m.entries[i].value) {
			return true
		}
...
func (m *Int16Float64HashMap) AllSatisfy(predicate func(int16, float64) bool) bool {
	for i := range m.entries {
		if m.entries[i].occupied && !predicate(m.entries[i].key, m.entries[i].value) {
			return false
		}
...
func (m *Int16Float64HashMap) NoneSatisfy(predicate func(int16, float64) bool) bool {
	for i := range m.entries {
		if m.entries[i].occupied && predicate(m.entries[i].key, m.entries[i].value) {
			return false
		}
...
func (m *Int16Float64HashMap) Count(predicate func(int16, float64) bool) int {
	count := 0
	for i := range m.entries {
		if m.entries[i].occupied && predicate(m.entries[i].key, m.entries[i].value) {
			count++
...
func (m *Int16Float64HashMap) String() string {
	if m.size == 0 {
		return "{}"
	}
...
func (m *Int16Float64HashMap) Equals(other *Int16Float64HashMap) bool {
...
func (m *Int16Float64HashMap) KeysToSlice() []int16 {
...
func (m *Int16Float64HashMap) ValuesToSlice() []float64 {
...
func (m *Int16Float64HashMap) ToImmutable() *ImmutableInt16Float64HashMap {
	return ImmutableInt16Float64HashMapFrom(m)
}

// InjectInto performs a left fold over all key-value pairs.
func (m *Int16Float64HashMap) InjectInto(initial float64, f func(float64, int16, float64) float64) float64 {
	result := initial
	for i := range m.entries {
		if m.entries[i].occupied {
...
func (m *Int16Float64HashMap) AddToValue(key int16, amount float64) float64 {
	if v, ok := m.Get(key); ok {
		newVal := v + amount
		m.Put(key, newVal)
		return newVal
...
// Returns the new value.
func (m *Int16Float64HashMap) UpdateValue(key int16, initialValue float64, f func(float64) float64) float64 {
	if v, ok := m.Get(key); ok {
		newVal := f(v)
		m.Put(key, newVal)
		return newVal
...
func (m *Int16Float64HashMap) WithKeyValue(key int16, value float64) *Int16Float64HashMap {
	m.Put(key, value)
	return m
}

// WithoutKey returns the map after removing the key (fluent API).
func (m *Int16Float64HashMap) WithoutKey(key int16) *Int16Float64HashMap {
	m.Remove(key)
	return m
}

// WithoutAllKeys removes all given keys (fluent API).
func (m *Int16Float64HashMap) WithoutAllKeys(keys []int16) *Int16Float64HashMap {
	for _, k := range keys {
		m.Remove(k)
	}
	return m
...
func (m *Int16Float64HashMap) SumOfValues() float64 {
	var sum float64
	for i := range m.entries {
		if m.entries[i].occupied {
...
// SynchronizedInt16Float64HashMap wrapper's Lock / RLock, or your own mutex) is required
...
func (m *Int16Float64HashMap) hashKey(key int16) uint64 {
	return func() uint64 { h := uint64(key) * 0x9E3779B97F4A7C15; return h ^ (h >> 32) }()
}

func (m *Int16Float64HashMap) needsResize() bool {
	return (m.size+1)*4 > len(m.entries)*3 // 0.75 load factor, integer math
}

func (m *Int16Float64HashMap) resize() {
	oldEntries := m.entries
	newCap := len(oldEntries) * 2
	if newCap == 0 {
		newCap = int16Float64HashMapDefaultCapacity
	}
	m.entries = make([]int16Float64HashMapEntry, newCap)
	m.size = 0
...
func (m *Int16Float64HashMap) rehashFrom(deleted int, mask int) {
	c := len(m.entries)
	idx := (deleted + 1) & mask
	for m.entries[idx].occupied {
		...
		if distCurrent > distGap {
			...
			m.entries[idx] = int16Float64HashMapEntry{}
			deleted = idx
		}
		idx = (idx + 1) & mask
		if idx == deleted {
...
func nextPowerOfTwoInt16Float64HashMap(n int) int {
	if n <= 0 {
		return 16
	}
	n--
