...
// NewInt16Float64WithCapacity creates a new empty Int16Float64 sized to hold at
...
func (m *Int16Float64) findIndex(key int16) (int, bool) {
	cap := len(m.entries)
	mask := cap - 1
	hash := m.hashKey(key)
	tag := uint8(hash & 0x7F)
...
func (m *Int16Float64) rehashForInsert() {
	cap := len(m.entries)
...
func (m *Int16Float64) rehashTo(newCap int) {
...
func (m *Int16Float64) insertNoGrow(key int16, value float64) {
	cap := len(m.entries)
	mask := cap - 1
	hash := m.hashKey(key)
	tag := uint8(hash & 0x7F)
