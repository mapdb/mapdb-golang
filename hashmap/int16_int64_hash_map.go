...
const (
	int16Int64DefaultCapacity = 16
	// Load factor 3/4 = 0.75, using integer math to avoid float conversion per insert.
)

// int16Int64Entry holds a single slot in the hash map for cache locality.
...
// Int16Int64 is an open-addressing hash map with int16 keys and int64 values.
type Int16Int64 struct {
	entries []int16Int64Entry
	size    int
}

// NewInt16Int64 creates a new empty Int16Int64 with default capacity.
func NewInt16Int64() *Int16Int64 {
	return NewInt16Int64WithCapacity(int16Int64DefaultCapacity)
}

// NewInt16Int64WithCapacity creates a new empty Int16Int64 with the given initial capacity.
func NewInt16Int64WithCapacity(capacity int) *Int16Int64 {
	cap := nextPowerOfTwoInt16Int64(capacity)
	return &Int16Int64{
		entries: make([]int16Int64Entry, cap),
		size:    0,
	}
}

// Int16Int64Of creates a new Int16Int64 from key-value pairs.
func Int16Int64Of(pairs ...struct {
	Key   int16
	Value int64
}) *Int16Int64 {
...
// Int16Int64BulkLoad builds a Int16Int64 from keys/values in a single pass,
...
// has more distinct keys than expected — use Int16Int64BulkLoadExact for the
// zero-rehash guarantee.
func Int16Int64BulkLoad(keys []int16, values []int64, policy pump.DuplicatePolicy) (*Int16Int64, error) {
	if len(keys) != len(values) {
		panic("mapdb: Int16Int64BulkLoad: len(keys) != len(values)")
	}
	m := &Int16Int64{entries: make([]int16Int64Entry, Int16Int64bulkCap(len(keys)))}
...
// Int16Int64BulkLoadExact is like Int16Int64BulkLoad but guarantees zero
...
func Int16Int64BulkLoadExact(keys []int16, values []int64, n int, policy pump.DuplicatePolicy) (*Int16Int64, error) {
	if len(keys) != len(values) {
		panic("mapdb: Int16Int64BulkLoadExact: len(keys) != len(values)")
	}
	if n < 0 {
		panic("mapdb: Int16Int64BulkLoadExact: negative n")
	}
	m := &Int16Int64{entries: make([]int16Int64Entry, Int16Int64bulkCap(n))}
	for i := range keys {
		if m.size >= n {
...
func (m *Int16Int64) bulkInsert(keys []int16, values []int64, policy pump.DuplicatePolicy) error {
	for i := range keys {
		if m.needsResize() {
			m.resize()
		}
...
func (m *Int16Int64) bulkPut(key int16, value int64, policy pump.DuplicatePolicy) (bool, error) {
...
// Int16Int64bulkCap returns the presized table capacity for n entries that
...
func Int16Int64bulkCap(n int) int {
	c := pump.HashCapacityFor(n)
	if c < int16Int64DefaultCapacity {
		return int16Int64DefaultCapacity
	}
	return c
}
...
func (m *Int16Int64) ContainsValue(value int64) bool {
...
func (m *Int16Int64) ForEachKey(f func(int16)) {
	for i := range m.entries {
		if m.entries[i].occupied {
			f(m.entries[i].key)
		}
...
func (m *Int16Int64) ForEachValue(f func(int64)) {
	for i := range m.entries {
		if m.entries[i].occupied {
			f(m.entries[i].value)
		}
...
func (m *Int16Int64) Equals(other *Int16Int64) bool {
...
func (m *Int16Int64) KeysToSlice() []int16 {
...
func (m *Int16Int64) ValuesToSlice() []int64 {
...
func (m *Int16Int64) ToImmutable() *ImmutableInt16Int64 {
	return ImmutableInt16Int64From(m)
}

// InjectInto performs a left fold over all key-value pairs.
func (m *Int16Int64) InjectInto(initial int64, f func(int64, int16, int64) int64) int64 {
	result := initial
	for i := range m.entries {
		if m.entries[i].occupied {
...
func (m *Int16Int64) AddToValue(key int16, amount int64) int64 {
	if v, ok := m.Get(key); ok {
		newVal := v + amount
		m.Put(key, newVal)
		return newVal
...
// Returns the new value.
func (m *Int16Int64) UpdateValue(key int16, initialValue int64, f func(int64) int64) int64 {
	if v, ok := m.Get(key); ok {
		newVal := f(v)
		m.Put(key, newVal)
		return newVal
...
func (m *Int16Int64) PutReturning(key int16, value int64) *Int16Int64 {
	m.Put(key, value)
	return m
}

// RemoveKeyReturning returns the map after removing the key (fluent API).
func (m *Int16Int64) RemoveKeyReturning(key int16) *Int16Int64 {
	m.Remove(key)
	return m
}

// WithoutAllKeys removes all given keys (fluent API).
func (m *Int16Int64) WithoutAllKeys(keys []int16) *Int16Int64 {
	for _, k := range keys {
		m.Remove(k)
	}
	return m
...
func (m *Int16Int64) SumOfValues() int64 {
	var sum int64
	for i := range m.entries {
		if m.entries[i].occupied {
...
// SynchronizedInt16Int64 wrapper's Lock / RLock, or your own mutex) is required
...
func (m *Int16Int64) hashKey(key int16) uint64 {
	h := uint64(key) * 0x9E3779B97F4A7C15
	return h ^ (h >> 32)
}

func (m *Int16Int64) needsResize() bool {
	return (m.size+1)*4 >= len(m.entries)*3 // 0.75 load factor, integer math
}

func (m *Int16Int64) resize() {
	oldEntries := m.entries
	newCap := len(oldEntries) * 2
	if newCap == 0 {
		newCap = int16Int64DefaultCapacity
	}
	m.entries = make([]int16Int64Entry, newCap)
	m.size = 0
...
func (m *Int16Int64) rehashFrom(deleted int, mask int) {
	c := len(m.entries)
	idx := (deleted + 1) & mask
...
func nextPowerOfTwoInt16Int64(n int) int {
	if n <= 0 {
		return 16
	}
	n--
