...
// NewInt32CharWithCapacity creates a new empty Int32Char sized to hold at
...
func (m *Int32Char) findIndex(key int32) (int, bool) {
	cap := len(m.entries)
	mask := cap - 1
	hash := m.hashKey(key)
	tag := uint8(hash & 0x7F)
...
func (m *Int32Char) rehashForInsert() {
	cap := len(m.entries)
...
func (m *Int32Char) rehashTo(newCap int) {
...
func (m *Int32Char) insertNoGrow(key int32, value uint16) {
	cap := len(m.entries)
	mask := cap - 1
	hash := m.hashKey(key)
	tag := uint8(hash & 0x7F)
