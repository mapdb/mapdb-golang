...
// SynchronizedInt32Float32HashMap wrapper's Lock / RLock, or your own mutex) is required
...
// Int32Float32Entry provides in-place check-and-modify operations for a single
// key. Not thread-safe — see Int32Float32HashMap.Entry.
	...
	for {
		...
		if e.m.entries[idx].key == e.key {
			...
			if prevLen != len(e.m.entries) || prevPtr != &e.m.entries[0] {
				panic("Int32Float32Entry.AndModify: map was resized during callback — do not mutate the map from within AndModify")
			}
			return e
		}
		idx = (idx + 1) & mask
