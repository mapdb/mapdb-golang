...
// has more distinct keys than expected — use Int32Float64BulkLoadExact for the
...
func Int32Float64BulkLoad(keys []int32, values []float64, policy pump.DuplicatePolicy) (*Int32Float64, error) {
	...
	m := &Int32Float64{entries: make([]int32Float64Entry, Int32Float64bulkCap(len(keys)))}
...
func Int32Float64BulkLoadExact(keys []int32, values []float64, n int, policy pump.DuplicatePolicy) (*Int32Float64, error) {
	...
	m := &Int32Float64{entries: make([]int32Float64Entry, Int32Float64bulkCap(n))}
	for i := range keys {
		if m.size >= n {
...
func (m *Int32Float64) bulkInsert(keys []int32, values []float64, policy pump.DuplicatePolicy) error {
	for i := range keys {
		if m.needsResize() {
			m.resize()
		}
...
func (m *Int32Float64) bulkPut(key int32, value float64, policy pump.DuplicatePolicy) (bool, error) {
...
func (m *Int32Float64) rehashFrom(deleted int, mask int) {
	c := len(m.entries)
	idx := (deleted + 1) & mask
