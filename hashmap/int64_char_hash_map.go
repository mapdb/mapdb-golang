...
// NewInt64CharWithCapacity creates a new empty Int64Char sized to hold at
...
func (m *Int64Char) findIndex(key int64) (int, bool) {
	cap := len(m.entries)
	mask := cap - 1
	hash := m.hashKey(key)
	tag := uint8(hash & 0x7F)
...
func (m *Int64Char) rehashForInsert() {
	cap := len(m.entries)
...
func (m *Int64Char) rehashTo(newCap int) {
...
func (m *Int64Char) insertNoGrow(key int64, value uint16) {
	cap := len(m.entries)
	mask := cap - 1
	hash := m.hashKey(key)
	tag := uint8(hash & 0x7F)
