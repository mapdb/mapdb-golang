...
// int64Float32Entry holds a single slot in the hash map for cache locality.
...
// Int64Float32 is an open-addressing hash map with int64 keys and float32 values.
type Int64Float32 struct {
	entries []int64Float32Entry
	size    int
}
...
func NewInt64Float32WithCapacity(capacity int) *Int64Float32 {
	...
	return &Int64Float32{
		entries: make([]int64Float32Entry, cap),
		size:    0,
	}
}

// Int64Float32Of creates a new Int64Float32 from key-value pairs.
func Int64Float32Of(pairs ...struct {
	Key   int64
	Value float32
}) *Int64Float32 {
	m := NewInt64Float32WithCapacity(len(pairs) * 2)
	for _, p := range pairs {
		m.Put(p.Key, p.Value)
	}
	return m
...
// Int64Float32BulkLoad builds a Int64Float32 from keys/values in a single pass,
...
// has more distinct keys than expected — use Int64Float32BulkLoadExact for the
// zero-rehash guarantee.
func Int64Float32BulkLoad(keys []int64, values []float32, policy pump.DuplicatePolicy) (*Int64Float32, error) {
	if len(keys) != len(values) {
		panic("mapdb: Int64Float32BulkLoad: len(keys) != len(values)")
	}
	m := &Int64Float32{entries: make([]int64Float32Entry, Int64Float32bulkCap(len(keys)))}
...
// Int64Float32BulkLoadExact is like Int64Float32BulkLoad but guarantees zero
...
func Int64Float32BulkLoadExact(keys []int64, values []float32, n int, policy pump.DuplicatePolicy) (*Int64Float32, error) {
	if len(keys) != len(values) {
		panic("mapdb: Int64Float32BulkLoadExact: len(keys) != len(values)")
	}
	if n < 0 {
		panic("mapdb: Int64Float32BulkLoadExact: negative n")
	}
	m := &Int64Float32{entries: make([]int64Float32Entry, Int64Float32bulkCap(n))}
	for i := range keys {
		if m.size >= n {
...
func (m *Int64Float32) bulkInsert(keys []int64, values []float32, policy pump.DuplicatePolicy) error {
	for i := range keys {
		if m.needsResize() {
			m.resize()
		}
...
func (m *Int64Float32) bulkPut(key int64, value float32, policy pump.DuplicatePolicy) (bool, error) {
...
// Int64Float32bulkCap returns the presized table capacity for n entries that
...
func Int64Float32bulkCap(n int) int {
	c := pump.HashCapacityFor(n)
	if c < int64Float32DefaultCapacity {
		return int64Float32DefaultCapacity
	}
	return c
}
...
func (m *Int64Float32) ForEachKey(f func(int64)) {
	for i := range m.entries {
		if m.entries[i].occupied {
			f(m.entries[i].key)
		}
...
func (m *Int64Float32) ForEachValue(f func(float32)) {
	for i := range m.entries {
		if m.entries[i].occupied {
			f(m.entries[i].value)
		}
...
func (m *Int64Float32) Equals(other *Int64Float32) bool {
...
func (m *Int64Float32) KeysToSlice() []int64 {
...
func (m *Int64Float32) ValuesToSlice() []float32 {
...
func (m *Int64Float32) ToImmutable() *ImmutableInt64Float32 {
	return ImmutableInt64Float32From(m)
}

// InjectInto performs a left fold over all key-value pairs.
func (m *Int64Float32) InjectInto(initial float32, f func(float32, int64, float32) float32) float32 {
	result := initial
	for i := range m.entries {
		if m.entries[i].occupied {
...
func (m *Int64Float32) AddToValue(key int64, amount float32) float32 {
	if v, ok := m.Get(key); ok {
		newVal := v + amount
		m.Put(key, newVal)
		return newVal
...
// Returns the new value.
func (m *Int64Float32) UpdateValue(key int64, initialValue float32, f func(float32) float32) float32 {
	if v, ok := m.Get(key); ok {
		newVal := f(v)
		m.Put(key, newVal)
		return newVal
...
func (m *Int64Float32) PutReturning(key int64, value float32) *Int64Float32 {
	m.Put(key, value)
	return m
}

// RemoveKeyReturning returns the map after removing the key (fluent API).
func (m *Int64Float32) RemoveKeyReturning(key int64) *Int64Float32 {
	m.Remove(key)
	return m
}

// WithoutAllKeys removes all given keys (fluent API).
func (m *Int64Float32) WithoutAllKeys(keys []int64) *Int64Float32 {
	for _, k := range keys {
		m.Remove(k)
	}
	return m
...
func (m *Int64Float32) SumOfValues() float32 {
	var sum float32
	for i := range m.entries {
		if m.entries[i].occupied {
...
// SynchronizedInt64Float32 wrapper's Lock / RLock, or your own mutex) is required
...
func (m *Int64Float32) resize() {
	...
	m.entries = make([]int64Float32Entry, newCap)
	m.size = 0
...
func (m *Int64Float32) rehashFrom(deleted int, mask int) {
	c := len(m.entries)
	idx := (deleted + 1) & mask
	for m.entries[idx].occupied {
		...
		if distCurrent > distGap {
			...
			m.entries[idx] = int64Float32Entry{}
			deleted = idx
		}
		idx = (idx + 1) & mask
		if idx == deleted {
