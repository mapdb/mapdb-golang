...
const (
	int64Float64DefaultCapacity = 16
	// Load factor 3/4 = 0.75, using integer math to avoid float conversion per insert.
)

// int64Float64Entry holds a single slot in the hash map for cache locality.
...
// Int64Float64 is an open-addressing hash map with int64 keys and float64 values.
type Int64Float64 struct {
	entries []int64Float64Entry
	size    int
}

// NewInt64Float64 creates a new empty Int64Float64 with default capacity.
func NewInt64Float64() *Int64Float64 {
	return NewInt64Float64WithCapacity(int64Float64DefaultCapacity)
}

// NewInt64Float64WithCapacity creates a new empty Int64Float64 with the given initial capacity.
func NewInt64Float64WithCapacity(capacity int) *Int64Float64 {
	cap := nextPowerOfTwoInt64Float64(capacity)
	return &Int64Float64{
		entries: make([]int64Float64Entry, cap),
		size:    0,
	}
}

// Int64Float64Of creates a new Int64Float64 from key-value pairs.
func Int64Float64Of(pairs ...struct {
	Key   int64
	Value float64
}) *Int64Float64 {
...
// Int64Float64BulkLoad builds a Int64Float64 from keys/values in a single pass,
...
// has more distinct keys than expected — use Int64Float64BulkLoadExact for the
// zero-rehash guarantee.
func Int64Float64BulkLoad(keys []int64, values []float64, policy pump.DuplicatePolicy) (*Int64Float64, error) {
	if len(keys) != len(values) {
		panic("mapdb: Int64Float64BulkLoad: len(keys) != len(values)")
	}
	m := &Int64Float64{entries: make([]int64Float64Entry, Int64Float64bulkCap(len(keys)))}
...
// Int64Float64BulkLoadExact is like Int64Float64BulkLoad but guarantees zero
...
func Int64Float64BulkLoadExact(keys []int64, values []float64, n int, policy pump.DuplicatePolicy) (*Int64Float64, error) {
	if len(keys) != len(values) {
		panic("mapdb: Int64Float64BulkLoadExact: len(keys) != len(values)")
	}
	if n < 0 {
		panic("mapdb: Int64Float64BulkLoadExact: negative n")
	}
	m := &Int64Float64{entries: make([]int64Float64Entry, Int64Float64bulkCap(n))}
	for i := range keys {
		if m.size >= n {
...
func (m *Int64Float64) bulkInsert(keys []int64, values []float64, policy pump.DuplicatePolicy) error {
	for i := range keys {
		if m.needsResize() {
			m.resize()
		}
...
func (m *Int64Float64) bulkPut(key int64, value float64, policy pump.DuplicatePolicy) (bool, error) {
...
// Int64Float64bulkCap returns the presized table capacity for n entries that
...
func Int64Float64bulkCap(n int) int {
	c := pump.HashCapacityFor(n)
	if c < int64Float64DefaultCapacity {
		return int64Float64DefaultCapacity
	}
	return c
}
...
func (m *Int64Float64) ContainsValue(value float64) bool {
...
func (m *Int64Float64) ForEachKey(f func(int64)) {
	for i := range m.entries {
		if m.entries[i].occupied {
			f(m.entries[i].key)
		}
...
func (m *Int64Float64) ForEachValue(f func(float64)) {
	for i := range m.entries {
		if m.entries[i].occupied {
			f(m.entries[i].value)
		}
...
func (m *Int64Float64) Equals(other *Int64Float64) bool {
...
func (m *Int64Float64) KeysToSlice() []int64 {
...
func (m *Int64Float64) ValuesToSlice() []float64 {
...
func (m *Int64Float64) ToImmutable() *ImmutableInt64Float64 {
	return ImmutableInt64Float64From(m)
}

// InjectInto performs a left fold over all key-value pairs.
func (m *Int64Float64) InjectInto(initial float64, f func(float64, int64, float64) float64) float64 {
	result := initial
	for i := range m.entries {
		if m.entries[i].occupied {
...
func (m *Int64Float64) AddToValue(key int64, amount float64) float64 {
	if v, ok := m.Get(key); ok {
		newVal := v + amount
		m.Put(key, newVal)
		return newVal
...
// Returns the new value.
func (m *Int64Float64) UpdateValue(key int64, initialValue float64, f func(float64) float64) float64 {
	if v, ok := m.Get(key); ok {
		newVal := f(v)
		m.Put(key, newVal)
		return newVal
...
func (m *Int64Float64) PutReturning(key int64, value float64) *Int64Float64 {
	m.Put(key, value)
	return m
}

// RemoveKeyReturning returns the map after removing the key (fluent API).
func (m *Int64Float64) RemoveKeyReturning(key int64) *Int64Float64 {
	m.Remove(key)
	return m
}

// WithoutAllKeys removes all given keys (fluent API).
func (m *Int64Float64) WithoutAllKeys(keys []int64) *Int64Float64 {
	for _, k := range keys {
		m.Remove(k)
	}
	return m
...
func (m *Int64Float64) SumOfValues() float64 {
	var sum float64
	for i := range m.entries {
		if m.entries[i].occupied {
...
// SynchronizedInt64Float64 wrapper's Lock / RLock, or your own mutex) is required
...
func (m *Int64Float64) hashKey(key int64) uint64 {
	h := uint64(key) * 0x9E3779B97F4A7C15
	return h ^ (h >> 32)
}

func (m *Int64Float64) needsResize() bool {
	return (m.size+1)*4 >= len(m.entries)*3 // 0.75 load factor, integer math
}

func (m *Int64Float64) resize() {
	oldEntries := m.entries
	newCap := len(oldEntries) * 2
	if newCap == 0 {
		newCap = int64Float64DefaultCapacity
	}
	m.entries = make([]int64Float64Entry, newCap)
	m.size = 0
...
func (m *Int64Float64) rehashFrom(deleted int, mask int) {
	c := len(m.entries)
	idx := (deleted + 1) & mask
	for m.entries[idx].occupied {
		...
		if distCurrent > distGap {
			...
			m.entries[idx] = int64Float64Entry{}
			deleted = idx
		}
		idx = (idx + 1) & mask
		if idx == deleted {
...
func nextPowerOfTwoInt64Float64(n int) int {
	if n <= 0 {
		return 16
	}
	n--
