...
const (
	int64Int64DefaultCapacity = 16
	// Load factor 3/4 = 0.75, using integer math to avoid float conversion per insert.
)

// int64Int64Entry holds a single slot in the hash map for cache locality.
...
// Int64Int64 is an open-addressing hash map with int64 keys and int64 values.
type Int64Int64 struct {
	entries []int64Int64Entry
	size    int
}

// NewInt64Int64 creates a new empty Int64Int64 with default capacity.
func NewInt64Int64() *Int64Int64 {
	return NewInt64Int64WithCapacity(int64Int64DefaultCapacity)
}

// NewInt64Int64WithCapacity creates a new empty Int64Int64 with the given initial capacity.
func NewInt64Int64WithCapacity(capacity int) *Int64Int64 {
	cap := nextPowerOfTwoInt64Int64(capacity)
	return &Int64Int64{
		entries: make([]int64Int64Entry, cap),
		size:    0,
	}
}

// Int64Int64Of creates a new Int64Int64 from key-value pairs.
func Int64Int64Of(pairs ...struct {
	Key   int64
	Value int64
}) *Int64Int64 {
...
// Int64Int64BulkLoad builds a Int64Int64 from keys/values in a single pass,
...
// has more distinct keys than expected — use Int64Int64BulkLoadExact for the
// zero-rehash guarantee.
func Int64Int64BulkLoad(keys []int64, values []int64, policy pump.DuplicatePolicy) (*Int64Int64, error) {
	if len(keys) != len(values) {
		panic("mapdb: Int64Int64BulkLoad: len(keys) != len(values)")
	}
	m := &Int64Int64{entries: make([]int64Int64Entry, Int64Int64bulkCap(len(keys)))}
...
// Int64Int64BulkLoadExact is like Int64Int64BulkLoad but guarantees zero
...
func Int64Int64BulkLoadExact(keys []int64, values []int64, n int, policy pump.DuplicatePolicy) (*Int64Int64, error) {
	if len(keys) != len(values) {
		panic("mapdb: Int64Int64BulkLoadExact: len(keys) != len(values)")
	}
	if n < 0 {
		panic("mapdb: Int64Int64BulkLoadExact: negative n")
	}
	m := &Int64Int64{entries: make([]int64Int64Entry, Int64Int64bulkCap(n))}
	for i := range keys {
		if m.size >= n {
...
func (m *Int64Int64) bulkInsert(keys []int64, values []int64, policy pump.DuplicatePolicy) error {
	for i := range keys {
		if m.needsResize() {
			m.resize()
		}
...
func (m *Int64Int64) bulkPut(key int64, value int64, policy pump.DuplicatePolicy) (bool, error) {
...
// Int64Int64bulkCap returns the presized table capacity for n entries that
...
func Int64Int64bulkCap(n int) int {
	c := pump.HashCapacityFor(n)
	if c < int64Int64DefaultCapacity {
		return int64Int64DefaultCapacity
	}
	return c
}
...
func (m *Int64Int64) ContainsValue(value int64) bool {
...
func (m *Int64Int64) ForEachKey(f func(int64)) {
	for i := range m.entries {
		if m.entries[i].occupied {
			f(m.entries[i].key)
		}
...
func (m *Int64Int64) ForEachValue(f func(int64)) {
	for i := range m.entries {
		if m.entries[i].occupied {
			f(m.entries[i].value)
		}
...
func (m *Int64Int64) Equals(other *Int64Int64) bool {
...
func (m *Int64Int64) KeysToSlice() []int64 {
...
func (m *Int64Int64) ValuesToSlice() []int64 {
	result := make([]int64, 0, m.size)
	for i := range m.entries {
		if m.entries[i].occupied {
...
func (m *Int64Int64) ToImmutable() *ImmutableInt64Int64 {
	return ImmutableInt64Int64From(m)
}

// InjectInto performs a left fold over all key-value pairs.
func (m *Int64Int64) InjectInto(initial int64, f func(int64, int64, int64) int64) int64 {
	result := initial
	for i := range m.entries {
		if m.entries[i].occupied {
...
func (m *Int64Int64) AddToValue(key int64, amount int64) int64 {
	if v, ok := m.Get(key); ok {
		newVal := v + amount
		m.Put(key, newVal)
		return newVal
...
// Returns the new value.
func (m *Int64Int64) UpdateValue(key int64, initialValue int64, f func(int64) int64) int64 {
	if v, ok := m.Get(key); ok {
		newVal := f(v)
		m.Put(key, newVal)
		return newVal
...
func (m *Int64Int64) PutReturning(key int64, value int64) *Int64Int64 {
	m.Put(key, value)
	return m
}

// RemoveKeyReturning returns the map after removing the key (fluent API).
func (m *Int64Int64) RemoveKeyReturning(key int64) *Int64Int64 {
	m.Remove(key)
	return m
}

// WithoutAllKeys removes all given keys (fluent API).
func (m *Int64Int64) WithoutAllKeys(keys []int64) *Int64Int64 {
	for _, k := range keys {
		m.Remove(k)
	}
	return m
...
func (m *Int64Int64) SumOfValues() int64 {
	var sum int64
	for i := range m.entries {
		if m.entries[i].occupied {
...
// SynchronizedInt64Int64 wrapper's Lock / RLock, or your own mutex) is required
...
func (m *Int64Int64) hashKey(key int64) uint64 {
	h := uint64(key) * 0x9E3779B97F4A7C15
	return h ^ (h >> 32)
}

func (m *Int64Int64) needsResize() bool {
...
func (m *Int64Int64) resize() {
	...
	m.entries = make([]int64Int64Entry, newCap)
	m.size = 0
...
func (m *Int64Int64) rehashFrom(deleted int, mask int) {
	c := len(m.entries)
	idx := (deleted + 1) & mask
...
func nextPowerOfTwoInt64Int64(n int) int {
	if n <= 0 {
		return 16
	}
	n--
