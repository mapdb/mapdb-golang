...
// NewInt64Int64WithCapacity creates a new empty Int64Int64 sized to hold at
...
func (m *Int64Int64) findIndex(key int64) (int, bool) {
	cap := len(m.entries)
	mask := cap - 1
	hash := m.hashKey(key)
	tag := uint8(hash & 0x7F)
...
func (m *Int64Int64) rehashForInsert() {
	cap := len(m.entries)
...
func (m *Int64Int64) rehashTo(newCap int) {
...
func (m *Int64Int64) insertNoGrow(key int64, value int64) {
	cap := len(m.entries)
	mask := cap - 1
	hash := m.hashKey(key)
	tag := uint8(hash & 0x7F)
