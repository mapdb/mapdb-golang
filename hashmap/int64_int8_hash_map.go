...
const (
	int64Int8DefaultCapacity = 16
	// Load factor 3/4 = 0.75, using integer math to avoid float conversion per insert.
)

// int64Int8Entry holds a single slot in the hash map for cache locality.
...
// Int64Int8 is an open-addressing hash map with int64 keys and int8 values.
type Int64Int8 struct {
	entries []int64Int8Entry
	size    int
}

// NewInt64Int8 creates a new empty Int64Int8 with default capacity.
func NewInt64Int8() *Int64Int8 {
	return NewInt64Int8WithCapacity(int64Int8DefaultCapacity)
}

// NewInt64Int8WithCapacity creates a new empty Int64Int8 with the given initial capacity.
func NewInt64Int8WithCapacity(capacity int) *Int64Int8 {
	cap := nextPowerOfTwoInt64Int8(capacity)
	return &Int64Int8{
		entries: make([]int64Int8Entry, cap),
		size:    0,
	}
}

// Int64Int8Of creates a new Int64Int8 from key-value pairs.
func Int64Int8Of(pairs ...struct {
	Key   int64
	Value int8
}) *Int64Int8 {
...
// Int64Int8BulkLoad builds a Int64Int8 from keys/values in a single pass,
...
// has more distinct keys than expected — use Int64Int8BulkLoadExact for the
// zero-rehash guarantee.
func Int64Int8BulkLoad(keys []int64, values []int8, policy pump.DuplicatePolicy) (*Int64Int8, error) {
	if len(keys) != len(values) {
		panic("mapdb: Int64Int8BulkLoad: len(keys) != len(values)")
	}
	m := &Int64Int8{entries: make([]int64Int8Entry, Int64Int8bulkCap(len(keys)))}
...
// Int64Int8BulkLoadExact is like Int64Int8BulkLoad but guarantees zero
...
func Int64Int8BulkLoadExact(keys []int64, values []int8, n int, policy pump.DuplicatePolicy) (*Int64Int8, error) {
	if len(keys) != len(values) {
		panic("mapdb: Int64Int8BulkLoadExact: len(keys) != len(values)")
	}
	if n < 0 {
		panic("mapdb: Int64Int8BulkLoadExact: negative n")
	}
	m := &Int64Int8{entries: make([]int64Int8Entry, Int64Int8bulkCap(n))}
	if len(keys) > n {
...
func (m *Int64Int8) bulkInsert(keys []int64, values []int8, policy pump.DuplicatePolicy) error {
	for i := range keys {
		if m.needsResize() {
			m.resize()
		}
...
func (m *Int64Int8) bulkPut(key int64, value int8, policy pump.DuplicatePolicy) (bool, error) {
...
// Int64Int8bulkCap returns the presized table capacity for n entries that
...
func Int64Int8bulkCap(n int) int {
	c := pump.HashCapacityFor(n)
	if c < int64Int8DefaultCapacity {
		return int64Int8DefaultCapacity
	}
	return c
}
...
func (m *Int64Int8) ContainsValue(value int8) bool {
...
func (m *Int64Int8) ForEachKey(f func(int64)) {
	for i := range m.entries {
		if m.entries[i].occupied {
			f(m.entries[i].key)
		}
...
func (m *Int64Int8) ForEachValue(f func(int8)) {
	for i := range m.entries {
		if m.entries[i].occupied {
			f(m.entries[i].value)
		}
...
func (m *Int64Int8) Equals(other *Int64Int8) bool {
...
func (m *Int64Int8) KeysToSlice() []int64 {
...
func (m *Int64Int8) ValuesToSlice() []int8 {
...
func (m *Int64Int8) ToImmutable() *ImmutableInt64Int8 {
	return ImmutableInt64Int8From(m)
}

// InjectInto performs a left fold over all key-value pairs.
func (m *Int64Int8) InjectInto(initial int8, f func(int8, int64, int8) int8) int8 {
	result := initial
	for i := range m.entries {
		if m.entries[i].occupied {
...
func (m *Int64Int8) AddToValue(key int64, amount int8) int8 {
	if v, ok := m.Get(key); ok {
		newVal := v + amount
		m.Put(key, newVal)
		return newVal
...
// Returns the new value.
func (m *Int64Int8) UpdateValue(key int64, initialValue int8, f func(int8) int8) int8 {
	if v, ok := m.Get(key); ok {
		newVal := f(v)
		m.Put(key, newVal)
		return newVal
...
func (m *Int64Int8) PutReturning(key int64, value int8) *Int64Int8 {
	m.Put(key, value)
	return m
}

// RemoveKeyReturning returns the map after removing the key (fluent API).
func (m *Int64Int8) RemoveKeyReturning(key int64) *Int64Int8 {
	m.Remove(key)
	return m
}

// WithoutAllKeys removes all given keys (fluent API).
func (m *Int64Int8) WithoutAllKeys(keys []int64) *Int64Int8 {
	for _, k := range keys {
		m.Remove(k)
	}
	return m
...
func (m *Int64Int8) SumOfValues() int8 {
	var sum int8
	for i := range m.entries {
		if m.entries[i].occupied {
...
// SynchronizedInt64Int8 wrapper's Lock / RLock, or your own mutex) is required
...
func (m *Int64Int8) hashKey(key int64) uint64 {
	h := uint64(key) * 0x9E3779B97F4A7C15
	return h ^ (h >> 32)
}

func (m *Int64Int8) needsResize() bool {
	return (m.size+1)*4 >= len(m.entries)*3 // 0.75 load factor, integer math
}

func (m *Int64Int8) resize() {
	oldEntries := m.entries
	newCap := len(oldEntries) * 2
	if newCap == 0 {
		newCap = int64Int8DefaultCapacity
	}
	m.entries = make([]int64Int8Entry, newCap)
	m.size = 0
...
func (m *Int64Int8) rehashFrom(deleted int, mask int) {
	c := len(m.entries)
	idx := (deleted + 1) & mask
...
func nextPowerOfTwoInt64Int8(n int) int {
	if n <= 0 {
		return 16
	}
	n--
