...
// NewInt64Int8WithCapacity creates a new empty Int64Int8 sized to hold at
...
func (m *Int64Int8) findIndex(key int64) (int, bool) {
	cap := len(m.entries)
	mask := cap - 1
	hash := m.hashKey(key)
	tag := uint8(hash & 0x7F)
...
func (m *Int64Int8) rehashForInsert() {
	cap := len(m.entries)
...
func (m *Int64Int8) rehashTo(newCap int) {
...
func (m *Int64Int8) insertNoGrow(key int64, value int8) {
	cap := len(m.entries)
	mask := cap - 1
	hash := m.hashKey(key)
	tag := uint8(hash & 0x7F)
