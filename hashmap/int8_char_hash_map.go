...
// has more distinct keys than expected — use Int8CharBulkLoadExact for the
...
func (m *Int8Char) bulkInsert(keys []int8, values []uint16, policy pump.DuplicatePolicy) error {
	for i := range keys {
		if m.needsResize() {
			m.resize()
		}
