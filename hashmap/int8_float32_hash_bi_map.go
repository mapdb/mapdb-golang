...
// Int8Float32BiMapBulkLoad builds a Int8Float32BiMap from keys/values in a
...
func Int8Float32BiMapBulkLoad(keys []int8, values []float32, policy pump.DuplicatePolicy) (*Int8Float32BiMap, error) {
	if len(keys) != len(values) {
		panic("mapdb: Int8Float32BiMapBulkLoad: len(keys) != len(values)")
	}
	m := &Int8Float32BiMap{
		forward: NewInt8Float32WithCapacity(Int8Float32bulkCap(len(keys))),
		reverse: NewFloat32Int8WithCapacity(Float32Int8bulkCap(len(values))),
	}
...
func (m *Int8Float32BiMap) All() iter.Seq2[int8, float32] {
