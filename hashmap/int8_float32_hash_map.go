...
const (
	int8Float32HashMapDefaultCapacity = 16
	// Load factor 3/4 = 0.75, using integer math to avoid float conversion per insert.
)

// int8Float32HashMapEntry holds a single slot in the hash map for cache locality.
type int8Float32HashMapEntry struct {
	key      int8
	value    float32
	occupied bool
}

// Int8Float32HashMap is an open-addressing hash map with int8 keys and float32 values.
type Int8Float32HashMap struct {
	entries []int8Float32HashMapEntry
	size    int
}

// NewInt8Float32HashMap creates a new empty Int8Float32HashMap with default capacity.
func NewInt8Float32HashMap() *Int8Float32HashMap {
	return NewInt8Float32HashMapWithCapacity(int8Float32HashMapDefaultCapacity)
}

// NewInt8Float32HashMapWithCapacity creates a new empty Int8Float32HashMap with the given initial capacity.
func NewInt8Float32HashMapWithCapacity(capacity int) *Int8Float32HashMap {
	cap := nextPowerOfTwoInt8Float32HashMap(capacity)
	return &Int8Float32HashMap{
		entries: make([]int8Float32HashMapEntry, cap),
		size:    0,
	}
}

// Int8Float32HashMapOf creates a new Int8Float32HashMap from key-value pairs.
func Int8Float32HashMapOf(pairs ...struct {
	Key   int8
	Value float32
}) *Int8Float32HashMap {
	m := NewInt8Float32HashMapWithCapacity(len(pairs) * 2)
	for _, p := range pairs {
		m.Put(p.Key, p.Value)
	}
	return m
...
func (m *Int8Float32HashMap) Put(key int8, value float32) (float32, bool) {
	if m.needsResize() {
		m.resize()
	}
	cap := len(m.entries)
...
func (m *Int8Float32HashMap) Get(key int8) (float32, bool) {
	cap := len(m.entries)
	if cap == 0 {
		return 0.0, false
	}
...
func (m *Int8Float32HashMap) GetOrDefault(key int8, defaultValue float32) float32 {
...
func (m *Int8Float32HashMap) Remove(key int8) (float32, bool) {
	cap := len(m.entries)
	if cap == 0 {
		return 0.0, false
	}
...
func (m *Int8Float32HashMap) ContainsKey(key int8) bool {
	_, ok := m.Get(key)
	return ok
}

// ContainsValue returns true if the map contains the given value.
func (m *Int8Float32HashMap) ContainsValue(value float32) bool {
...
func (m *Int8Float32HashMap) Size() int {
	return m.size
}
...
func (m *Int8Float32HashMap) Clear() {
	for i := range m.entries {
		m.entries[i] = int8Float32HashMapEntry{}
	}
	m.size = 0
}

// All returns an iter.Seq2 that yields all key-value pairs.
func (m *Int8Float32HashMap) All() iter.Seq2[int8, float32] {
...
func (m *Int8Float32HashMap) Keys() iter.Seq[int8] {
...
func (m *Int8Float32HashMap) Values() iter.Seq[float32] {
...
func (m *Int8Float32HashMap) ForEach(f func(int8, float32)) {
	for i := range m.entries {
		if m.entries[i].occupied {
...
func (m *Int8Float32HashMap) ForEachKey(f func(int8)) {
	for i := range m.entries {
		if m.entries[i].occupied {
			f(m.entries[i].key)
		}
...
func (m *Int8Float32HashMap) ForEachValue(f func(float32)) {
	for i := range m.entries {
		if m.entries[i].occupied {
			f(m.entries[i].value)
		}
...
func (m *Int8Float32HashMap) Select(predicate func(int8, float32) bool) *Int8Float32HashMap {
	result := NewInt8Float32HashMap()
	for i := range m.entries {
...
func (m *Int8Float32HashMap) Reject(predicate func(int8, float32) bool) *Int8Float32HashMap {
	result := NewInt8Float32HashMap()
	for i := range m.entries {
		if m.entries[i].occupied && !predicate(m.entries[i].key, m.entries[i].value) {
			result.Put(m.entries[i].key, m.entries[i].value)
...
func (m *Int8Float32HashMap) Detect(predicate func(int8, float32) bool) (int8, float32, bool) {
	for i := range m.entries {
		if m.entries[i].occupied && predicate(m.entries[i].key, m.entries[i].value) {
...
func (m *Int8Float32HashMap) AnySatisfy(predicate func(int8, float32) bool) bool {
	for i := range m.entries {
		if m.entries[i].occupied && predicate(m.entries[i].key, m.entries[i].value) {
			return true
		}
...
func (m *Int8Float32HashMap) AllSatisfy(predicate func(int8, float32) bool) bool {
	for i := range m.entries {
		if m.entries[i].occupied && !predicate(m.entries[i].key, m.entries[i].value) {
			return false
		}
...
func (m *Int8Float32HashMap) NoneSatisfy(predicate func(int8, float32) bool) bool {
	for i := range m.entries {
		if m.entries[i].occupied && predicate(m.entries[i].key, m.entries[i].value) {
			return false
		}
...
func (m *Int8Float32HashMap) Count(predicate func(int8, float32) bool) int {
	count := 0
	for i := range m.entries {
		if m.entries[i].occupied && predicate(m.entries[i].key, m.entries[i].value) {
			count++
...
func (m *Int8Float32HashMap) String() string {
	if m.size == 0 {
		return "{}"
	}
...
func (m *Int8Float32HashMap) Equals(other *Int8Float32HashMap) bool {
...
func (m *Int8Float32HashMap) KeysToSlice() []int8 {
...
func (m *Int8Float32HashMap) ValuesToSlice() []float32 {
...
func (m *Int8Float32HashMap) ToImmutable() *ImmutableInt8Float32HashMap {
	return ImmutableInt8Float32HashMapFrom(m)
}

// InjectInto performs a left fold over all key-value pairs.
func (m *Int8Float32HashMap) InjectInto(initial float32, f func(float32, int8, float32) float32) float32 {
	result := initial
	for i := range m.entries {
		if m.entries[i].occupied {
...
func (m *Int8Float32HashMap) AddToValue(key int8, amount float32) float32 {
	if v, ok := m.Get(key); ok {
		newVal := v + amount
		m.Put(key, newVal)
		return newVal
...
// Returns the new value.
func (m *Int8Float32HashMap) UpdateValue(key int8, initialValue float32, f func(float32) float32) float32 {
	if v, ok := m.Get(key); ok {
		newVal := f(v)
		m.Put(key, newVal)
		return newVal
...
func (m *Int8Float32HashMap) WithKeyValue(key int8, value float32) *Int8Float32HashMap {
	m.Put(key, value)
	return m
}

// WithoutKey returns the map after removing the key (fluent API).
func (m *Int8Float32HashMap) WithoutKey(key int8) *Int8Float32HashMap {
	m.Remove(key)
	return m
}

// WithoutAllKeys removes all given keys (fluent API).
func (m *Int8Float32HashMap) WithoutAllKeys(keys []int8) *Int8Float32HashMap {
	for _, k := range keys {
		m.Remove(k)
	}
	return m
...
func (m *Int8Float32HashMap) SumOfValues() float32 {
	var sum float32
	for i := range m.entries {
		if m.entries[i].occupied {
...
func (m *Int8Float32HashMap) resize() {
	oldEntries := m.entries
	newCap := len(oldEntries) * 2
	if newCap == 0 {
		newCap = int8Float32HashMapDefaultCapacity
	}
	m.entries = make([]int8Float32HashMapEntry, newCap)
	m.size = 0
...
func (m *Int8Float32HashMap) rehashFrom(deleted int, mask int) {
	c := len(m.entries)
	idx := (deleted + 1) & mask
	for m.entries[idx].occupied {
		...
		if distCurrent > distGap {
			...
			m.entries[idx] = int8Float32HashMapEntry{}
			deleted = idx
		}
		idx = (idx + 1) & mask
		if idx == deleted {
...
func nextPowerOfTwoInt8Float32HashMap(n int) int {
	if n <= 0 {
		return 16
	}
	n--
