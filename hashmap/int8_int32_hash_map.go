...
// NewInt8Int32WithCapacity creates a new empty Int8Int32 with the given initial capacity.
...
// Int8Int32BulkLoad builds a Int8Int32 from keys/values in a single pass,
...
// has more distinct keys than expected — use Int8Int32BulkLoadExact for the
// zero-rehash guarantee.
func Int8Int32BulkLoad(keys []int8, values []int32, policy pump.DuplicatePolicy) (*Int8Int32, error) {
	if len(keys) != len(values) {
		panic("mapdb: Int8Int32BulkLoad: len(keys) != len(values)")
	}
	m := &Int8Int32{entries: make([]int8Int32Entry, Int8Int32bulkCap(len(keys)))}
...
// Int8Int32BulkLoadExact is like Int8Int32BulkLoad but guarantees zero
...
func Int8Int32BulkLoadExact(keys []int8, values []int32, n int, policy pump.DuplicatePolicy) (*Int8Int32, error) {
	if len(keys) != len(values) {
		panic("mapdb: Int8Int32BulkLoadExact: len(keys) != len(values)")
	}
	if n < 0 {
		panic("mapdb: Int8Int32BulkLoadExact: negative n")
	}
	m := &Int8Int32{entries: make([]int8Int32Entry, Int8Int32bulkCap(n))}
	for i := range keys {
		if m.size >= n {
...
func (m *Int8Int32) bulkInsert(keys []int8, values []int32, policy pump.DuplicatePolicy) error {
	for i := range keys {
		if m.needsResize() {
			m.resize()
		}
...
func (m *Int8Int32) bulkPut(key int8, value int32, policy pump.DuplicatePolicy) (bool, error) {
...
// Int8Int32bulkCap returns the presized table capacity for n entries that
...
func Int8Int32bulkCap(n int) int {
	c := pump.HashCapacityFor(n)
	if c < int8Int32DefaultCapacity {
		return int8Int32DefaultCapacity
	}
	return c
}
...
func (m *Int8Int32) resize() {
	oldEntries := m.entries
	newCap := len(oldEntries) * 2
	if newCap == 0 {
		newCap = int8Int32DefaultCapacity
	}
...
func (m *Int8Int32) rehashFrom(deleted int, mask int) {
	c := len(m.entries)
	idx := (deleted + 1) & mask
...
func nextPowerOfTwoInt8Int32(n int) int {
	if n <= 0 {
		return 16
	}
	n--
