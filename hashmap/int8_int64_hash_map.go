...
// NewInt8Int64WithCapacity creates a new empty Int8Int64 sized to hold at
...
func (m *Int8Int64) findIndex(key int8) (int, bool) {
	cap := len(m.entries)
	mask := cap - 1
	hash := m.hashKey(key)
	tag := uint8(hash & 0x7F)
...
func (m *Int8Int64) rehashForInsert() {
	cap := len(m.entries)
...
func (m *Int8Int64) rehashTo(newCap int) {
...
func (m *Int8Int64) insertNoGrow(key int8, value int64) {
	cap := len(m.entries)
	mask := cap - 1
	hash := m.hashKey(key)
	tag := uint8(hash & 0x7F)
