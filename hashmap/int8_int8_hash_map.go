...
// NewInt8Int8WithCapacity creates a new empty Int8Int8 sized to hold at
...
func (m *Int8Int8) findIndex(key int8) (int, bool) {
	cap := len(m.entries)
	mask := cap - 1
	hash := m.hashKey(key)
	tag := uint8(hash & 0x7F)
...
func (m *Int8Int8) rehashForInsert() {
	cap := len(m.entries)
...
func (m *Int8Int8) rehashTo(newCap int) {
...
func (m *Int8Int8) insertNoGrow(key int8, value int8) {
	cap := len(m.entries)
	mask := cap - 1
	hash := m.hashKey(key)
	tag := uint8(hash & 0x7F)
