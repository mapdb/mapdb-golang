...
// int8Int8Entry holds a single slot in the hash map for cache locality.
...
// Int8Int8 is an open-addressing hash map with int8 keys and int8 values.
type Int8Int8 struct {
	entries []int8Int8Entry
	size    int
}
...
func NewInt8Int8WithCapacity(capacity int) *Int8Int8 {
	...
	return &Int8Int8{
		entries: make([]int8Int8Entry, cap),
		size:    0,
	}
}

// Int8Int8Of creates a new Int8Int8 from key-value pairs.
func Int8Int8Of(pairs ...struct {
	Key   int8
	Value int8
}) *Int8Int8 {
...
// Int8Int8BulkLoad builds a Int8Int8 from keys/values in a single pass,
...
// has more distinct keys than expected — use Int8Int8BulkLoadExact for the
// zero-rehash guarantee.
func Int8Int8BulkLoad(keys []int8, values []int8, policy pump.DuplicatePolicy) (*Int8Int8, error) {
	if len(keys) != len(values) {
		panic("mapdb: Int8Int8BulkLoad: len(keys) != len(values)")
	}
	m := &Int8Int8{entries: make([]int8Int8Entry, Int8Int8bulkCap(len(keys)))}
...
// Int8Int8BulkLoadExact is like Int8Int8BulkLoad but guarantees zero
...
func Int8Int8BulkLoadExact(keys []int8, values []int8, n int, policy pump.DuplicatePolicy) (*Int8Int8, error) {
	if len(keys) != len(values) {
		panic("mapdb: Int8Int8BulkLoadExact: len(keys) != len(values)")
	}
	if n < 0 {
		panic("mapdb: Int8Int8BulkLoadExact: negative n")
	}
	m := &Int8Int8{entries: make([]int8Int8Entry, Int8Int8bulkCap(n))}
	if len(keys) > n {
...
func (m *Int8Int8) bulkInsert(keys []int8, values []int8, policy pump.DuplicatePolicy) error {
	for i := range keys {
		if m.needsResize() {
			m.resize()
		}
...
func (m *Int8Int8) bulkPut(key int8, value int8, policy pump.DuplicatePolicy) (bool, error) {
...
// Int8Int8bulkCap returns the presized table capacity for n entries that
...
func Int8Int8bulkCap(n int) int {
	c := pump.HashCapacityFor(n)
	if c < int8Int8DefaultCapacity {
		return int8Int8DefaultCapacity
	}
	return c
}
...
func (m *Int8Int8) ForEachKey(f func(int8)) {
	for i := range m.entries {
		if m.entries[i].occupied {
			f(m.entries[i].key)
		}
...
func (m *Int8Int8) ForEachValue(f func(int8)) {
	for i := range m.entries {
		if m.entries[i].occupied {
			f(m.entries[i].value)
		}
...
func (m *Int8Int8) Equals(other *Int8Int8) bool {
...
func (m *Int8Int8) KeysToSlice() []int8 {
...
func (m *Int8Int8) ValuesToSlice() []int8 {
	result := make([]int8, 0, m.size)
	for i := range m.entries {
		if m.entries[i].occupied {
...
func (m *Int8Int8) ToImmutable() *ImmutableInt8Int8 {
	return ImmutableInt8Int8From(m)
}

// InjectInto performs a left fold over all key-value pairs.
func (m *Int8Int8) InjectInto(initial int8, f func(int8, int8, int8) int8) int8 {
	result := initial
	for i := range m.entries {
		if m.entries[i].occupied {
...
func (m *Int8Int8) AddToValue(key int8, amount int8) int8 {
	if v, ok := m.Get(key); ok {
		newVal := v + amount
		m.Put(key, newVal)
		return newVal
...
// Returns the new value.
func (m *Int8Int8) UpdateValue(key int8, initialValue int8, f func(int8) int8) int8 {
	if v, ok := m.Get(key); ok {
		newVal := f(v)
		m.Put(key, newVal)
		return newVal
...
func (m *Int8Int8) PutReturning(key int8, value int8) *Int8Int8 {
	m.Put(key, value)
	return m
}

// RemoveKeyReturning returns the map after removing the key (fluent API).
func (m *Int8Int8) RemoveKeyReturning(key int8) *Int8Int8 {
	m.Remove(key)
	return m
}

// WithoutAllKeys removes all given keys (fluent API).
func (m *Int8Int8) WithoutAllKeys(keys []int8) *Int8Int8 {
	for _, k := range keys {
		m.Remove(k)
	}
	return m
...
func (m *Int8Int8) SumOfValues() int8 {
	var sum int8
	for i := range m.entries {
		if m.entries[i].occupied {
...
// SynchronizedInt8Int8 wrapper's Lock / RLock, or your own mutex) is required
...
func (m *Int8Int8) resize() {
	...
	m.entries = make([]int8Int8Entry, newCap)
	m.size = 0
...
func (m *Int8Int8) rehashFrom(deleted int, mask int) {
	c := len(m.entries)
	idx := (deleted + 1) & mask
