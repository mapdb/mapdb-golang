...
func TestImmutableInt64_Generated_Union(t *testing.T) {
	a := Int64Of(1, 2, 3).ToImmutable()
	b := Int64Of(3, 4, 5).ToImmutable()
	u := a.Union(b)
	if u.Len() != 5 {
		t.Errorf("Union size = %d, want 5", u.Len())
	}
}
func TestImmutableInt64_Generated_Intersect(t *testing.T) {
	a := Int64Of(1, 2, 3).ToImmutable()
	b := Int64Of(2, 3, 4).ToImmutable()
	i := a.Intersect(b)
	if i.Len() != 2 {
		t.Errorf("Intersect size = %d, want 2", i.Len())
	}
}
func TestImmutableInt64_Generated_Difference(t *testing.T) {
	a := Int64Of(1, 2, 3).ToImmutable()
	b := Int64Of(2, 3, 4).ToImmutable()
	d := a.Difference(b)
	if d.Len() != 1 {
