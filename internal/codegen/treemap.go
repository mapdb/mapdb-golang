...
func genTreeMap() error {
	...
	if err := genCmpFloat("treemap"); err != nil {
		return err
	}
...
// reproduces All in ascending key order — so a *{{.MapName}} satisfies
// par.Segmenter2[{{.KeyType}}, {{.ValType}}] and feeds par.From2 with ORDERED
...
func (m *{{.MapName}}) Segments2(n int) []iter.Seq2[{{.KeyType}}, {{.ValType}}] {
	ranges := segment.SplitRanges(m.size, n)
	segs := make([]iter.Seq2[{{.KeyType}}, {{.ValType}}], len(ranges))
	for i := range ranges {
		lo, hi := ranges[i][0], ranges[i][1] // per-iteration copies for this view's closure
		segs[i] = func(yield func({{.KeyType}}, {{.ValType}}) bool) {
			// In-order walk yielding pairs whose 0-based key rank is in [lo, hi).
			// base is the rank of the first element in node's subtree.
			var walk func(node *{{.NodeName}}TreeNode, base int) bool
			walk = func(node *{{.NodeName}}TreeNode, base int) bool {
				if node == nil {
					return true
				}
				rank := base + {{.NodeName}}TreeNodeSize(node.left)
