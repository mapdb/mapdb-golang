...
// (NewInt16(0, 10, 3) yields 0, 3, 6, 9; its reverse is 9, 6, 3, 0).
// Panics if step is the minimum int16, whose negation is unrepresentable.
func (iv *Int16) Reversed() *Int16 {
	...
	var last int16
	if iv.step > 0 {
		last = int16(uint64(int64(iv.to)) - rem)
	} else {
		last = int16(uint64(int64(iv.to)) + rem)
	}
	return &Int16{from: last, to: iv.from, step: -iv.step}
}
