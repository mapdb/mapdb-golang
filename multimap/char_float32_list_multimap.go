...
// CharFloat32ListMultimap is a list multimap from uint16 keys to float32 values.
// Each key maps to a slice of values, preserving insertion order per key.
type CharFloat32ListMultimap struct {
	data map[uint16][]float32
	size int
}

// NewCharFloat32ListMultimap creates a new empty CharFloat32ListMultimap.
func NewCharFloat32ListMultimap() *CharFloat32ListMultimap {
	return &CharFloat32ListMultimap{
...
func (m *CharFloat32ListMultimap) Get(key uint16) []float32 {
	return m.GetAll(key)
}

// GetAll returns a copy of the values for the given key.
func (m *CharFloat32ListMultimap) GetAll(key uint16) []float32 {
	vals := m.data[key]
	if vals == nil {
		return nil
	}
...
func (m *CharFloat32ListMultimap) RemoveAll(key uint16) []float32 {
	vals, ok := m.data[key]
	if !ok {
		return nil
	}
...
func (m *CharFloat32ListMultimap) ContainsKey(key uint16) bool {
	_, ok := m.data[key]
	return ok
}

// ContainsKeyValue returns true if the multimap contains the given key-value pair.
func (m *CharFloat32ListMultimap) ContainsKeyValue(key uint16, value float32) bool {
	vals, ok := m.data[key]
	if !ok {
		return false
	}
...
func (m *CharFloat32ListMultimap) KeysCount() int {
	return len(m.data)
}
...
func (m *CharFloat32ListMultimap) Clear() {
	m.data = make(map[uint16][]float32)
	m.size = 0
}

// ForEach calls the given function for each key-value pair.
func (m *CharFloat32ListMultimap) ForEach(f func(uint16, float32)) {
...
func (m *CharFloat32ListMultimap) ForEachKeyValues(f func(uint16, []float32)) {
	for key, vals := range m.data {
...
func (m *CharFloat32ListMultimap) Keys() []uint16 {
...
func (m *CharFloat32ListMultimap) Values() []float32 {
...
func (m *CharFloat32ListMultimap) Select(predicate func(uint16, float32) bool) *CharFloat32ListMultimap {
	result := NewCharFloat32ListMultimap()
	for key, vals := range m.data {
		for _, val := range vals {
...
func (m *CharFloat32ListMultimap) Reject(predicate func(uint16, float32) bool) *CharFloat32ListMultimap {
	result := NewCharFloat32ListMultimap()
	for key, vals := range m.data {
		for _, val := range vals {
			if !predicate(key, val) {
...
func (m *CharFloat32ListMultimap) String() string {
	if m.size == 0 {
		return "{}"
	}
...
func (m *CharFloat32ListMultimap) Equals(other *CharFloat32ListMultimap) bool {
...
func (m *CharFloat32ListMultimap) KeysToSlice() []uint16 {
	return m.Keys()
}

// ValuesToSlice returns all values as a slice.
func (m *CharFloat32ListMultimap) ValuesToSlice() []float32 {
	return m.Values()
}

// WithKeyValue adds a key-value pair and returns the multimap (fluent API).
func (m *CharFloat32ListMultimap) WithKeyValue(key uint16, value float32) *CharFloat32ListMultimap {
	m.Put(key, value)
	return m
}

// WithoutKey removes all values for the key and returns the multimap (fluent API).
func (m *CharFloat32ListMultimap) WithoutKey(key uint16) *CharFloat32ListMultimap {
	m.RemoveAll(key)
	return m
}
