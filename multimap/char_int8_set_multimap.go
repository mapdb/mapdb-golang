...
// CharInt8SetMultimap is a set multimap from uint16 keys to int8 values.
// Each key maps to a set of unique values (duplicates on Put are silently dropped).
type CharInt8SetMultimap struct {
	data map[uint16][]int8
	size int
}

// NewCharInt8SetMultimap creates a new empty CharInt8SetMultimap.
func NewCharInt8SetMultimap() *CharInt8SetMultimap {
	return &CharInt8SetMultimap{
...
func (m *CharInt8SetMultimap) Put(key uint16, value int8) {
...
func (m *CharInt8SetMultimap) Get(key uint16) []int8 {
	return m.GetAll(key)
}

// GetAll returns a copy of the values for the given key.
func (m *CharInt8SetMultimap) GetAll(key uint16) []int8 {
	vals := m.data[key]
	if vals == nil {
		return nil
	}
...
func (m *CharInt8SetMultimap) RemoveAll(key uint16) []int8 {
	vals, ok := m.data[key]
	if !ok {
		return nil
	}
...
func (m *CharInt8SetMultimap) ContainsKey(key uint16) bool {
	_, ok := m.data[key]
	return ok
}

// ContainsKeyValue returns true if the multimap contains the given key-value pair.
func (m *CharInt8SetMultimap) ContainsKeyValue(key uint16, value int8) bool {
	vals, ok := m.data[key]
	if !ok {
		return false
	}
...
func (m *CharInt8SetMultimap) KeysCount() int {
	return len(m.data)
}

// Size returns the total number of values across all keys.
func (m *CharInt8SetMultimap) Size() int {
	return m.size
}

// IsEmpty returns true if the multimap contains no values.
func (m *CharInt8SetMultimap) IsEmpty() bool {
	return m.size == 0
}

// Clear removes all entries from the multimap.
func (m *CharInt8SetMultimap) Clear() {
	m.data = make(map[uint16][]int8)
	m.size = 0
}

// ForEach calls the given function for each key-value pair.
func (m *CharInt8SetMultimap) ForEach(f func(uint16, int8)) {
...
func (m *CharInt8SetMultimap) ForEachKeyValues(f func(uint16, []int8)) {
	for key, vals := range m.data {
...
func (m *CharInt8SetMultimap) Keys() []uint16 {
...
func (m *CharInt8SetMultimap) Values() []int8 {
...
func (m *CharInt8SetMultimap) Select(predicate func(uint16, int8) bool) *CharInt8SetMultimap {
	result := NewCharInt8SetMultimap()
	for key, vals := range m.data {
		for _, val := range vals {
...
func (m *CharInt8SetMultimap) Reject(predicate func(uint16, int8) bool) *CharInt8SetMultimap {
	result := NewCharInt8SetMultimap()
	for key, vals := range m.data {
		for _, val := range vals {
			if !predicate(key, val) {
...
func (m *CharInt8SetMultimap) String() string {
	if m.size == 0 {
		return "{}"
	}
...
func (m *CharInt8SetMultimap) Equals(other *CharInt8SetMultimap) bool {
...
func (m *CharInt8SetMultimap) KeysToSlice() []uint16 {
	return m.Keys()
}

// ValuesToSlice returns all values as a slice.
func (m *CharInt8SetMultimap) ValuesToSlice() []int8 {
	return m.Values()
}

// WithKeyValue adds a key-value pair and returns the multimap (fluent API).
func (m *CharInt8SetMultimap) WithKeyValue(key uint16, value int8) *CharInt8SetMultimap {
	m.Put(key, value)
	return m
}

// WithoutKey removes all values for the key and returns the multimap (fluent API).
func (m *CharInt8SetMultimap) WithoutKey(key uint16) *CharInt8SetMultimap {
	m.RemoveAll(key)
	return m
}
