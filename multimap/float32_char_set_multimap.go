// Code generated by internal/codegen; DO NOT EDIT.

package multimap

import (
	"fmt"
	"iter"
	"math"
	"strings"

	"github.com/mapdb/mapdb-golang/pump"
)

// Float32CharSet is a set multimap from float32 keys to uint16 values.
// Each key maps to a set of unique values (duplicates on Put are silently dropped).
type Float32CharSet struct {
	data map[uint32][]uint16
	keys map[uint32]float32
	size int
}

// NewFloat32CharSet creates a new empty Float32CharSet.
func NewFloat32CharSet() *Float32CharSet {
	return &Float32CharSet{
		data: make(map[uint32][]uint16),
		keys: make(map[uint32]float32),
		size: 0,
	}
}

// Float32CharSetBulkLoad builds a Float32CharSet from keys/values in a single
// pass, presizing the backing map for the input. keys[i] and values[i] form one
// pair (a length mismatch panics). The input need not be sorted; per-key value
// duplicates are dropped exactly as repeated Put. Duplicate keys are the normal
// grouping case, so the duplicate policy does not apply.
func Float32CharSetBulkLoad(keys []float32, values []uint16) *Float32CharSet {
	if len(keys) != len(values) {
		panic("mapdb: Float32CharSetBulkLoad: len(keys) != len(values)")
	}
	m := &Float32CharSet{
		data: make(map[uint32][]uint16, len(keys)),
		keys: make(map[uint32]float32, len(keys)),
	}
	for i := range keys {
		m.Put(keys[i], values[i])
	}
	return m
}

// NewFloat32CharSetFromSortedKeyValues builds a Float32CharSet from input sorted
// by ascending key and, within each key, ascending value. It validates both key
// monotonicity AND per-key value monotonicity (using the value type's own
// comparator — the IEEE-754 total order for float values) in one pass, deduping
// equal values per key (the sorted equivalent of the linear-scan dedupe Put
// performs). keys[i] and values[i] form one pair (a length mismatch panics).
// Out-of-order keys, or values that descend within a key run, return
// pump.ErrNotSorted before any partial collection is built. The result is
// observably identical to the same pairs inserted with Put; if your values are
// not sorted within each key, use Float32CharSetBulkLoad instead.
func NewFloat32CharSetFromSortedKeyValues(keys []float32, values []uint16) (*Float32CharSet, error) {
	if len(keys) != len(values) {
		panic("mapdb: NewFloat32CharSetFromSortedKeyValues: len(keys) != len(values)")
	}
	m := &Float32CharSet{
		data: make(map[uint32][]uint16),
		keys: make(map[uint32]float32),
	}
	i := 0
	for i < len(keys) {
		key := keys[i]
		if i > 0 && cmpKeyFloat32(key, keys[i-1]) <= 0 {
			return nil, pump.ErrNotSorted
		}
		j := i
		run := []uint16{}
		for j < len(keys) && cmpKeyFloat32(keys[j], key) == 0 {
			v := values[j]
			if len(run) > 0 {
				c := cmpKeyChar(run[len(run)-1], v)
				if c > 0 {
					return nil, pump.ErrNotSorted // value descends within key run
				}
				if c == 0 {
					j++
					continue // adjacent duplicate value (input is sorted, so equals are adjacent)
				}
			}
			run = append(run, v)
			j++
		}
		kb := math.Float32bits(key)
		m.data[kb] = run
		m.keys[kb] = key
		m.size += len(run)
		i = j
	}
	return m, nil
}

// Put adds a value to the set for the given key. Idempotent: a duplicate
// value for the same key is silently dropped.
func (m *Float32CharSet) Put(key float32, value uint16) {
	if m.data == nil {
		m.data = make(map[uint32][]uint16)
		m.keys = make(map[uint32]float32)
	}
	kb := math.Float32bits(key)
	for _, existing := range m.data[kb] {
		if existing == value {
			return
		}
	}
	m.data[kb] = append(m.data[kb], value)
	m.keys[kb] = key
	m.size++
}

// Get returns a copy of the values for the given key. Returns nil if the key is absent.
func (m *Float32CharSet) Get(key float32) []uint16 {
	return m.GetAll(key)
}

// GetAll returns a copy of the values for the given key.
func (m *Float32CharSet) GetAll(key float32) []uint16 {
	vals := m.data[math.Float32bits(key)]
	if vals == nil {
		return nil
	}
	result := make([]uint16, len(vals))
	copy(result, vals)
	return result
}

// RemoveAll removes all values for the given key and returns them.
func (m *Float32CharSet) RemoveAll(key float32) []uint16 {
	kb := math.Float32bits(key)
	vals, ok := m.data[kb]
	if !ok {
		return nil
	}
	delete(m.data, kb)
	delete(m.keys, kb)
	m.size -= len(vals)
	return vals
}

// ContainsKey returns true if the multimap contains the given key.
func (m *Float32CharSet) ContainsKey(key float32) bool {
	_, ok := m.data[math.Float32bits(key)]
	return ok
}

// ContainsKeyValue returns true if the multimap contains the given key-value pair.
func (m *Float32CharSet) ContainsKeyValue(key float32, value uint16) bool {
	vals, ok := m.data[math.Float32bits(key)]
	if !ok {
		return false
	}
	for _, v := range vals {
		if v == value {
			return true
		}
	}
	return false
}

// KeysCount returns the number of distinct keys.
func (m *Float32CharSet) KeysCount() int {
	return len(m.data)
}

// Len returns the number of elements. Use m.Len() == 0 to test for emptiness.
func (m *Float32CharSet) Len() int {
	return m.size
}

// Clear removes all entries from the multimap.
func (m *Float32CharSet) Clear() {
	m.data = make(map[uint32][]uint16)
	m.keys = make(map[uint32]float32)
	m.size = 0
}

// ForEach calls the given function for each key-value pair.
func (m *Float32CharSet) ForEach(f func(float32, uint16)) {
	for kb, vals := range m.data {
		key := m.keys[kb]
		for _, val := range vals {
			f(key, val)
		}
	}
}

// All returns an iter.Seq2 that yields every key-value pair (law 1). Iteration
// order is unspecified — keys follow Go map order; a key's values keep their
// set-insertion order. Unlike ForEach it is rangeable and honors an early break,
// and it bridges into the seq/par layers via seq.From2(m.All()).
func (m *Float32CharSet) All() iter.Seq2[float32, uint16] {
	return func(yield func(float32, uint16) bool) {
		for kb, vals := range m.data {
			key := m.keys[kb]
			for _, val := range vals {
				if !yield(key, val) {
					return
				}
			}
		}
	}
}

// ForEachKeyValues calls the given function for each key with a copy of its values.
func (m *Float32CharSet) ForEachKeyValues(f func(float32, []uint16)) {
	for kb, vals := range m.data {
		key := m.keys[kb]
		copied := make([]uint16, len(vals))
		copy(copied, vals)
		f(key, copied)
	}
}

// Keys returns a slice of all distinct keys.
func (m *Float32CharSet) Keys() []float32 {
	result := make([]float32, 0, len(m.data))
	for _, key := range m.keys {
		result = append(result, key)
	}
	return result
}

// Values returns a slice of all values across all keys.
func (m *Float32CharSet) Values() []uint16 {
	result := make([]uint16, 0, m.size)
	for _, vals := range m.data {
		result = append(result, vals...)
	}
	return result
}

// Select returns a new multimap containing only key-value pairs that satisfy the predicate.
func (m *Float32CharSet) Select(predicate func(float32, uint16) bool) *Float32CharSet {
	result := NewFloat32CharSet()
	for kb, vals := range m.data {
		key := m.keys[kb]
		for _, val := range vals {
			if predicate(key, val) {
				result.Put(key, val)
			}
		}
	}
	return result
}

// Reject returns a new multimap containing only key-value pairs that do not satisfy the predicate.
func (m *Float32CharSet) Reject(predicate func(float32, uint16) bool) *Float32CharSet {
	result := NewFloat32CharSet()
	for kb, vals := range m.data {
		key := m.keys[kb]
		for _, val := range vals {
			if !predicate(key, val) {
				result.Put(key, val)
			}
		}
	}
	return result
}

// String returns a string representation of the multimap.
func (m *Float32CharSet) String() string {
	if m.size == 0 {
		return "{}"
	}
	var sb strings.Builder
	sb.WriteString("{")
	first := true
	for kb, vals := range m.data {
		key := m.keys[kb]
		if !first {
			sb.WriteString(", ")
		}
		fmt.Fprintf(&sb, "%v=[", key)
		for i, val := range vals {
			if i > 0 {
				sb.WriteString(", ")
			}
			fmt.Fprintf(&sb, "%v", val)
		}
		sb.WriteString("]")
		first = false
	}
	sb.WriteString("}")
	return sb.String()
}

// Equals returns true if the other multimap has the same key-value pairs in the same order per key.
func (m *Float32CharSet) Equals(other *Float32CharSet) bool {
	if m.size != other.size {
		return false
	}
	if len(m.data) != len(other.data) {
		return false
	}
	for kb, vals := range m.data {
		otherVals, ok := other.data[kb]
		if !ok || len(vals) != len(otherVals) {
			return false
		}
		for i, val := range vals {
			if !(val == otherVals[i]) {
				return false
			}
		}
	}
	return true
}

// KeysToSlice returns all distinct keys as a slice.
func (m *Float32CharSet) KeysToSlice() []float32 {
	return m.Keys()
}

// ValuesToSlice returns all values as a slice.
func (m *Float32CharSet) ValuesToSlice() []uint16 {
	return m.Values()
}

// PutReturning adds a key-value pair and returns the multimap (fluent API).
func (m *Float32CharSet) PutReturning(key float32, value uint16) *Float32CharSet {
	m.Put(key, value)
	return m
}

// RemoveKeyReturning removes all values for the key and returns the multimap (fluent API).
func (m *Float32CharSet) RemoveKeyReturning(key float32) *Float32CharSet {
	m.RemoveAll(key)
	return m
}
