...
type Float32Float32List struct {
	data map[uint32][]float32
	keys map[uint32]float32
	size int
}

// NewFloat32Float32List creates a new empty Float32Float32List.
func NewFloat32Float32List() *Float32Float32List {
	return &Float32Float32List{
...
func (m *Float32Float32List) Put(key float32, value float32) {
	if m.data == nil {
...
func (m *Float32Float32List) Get(key float32) []float32 {
	return m.GetAll(key)
}

// GetAll returns a copy of the values for the given key.
func (m *Float32Float32List) GetAll(key float32) []float32 {
...
func (m *Float32Float32List) RemoveAll(key float32) []float32 {
	kb := math.Float32bits(key)
	vals, ok := m.data[kb]
	if !ok {
		return nil
...
func (m *Float32Float32List) ContainsKey(key float32) bool {
	_, ok := m.data[math.Float32bits(key)]
	return ok
}

// ContainsKeyValue returns true if the multimap contains the given key-value pair.
func (m *Float32Float32List) ContainsKeyValue(key float32, value float32) bool {
...
func (m *Float32Float32List) KeysCount() int {
	return len(m.data)
}

// Len returns the number of elements. Use m.Len() == 0 to test for emptiness.
func (m *Float32Float32List) Len() int {
	return m.size
}

// Clear removes all entries from the multimap.
func (m *Float32Float32List) Clear() {
	m.data = make(map[uint32][]float32)
	m.keys = make(map[uint32]float32)
	m.size = 0
}
...
func (m *Float32Float32List) ForEach(f func(float32, float32)) {
...
func (m *Float32Float32List) All() iter.Seq2[float32, float32] {
...
func (m *Float32Float32List) ForEachKeyValues(f func(float32, []float32)) {
	for kb, vals := range m.data {
		key := m.keys[kb]
...
func (m *Float32Float32List) Keys() []float32 {
...
func (m *Float32Float32List) Values() []float32 {
...
func (m *Float32Float32List) Select(predicate func(float32, float32) bool) *Float32Float32List {
	result := NewFloat32Float32List()
	for kb, vals := range m.data {
		key := m.keys[kb]
		for _, val := range vals {
...
func (m *Float32Float32List) Reject(predicate func(float32, float32) bool) *Float32Float32List {
	result := NewFloat32Float32List()
	for kb, vals := range m.data {
		key := m.keys[kb]
		for _, val := range vals {
...
func (m *Float32Float32List) String() string {
	if m.size == 0 {
		return "{}"
	}
...
func (m *Float32Float32List) Equals(other *Float32Float32List) bool {
...
func (m *Float32Float32List) KeysToSlice() []float32 {
	return m.Keys()
}

// ValuesToSlice returns all values as a slice.
func (m *Float32Float32List) ValuesToSlice() []float32 {
	return m.Values()
}

// PutReturning adds a key-value pair and returns the multimap (fluent API).
func (m *Float32Float32List) PutReturning(key float32, value float32) *Float32Float32List {
	m.Put(key, value)
	return m
}

// RemoveKeyReturning removes all values for the key and returns the multimap (fluent API).
func (m *Float32Float32List) RemoveKeyReturning(key float32) *Float32Float32List {
	m.RemoveAll(key)
	return m
}
