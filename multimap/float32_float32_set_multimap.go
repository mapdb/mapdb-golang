...
// Float32Float32Set is a set multimap from float32 keys to float32 values.
// Each key maps to a set of unique values (duplicates on Put are silently dropped).
type Float32Float32Set struct {
	data map[uint32][]float32
	keys map[uint32]float32
	size int
}

// NewFloat32Float32Set creates a new empty Float32Float32Set.
func NewFloat32Float32Set() *Float32Float32Set {
	return &Float32Float32Set{
...
// Float32Float32SetBulkLoad builds a Float32Float32Set from keys/values in a single
...
func Float32Float32SetBulkLoad(keys []float32, values []float32) *Float32Float32Set {
	if len(keys) != len(values) {
		panic("mapdb: Float32Float32SetBulkLoad: len(keys) != len(values)")
	}
	m := &Float32Float32Set{
...
// NewFloat32Float32SetFromSortedKeyValues builds a Float32Float32Set from input sorted
...
// not sorted within each key, use Float32Float32SetBulkLoad instead.
func NewFloat32Float32SetFromSortedKeyValues(keys []float32, values []float32) (*Float32Float32Set, error) {
	if len(keys) != len(values) {
		panic("mapdb: NewFloat32Float32SetFromSortedKeyValues: len(keys) != len(values)")
	}
	m := &Float32Float32Set{
		data: make(map[uint32][]float32),
		keys: make(map[uint32]float32),
...
func (m *Float32Float32Set) Put(key float32, value float32) {
	if m.data == nil {
...
func (m *Float32Float32Set) Get(key float32) []float32 {
	return m.GetAll(key)
}

// GetAll returns a copy of the values for the given key.
func (m *Float32Float32Set) GetAll(key float32) []float32 {
...
func (m *Float32Float32Set) RemoveAll(key float32) []float32 {
	kb := math.Float32bits(key)
	vals, ok := m.data[kb]
	if !ok {
		return nil
...
func (m *Float32Float32Set) ContainsKey(key float32) bool {
	_, ok := m.data[math.Float32bits(key)]
	return ok
}

// ContainsKeyValue returns true if the multimap contains the given key-value pair.
func (m *Float32Float32Set) ContainsKeyValue(key float32, value float32) bool {
...
func (m *Float32Float32Set) KeysCount() int {
	return len(m.data)
}

// Len returns the number of elements. Use m.Len() == 0 to test for emptiness.
func (m *Float32Float32Set) Len() int {
	return m.size
}

// Clear removes all entries from the multimap.
func (m *Float32Float32Set) Clear() {
	m.data = make(map[uint32][]float32)
	m.keys = make(map[uint32]float32)
	m.size = 0
}
...
func (m *Float32Float32Set) ForEach(f func(float32, float32)) {
...
func (m *Float32Float32Set) All() iter.Seq2[float32, float32] {
...
func (m *Float32Float32Set) ForEachKeyValues(f func(float32, []float32)) {
	for kb, vals := range m.data {
		key := m.keys[kb]
...
func (m *Float32Float32Set) Keys() []float32 {
...
func (m *Float32Float32Set) Values() []float32 {
...
func (m *Float32Float32Set) Select(predicate func(float32, float32) bool) *Float32Float32Set {
	result := NewFloat32Float32Set()
	for kb, vals := range m.data {
		key := m.keys[kb]
		for _, val := range vals {
...
func (m *Float32Float32Set) Reject(predicate func(float32, float32) bool) *Float32Float32Set {
	result := NewFloat32Float32Set()
	for kb, vals := range m.data {
		key := m.keys[kb]
		for _, val := range vals {
...
func (m *Float32Float32Set) String() string {
	if m.size == 0 {
		return "{}"
	}
...
func (m *Float32Float32Set) Equals(other *Float32Float32Set) bool {
...
func (m *Float32Float32Set) KeysToSlice() []float32 {
	return m.Keys()
}

// ValuesToSlice returns all values as a slice.
func (m *Float32Float32Set) ValuesToSlice() []float32 {
	return m.Values()
}

// PutReturning adds a key-value pair and returns the multimap (fluent API).
func (m *Float32Float32Set) PutReturning(key float32, value float32) *Float32Float32Set {
	m.Put(key, value)
	return m
}

// RemoveKeyReturning removes all values for the key and returns the multimap (fluent API).
func (m *Float32Float32Set) RemoveKeyReturning(key float32) *Float32Float32Set {
	m.RemoveAll(key)
	return m
}
