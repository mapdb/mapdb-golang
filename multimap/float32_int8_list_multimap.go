...
// Float32Int8ListBulkLoad builds a Float32Int8List from keys/values in a single
...
// every value).
func Float32Int8ListBulkLoad(keys []float32, values []int8) *Float32Int8List {
	if len(keys) != len(values) {
		panic("mapdb: Float32Int8ListBulkLoad: len(keys) != len(values)")
	}
	m := &Float32Int8List{
...
// NewFloat32Int8ListFromSortedKeys builds a Float32Int8List from input grouped
...
func NewFloat32Int8ListFromSortedKeys(keys []float32, values []int8) (*Float32Int8List, error) {
	if len(keys) != len(values) {
		panic("mapdb: NewFloat32Int8ListFromSortedKeys: len(keys) != len(values)")
	}
	m := &Float32Int8List{
		data: make(map[uint32][]int8),
		keys: make(map[uint32]float32),
...
// NewFloat32Int8ListFromSortedKeyValues builds a Float32Int8List from input
...
// not sorted within each key, use Float32Int8ListBulkLoad instead.
func NewFloat32Int8ListFromSortedKeyValues(keys []float32, values []int8) (*Float32Int8List, error) {
	if len(keys) != len(values) {
		panic("mapdb: NewFloat32Int8ListFromSortedKeyValues: len(keys) != len(values)")
	}
	m := &Float32Int8List{
		data: make(map[uint32][]int8),
		keys: make(map[uint32]float32),
...
func (m *Float32Int8List) All() iter.Seq2[float32, int8] {
