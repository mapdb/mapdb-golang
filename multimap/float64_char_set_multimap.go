...
// Float64CharSet is a set multimap from float64 keys to uint16 values.
// Each key maps to a set of unique values (duplicates on Put are silently dropped).
type Float64CharSet struct {
	data map[uint64][]uint16
	keys map[uint64]float64
	size int
}

// NewFloat64CharSet creates a new empty Float64CharSet.
func NewFloat64CharSet() *Float64CharSet {
	return &Float64CharSet{
...
// Float64CharSetBulkLoad builds a Float64CharSet from keys/values in a single
...
func Float64CharSetBulkLoad(keys []float64, values []uint16) *Float64CharSet {
	if len(keys) != len(values) {
		panic("mapdb: Float64CharSetBulkLoad: len(keys) != len(values)")
	}
	m := &Float64CharSet{
...
// NewFloat64CharSetFromSortedKeyValues builds a Float64CharSet from input sorted
...
// not sorted within each key, use Float64CharSetBulkLoad instead.
func NewFloat64CharSetFromSortedKeyValues(keys []float64, values []uint16) (*Float64CharSet, error) {
	if len(keys) != len(values) {
		panic("mapdb: NewFloat64CharSetFromSortedKeyValues: len(keys) != len(values)")
	}
	m := &Float64CharSet{
		data: make(map[uint64][]uint16),
		keys: make(map[uint64]float64),
...
func (m *Float64CharSet) Put(key float64, value uint16) {
	if m.data == nil {
...
func (m *Float64CharSet) Get(key float64) []uint16 {
	return m.GetAll(key)
}

// GetAll returns a copy of the values for the given key.
func (m *Float64CharSet) GetAll(key float64) []uint16 {
...
func (m *Float64CharSet) RemoveAll(key float64) []uint16 {
	kb := math.Float64bits(key)
	vals, ok := m.data[kb]
	if !ok {
		return nil
...
func (m *Float64CharSet) ContainsKey(key float64) bool {
	_, ok := m.data[math.Float64bits(key)]
	return ok
}

// ContainsKeyValue returns true if the multimap contains the given key-value pair.
func (m *Float64CharSet) ContainsKeyValue(key float64, value uint16) bool {
...
func (m *Float64CharSet) KeysCount() int {
	return len(m.data)
}

// Len returns the number of elements. Use m.Len() == 0 to test for emptiness.
func (m *Float64CharSet) Len() int {
	return m.size
}

// Clear removes all entries from the multimap.
func (m *Float64CharSet) Clear() {
	m.data = make(map[uint64][]uint16)
	m.keys = make(map[uint64]float64)
	m.size = 0
}
...
func (m *Float64CharSet) ForEach(f func(float64, uint16)) {
...
func (m *Float64CharSet) All() iter.Seq2[float64, uint16] {
...
func (m *Float64CharSet) ForEachKeyValues(f func(float64, []uint16)) {
	for kb, vals := range m.data {
		key := m.keys[kb]
...
func (m *Float64CharSet) Keys() []float64 {
...
func (m *Float64CharSet) Values() []uint16 {
...
func (m *Float64CharSet) Select(predicate func(float64, uint16) bool) *Float64CharSet {
	result := NewFloat64CharSet()
	for kb, vals := range m.data {
		key := m.keys[kb]
		for _, val := range vals {
...
func (m *Float64CharSet) Reject(predicate func(float64, uint16) bool) *Float64CharSet {
	result := NewFloat64CharSet()
	for kb, vals := range m.data {
		key := m.keys[kb]
		for _, val := range vals {
...
func (m *Float64CharSet) String() string {
	if m.size == 0 {
		return "{}"
	}
...
func (m *Float64CharSet) Equals(other *Float64CharSet) bool {
...
func (m *Float64CharSet) KeysToSlice() []float64 {
	return m.Keys()
}

// ValuesToSlice returns all values as a slice.
func (m *Float64CharSet) ValuesToSlice() []uint16 {
	return m.Values()
}

// PutReturning adds a key-value pair and returns the multimap (fluent API).
func (m *Float64CharSet) PutReturning(key float64, value uint16) *Float64CharSet {
	m.Put(key, value)
	return m
}

// RemoveKeyReturning removes all values for the key and returns the multimap (fluent API).
func (m *Float64CharSet) RemoveKeyReturning(key float64) *Float64CharSet {
	m.RemoveAll(key)
	return m
}
