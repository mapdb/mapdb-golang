...
// Float64Float64List is a list multimap from float64 keys to float64 values.
// Each key maps to a slice of values, preserving insertion order per key.
type Float64Float64List struct {
	data map[uint64][]float64
	keys map[uint64]float64
	size int
}

// NewFloat64Float64List creates a new empty Float64Float64List.
func NewFloat64Float64List() *Float64Float64List {
	return &Float64Float64List{
...
// Float64Float64ListBulkLoad builds a Float64Float64List from keys/values in a single
...
// every value).
func Float64Float64ListBulkLoad(keys []float64, values []float64) *Float64Float64List {
	if len(keys) != len(values) {
		panic("mapdb: Float64Float64ListBulkLoad: len(keys) != len(values)")
	}
	m := &Float64Float64List{
		data: make(map[uint64][]float64, len(keys)),
...
// NewFloat64Float64ListFromSortedKeys builds a Float64Float64List from input grouped
...
func NewFloat64Float64ListFromSortedKeys(keys []float64, values []float64) (*Float64Float64List, error) {
	if len(keys) != len(values) {
		panic("mapdb: NewFloat64Float64ListFromSortedKeys: len(keys) != len(values)")
	}
	m := &Float64Float64List{
		data: make(map[uint64][]float64),
		keys: make(map[uint64]float64),
...
// NewFloat64Float64ListFromSortedKeyValues builds a Float64Float64List from input
...
// not sorted within each key, use Float64Float64ListBulkLoad instead.
func NewFloat64Float64ListFromSortedKeyValues(keys []float64, values []float64) (*Float64Float64List, error) {
	if len(keys) != len(values) {
		panic("mapdb: NewFloat64Float64ListFromSortedKeyValues: len(keys) != len(values)")
	}
	m := &Float64Float64List{
		data: make(map[uint64][]float64),
		keys: make(map[uint64]float64),
...
func (m *Float64Float64List) Put(key float64, value float64) {
	if m.data == nil {
...
func (m *Float64Float64List) Get(key float64) []float64 {
	return m.GetAll(key)
}

// GetAll returns a copy of the values for the given key.
func (m *Float64Float64List) GetAll(key float64) []float64 {
...
func (m *Float64Float64List) RemoveAll(key float64) []float64 {
	kb := math.Float64bits(key)
	vals, ok := m.data[kb]
	if !ok {
		return nil
...
func (m *Float64Float64List) ContainsKey(key float64) bool {
	_, ok := m.data[math.Float64bits(key)]
	return ok
}

// ContainsKeyValue returns true if the multimap contains the given key-value pair.
func (m *Float64Float64List) ContainsKeyValue(key float64, value float64) bool {
...
func (m *Float64Float64List) KeysCount() int {
	return len(m.data)
}

// Len returns the number of elements. Use m.Len() == 0 to test for emptiness.
func (m *Float64Float64List) Len() int {
	return m.size
}

// Clear removes all entries from the multimap.
func (m *Float64Float64List) Clear() {
	m.data = make(map[uint64][]float64)
	m.keys = make(map[uint64]float64)
	m.size = 0
}
...
func (m *Float64Float64List) ForEach(f func(float64, float64)) {
...
func (m *Float64Float64List) All() iter.Seq2[float64, float64] {
...
func (m *Float64Float64List) ForEachKeyValues(f func(float64, []float64)) {
	for kb, vals := range m.data {
		key := m.keys[kb]
...
func (m *Float64Float64List) Keys() []float64 {
...
func (m *Float64Float64List) Values() []float64 {
...
func (m *Float64Float64List) Select(predicate func(float64, float64) bool) *Float64Float64List {
	result := NewFloat64Float64List()
	for kb, vals := range m.data {
		key := m.keys[kb]
		for _, val := range vals {
...
func (m *Float64Float64List) Reject(predicate func(float64, float64) bool) *Float64Float64List {
	result := NewFloat64Float64List()
	for kb, vals := range m.data {
		key := m.keys[kb]
		for _, val := range vals {
...
func (m *Float64Float64List) String() string {
	if m.size == 0 {
		return "{}"
	}
...
func (m *Float64Float64List) Equals(other *Float64Float64List) bool {
...
func (m *Float64Float64List) KeysToSlice() []float64 {
	return m.Keys()
}

// ValuesToSlice returns all values as a slice.
func (m *Float64Float64List) ValuesToSlice() []float64 {
	return m.Values()
}

// PutReturning adds a key-value pair and returns the multimap (fluent API).
func (m *Float64Float64List) PutReturning(key float64, value float64) *Float64Float64List {
	m.Put(key, value)
	return m
}

// RemoveKeyReturning removes all values for the key and returns the multimap (fluent API).
func (m *Float64Float64List) RemoveKeyReturning(key float64) *Float64Float64List {
	m.RemoveAll(key)
	return m
}
