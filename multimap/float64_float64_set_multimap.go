...
// Float64Float64Set is a set multimap from float64 keys to float64 values.
// Each key maps to a set of unique values (duplicates on Put are silently dropped).
type Float64Float64Set struct {
	data map[uint64][]float64
	keys map[uint64]float64
	size int
}

// NewFloat64Float64Set creates a new empty Float64Float64Set.
func NewFloat64Float64Set() *Float64Float64Set {
	return &Float64Float64Set{
...
// Float64Float64SetBulkLoad builds a Float64Float64Set from keys/values in a single
...
func Float64Float64SetBulkLoad(keys []float64, values []float64) *Float64Float64Set {
	if len(keys) != len(values) {
		panic("mapdb: Float64Float64SetBulkLoad: len(keys) != len(values)")
	}
	m := &Float64Float64Set{
...
// NewFloat64Float64SetFromSortedKeyValues builds a Float64Float64Set from input sorted
...
// not sorted within each key, use Float64Float64SetBulkLoad instead.
func NewFloat64Float64SetFromSortedKeyValues(keys []float64, values []float64) (*Float64Float64Set, error) {
	if len(keys) != len(values) {
		panic("mapdb: NewFloat64Float64SetFromSortedKeyValues: len(keys) != len(values)")
	}
	m := &Float64Float64Set{
		data: make(map[uint64][]float64),
		keys: make(map[uint64]float64),
...
func (m *Float64Float64Set) Put(key float64, value float64) {
	if m.data == nil {
...
func (m *Float64Float64Set) Get(key float64) []float64 {
	return m.GetAll(key)
}

// GetAll returns a copy of the values for the given key.
func (m *Float64Float64Set) GetAll(key float64) []float64 {
...
func (m *Float64Float64Set) RemoveAll(key float64) []float64 {
	kb := math.Float64bits(key)
	vals, ok := m.data[kb]
	if !ok {
		return nil
...
func (m *Float64Float64Set) ContainsKey(key float64) bool {
	_, ok := m.data[math.Float64bits(key)]
	return ok
}

// ContainsKeyValue returns true if the multimap contains the given key-value pair.
func (m *Float64Float64Set) ContainsKeyValue(key float64, value float64) bool {
...
func (m *Float64Float64Set) KeysCount() int {
	return len(m.data)
}

// Len returns the number of elements. Use m.Len() == 0 to test for emptiness.
func (m *Float64Float64Set) Len() int {
	return m.size
}

// Clear removes all entries from the multimap.
func (m *Float64Float64Set) Clear() {
	m.data = make(map[uint64][]float64)
	m.keys = make(map[uint64]float64)
	m.size = 0
}
...
func (m *Float64Float64Set) ForEach(f func(float64, float64)) {
...
func (m *Float64Float64Set) All() iter.Seq2[float64, float64] {
...
func (m *Float64Float64Set) ForEachKeyValues(f func(float64, []float64)) {
	for kb, vals := range m.data {
		key := m.keys[kb]
...
func (m *Float64Float64Set) Keys() []float64 {
...
func (m *Float64Float64Set) Values() []float64 {
...
func (m *Float64Float64Set) Select(predicate func(float64, float64) bool) *Float64Float64Set {
	result := NewFloat64Float64Set()
	for kb, vals := range m.data {
		key := m.keys[kb]
		for _, val := range vals {
...
func (m *Float64Float64Set) Reject(predicate func(float64, float64) bool) *Float64Float64Set {
	result := NewFloat64Float64Set()
	for kb, vals := range m.data {
		key := m.keys[kb]
		for _, val := range vals {
...
func (m *Float64Float64Set) String() string {
	if m.size == 0 {
		return "{}"
	}
...
func (m *Float64Float64Set) Equals(other *Float64Float64Set) bool {
...
func (m *Float64Float64Set) KeysToSlice() []float64 {
	return m.Keys()
}

// ValuesToSlice returns all values as a slice.
func (m *Float64Float64Set) ValuesToSlice() []float64 {
	return m.Values()
}

// PutReturning adds a key-value pair and returns the multimap (fluent API).
func (m *Float64Float64Set) PutReturning(key float64, value float64) *Float64Float64Set {
	m.Put(key, value)
	return m
}

// RemoveKeyReturning removes all values for the key and returns the multimap (fluent API).
func (m *Float64Float64Set) RemoveKeyReturning(key float64) *Float64Float64Set {
	m.RemoveAll(key)
	return m
}
