...
// Int16Float64ListBulkLoad builds a Int16Float64List from keys/values in a single
...
// every value).
func Int16Float64ListBulkLoad(keys []int16, values []float64) *Int16Float64List {
	if len(keys) != len(values) {
		panic("mapdb: Int16Float64ListBulkLoad: len(keys) != len(values)")
	}
	m := &Int16Float64List{
		data: make(map[int16][]float64, len(keys)),
	}
	for i := range keys {
...
// NewInt16Float64ListFromSortedKeys builds a Int16Float64List from input grouped
...
func NewInt16Float64ListFromSortedKeys(keys []int16, values []float64) (*Int16Float64List, error) {
	if len(keys) != len(values) {
		panic("mapdb: NewInt16Float64ListFromSortedKeys: len(keys) != len(values)")
	}
	m := &Int16Float64List{
		data: make(map[int16][]float64),
	}
...
// NewInt16Float64ListFromSortedKeyValues builds a Int16Float64List from input
...
// not sorted within each key, use Int16Float64ListBulkLoad instead.
func NewInt16Float64ListFromSortedKeyValues(keys []int16, values []float64) (*Int16Float64List, error) {
	if len(keys) != len(values) {
		panic("mapdb: NewInt16Float64ListFromSortedKeyValues: len(keys) != len(values)")
	}
	m := &Int16Float64List{
		data: make(map[int16][]float64),
	}
...
func (m *Int16Float64List) All() iter.Seq2[int16, float64] {
