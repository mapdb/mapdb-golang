...
// NewInt64Float32ListFromSortedKeyValues builds a Int64Float32List from input
...
// not sorted within each key, use Int64Float32ListBulkLoad instead.
func NewInt64Float32ListFromSortedKeyValues(keys []int64, values []float32) (*Int64Float32List, error) {
	if len(keys) != len(values) {
		panic("mapdb: NewInt64Float32ListFromSortedKeyValues: len(keys) != len(values)")
	}
	m := &Int64Float32List{
		data: make(map[int64][]float32),
	}
...
func (m *Int64Float32List) All() iter.Seq2[int64, float32] {
