...
// Int8Float64List is a list multimap from int8 keys to float64 values.
// Each key maps to a slice of values, preserving insertion order per key.
type Int8Float64List struct {
	data map[int8][]float64
	size int
}

// NewInt8Float64List creates a new empty Int8Float64List.
func NewInt8Float64List() *Int8Float64List {
	return &Int8Float64List{
		data: make(map[int8][]float64),
		size: 0,
	}
}

// Int8Float64ListBulkLoad builds a Int8Float64List from keys/values in a single
...
// every value).
func Int8Float64ListBulkLoad(keys []int8, values []float64) *Int8Float64List {
	if len(keys) != len(values) {
		panic("mapdb: Int8Float64ListBulkLoad: len(keys) != len(values)")
	}
	m := &Int8Float64List{
...
// NewInt8Float64ListFromSortedKeys builds a Int8Float64List from input grouped
...
func NewInt8Float64ListFromSortedKeys(keys []int8, values []float64) (*Int8Float64List, error) {
	if len(keys) != len(values) {
		panic("mapdb: NewInt8Float64ListFromSortedKeys: len(keys) != len(values)")
	}
	m := &Int8Float64List{
		data: make(map[int8][]float64),
	}
...
// NewInt8Float64ListFromSortedKeyValues builds a Int8Float64List from input
...
// not sorted within each key, use Int8Float64ListBulkLoad instead.
func NewInt8Float64ListFromSortedKeyValues(keys []int8, values []float64) (*Int8Float64List, error) {
	if len(keys) != len(values) {
		panic("mapdb: NewInt8Float64ListFromSortedKeyValues: len(keys) != len(values)")
	}
	m := &Int8Float64List{
		data: make(map[int8][]float64),
	}
...
func (m *Int8Float64List) Put(key int8, value float64) {
	if m.data == nil {
...
func (m *Int8Float64List) Get(key int8) []float64 {
	return m.GetAll(key)
}

// GetAll returns a copy of the values for the given key.
func (m *Int8Float64List) GetAll(key int8) []float64 {
	vals := m.data[key]
	if vals == nil {
		return nil
	}
...
func (m *Int8Float64List) RemoveAll(key int8) []float64 {
	vals, ok := m.data[key]
	if !ok {
		return nil
	}
...
func (m *Int8Float64List) ContainsKey(key int8) bool {
	_, ok := m.data[key]
	return ok
}

// ContainsKeyValue returns true if the multimap contains the given key-value pair.
func (m *Int8Float64List) ContainsKeyValue(key int8, value float64) bool {
	vals, ok := m.data[key]
	if !ok {
		return false
	}
...
func (m *Int8Float64List) KeysCount() int {
	return len(m.data)
}

// Len returns the number of elements. Use m.Len() == 0 to test for emptiness.
func (m *Int8Float64List) Len() int {
	return m.size
}

// Clear removes all entries from the multimap.
func (m *Int8Float64List) Clear() {
	m.data = make(map[int8][]float64)
	m.size = 0
}

// ForEach calls the given function for each key-value pair.
func (m *Int8Float64List) ForEach(f func(int8, float64)) {
...
func (m *Int8Float64List) All() iter.Seq2[int8, float64] {
...
func (m *Int8Float64List) ForEachKeyValues(f func(int8, []float64)) {
	for key, vals := range m.data {
...
func (m *Int8Float64List) Keys() []int8 {
...
func (m *Int8Float64List) Values() []float64 {
...
func (m *Int8Float64List) Select(predicate func(int8, float64) bool) *Int8Float64List {
	result := NewInt8Float64List()
	for key, vals := range m.data {
		for _, val := range vals {
...
func (m *Int8Float64List) Reject(predicate func(int8, float64) bool) *Int8Float64List {
	result := NewInt8Float64List()
	for key, vals := range m.data {
		for _, val := range vals {
			if !predicate(key, val) {
...
func (m *Int8Float64List) String() string {
	if m.size == 0 {
		return "{}"
	}
...
func (m *Int8Float64List) Equals(other *Int8Float64List) bool {
...
func (m *Int8Float64List) KeysToSlice() []int8 {
	return m.Keys()
}

// ValuesToSlice returns all values as a slice.
func (m *Int8Float64List) ValuesToSlice() []float64 {
	return m.Values()
}

// PutReturning adds a key-value pair and returns the multimap (fluent API).
func (m *Int8Float64List) PutReturning(key int8, value float64) *Int8Float64List {
	m.Put(key, value)
	return m
}

// RemoveKeyReturning removes all values for the key and returns the multimap (fluent API).
func (m *Int8Float64List) RemoveKeyReturning(key int8) *Int8Float64List {
	m.RemoveAll(key)
	return m
}
