...
// CharOf creates a new Char and heapifies the given values in O(n).
func CharOf(values ...uint16) *Char {
	q := &Char{items: make([]uint16, len(values))}
	copy(q.items, values)
	if len(q.items) > 1 {
...
// for emptiness.
func (q *Char) Len() int { return len(q.items) }

// Clear removes all elements.
func (q *Char) Clear() { q.items = q.items[:0] }

// Contains returns true if the queue contains the given value. O(n).
func (q *Char) Contains(value uint16) bool {
...
func (q *Char) DrainSorted() []uint16 {
...
func (q *Char) less(a, b int) bool {
	return q.items[a] < q.items[b]
}

func (q *Char) siftUp(start int) {
	i := start
	for i > 0 {
		parent := (i - 1) / 2
		if q.less(i, parent) {
...
func (q *Char) siftDown(start int) {
	i := start
	n := len(q.items)
	for {
		left := 2*i + 1
