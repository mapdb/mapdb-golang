...
// Int64 is a min-heap priority queue of int64 values.
...
// Int64Of creates a new Int64 and heapifies the given values in O(n).
func Int64Of(values ...int64) *Int64 {
	q := &Int64{items: make([]int64, len(values))}
	copy(q.items, values)
	if len(q.items) > 1 {
...
// for emptiness.
func (q *Int64) Len() int { return len(q.items) }

// Clear removes all elements.
func (q *Int64) Clear() { q.items = q.items[:0] }

// Contains returns true if the queue contains the given value. O(n).
func (q *Int64) Contains(value int64) bool {
...
func (q *Int64) All() iter.Seq[int64] {
...
func (q *Int64) less(a, b int) bool {
	return q.items[a] < q.items[b]
}

func (q *Int64) siftUp(start int) {
	i := start
	for i > 0 {
		parent := (i - 1) / 2
		if q.less(i, parent) {
...
func (q *Int64) siftDown(start int) {
	i := start
	n := len(q.items)
	for {
		left := 2*i + 1
