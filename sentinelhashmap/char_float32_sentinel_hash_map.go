...
const (
	...
	charFloat32EmptyKey   = uint16(0)
	charFloat32RemovedKey = uint16(1)
)

// CharFloat32 is a sentinel-based open-addressing hash map with uint16 keys and float32 values.
...
// The size is a hint; use CharFloat32BulkLoadExact for the zero-rehash
// guarantee.
func CharFloat32BulkLoad(keys []uint16, values []float32, policy pump.DuplicatePolicy) (*CharFloat32, error) {
	...
	m := NewCharFloat32WithCapacity(CharFloat32bulkCap(len(keys)))
	for i := range keys {
...
func CharFloat32BulkLoadExact(keys []uint16, values []float32, n int, policy pump.DuplicatePolicy) (*CharFloat32, error) {
	...
	m := NewCharFloat32WithCapacity(CharFloat32bulkCap(n))
	if len(keys) > n {
...
// rehashes.
func (m *CharFloat32) bulkPut(key uint16, value float32, policy pump.DuplicatePolicy) error {
...
func (m *CharFloat32) putRegular(key uint16, value float32) (float32, bool) {
	if m.needsResize() {
	...
	empty := charFloat32EmptyKey
	removed := charFloat32RemovedKey
	firstRemoved := -1
...
func (m *CharFloat32) removeRegular(key uint16) (float32, bool) {
	cap := len(m.keys)
	if cap == 0 {
		return 0.0, false
	}
...
func (m *CharFloat32) All() iter.Seq2[uint16, float32] {
	return func(yield func(uint16, float32) bool) {
		...
		if m.oneKeyPresent {
			if !yield(charFloat32RemovedKey, m.oneKeyValue) {
				return
			}
		}
		empty := charFloat32EmptyKey
...
func (m *CharFloat32) Keys() iter.Seq[uint16] {
	return func(yield func(uint16) bool) {
		...
		if m.oneKeyPresent {
			if !yield(charFloat32RemovedKey) {
				return
			}
		}
		empty := charFloat32EmptyKey
...
func (m *CharFloat32) needsRehash() bool {
...
func (m *CharFloat32) resize(newCap int) {
	oldKeys := m.keys
	oldValues := m.values
	if newCap == 0 {
	...
	if savedOnePresent {
		m.Put(charFloat32RemovedKey, savedOneValue)
	}
