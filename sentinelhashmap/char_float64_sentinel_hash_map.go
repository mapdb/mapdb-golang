...
const (
	...
	charFloat64EmptyKey   = uint16(0)
	charFloat64RemovedKey = uint16(1)
)

// CharFloat64 is a sentinel-based open-addressing hash map with uint16 keys and float64 values.
...
// The size is a hint; use CharFloat64BulkLoadExact for the zero-rehash
// guarantee.
func CharFloat64BulkLoad(keys []uint16, values []float64, policy pump.DuplicatePolicy) (*CharFloat64, error) {
	...
	m := NewCharFloat64WithCapacity(CharFloat64bulkCap(len(keys)))
	for i := range keys {
...
func CharFloat64BulkLoadExact(keys []uint16, values []float64, n int, policy pump.DuplicatePolicy) (*CharFloat64, error) {
	...
	m := NewCharFloat64WithCapacity(CharFloat64bulkCap(n))
	if len(keys) > n {
...
// rehashes.
func (m *CharFloat64) bulkPut(key uint16, value float64, policy pump.DuplicatePolicy) error {
...
func (m *CharFloat64) putRegular(key uint16, value float64) (float64, bool) {
	if m.needsResize() {
	...
	empty := charFloat64EmptyKey
	removed := charFloat64RemovedKey
	firstRemoved := -1
...
func (m *CharFloat64) removeRegular(key uint16) (float64, bool) {
	cap := len(m.keys)
	if cap == 0 {
		return 0.0, false
	}
	...
	for probes := 0; probes < cap; probes++ {
		...
		if k == key {
			old := m.values[idx]
			m.keys[idx] = charFloat64RemovedKey
			m.values[idx] = 0.0
			m.size--
			m.tombstones++
			return old, true
...
func (m *CharFloat64) All() iter.Seq2[uint16, float64] {
	return func(yield func(uint16, float64) bool) {
		...
		if m.oneKeyPresent {
			if !yield(charFloat64RemovedKey, m.oneKeyValue) {
				return
			}
		}
		empty := charFloat64EmptyKey
...
func (m *CharFloat64) Keys() iter.Seq[uint16] {
	return func(yield func(uint16) bool) {
		...
		if m.oneKeyPresent {
			if !yield(charFloat64RemovedKey) {
				return
			}
		}
		empty := charFloat64EmptyKey
...
func (m *CharFloat64) needsRehash() bool {
...
func (m *CharFloat64) resize(newCap int) {
	oldKeys := m.keys
	oldValues := m.values
	if newCap == 0 {
	...
	if savedOnePresent {
		m.Put(charFloat64RemovedKey, savedOneValue)
	}
