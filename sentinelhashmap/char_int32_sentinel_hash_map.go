...
const (
	...
	charInt32EmptyKey   = uint16(0)
	charInt32RemovedKey = uint16(1)
)

// CharInt32 is a sentinel-based open-addressing hash map with uint16 keys and int32 values.
...
// The size is a hint; use CharInt32BulkLoadExact for the zero-rehash
// guarantee.
func CharInt32BulkLoad(keys []uint16, values []int32, policy pump.DuplicatePolicy) (*CharInt32, error) {
	...
	m := NewCharInt32WithCapacity(CharInt32bulkCap(len(keys)))
	for i := range keys {
...
// rehashes.
func (m *CharInt32) bulkPut(key uint16, value int32, policy pump.DuplicatePolicy) error {
...
func (m *CharInt32) putRegular(key uint16, value int32) (int32, bool) {
	if m.needsResize() {
	...
	empty := charInt32EmptyKey
	removed := charInt32RemovedKey
	firstRemoved := -1
...
func (m *CharInt32) removeRegular(key uint16) (int32, bool) {
	cap := len(m.keys)
	if cap == 0 {
		return 0, false
	}
	...
	for probes := 0; probes < cap; probes++ {
		...
		if k == key {
			old := m.values[idx]
			m.keys[idx] = charInt32RemovedKey
			m.values[idx] = 0
			m.size--
			m.tombstones++
			return old, true
...
func (m *CharInt32) All() iter.Seq2[uint16, int32] {
	return func(yield func(uint16, int32) bool) {
		...
		if m.oneKeyPresent {
			if !yield(charInt32RemovedKey, m.oneKeyValue) {
				return
			}
		}
		empty := charInt32EmptyKey
...
func (m *CharInt32) needsRehash() bool {
...
func (m *CharInt32) resize(newCap int) {
	oldKeys := m.keys
	oldValues := m.values
	if newCap == 0 {
	...
	if savedOnePresent {
		m.Put(charInt32RemovedKey, savedOneValue)
	}
