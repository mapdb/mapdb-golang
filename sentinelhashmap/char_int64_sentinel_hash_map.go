// Code generated by internal/codegen; DO NOT EDIT.

package sentinelhashmap

import (
	"fmt"
	"iter"
	"strings"

	"github.com/mapdb/mapdb-golang/pump"
)

const (
	charInt64DefaultCapacity = 16
	// Load factor 3/4 = 0.75, using integer math to avoid float conversion per insert.
	charInt64EmptyKey   = uint16(0)
	charInt64RemovedKey = uint16(1)
)

// CharInt64 is a sentinel-based open-addressing hash map with uint16 keys and int64 values.
// It uses sentinel values (0=empty, 1=removed) to track slot state.
// Keys 0 and 1 are stored separately in dedicated fields.
type CharInt64 struct {
	keys       []uint16
	values     []int64
	size       int
	tombstones int

	// Sentinel key storage — keys 0 and 1 are valid user keys but also
	// serve as empty/removed markers in the table, so we store them separately.
	zeroKeyPresent bool
	zeroKeyValue   int64
	oneKeyPresent  bool
	oneKeyValue    int64
}

// NewCharInt64 creates a new empty CharInt64 with default capacity.
func NewCharInt64() *CharInt64 {
	return NewCharInt64WithCapacity(charInt64DefaultCapacity)
}

// NewCharInt64WithCapacity creates a new empty CharInt64 with the given initial capacity.
func NewCharInt64WithCapacity(capacity int) *CharInt64 {
	cap := nextPowerOfTwoCharInt64(capacity)
	return &CharInt64{
		keys:   make([]uint16, cap),
		values: make([]int64, cap),
		size:   0,
	}
}

// CharInt64BulkLoad builds a CharInt64 from keys/values in a single pass,
// presizing the table once to fit len(keys) at the 0.75 load factor. keys[i] and
// values[i] form one entry (a length mismatch panics). The input need not be
// sorted. Sentinel keys (0/1/-0.0) are routed through their dedicated fields by
// the ordinary Put path, so the result is observably identical to one-by-one Put.
//
// On a duplicate key it returns pump.ErrDuplicateKey unless policy is
// pump.IgnoreDuplicates, in which case the first value for a key is kept.
// The size is a hint; use CharInt64BulkLoadExact for the zero-rehash
// guarantee.
func CharInt64BulkLoad(keys []uint16, values []int64, policy pump.DuplicatePolicy) (*CharInt64, error) {
	if len(keys) != len(values) {
		panic("mapdb: CharInt64BulkLoad: len(keys) != len(values)")
	}
	m := NewCharInt64WithCapacity(CharInt64bulkCap(len(keys)))
	for i := range keys {
		if err := m.bulkPut(keys[i], values[i], policy); err != nil {
			return nil, err
		}
	}
	return m, nil
}

// CharInt64BulkLoadExact is like CharInt64BulkLoad but guarantees zero
// mid-load rehash: the table is sized for exactly n consumed entries. It returns
// pump.ErrTooManyElements if the source yields more than n entries, even when
// the extra entries are duplicate keys skipped by pump.IgnoreDuplicates. n must
// be non-negative (negative panics).
func CharInt64BulkLoadExact(keys []uint16, values []int64, n int, policy pump.DuplicatePolicy) (*CharInt64, error) {
	if len(keys) != len(values) {
		panic("mapdb: CharInt64BulkLoadExact: len(keys) != len(values)")
	}
	if n < 0 {
		panic("mapdb: CharInt64BulkLoadExact: negative n")
	}
	m := NewCharInt64WithCapacity(CharInt64bulkCap(n))
	if len(keys) > n {
		return nil, pump.ErrTooManyElements
	}
	for i := range keys {
		if err := m.bulkPut(keys[i], values[i], policy); err != nil {
			return nil, err
		}
	}
	return m, nil
}

// bulkPut inserts a single entry, applying the duplicate policy. It reuses Put so
// the sentinel-key routing stays in one place; the presize means Put never
// rehashes.
func (m *CharInt64) bulkPut(key uint16, value int64, policy pump.DuplicatePolicy) error {
	if m.ContainsKey(key) {
		if policy == pump.IgnoreDuplicates {
			return nil
		}
		return pump.ErrDuplicateKey
	}
	m.Put(key, value)
	return nil
}

// CharInt64bulkCap returns the presized table capacity for n entries that
// avoids any mid-load rehash, floored at the family default.
func CharInt64bulkCap(n int) int {
	c := pump.HashCapacityFor(n)
	if c < charInt64DefaultCapacity {
		return charInt64DefaultCapacity
	}
	return c
}

// Put inserts or updates a key-value pair. Returns the previous value and true if the key existed.
func (m *CharInt64) Put(key uint16, value int64) (int64, bool) {
	if key == charInt64EmptyKey {
		old := m.zeroKeyValue
		existed := m.zeroKeyPresent
		m.zeroKeyValue = value
		if !m.zeroKeyPresent {
			m.zeroKeyPresent = true
			m.size++
		}
		return old, existed
	}
	if key == charInt64RemovedKey {
		old := m.oneKeyValue
		existed := m.oneKeyPresent
		m.oneKeyValue = value
		if !m.oneKeyPresent {
			m.oneKeyPresent = true
			m.size++
		}
		return old, existed
	}
	return m.putRegular(key, value)
}

func (m *CharInt64) putRegular(key uint16, value int64) (int64, bool) {
	if m.needsResize() {
		m.resize(len(m.keys) * 2)
	} else if m.needsRehash() {
		m.resize(len(m.keys))
	}
	cap := len(m.keys)
	mask := cap - 1
	idx := int(m.hashKey(key)) & mask
	empty := charInt64EmptyKey
	removed := charInt64RemovedKey
	firstRemoved := -1

	for probes := 0; probes < cap; probes++ {
		k := m.keys[idx]
		if k == empty {
			if firstRemoved >= 0 {
				idx = firstRemoved
				m.tombstones--
			}
			m.keys[idx] = key
			m.values[idx] = value
			m.size++
			return 0, false
		}
		if k == removed {
			if firstRemoved < 0 {
				firstRemoved = idx
			}
			idx = (idx + 1) & mask
			continue
		}
		if k == key {
			old := m.values[idx]
			m.values[idx] = value
			return old, true
		}
		idx = (idx + 1) & mask
	}
	// Defend against a full probe even if the occupancy invariant changes.
	// Reuse a removed slot or grow and retry.
	if firstRemoved >= 0 {
		m.keys[firstRemoved] = key
		m.values[firstRemoved] = value
		m.tombstones--
		m.size++
		return 0, false
	}
	m.resize(cap * 2)
	return m.putRegular(key, value)
}

// Get returns the value for the given key and true if found, or the zero value and false if not.
func (m *CharInt64) Get(key uint16) (int64, bool) {
	if key == charInt64EmptyKey {
		if m.zeroKeyPresent {
			return m.zeroKeyValue, true
		}
		return 0, false
	}
	if key == charInt64RemovedKey {
		if m.oneKeyPresent {
			return m.oneKeyValue, true
		}
		return 0, false
	}
	cap := len(m.keys)
	if cap == 0 {
		return 0, false
	}
	mask := cap - 1
	idx := int(m.hashKey(key)) & mask
	empty := charInt64EmptyKey

	for probes := 0; probes < cap; probes++ {
		k := m.keys[idx]
		if k == empty {
			return 0, false
		}
		if k == key {
			return m.values[idx], true
		}
		idx = (idx + 1) & mask
	}
	return 0, false
}

// GetOrDefault returns the value for the given key if present, or the default value otherwise.
func (m *CharInt64) GetOrDefault(key uint16, defaultValue int64) int64 {
	if v, ok := m.Get(key); ok {
		return v
	}
	return defaultValue
}

// Remove deletes the entry for the given key. Returns the previous value and true if the key existed.
func (m *CharInt64) Remove(key uint16) (int64, bool) {
	if key == charInt64EmptyKey {
		if m.zeroKeyPresent {
			old := m.zeroKeyValue
			m.zeroKeyPresent = false
			m.zeroKeyValue = 0
			m.size--
			return old, true
		}
		return 0, false
	}
	if key == charInt64RemovedKey {
		if m.oneKeyPresent {
			old := m.oneKeyValue
			m.oneKeyPresent = false
			m.oneKeyValue = 0
			m.size--
			return old, true
		}
		return 0, false
	}
	return m.removeRegular(key)
}

func (m *CharInt64) removeRegular(key uint16) (int64, bool) {
	cap := len(m.keys)
	if cap == 0 {
		return 0, false
	}
	mask := cap - 1
	idx := int(m.hashKey(key)) & mask
	empty := charInt64EmptyKey

	for probes := 0; probes < cap; probes++ {
		k := m.keys[idx]
		if k == empty {
			return 0, false
		}
		if k == key {
			old := m.values[idx]
			m.keys[idx] = charInt64RemovedKey
			m.values[idx] = 0
			m.size--
			m.tombstones++
			return old, true
		}
		idx = (idx + 1) & mask
	}
	return 0, false
}

// ContainsKey returns true if the map contains the given key.
func (m *CharInt64) ContainsKey(key uint16) bool {
	_, ok := m.Get(key)
	return ok
}

// ContainsValue returns true if the map contains the given value.
func (m *CharInt64) ContainsValue(value int64) bool {
	empty := charInt64EmptyKey
	removed := charInt64RemovedKey
	if m.zeroKeyPresent && m.zeroKeyValue == value {
		return true
	}
	if m.oneKeyPresent && m.oneKeyValue == value {
		return true
	}
	for i := range m.keys {
		if m.keys[i] != empty && m.keys[i] != removed && m.values[i] == value {
			return true
		}
	}
	return false
}

// Len returns the number of elements. Use m.Len() == 0 to test for emptiness.
func (m *CharInt64) Len() int {
	return m.size
}

// Clear removes all entries from the map.
func (m *CharInt64) Clear() {
	for i := range m.keys {
		m.keys[i] = 0
		m.values[i] = 0
	}
	m.zeroKeyPresent = false
	m.zeroKeyValue = 0
	m.oneKeyPresent = false
	m.oneKeyValue = 0
	m.size = 0
	m.tombstones = 0
}

// All returns an iter.Seq2 that yields all key-value pairs.
func (m *CharInt64) All() iter.Seq2[uint16, int64] {
	return func(yield func(uint16, int64) bool) {
		if m.zeroKeyPresent {
			if !yield(0, m.zeroKeyValue) {
				return
			}
		}
		if m.oneKeyPresent {
			if !yield(charInt64RemovedKey, m.oneKeyValue) {
				return
			}
		}
		empty := charInt64EmptyKey
		removed := charInt64RemovedKey
		for i := range m.keys {
			if m.keys[i] != empty && m.keys[i] != removed {
				if !yield(m.keys[i], m.values[i]) {
					return
				}
			}
		}
	}
}

// Keys returns an iter.Seq that yields all keys.
func (m *CharInt64) Keys() iter.Seq[uint16] {
	return func(yield func(uint16) bool) {
		if m.zeroKeyPresent {
			if !yield(0) {
				return
			}
		}
		if m.oneKeyPresent {
			if !yield(charInt64RemovedKey) {
				return
			}
		}
		empty := charInt64EmptyKey
		removed := charInt64RemovedKey
		for i := range m.keys {
			if m.keys[i] != empty && m.keys[i] != removed {
				if !yield(m.keys[i]) {
					return
				}
			}
		}
	}
}

// Values returns an iter.Seq that yields all values.
func (m *CharInt64) Values() iter.Seq[int64] {
	return func(yield func(int64) bool) {
		if m.zeroKeyPresent {
			if !yield(m.zeroKeyValue) {
				return
			}
		}
		if m.oneKeyPresent {
			if !yield(m.oneKeyValue) {
				return
			}
		}
		empty := charInt64EmptyKey
		removed := charInt64RemovedKey
		for i := range m.keys {
			if m.keys[i] != empty && m.keys[i] != removed {
				if !yield(m.values[i]) {
					return
				}
			}
		}
	}
}

// ForEach calls the given function for each key-value pair.
func (m *CharInt64) ForEach(f func(uint16, int64)) {
	for k, v := range m.All() {
		f(k, v)
	}
}

// Select returns a new map containing only the key-value pairs that satisfy the predicate.
func (m *CharInt64) Select(predicate func(uint16, int64) bool) *CharInt64 {
	result := NewCharInt64()
	for k, v := range m.All() {
		if predicate(k, v) {
			result.Put(k, v)
		}
	}
	return result
}

// Reject returns a new map containing only the key-value pairs that do not satisfy the predicate.
func (m *CharInt64) Reject(predicate func(uint16, int64) bool) *CharInt64 {
	result := NewCharInt64()
	for k, v := range m.All() {
		if !predicate(k, v) {
			result.Put(k, v)
		}
	}
	return result
}

// AnySatisfy returns true if any key-value pair satisfies the predicate.
func (m *CharInt64) AnySatisfy(predicate func(uint16, int64) bool) bool {
	for k, v := range m.All() {
		if predicate(k, v) {
			return true
		}
	}
	return false
}

// AllSatisfy returns true if all key-value pairs satisfy the predicate.
func (m *CharInt64) AllSatisfy(predicate func(uint16, int64) bool) bool {
	for k, v := range m.All() {
		if !predicate(k, v) {
			return false
		}
	}
	return true
}

// NoneSatisfy returns true if no key-value pair satisfies the predicate.
func (m *CharInt64) NoneSatisfy(predicate func(uint16, int64) bool) bool {
	return !m.AnySatisfy(predicate)
}

// String returns a string representation of the map.
func (m *CharInt64) String() string {
	if m.size == 0 {
		return "{}"
	}
	var sb strings.Builder
	sb.WriteString("{")
	first := true
	for k, v := range m.All() {
		if !first {
			sb.WriteString(", ")
		}
		fmt.Fprintf(&sb, "%v: %v", k, v)
		first = false
	}
	sb.WriteString("}")
	return sb.String()
}

func (m *CharInt64) hashKey(key uint16) uint64 {
	h := uint64(key) * 0x9E3779B97F4A7C15
	return h ^ (h >> 32)
}

func (m *CharInt64) needsResize() bool {
	// Count only regular entries (not sentinel entries) for load factor
	regularEntries := m.size
	if m.zeroKeyPresent {
		regularEntries--
	}
	if m.oneKeyPresent {
		regularEntries--
	}
	return (regularEntries+1)*4 >= len(m.keys)*3 // 0.75 load factor, integer math
}

func (m *CharInt64) needsRehash() bool {
	// Rebuild at 7/8 physical occupancy. The gap above the 3/4 live growth
	// threshold leaves proportional room for tombstones under stable-size churn.
	regularEntries := m.size
	if m.zeroKeyPresent {
		regularEntries--
	}
	if m.oneKeyPresent {
		regularEntries--
	}
	return (regularEntries+m.tombstones+1)*8 >= len(m.keys)*7
}

func (m *CharInt64) resize(newCap int) {
	oldKeys := m.keys
	oldValues := m.values
	if newCap == 0 {
		newCap = charInt64DefaultCapacity
	}

	// Save sentinel state
	savedZeroPresent := m.zeroKeyPresent
	savedZeroValue := m.zeroKeyValue
	savedOnePresent := m.oneKeyPresent
	savedOneValue := m.oneKeyValue

	m.keys = make([]uint16, newCap)
	m.values = make([]int64, newCap)
	m.size = 0
	m.tombstones = 0
	m.zeroKeyPresent = false
	m.oneKeyPresent = false

	// Re-insert sentinel entries
	if savedZeroPresent {
		m.Put(0, savedZeroValue)
	}
	if savedOnePresent {
		m.Put(charInt64RemovedKey, savedOneValue)
	}

	// Re-insert regular entries
	empty := charInt64EmptyKey
	removed := charInt64RemovedKey
	for i := range oldKeys {
		if oldKeys[i] != empty && oldKeys[i] != removed {
			m.Put(oldKeys[i], oldValues[i])
		}
	}
}

func nextPowerOfTwoCharInt64(n int) int {
	if n <= 0 {
		return 16
	}
	n--
	n |= n >> 1
	n |= n >> 2
	n |= n >> 4
	n |= n >> 8
	n |= n >> 16
	n |= n >> 32 // no-op on 32-bit platforms (Go shifts are width-defined), required on 64-bit
	n++
	return n
}
