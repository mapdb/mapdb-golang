// Code generated by internal/codegen; DO NOT EDIT.

package sentinelhashmap

import (
	"fmt"
	"iter"
	"math"
	"strings"

	"github.com/mapdb/mapdb-golang/pump"
)

const (
	float32Int32DefaultCapacity = 16
	// Load factor 3/4 = 0.75, using integer math to avoid float conversion per insert.
	float32Int32EmptyKey   = float32(0)
	float32Int32RemovedKey = float32(1)
	// float32Int32NegZeroBits is the IEEE-754 bit pattern of -0.0, routed to a
	// dedicated field so -0.0 stays distinct from +0.0 (which collides with
	// the empty sentinel) and from the table.
	float32Int32NegZeroBits = uint32(0x80000000)
)

// Float32Int32 is a sentinel-based open-addressing hash map with float32 keys and int32 values.
// It uses sentinel values (0=empty, 1=removed) to track slot state.
// Keys 0 and 1 are stored separately in dedicated fields.
type Float32Int32 struct {
	keys       []float32
	values     []int32
	size       int
	tombstones int

	// Sentinel key storage — keys 0 and 1 are valid user keys but also
	// serve as empty/removed markers in the table, so we store them separately.
	zeroKeyPresent    bool
	zeroKeyValue      int32
	negZeroKeyPresent bool
	negZeroKeyValue   int32
	oneKeyPresent     bool
	oneKeyValue       int32
}

// NewFloat32Int32 creates a new empty Float32Int32 with default capacity.
func NewFloat32Int32() *Float32Int32 {
	return NewFloat32Int32WithCapacity(float32Int32DefaultCapacity)
}

// NewFloat32Int32WithCapacity creates a new empty Float32Int32 with the given initial capacity.
func NewFloat32Int32WithCapacity(capacity int) *Float32Int32 {
	cap := nextPowerOfTwoFloat32Int32(capacity)
	return &Float32Int32{
		keys:   make([]float32, cap),
		values: make([]int32, cap),
		size:   0,
	}
}

// Float32Int32BulkLoad builds a Float32Int32 from keys/values in a single pass,
// presizing the table once to fit len(keys) at the 0.75 load factor. keys[i] and
// values[i] form one entry (a length mismatch panics). The input need not be
// sorted. Sentinel keys (0/1/-0.0) are routed through their dedicated fields by
// the ordinary Put path, so the result is observably identical to one-by-one Put.
//
// On a duplicate key it returns pump.ErrDuplicateKey unless policy is
// pump.IgnoreDuplicates, in which case the first value for a key is kept.
// The size is a hint; use Float32Int32BulkLoadExact for the zero-rehash
// guarantee.
func Float32Int32BulkLoad(keys []float32, values []int32, policy pump.DuplicatePolicy) (*Float32Int32, error) {
	if len(keys) != len(values) {
		panic("mapdb: Float32Int32BulkLoad: len(keys) != len(values)")
	}
	m := NewFloat32Int32WithCapacity(Float32Int32bulkCap(len(keys)))
	for i := range keys {
		if err := m.bulkPut(keys[i], values[i], policy); err != nil {
			return nil, err
		}
	}
	return m, nil
}

// Float32Int32BulkLoadExact is like Float32Int32BulkLoad but guarantees zero
// mid-load rehash: the table is sized for exactly n consumed entries. It returns
// pump.ErrTooManyElements if the source yields more than n entries, even when
// the extra entries are duplicate keys skipped by pump.IgnoreDuplicates. n must
// be non-negative (negative panics).
func Float32Int32BulkLoadExact(keys []float32, values []int32, n int, policy pump.DuplicatePolicy) (*Float32Int32, error) {
	if len(keys) != len(values) {
		panic("mapdb: Float32Int32BulkLoadExact: len(keys) != len(values)")
	}
	if n < 0 {
		panic("mapdb: Float32Int32BulkLoadExact: negative n")
	}
	m := NewFloat32Int32WithCapacity(Float32Int32bulkCap(n))
	if len(keys) > n {
		return nil, pump.ErrTooManyElements
	}
	for i := range keys {
		if err := m.bulkPut(keys[i], values[i], policy); err != nil {
			return nil, err
		}
	}
	return m, nil
}

// bulkPut inserts a single entry, applying the duplicate policy. It reuses Put so
// the sentinel-key routing stays in one place; the presize means Put never
// rehashes.
func (m *Float32Int32) bulkPut(key float32, value int32, policy pump.DuplicatePolicy) error {
	if m.ContainsKey(key) {
		if policy == pump.IgnoreDuplicates {
			return nil
		}
		return pump.ErrDuplicateKey
	}
	m.Put(key, value)
	return nil
}

// Float32Int32bulkCap returns the presized table capacity for n entries that
// avoids any mid-load rehash, floored at the family default.
func Float32Int32bulkCap(n int) int {
	c := pump.HashCapacityFor(n)
	if c < float32Int32DefaultCapacity {
		return float32Int32DefaultCapacity
	}
	return c
}

// Put inserts or updates a key-value pair. Returns the previous value and true if the key existed.
func (m *Float32Int32) Put(key float32, value int32) (int32, bool) {
	if math.Float32bits(key) == math.Float32bits(float32Int32EmptyKey) {
		old := m.zeroKeyValue
		existed := m.zeroKeyPresent
		m.zeroKeyValue = value
		if !m.zeroKeyPresent {
			m.zeroKeyPresent = true
			m.size++
		}
		return old, existed
	}
	if math.Float32bits(key) == float32Int32NegZeroBits {
		old := m.negZeroKeyValue
		existed := m.negZeroKeyPresent
		m.negZeroKeyValue = value
		if !m.negZeroKeyPresent {
			m.negZeroKeyPresent = true
			m.size++
		}
		return old, existed
	}
	if key == float32Int32RemovedKey {
		old := m.oneKeyValue
		existed := m.oneKeyPresent
		m.oneKeyValue = value
		if !m.oneKeyPresent {
			m.oneKeyPresent = true
			m.size++
		}
		return old, existed
	}
	return m.putRegular(key, value)
}

func (m *Float32Int32) putRegular(key float32, value int32) (int32, bool) {
	if m.needsResize() {
		m.resize(len(m.keys) * 2)
	} else if m.needsRehash() {
		m.resize(len(m.keys))
	}
	cap := len(m.keys)
	mask := cap - 1
	idx := int(m.hashKey(key)) & mask
	empty := float32Int32EmptyKey
	removed := float32Int32RemovedKey
	firstRemoved := -1

	for probes := 0; probes < cap; probes++ {
		k := m.keys[idx]
		if k == empty {
			if firstRemoved >= 0 {
				idx = firstRemoved
				m.tombstones--
			}
			m.keys[idx] = key
			m.values[idx] = value
			m.size++
			return 0, false
		}
		if k == removed {
			if firstRemoved < 0 {
				firstRemoved = idx
			}
			idx = (idx + 1) & mask
			continue
		}
		if math.Float32bits(k) == math.Float32bits(key) {
			old := m.values[idx]
			m.values[idx] = value
			return old, true
		}
		idx = (idx + 1) & mask
	}
	// Defend against a full probe even if the occupancy invariant changes.
	// Reuse a removed slot or grow and retry.
	if firstRemoved >= 0 {
		m.keys[firstRemoved] = key
		m.values[firstRemoved] = value
		m.tombstones--
		m.size++
		return 0, false
	}
	m.resize(cap * 2)
	return m.putRegular(key, value)
}

// Get returns the value for the given key and true if found, or the zero value and false if not.
func (m *Float32Int32) Get(key float32) (int32, bool) {
	if math.Float32bits(key) == math.Float32bits(float32Int32EmptyKey) {
		if m.zeroKeyPresent {
			return m.zeroKeyValue, true
		}
		return 0, false
	}
	if math.Float32bits(key) == float32Int32NegZeroBits {
		if m.negZeroKeyPresent {
			return m.negZeroKeyValue, true
		}
		return 0, false
	}
	if key == float32Int32RemovedKey {
		if m.oneKeyPresent {
			return m.oneKeyValue, true
		}
		return 0, false
	}
	cap := len(m.keys)
	if cap == 0 {
		return 0, false
	}
	mask := cap - 1
	idx := int(m.hashKey(key)) & mask
	empty := float32Int32EmptyKey

	for probes := 0; probes < cap; probes++ {
		k := m.keys[idx]
		if k == empty {
			return 0, false
		}
		if math.Float32bits(k) == math.Float32bits(key) {
			return m.values[idx], true
		}
		idx = (idx + 1) & mask
	}
	return 0, false
}

// GetOrDefault returns the value for the given key if present, or the default value otherwise.
func (m *Float32Int32) GetOrDefault(key float32, defaultValue int32) int32 {
	if v, ok := m.Get(key); ok {
		return v
	}
	return defaultValue
}

// Remove deletes the entry for the given key. Returns the previous value and true if the key existed.
func (m *Float32Int32) Remove(key float32) (int32, bool) {
	if math.Float32bits(key) == math.Float32bits(float32Int32EmptyKey) {
		if m.zeroKeyPresent {
			old := m.zeroKeyValue
			m.zeroKeyPresent = false
			m.zeroKeyValue = 0
			m.size--
			return old, true
		}
		return 0, false
	}
	if math.Float32bits(key) == float32Int32NegZeroBits {
		if m.negZeroKeyPresent {
			old := m.negZeroKeyValue
			m.negZeroKeyPresent = false
			m.negZeroKeyValue = 0
			m.size--
			return old, true
		}
		return 0, false
	}
	if key == float32Int32RemovedKey {
		if m.oneKeyPresent {
			old := m.oneKeyValue
			m.oneKeyPresent = false
			m.oneKeyValue = 0
			m.size--
			return old, true
		}
		return 0, false
	}
	return m.removeRegular(key)
}

func (m *Float32Int32) removeRegular(key float32) (int32, bool) {
	cap := len(m.keys)
	if cap == 0 {
		return 0, false
	}
	mask := cap - 1
	idx := int(m.hashKey(key)) & mask
	empty := float32Int32EmptyKey

	for probes := 0; probes < cap; probes++ {
		k := m.keys[idx]
		if k == empty {
			return 0, false
		}
		if math.Float32bits(k) == math.Float32bits(key) {
			old := m.values[idx]
			m.keys[idx] = float32Int32RemovedKey
			m.values[idx] = 0
			m.size--
			m.tombstones++
			return old, true
		}
		idx = (idx + 1) & mask
	}
	return 0, false
}

// ContainsKey returns true if the map contains the given key.
func (m *Float32Int32) ContainsKey(key float32) bool {
	_, ok := m.Get(key)
	return ok
}

// ContainsValue returns true if the map contains the given value.
func (m *Float32Int32) ContainsValue(value int32) bool {
	empty := float32Int32EmptyKey
	removed := float32Int32RemovedKey
	if m.zeroKeyPresent && m.zeroKeyValue == value {
		return true
	}
	if m.negZeroKeyPresent && m.negZeroKeyValue == value {
		return true
	}
	if m.oneKeyPresent && m.oneKeyValue == value {
		return true
	}
	for i := range m.keys {
		if m.keys[i] != empty && m.keys[i] != removed && m.values[i] == value {
			return true
		}
	}
	return false
}

// Len returns the number of elements. Use m.Len() == 0 to test for emptiness.
func (m *Float32Int32) Len() int {
	return m.size
}

// Clear removes all entries from the map.
func (m *Float32Int32) Clear() {
	for i := range m.keys {
		m.keys[i] = 0.0
		m.values[i] = 0
	}
	m.zeroKeyPresent = false
	m.zeroKeyValue = 0
	m.negZeroKeyPresent = false
	m.negZeroKeyValue = 0
	m.oneKeyPresent = false
	m.oneKeyValue = 0
	m.size = 0
	m.tombstones = 0
}

// All returns an iter.Seq2 that yields all key-value pairs.
func (m *Float32Int32) All() iter.Seq2[float32, int32] {
	return func(yield func(float32, int32) bool) {
		if m.zeroKeyPresent {
			if !yield(0.0, m.zeroKeyValue) {
				return
			}
		}
		if m.negZeroKeyPresent {
			if !yield(float32(math.Copysign(0, -1)), m.negZeroKeyValue) {
				return
			}
		}
		if m.oneKeyPresent {
			if !yield(float32Int32RemovedKey, m.oneKeyValue) {
				return
			}
		}
		empty := float32Int32EmptyKey
		removed := float32Int32RemovedKey
		for i := range m.keys {
			if m.keys[i] != empty && m.keys[i] != removed {
				if !yield(m.keys[i], m.values[i]) {
					return
				}
			}
		}
	}
}

// Keys returns an iter.Seq that yields all keys.
func (m *Float32Int32) Keys() iter.Seq[float32] {
	return func(yield func(float32) bool) {
		if m.zeroKeyPresent {
			if !yield(0.0) {
				return
			}
		}
		if m.negZeroKeyPresent {
			if !yield(float32(math.Copysign(0, -1))) {
				return
			}
		}
		if m.oneKeyPresent {
			if !yield(float32Int32RemovedKey) {
				return
			}
		}
		empty := float32Int32EmptyKey
		removed := float32Int32RemovedKey
		for i := range m.keys {
			if m.keys[i] != empty && m.keys[i] != removed {
				if !yield(m.keys[i]) {
					return
				}
			}
		}
	}
}

// Values returns an iter.Seq that yields all values.
func (m *Float32Int32) Values() iter.Seq[int32] {
	return func(yield func(int32) bool) {
		if m.zeroKeyPresent {
			if !yield(m.zeroKeyValue) {
				return
			}
		}
		if m.negZeroKeyPresent {
			if !yield(m.negZeroKeyValue) {
				return
			}
		}
		if m.oneKeyPresent {
			if !yield(m.oneKeyValue) {
				return
			}
		}
		empty := float32Int32EmptyKey
		removed := float32Int32RemovedKey
		for i := range m.keys {
			if m.keys[i] != empty && m.keys[i] != removed {
				if !yield(m.values[i]) {
					return
				}
			}
		}
	}
}

// ForEach calls the given function for each key-value pair.
func (m *Float32Int32) ForEach(f func(float32, int32)) {
	for k, v := range m.All() {
		f(k, v)
	}
}

// Select returns a new map containing only the key-value pairs that satisfy the predicate.
func (m *Float32Int32) Select(predicate func(float32, int32) bool) *Float32Int32 {
	result := NewFloat32Int32()
	for k, v := range m.All() {
		if predicate(k, v) {
			result.Put(k, v)
		}
	}
	return result
}

// Reject returns a new map containing only the key-value pairs that do not satisfy the predicate.
func (m *Float32Int32) Reject(predicate func(float32, int32) bool) *Float32Int32 {
	result := NewFloat32Int32()
	for k, v := range m.All() {
		if !predicate(k, v) {
			result.Put(k, v)
		}
	}
	return result
}

// AnySatisfy returns true if any key-value pair satisfies the predicate.
func (m *Float32Int32) AnySatisfy(predicate func(float32, int32) bool) bool {
	for k, v := range m.All() {
		if predicate(k, v) {
			return true
		}
	}
	return false
}

// AllSatisfy returns true if all key-value pairs satisfy the predicate.
func (m *Float32Int32) AllSatisfy(predicate func(float32, int32) bool) bool {
	for k, v := range m.All() {
		if !predicate(k, v) {
			return false
		}
	}
	return true
}

// NoneSatisfy returns true if no key-value pair satisfies the predicate.
func (m *Float32Int32) NoneSatisfy(predicate func(float32, int32) bool) bool {
	return !m.AnySatisfy(predicate)
}

// String returns a string representation of the map.
func (m *Float32Int32) String() string {
	if m.size == 0 {
		return "{}"
	}
	var sb strings.Builder
	sb.WriteString("{")
	first := true
	for k, v := range m.All() {
		if !first {
			sb.WriteString(", ")
		}
		fmt.Fprintf(&sb, "%v: %v", k, v)
		first = false
	}
	sb.WriteString("}")
	return sb.String()
}

func (m *Float32Int32) hashKey(key float32) uint64 {
	h := uint64(math.Float32bits(key)) * 0x9E3779B97F4A7C15
	return h ^ (h >> 32)
}

func (m *Float32Int32) needsResize() bool {
	// Count only regular entries (not sentinel entries) for load factor
	regularEntries := m.size
	if m.zeroKeyPresent {
		regularEntries--
	}
	if m.negZeroKeyPresent {
		regularEntries--
	}
	if m.oneKeyPresent {
		regularEntries--
	}
	return (regularEntries+1)*4 >= len(m.keys)*3 // 0.75 load factor, integer math
}

func (m *Float32Int32) needsRehash() bool {
	// Rebuild at 7/8 physical occupancy. The gap above the 3/4 live growth
	// threshold leaves proportional room for tombstones under stable-size churn.
	regularEntries := m.size
	if m.zeroKeyPresent {
		regularEntries--
	}
	if m.negZeroKeyPresent {
		regularEntries--
	}
	if m.oneKeyPresent {
		regularEntries--
	}
	return (regularEntries+m.tombstones+1)*8 >= len(m.keys)*7
}

func (m *Float32Int32) resize(newCap int) {
	oldKeys := m.keys
	oldValues := m.values
	if newCap == 0 {
		newCap = float32Int32DefaultCapacity
	}

	// Save sentinel state
	savedZeroPresent := m.zeroKeyPresent
	savedZeroValue := m.zeroKeyValue
	savedNegZeroPresent := m.negZeroKeyPresent
	savedNegZeroValue := m.negZeroKeyValue
	savedOnePresent := m.oneKeyPresent
	savedOneValue := m.oneKeyValue

	m.keys = make([]float32, newCap)
	m.values = make([]int32, newCap)
	m.size = 0
	m.tombstones = 0
	m.zeroKeyPresent = false
	m.negZeroKeyPresent = false
	m.oneKeyPresent = false

	// Re-insert sentinel entries
	if savedZeroPresent {
		m.Put(0.0, savedZeroValue)
	}
	if savedNegZeroPresent {
		m.Put(float32(math.Copysign(0, -1)), savedNegZeroValue)
	}
	if savedOnePresent {
		m.Put(float32Int32RemovedKey, savedOneValue)
	}

	// Re-insert regular entries
	empty := float32Int32EmptyKey
	removed := float32Int32RemovedKey
	for i := range oldKeys {
		if oldKeys[i] != empty && oldKeys[i] != removed {
			m.Put(oldKeys[i], oldValues[i])
		}
	}
}

func nextPowerOfTwoFloat32Int32(n int) int {
	if n <= 0 {
		return 16
	}
	n--
	n |= n >> 1
	n |= n >> 2
	n |= n >> 4
	n |= n >> 8
	n |= n >> 16
	n |= n >> 32 // no-op on 32-bit platforms (Go shifts are width-defined), required on 64-bit
	n++
	return n
}
