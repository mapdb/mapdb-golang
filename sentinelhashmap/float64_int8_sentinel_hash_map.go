...
const (
	...
	float64Int8EmptyKey   = float64(0)
	float64Int8RemovedKey = float64(1)
	// float64Int8NegZeroBits is the IEEE-754 bit pattern of -0.0, routed to a
	// dedicated field so -0.0 stays distinct from +0.0 (which collides with
	// the empty sentinel) and from the table.
	float64Int8NegZeroBits = uint64(0x8000000000000000)
)

// Float64Int8 is a sentinel-based open-addressing hash map with float64 keys and int8 values.
...
// Float64Int8BulkLoad builds a Float64Int8 from keys/values in a single pass,
...
// The size is a hint; use Float64Int8BulkLoadExact for the zero-rehash
// guarantee.
func Float64Int8BulkLoad(keys []float64, values []int8, policy pump.DuplicatePolicy) (*Float64Int8, error) {
	if len(keys) != len(values) {
		panic("mapdb: Float64Int8BulkLoad: len(keys) != len(values)")
	}
	m := NewFloat64Int8WithCapacity(Float64Int8bulkCap(len(keys)))
	for i := range keys {
...
// Float64Int8BulkLoadExact is like Float64Int8BulkLoad but guarantees zero
...
func Float64Int8BulkLoadExact(keys []float64, values []int8, n int, policy pump.DuplicatePolicy) (*Float64Int8, error) {
	if len(keys) != len(values) {
		panic("mapdb: Float64Int8BulkLoadExact: len(keys) != len(values)")
	}
	if n < 0 {
		panic("mapdb: Float64Int8BulkLoadExact: negative n")
	}
	m := NewFloat64Int8WithCapacity(Float64Int8bulkCap(n))
	if len(keys) > n {
...
// rehashes.
func (m *Float64Int8) bulkPut(key float64, value int8, policy pump.DuplicatePolicy) error {
...
// Float64Int8bulkCap returns the presized table capacity for n entries that
// avoids any mid-load rehash, floored at the family default.
func Float64Int8bulkCap(n int) int {
	c := pump.HashCapacityFor(n)
	if c < float64Int8DefaultCapacity {
		return float64Int8DefaultCapacity
	}
	return c
}
...
func (m *Float64Int8) Put(key float64, value int8) (int8, bool) {
	if math.Float64bits(key) == math.Float64bits(float64Int8EmptyKey) {
		old := m.zeroKeyValue
	...
	if math.Float64bits(key) == float64Int8NegZeroBits {
...
func (m *Float64Int8) putRegular(key float64, value int8) (int8, bool) {
	if m.needsResize() {
	...
	empty := float64Int8EmptyKey
	removed := float64Int8RemovedKey
	firstRemoved := -1
...
func (m *Float64Int8) removeRegular(key float64) (int8, bool) {
	cap := len(m.keys)
	if cap == 0 {
		return 0, false
	}
	...
	for probes := 0; probes < cap; probes++ {
		...
		if math.Float64bits(k) == math.Float64bits(key) {
			old := m.values[idx]
			m.keys[idx] = float64Int8RemovedKey
			m.values[idx] = 0
			m.size--
			m.tombstones++
			return old, true
...
func (m *Float64Int8) All() iter.Seq2[float64, int8] {
	return func(yield func(float64, int8) bool) {
		...
		if m.oneKeyPresent {
			if !yield(float64Int8RemovedKey, m.oneKeyValue) {
				return
			}
		}
		empty := float64Int8EmptyKey
...
func (m *Float64Int8) Keys() iter.Seq[float64] {
	return func(yield func(float64) bool) {
		...
		if m.oneKeyPresent {
			if !yield(float64Int8RemovedKey) {
				return
			}
		}
		empty := float64Int8EmptyKey
...
func (m *Float64Int8) needsRehash() bool {
...
func (m *Float64Int8) resize(newCap int) {
	oldKeys := m.keys
	oldValues := m.values
	if newCap == 0 {
	...
	if savedOnePresent {
		m.Put(float64Int8RemovedKey, savedOneValue)
	}
