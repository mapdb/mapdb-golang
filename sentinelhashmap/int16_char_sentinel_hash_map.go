...
const (
	...
	int16CharEmptyKey   = int16(0)
	int16CharRemovedKey = int16(1)
)

// Int16Char is a sentinel-based open-addressing hash map with int16 keys and uint16 values.
...
// The size is a hint; use Int16CharBulkLoadExact for the zero-rehash
// guarantee.
func Int16CharBulkLoad(keys []int16, values []uint16, policy pump.DuplicatePolicy) (*Int16Char, error) {
	...
	m := NewInt16CharWithCapacity(Int16CharbulkCap(len(keys)))
	for i := range keys {
...
func Int16CharBulkLoadExact(keys []int16, values []uint16, n int, policy pump.DuplicatePolicy) (*Int16Char, error) {
	...
	m := NewInt16CharWithCapacity(Int16CharbulkCap(n))
	if len(keys) > n {
...
// rehashes.
func (m *Int16Char) bulkPut(key int16, value uint16, policy pump.DuplicatePolicy) error {
...
func (m *Int16Char) putRegular(key int16, value uint16) (uint16, bool) {
	if m.needsResize() {
	...
	empty := int16CharEmptyKey
	removed := int16CharRemovedKey
	firstRemoved := -1
...
func (m *Int16Char) removeRegular(key int16) (uint16, bool) {
	cap := len(m.keys)
	if cap == 0 {
		return 0, false
	}
	...
	for probes := 0; probes < cap; probes++ {
		...
		if k == key {
			old := m.values[idx]
			m.keys[idx] = int16CharRemovedKey
			m.values[idx] = 0
			m.size--
			m.tombstones++
			return old, true
...
func (m *Int16Char) All() iter.Seq2[int16, uint16] {
	return func(yield func(int16, uint16) bool) {
		...
		if m.oneKeyPresent {
			if !yield(int16CharRemovedKey, m.oneKeyValue) {
				return
			}
		}
		empty := int16CharEmptyKey
...
func (m *Int16Char) needsRehash() bool {
...
func (m *Int16Char) resize(newCap int) {
	oldKeys := m.keys
	oldValues := m.values
	if newCap == 0 {
	...
	if savedOnePresent {
		m.Put(int16CharRemovedKey, savedOneValue)
	}
