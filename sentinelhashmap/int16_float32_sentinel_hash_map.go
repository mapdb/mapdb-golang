...
const (
	...
	int16Float32EmptyKey   = int16(0)
	int16Float32RemovedKey = int16(1)
)

// Int16Float32 is a sentinel-based open-addressing hash map with int16 keys and float32 values.
...
// Int16Float32BulkLoad builds a Int16Float32 from keys/values in a single pass,
...
// The size is a hint; use Int16Float32BulkLoadExact for the zero-rehash
// guarantee.
func Int16Float32BulkLoad(keys []int16, values []float32, policy pump.DuplicatePolicy) (*Int16Float32, error) {
	if len(keys) != len(values) {
		panic("mapdb: Int16Float32BulkLoad: len(keys) != len(values)")
	}
	m := NewInt16Float32WithCapacity(Int16Float32bulkCap(len(keys)))
	for i := range keys {
...
// Int16Float32BulkLoadExact is like Int16Float32BulkLoad but guarantees zero
...
func Int16Float32BulkLoadExact(keys []int16, values []float32, n int, policy pump.DuplicatePolicy) (*Int16Float32, error) {
	if len(keys) != len(values) {
		panic("mapdb: Int16Float32BulkLoadExact: len(keys) != len(values)")
	}
	if n < 0 {
		panic("mapdb: Int16Float32BulkLoadExact: negative n")
	}
	m := NewInt16Float32WithCapacity(Int16Float32bulkCap(n))
	if len(keys) > n {
...
// rehashes.
func (m *Int16Float32) bulkPut(key int16, value float32, policy pump.DuplicatePolicy) error {
...
// Int16Float32bulkCap returns the presized table capacity for n entries that
// avoids any mid-load rehash, floored at the family default.
func Int16Float32bulkCap(n int) int {
	c := pump.HashCapacityFor(n)
	if c < int16Float32DefaultCapacity {
		return int16Float32DefaultCapacity
	}
	return c
}
...
func (m *Int16Float32) putRegular(key int16, value float32) (float32, bool) {
	if m.needsResize() {
...
func (m *Int16Float32) removeRegular(key int16) (float32, bool) {
	cap := len(m.keys)
	if cap == 0 {
		return 0.0, false
	}
	...
	for probes := 0; probes < cap; probes++ {
		...
		if k == key {
			old := m.values[idx]
			m.keys[idx] = int16Float32RemovedKey
			m.values[idx] = 0.0
			m.size--
			m.tombstones++
			return old, true
...
func (m *Int16Float32) All() iter.Seq2[int16, float32] {
	return func(yield func(int16, float32) bool) {
		...
		if m.oneKeyPresent {
			if !yield(int16Float32RemovedKey, m.oneKeyValue) {
				return
			}
		}
		empty := int16Float32EmptyKey
...
func (m *Int16Float32) Keys() iter.Seq[int16] {
	return func(yield func(int16) bool) {
		...
		if m.oneKeyPresent {
			if !yield(int16Float32RemovedKey) {
				return
			}
		}
		empty := int16Float32EmptyKey
...
func (m *Int16Float32) resize(newCap int) {
	oldKeys := m.keys
	oldValues := m.values
	if newCap == 0 {
	...
	if savedOnePresent {
		m.Put(int16Float32RemovedKey, savedOneValue)
	}
