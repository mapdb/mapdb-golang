...
const (
	...
	int16Int32EmptyKey   = int16(0)
	int16Int32RemovedKey = int16(1)
)

// Int16Int32 is a sentinel-based open-addressing hash map with int16 keys and int32 values.
...
// The size is a hint; use Int16Int32BulkLoadExact for the zero-rehash
// guarantee.
func Int16Int32BulkLoad(keys []int16, values []int32, policy pump.DuplicatePolicy) (*Int16Int32, error) {
	...
	m := NewInt16Int32WithCapacity(Int16Int32bulkCap(len(keys)))
	for i := range keys {
...
func Int16Int32BulkLoadExact(keys []int16, values []int32, n int, policy pump.DuplicatePolicy) (*Int16Int32, error) {
	...
	m := NewInt16Int32WithCapacity(Int16Int32bulkCap(n))
	if len(keys) > n {
...
// rehashes.
func (m *Int16Int32) bulkPut(key int16, value int32, policy pump.DuplicatePolicy) error {
...
func (m *Int16Int32) putRegular(key int16, value int32) (int32, bool) {
	if m.needsResize() {
	...
	empty := int16Int32EmptyKey
	removed := int16Int32RemovedKey
	firstRemoved := -1
...
func (m *Int16Int32) removeRegular(key int16) (int32, bool) {
	cap := len(m.keys)
	if cap == 0 {
		return 0, false
	}
	...
	for probes := 0; probes < cap; probes++ {
		...
		if k == key {
			old := m.values[idx]
			m.keys[idx] = int16Int32RemovedKey
			m.values[idx] = 0
			m.size--
			m.tombstones++
			return old, true
...
func (m *Int16Int32) All() iter.Seq2[int16, int32] {
	return func(yield func(int16, int32) bool) {
		...
		if m.oneKeyPresent {
			if !yield(int16Int32RemovedKey, m.oneKeyValue) {
				return
			}
		}
		empty := int16Int32EmptyKey
...
func (m *Int16Int32) needsRehash() bool {
...
func (m *Int16Int32) resize(newCap int) {
	oldKeys := m.keys
	oldValues := m.values
	if newCap == 0 {
	...
	if savedOnePresent {
		m.Put(int16Int32RemovedKey, savedOneValue)
	}
