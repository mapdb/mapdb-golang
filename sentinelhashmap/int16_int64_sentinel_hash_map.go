...
const (
	...
	int16Int64EmptyKey   = int16(0)
	int16Int64RemovedKey = int16(1)
)

// Int16Int64 is a sentinel-based open-addressing hash map with int16 keys and int64 values.
...
// The size is a hint; use Int16Int64BulkLoadExact for the zero-rehash
// guarantee.
func Int16Int64BulkLoad(keys []int16, values []int64, policy pump.DuplicatePolicy) (*Int16Int64, error) {
	...
	m := NewInt16Int64WithCapacity(Int16Int64bulkCap(len(keys)))
	for i := range keys {
...
func Int16Int64BulkLoadExact(keys []int16, values []int64, n int, policy pump.DuplicatePolicy) (*Int16Int64, error) {
	...
	m := NewInt16Int64WithCapacity(Int16Int64bulkCap(n))
	if len(keys) > n {
...
// rehashes.
func (m *Int16Int64) bulkPut(key int16, value int64, policy pump.DuplicatePolicy) error {
...
func (m *Int16Int64) putRegular(key int16, value int64) (int64, bool) {
	if m.needsResize() {
	...
	empty := int16Int64EmptyKey
	removed := int16Int64RemovedKey
	firstRemoved := -1
...
func (m *Int16Int64) removeRegular(key int16) (int64, bool) {
	cap := len(m.keys)
	if cap == 0 {
		return 0, false
	}
	...
	for probes := 0; probes < cap; probes++ {
		...
		if k == key {
			old := m.values[idx]
			m.keys[idx] = int16Int64RemovedKey
			m.values[idx] = 0
			m.size--
			m.tombstones++
			return old, true
...
func (m *Int16Int64) All() iter.Seq2[int16, int64] {
	return func(yield func(int16, int64) bool) {
		...
		if m.oneKeyPresent {
			if !yield(int16Int64RemovedKey, m.oneKeyValue) {
				return
			}
		}
		empty := int16Int64EmptyKey
...
func (m *Int16Int64) needsRehash() bool {
...
func (m *Int16Int64) resize(newCap int) {
	oldKeys := m.keys
	oldValues := m.values
	if newCap == 0 {
	...
	if savedOnePresent {
		m.Put(int16Int64RemovedKey, savedOneValue)
	}
