...
const (
	...
	int16Int8EmptyKey   = int16(0)
	int16Int8RemovedKey = int16(1)
)

// Int16Int8 is a sentinel-based open-addressing hash map with int16 keys and int8 values.
...
// The size is a hint; use Int16Int8BulkLoadExact for the zero-rehash
// guarantee.
func Int16Int8BulkLoad(keys []int16, values []int8, policy pump.DuplicatePolicy) (*Int16Int8, error) {
	...
	m := NewInt16Int8WithCapacity(Int16Int8bulkCap(len(keys)))
	for i := range keys {
...
func Int16Int8BulkLoadExact(keys []int16, values []int8, n int, policy pump.DuplicatePolicy) (*Int16Int8, error) {
	...
	m := NewInt16Int8WithCapacity(Int16Int8bulkCap(n))
	if len(keys) > n {
...
// rehashes.
func (m *Int16Int8) bulkPut(key int16, value int8, policy pump.DuplicatePolicy) error {
...
func (m *Int16Int8) putRegular(key int16, value int8) (int8, bool) {
	if m.needsResize() {
	...
	empty := int16Int8EmptyKey
	removed := int16Int8RemovedKey
	firstRemoved := -1
...
func (m *Int16Int8) removeRegular(key int16) (int8, bool) {
	cap := len(m.keys)
	if cap == 0 {
		return 0, false
	}
	...
	for probes := 0; probes < cap; probes++ {
		...
		if k == key {
			old := m.values[idx]
			m.keys[idx] = int16Int8RemovedKey
			m.values[idx] = 0
			m.size--
			m.tombstones++
			return old, true
...
func (m *Int16Int8) All() iter.Seq2[int16, int8] {
	return func(yield func(int16, int8) bool) {
		...
		if m.oneKeyPresent {
			if !yield(int16Int8RemovedKey, m.oneKeyValue) {
				return
			}
		}
		empty := int16Int8EmptyKey
...
func (m *Int16Int8) needsRehash() bool {
...
func (m *Int16Int8) resize(newCap int) {
	oldKeys := m.keys
	oldValues := m.values
	if newCap == 0 {
	...
	if savedOnePresent {
		m.Put(int16Int8RemovedKey, savedOneValue)
	}
