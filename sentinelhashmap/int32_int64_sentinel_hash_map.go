...
const (
	...
	int32Int64EmptyKey   = int32(0)
	int32Int64RemovedKey = int32(1)
)

// Int32Int64 is a sentinel-based open-addressing hash map with int32 keys and int64 values.
...
// The size is a hint; use Int32Int64BulkLoadExact for the zero-rehash
// guarantee.
func Int32Int64BulkLoad(keys []int32, values []int64, policy pump.DuplicatePolicy) (*Int32Int64, error) {
	...
	m := NewInt32Int64WithCapacity(Int32Int64bulkCap(len(keys)))
	for i := range keys {
...
func Int32Int64BulkLoadExact(keys []int32, values []int64, n int, policy pump.DuplicatePolicy) (*Int32Int64, error) {
	...
	m := NewInt32Int64WithCapacity(Int32Int64bulkCap(n))
	if len(keys) > n {
...
// rehashes.
func (m *Int32Int64) bulkPut(key int32, value int64, policy pump.DuplicatePolicy) error {
...
func (m *Int32Int64) putRegular(key int32, value int64) (int64, bool) {
	if m.needsResize() {
	...
	empty := int32Int64EmptyKey
	removed := int32Int64RemovedKey
	firstRemoved := -1
...
func (m *Int32Int64) removeRegular(key int32) (int64, bool) {
	cap := len(m.keys)
	if cap == 0 {
		return 0, false
	}
	...
	for probes := 0; probes < cap; probes++ {
		...
		if k == key {
			old := m.values[idx]
			m.keys[idx] = int32Int64RemovedKey
			m.values[idx] = 0
			m.size--
			m.tombstones++
			return old, true
...
func (m *Int32Int64) All() iter.Seq2[int32, int64] {
	return func(yield func(int32, int64) bool) {
		...
		if m.oneKeyPresent {
			if !yield(int32Int64RemovedKey, m.oneKeyValue) {
				return
			}
		}
		empty := int32Int64EmptyKey
...
func (m *Int32Int64) needsRehash() bool {
...
func (m *Int32Int64) resize(newCap int) {
	oldKeys := m.keys
	oldValues := m.values
	if newCap == 0 {
	...
	if savedOnePresent {
		m.Put(int32Int64RemovedKey, savedOneValue)
	}
