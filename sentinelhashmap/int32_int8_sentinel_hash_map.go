...
const (
	...
	int32Int8EmptyKey   = int32(0)
	int32Int8RemovedKey = int32(1)
)

// Int32Int8 is a sentinel-based open-addressing hash map with int32 keys and int8 values.
...
// The size is a hint; use Int32Int8BulkLoadExact for the zero-rehash
// guarantee.
func Int32Int8BulkLoad(keys []int32, values []int8, policy pump.DuplicatePolicy) (*Int32Int8, error) {
	...
	m := NewInt32Int8WithCapacity(Int32Int8bulkCap(len(keys)))
	for i := range keys {
...
func Int32Int8BulkLoadExact(keys []int32, values []int8, n int, policy pump.DuplicatePolicy) (*Int32Int8, error) {
	...
	m := NewInt32Int8WithCapacity(Int32Int8bulkCap(n))
	if len(keys) > n {
...
// rehashes.
func (m *Int32Int8) bulkPut(key int32, value int8, policy pump.DuplicatePolicy) error {
...
func (m *Int32Int8) putRegular(key int32, value int8) (int8, bool) {
	if m.needsResize() {
	...
	empty := int32Int8EmptyKey
	removed := int32Int8RemovedKey
	firstRemoved := -1
...
func (m *Int32Int8) removeRegular(key int32) (int8, bool) {
	cap := len(m.keys)
	if cap == 0 {
		return 0, false
	}
	...
	for probes := 0; probes < cap; probes++ {
		...
		if k == key {
			old := m.values[idx]
			m.keys[idx] = int32Int8RemovedKey
			m.values[idx] = 0
			m.size--
			m.tombstones++
			return old, true
...
func (m *Int32Int8) All() iter.Seq2[int32, int8] {
	return func(yield func(int32, int8) bool) {
		...
		if m.oneKeyPresent {
			if !yield(int32Int8RemovedKey, m.oneKeyValue) {
				return
			}
		}
		empty := int32Int8EmptyKey
...
func (m *Int32Int8) needsRehash() bool {
...
func (m *Int32Int8) resize(newCap int) {
	oldKeys := m.keys
	oldValues := m.values
	if newCap == 0 {
	...
	if savedOnePresent {
		m.Put(int32Int8RemovedKey, savedOneValue)
	}
