...
const (
	...
	int64Int8EmptyKey   = int64(0)
	int64Int8RemovedKey = int64(1)
)

// Int64Int8 is a sentinel-based open-addressing hash map with int64 keys and int8 values.
...
// The size is a hint; use Int64Int8BulkLoadExact for the zero-rehash
// guarantee.
func Int64Int8BulkLoad(keys []int64, values []int8, policy pump.DuplicatePolicy) (*Int64Int8, error) {
	...
	m := NewInt64Int8WithCapacity(Int64Int8bulkCap(len(keys)))
	for i := range keys {
...
func Int64Int8BulkLoadExact(keys []int64, values []int8, n int, policy pump.DuplicatePolicy) (*Int64Int8, error) {
	...
	m := NewInt64Int8WithCapacity(Int64Int8bulkCap(n))
	if len(keys) > n {
...
// rehashes.
func (m *Int64Int8) bulkPut(key int64, value int8, policy pump.DuplicatePolicy) error {
...
func (m *Int64Int8) putRegular(key int64, value int8) (int8, bool) {
	if m.needsResize() {
	...
	empty := int64Int8EmptyKey
	removed := int64Int8RemovedKey
	firstRemoved := -1
...
func (m *Int64Int8) removeRegular(key int64) (int8, bool) {
	cap := len(m.keys)
	if cap == 0 {
		return 0, false
	}
	...
	for probes := 0; probes < cap; probes++ {
		...
		if k == key {
			old := m.values[idx]
			m.keys[idx] = int64Int8RemovedKey
			m.values[idx] = 0
			m.size--
			m.tombstones++
			return old, true
...
func (m *Int64Int8) All() iter.Seq2[int64, int8] {
	return func(yield func(int64, int8) bool) {
		...
		if m.oneKeyPresent {
			if !yield(int64Int8RemovedKey, m.oneKeyValue) {
				return
			}
		}
		empty := int64Int8EmptyKey
...
func (m *Int64Int8) needsRehash() bool {
...
func (m *Int64Int8) resize(newCap int) {
	oldKeys := m.keys
	oldValues := m.values
	if newCap == 0 {
	...
	if savedOnePresent {
		m.Put(int64Int8RemovedKey, savedOneValue)
	}
