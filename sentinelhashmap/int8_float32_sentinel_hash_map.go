...
// The size is a hint; use Int8Float32BulkLoadExact for the zero-rehash
// guarantee.
func Int8Float32BulkLoad(keys []int8, values []float32, policy pump.DuplicatePolicy) (*Int8Float32, error) {
	...
	m := NewInt8Float32WithCapacity(Int8Float32bulkCap(len(keys)))
	for i := range keys {
...
func Int8Float32BulkLoadExact(keys []int8, values []float32, n int, policy pump.DuplicatePolicy) (*Int8Float32, error) {
	...
	m := NewInt8Float32WithCapacity(Int8Float32bulkCap(n))
	if len(keys) > n {
...
// rehashes.
func (m *Int8Float32) bulkPut(key int8, value float32, policy pump.DuplicatePolicy) error {
...
func (m *Int8Float32) resize(newCap int) {
	oldKeys := m.keys
	oldValues := m.values
	if newCap == 0 {
