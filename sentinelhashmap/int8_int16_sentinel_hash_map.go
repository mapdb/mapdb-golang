...
const (
	...
	int8Int16EmptyKey   = int8(0)
	int8Int16RemovedKey = int8(1)
)

// Int8Int16 is a sentinel-based open-addressing hash map with int8 keys and int16 values.
...
// The size is a hint; use Int8Int16BulkLoadExact for the zero-rehash
// guarantee.
func Int8Int16BulkLoad(keys []int8, values []int16, policy pump.DuplicatePolicy) (*Int8Int16, error) {
	...
	m := NewInt8Int16WithCapacity(Int8Int16bulkCap(len(keys)))
	for i := range keys {
...
func Int8Int16BulkLoadExact(keys []int8, values []int16, n int, policy pump.DuplicatePolicy) (*Int8Int16, error) {
	...
	m := NewInt8Int16WithCapacity(Int8Int16bulkCap(n))
	if len(keys) > n {
...
// rehashes.
func (m *Int8Int16) bulkPut(key int8, value int16, policy pump.DuplicatePolicy) error {
...
func (m *Int8Int16) putRegular(key int8, value int16) (int16, bool) {
	if m.needsResize() {
	...
	empty := int8Int16EmptyKey
	removed := int8Int16RemovedKey
	firstRemoved := -1
...
func (m *Int8Int16) removeRegular(key int8) (int16, bool) {
	cap := len(m.keys)
	if cap == 0 {
		return 0, false
	}
	...
	for probes := 0; probes < cap; probes++ {
		...
		if k == key {
			old := m.values[idx]
			m.keys[idx] = int8Int16RemovedKey
			m.values[idx] = 0
			m.size--
			m.tombstones++
			return old, true
...
func (m *Int8Int16) All() iter.Seq2[int8, int16] {
	return func(yield func(int8, int16) bool) {
		...
		if m.oneKeyPresent {
			if !yield(int8Int16RemovedKey, m.oneKeyValue) {
				return
			}
		}
		empty := int8Int16EmptyKey
...
func (m *Int8Int16) needsRehash() bool {
...
func (m *Int8Int16) resize(newCap int) {
	oldKeys := m.keys
	oldValues := m.values
	if newCap == 0 {
	...
	if savedOnePresent {
		m.Put(int8Int16RemovedKey, savedOneValue)
	}
