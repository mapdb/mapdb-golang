...
const (
	...
	int8Int32EmptyKey   = int8(0)
	int8Int32RemovedKey = int8(1)
)

// Int8Int32 is a sentinel-based open-addressing hash map with int8 keys and int32 values.
...
// The size is a hint; use Int8Int32BulkLoadExact for the zero-rehash
// guarantee.
func Int8Int32BulkLoad(keys []int8, values []int32, policy pump.DuplicatePolicy) (*Int8Int32, error) {
	...
	m := NewInt8Int32WithCapacity(Int8Int32bulkCap(len(keys)))
	for i := range keys {
...
func Int8Int32BulkLoadExact(keys []int8, values []int32, n int, policy pump.DuplicatePolicy) (*Int8Int32, error) {
	...
	m := NewInt8Int32WithCapacity(Int8Int32bulkCap(n))
	if len(keys) > n {
...
// rehashes.
func (m *Int8Int32) bulkPut(key int8, value int32, policy pump.DuplicatePolicy) error {
...
func (m *Int8Int32) putRegular(key int8, value int32) (int32, bool) {
	if m.needsResize() {
...
func (m *Int8Int32) removeRegular(key int8) (int32, bool) {
	cap := len(m.keys)
	if cap == 0 {
		return 0, false
	}
	...
	for probes := 0; probes < cap; probes++ {
		...
		if k == key {
			old := m.values[idx]
			m.keys[idx] = int8Int32RemovedKey
			m.values[idx] = 0
			m.size--
			m.tombstones++
			return old, true
...
func (m *Int8Int32) All() iter.Seq2[int8, int32] {
	return func(yield func(int8, int32) bool) {
		...
		if m.oneKeyPresent {
			if !yield(int8Int32RemovedKey, m.oneKeyValue) {
				return
			}
		}
		empty := int8Int32EmptyKey
...
func (m *Int8Int32) resize(newCap int) {
	oldKeys := m.keys
	oldValues := m.values
	if newCap == 0 {
	...
	if savedOnePresent {
		m.Put(int8Int32RemovedKey, savedOneValue)
	}
