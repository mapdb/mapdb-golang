...
const (
	...
	int8Int64EmptyKey   = int8(0)
	int8Int64RemovedKey = int8(1)
)

// Int8Int64 is a sentinel-based open-addressing hash map with int8 keys and int64 values.
...
// The size is a hint; use Int8Int64BulkLoadExact for the zero-rehash
// guarantee.
func Int8Int64BulkLoad(keys []int8, values []int64, policy pump.DuplicatePolicy) (*Int8Int64, error) {
	...
	m := NewInt8Int64WithCapacity(Int8Int64bulkCap(len(keys)))
	for i := range keys {
...
func Int8Int64BulkLoadExact(keys []int8, values []int64, n int, policy pump.DuplicatePolicy) (*Int8Int64, error) {
	...
	m := NewInt8Int64WithCapacity(Int8Int64bulkCap(n))
	if len(keys) > n {
...
// rehashes.
func (m *Int8Int64) bulkPut(key int8, value int64, policy pump.DuplicatePolicy) error {
...
func (m *Int8Int64) putRegular(key int8, value int64) (int64, bool) {
	if m.needsResize() {
	...
	empty := int8Int64EmptyKey
	removed := int8Int64RemovedKey
	firstRemoved := -1
...
func (m *Int8Int64) removeRegular(key int8) (int64, bool) {
	cap := len(m.keys)
	if cap == 0 {
		return 0, false
	}
	...
	for probes := 0; probes < cap; probes++ {
		...
		if k == key {
			old := m.values[idx]
			m.keys[idx] = int8Int64RemovedKey
			m.values[idx] = 0
			m.size--
			m.tombstones++
			return old, true
...
func (m *Int8Int64) All() iter.Seq2[int8, int64] {
	return func(yield func(int8, int64) bool) {
		...
		if m.oneKeyPresent {
			if !yield(int8Int64RemovedKey, m.oneKeyValue) {
				return
			}
		}
		empty := int8Int64EmptyKey
...
func (m *Int8Int64) needsRehash() bool {
...
func (m *Int8Int64) resize(newCap int) {
	oldKeys := m.keys
	oldValues := m.values
	if newCap == 0 {
	...
	if savedOnePresent {
		m.Put(int8Int64RemovedKey, savedOneValue)
	}
