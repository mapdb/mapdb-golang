...
// reproduces All in ascending key order — so a *CharChar satisfies
// par.Segmenter2[uint16, uint16] and feeds par.From2 with ORDERED
...
func (m *CharChar) Segments2(n int) []iter.Seq2[uint16, uint16] {
	ranges := segment.SplitRanges(m.size, n)
	segs := make([]iter.Seq2[uint16, uint16], len(ranges))
	for i := range ranges {
		lo, hi := ranges[i][0], ranges[i][1] // per-iteration copies for this view's closure
		segs[i] = func(yield func(uint16, uint16) bool) {
			// In-order walk yielding pairs whose 0-based key rank is in [lo, hi).
			// base is the rank of the first element in node's subtree.
			var walk func(node *charCharTreeNode, base int) bool
			walk = func(node *charCharTreeNode, base int) bool {
				if node == nil {
					return true
				}
				rank := base + charCharTreeNodeSize(node.left)
...
func (m *CharChar) buildcharChar(keys []uint16, values []uint16, lo, hi, level, redLevel int, parent *charCharTreeNode) *charCharTreeNode {
	...
	node.size = 1 + charCharTreeNodeSize(node.left) + charCharTreeNodeSize(node.right)
	return node
}
