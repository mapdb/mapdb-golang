...
// charFloat64TreeNodeSize returns the subtree size of a node link (0 if nil).
func charFloat64TreeNodeSize(n *charFloat64TreeNode) int {
	if n == nil {
		return 0
	}
	return n.size
...
// charFloat64TreeNodeFixSize recomputes a node's cached subtree size from its
// children. Called after any rotation or child relink so the augmentation stays
// consistent.
func charFloat64TreeNodeFixSize(n *charFloat64TreeNode) {
	n.size = 1 + charFloat64TreeNodeSize(n.left) + charFloat64TreeNodeSize(n.right)
}
...
func (m *CharFloat64) Put(key uint16, value float64) (float64, bool) {
	if m.root == nil {
		m.root = &charFloat64TreeNode{key: key, value: value, color: charFloat64TreeNodeBlack, size: 1}
		m.size++
		return 0.0, false
	}
	node := m.root
	for {
		if key < node.key {
			if node.left == nil {
				node.left = &charFloat64TreeNode{key: key, value: value, parent: node, color: charFloat64TreeNodeRed, size: 1}
			...
			if node.right == nil {
				node.right = &charFloat64TreeNode{key: key, value: value, parent: node, color: charFloat64TreeNodeRed, size: 1}
				m.incSizeToRoot(node)
...
// reproduces All in ascending key order — so a *CharFloat64 satisfies
// par.Segmenter2[uint16, float64] and feeds par.From2 with ORDERED
...
func (m *CharFloat64) Segments2(n int) []iter.Seq2[uint16, float64] {
	ranges := segment.SplitRanges(m.size, n)
	segs := make([]iter.Seq2[uint16, float64], len(ranges))
	for i := range ranges {
		lo, hi := ranges[i][0], ranges[i][1] // per-iteration copies for this view's closure
		segs[i] = func(yield func(uint16, float64) bool) {
			// In-order walk yielding pairs whose 0-based key rank is in [lo, hi).
			// base is the rank of the first element in node's subtree.
			var walk func(node *charFloat64TreeNode, base int) bool
			walk = func(node *charFloat64TreeNode, base int) bool {
				if node == nil {
					return true
				}
				rank := base + charFloat64TreeNodeSize(node.left)
...
// query; never mutates.
func (m *CharFloat64) Rank(key uint16) int {
	rank := 0
	node := m.root
	for node != nil {
		if key < node.key {
			...
			rank += 1 + charFloat64TreeNodeSize(node.left)
			node = node.right
		} else {
			// key == node.key: exactly the left subtree is strictly less.
			return rank + charFloat64TreeNodeSize(node.left)
		}
	}
	return rank
}
...
func (m *CharFloat64) SelectKey(i int) (uint16, bool) {
	node := m.selectNode(i)
	if node == nil {
		return 0, false
	}
...
func (m *CharFloat64) SelectEntry(i int) (uint16, float64, bool) {
	node := m.selectNode(i)
	if node == nil {
		return 0, 0.0, false
	}
...
// O(log n).
func (m *CharFloat64) selectNode(i int) *charFloat64TreeNode {
	if i < 0 {
		return nil
	}
	node := m.root
	for node != nil {
		left := charFloat64TreeNodeSize(node.left)
		if i < left {
			node = node.left
		} else if i == left {
			return node
...
func (m *CharFloat64) rotateLeft(x *charFloat64TreeNode) {
	...
	charFloat64TreeNodeFixSize(x)
	charFloat64TreeNodeFixSize(y)
}
...
func (m *CharFloat64) incSizeToRoot(n *charFloat64TreeNode) {
...
func (m *CharFloat64) fixSizeToRoot(n *charFloat64TreeNode) {
	for ; n != nil; n = n.parent {
		charFloat64TreeNodeFixSize(n)
	}
}
...
func (m *CharFloat64) deleteNode(z *charFloat64TreeNode) {
	...
	var fixSizeFrom *charFloat64TreeNode
...
// NewCharFloat64FromSorted builds a CharFloat64 from presorted, ascending keys
...
func NewCharFloat64FromSorted(keys []uint16, values []float64, policy pump.DuplicatePolicy) (*CharFloat64, error) {
	if len(keys) != len(values) {
		panic("mapdb: NewCharFloat64FromSorted: len(keys) != len(values)")
	}
	dk, dv, err := dedupCharFloat64Sorted(keys, values, policy)
	if err != nil {
		return nil, err
	}
	m := NewCharFloat64()
	m.root = m.buildcharFloat64(dk, dv, 0, len(dk)-1, 0, pump.RedBlackRedLevel(len(dk)), nil)
	m.size = len(dk)
	return m, nil
}

// dedupCharFloat64Sorted validates ascending order and applies the duplicate
// policy, returning compacted key/value slices. Equal adjacent keys are the only
// duplicates possible in sorted input.
func dedupCharFloat64Sorted(keys []uint16, values []float64, policy pump.DuplicatePolicy) ([]uint16, []float64, error) {
	if len(keys) == 0 {
	...
	for i := 1; i < len(keys); i++ {
		cmp := cmpCharFloat64(keys[i], keys[i-1])
		if cmp < 0 {
...
// cmpCharFloat64 is the three-way ordering used by the bulk-load validator for
// integer/char keys (float keys use the IEEE total-order helper instead).
func cmpCharFloat64(a, b uint16) int {
	switch {
	case a < b:
		return -1
	case a > b:
...
// buildcharFloat64 recursively builds a perfectly balanced subtree from the
...
func (m *CharFloat64) buildcharFloat64(keys []uint16, values []float64, lo, hi, level, redLevel int, parent *charFloat64TreeNode) *charFloat64TreeNode {
	if lo > hi {
		return nil
	}
	mid := (lo + hi) / 2
	node := &charFloat64TreeNode{key: keys[mid], value: values[mid], parent: parent, color: charFloat64TreeNodeBlack}
	if level == redLevel {
		node.color = charFloat64TreeNodeRed
	}
...
// CharFloat64Sink is a streaming builder for a CharFloat64: callers Put
// ascending entries one at a time, then Build the finished map. It is a thin
// wrapper over NewCharFloat64FromSorted (entries are buffered, then built in one
...
// NewCharFloat64Sink creates a streaming sink with the given duplicate policy.
func NewCharFloat64Sink(policy pump.DuplicatePolicy) *CharFloat64Sink {
	return &CharFloat64Sink{policy: policy}
}
...
func (s *CharFloat64Sink) Put(key uint16, value float64) {
	if s.done {
		panic("mapdb: Put on a finished CharFloat64Sink")
	}
...
func (s *CharFloat64Sink) Build() (*CharFloat64, error) {
	if s.done {
		panic("mapdb: Build on a finished CharFloat64Sink")
	}
	s.done = true
	return NewCharFloat64FromSorted(s.keys, s.values, s.policy)
}
