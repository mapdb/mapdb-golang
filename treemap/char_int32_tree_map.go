...
// charInt32TreeNodeSize returns the subtree size of a node link (0 if nil).
func charInt32TreeNodeSize(n *charInt32TreeNode) int {
	if n == nil {
		return 0
	}
	return n.size
...
// charInt32TreeNodeFixSize recomputes a node's cached subtree size from its
// children. Called after any rotation or child relink so the augmentation stays
// consistent.
func charInt32TreeNodeFixSize(n *charInt32TreeNode) {
	n.size = 1 + charInt32TreeNodeSize(n.left) + charInt32TreeNodeSize(n.right)
}

// CharInt32 is a sorted map with uint16 keys and int32 values, backed by a red-black tree.
...
// NewCharInt32 creates a new empty sorted map.
...
func (m *CharInt32) Put(key uint16, value int32) (int32, bool) {
	if m.root == nil {
		m.root = &charInt32TreeNode{key: key, value: value, color: charInt32TreeNodeBlack, size: 1}
		m.size++
		return 0, false
	}
	node := m.root
	for {
		if key < node.key {
			if node.left == nil {
				node.left = &charInt32TreeNode{key: key, value: value, parent: node, color: charInt32TreeNodeRed, size: 1}
			...
			if node.right == nil {
				node.right = &charInt32TreeNode{key: key, value: value, parent: node, color: charInt32TreeNodeRed, size: 1}
				m.incSizeToRoot(node)
...
func (m *CharInt32) Min() (uint16, int32, bool) {
	if m.root == nil {
		return 0, 0, false
	}
...
func (m *CharInt32) Max() (uint16, int32, bool) {
	if m.root == nil {
		return 0, 0, false
	}
...
func (m *CharInt32) Floor(key uint16) (uint16, int32, bool) {
...
func (m *CharInt32) Ceiling(key uint16) (uint16, int32, bool) {
	var result *charInt32TreeNode
	node := m.root
	for node != nil {
		if key == node.key {
...
// reproduces All in ascending key order — so a *CharInt32 satisfies
// par.Segmenter2[uint16, int32] and feeds par.From2 with ORDERED
...
func (m *CharInt32) Segments2(n int) []iter.Seq2[uint16, int32] {
	ranges := segment.SplitRanges(m.size, n)
	segs := make([]iter.Seq2[uint16, int32], len(ranges))
	for i := range ranges {
		lo, hi := ranges[i][0], ranges[i][1] // per-iteration copies for this view's closure
		segs[i] = func(yield func(uint16, int32) bool) {
			// In-order walk yielding pairs whose 0-based key rank is in [lo, hi).
			// base is the rank of the first element in node's subtree.
			var walk func(node *charInt32TreeNode, base int) bool
			walk = func(node *charInt32TreeNode, base int) bool {
				if node == nil {
					return true
				}
				rank := base + charInt32TreeNodeSize(node.left)
...
func (m *CharInt32) RangeKeys(fromKey, toKey uint16) iter.Seq2[uint16, int32] {
	return func(yield func(uint16, int32) bool) {
		for k, v := range m.All() {
			if k < fromKey {
				continue
...
func (m *CharInt32) Higher(key uint16) (uint16, int32, bool) {
	var result *charInt32TreeNode
	node := m.root
	for node != nil {
		if key < node.key {
...
func (m *CharInt32) Lower(key uint16) (uint16, int32, bool) {
	var result *charInt32TreeNode
	node := m.root
	for node != nil {
		if key > node.key {
...
func (m *CharInt32) HeadMap(toKey uint16) iter.Seq2[uint16, int32] {
	return func(yield func(uint16, int32) bool) {
		for k, v := range m.All() {
			if k >= toKey {
				return
...
func (m *CharInt32) TailMap(fromKey uint16) iter.Seq2[uint16, int32] {
	return func(yield func(uint16, int32) bool) {
		for k, v := range m.All() {
			if k < fromKey {
				continue
...
func (m *CharInt32) PollLastEntry() (uint16, int32, bool) {
	k, v, ok := m.Max()
	if !ok {
		return 0, 0, false
	}
...
func (m *CharInt32) DescendingMap() iter.Seq2[uint16, int32] {
	return func(yield func(uint16, int32) bool) {
...
func (m *CharInt32) DescendingKeys() iter.Seq[uint16] {
	return func(yield func(uint16) bool) {
...
// query; never mutates.
func (m *CharInt32) Rank(key uint16) int {
	rank := 0
	node := m.root
	for node != nil {
		if key < node.key {
			...
			rank += 1 + charInt32TreeNodeSize(node.left)
			node = node.right
		} else {
			// key == node.key: exactly the left subtree is strictly less.
			return rank + charInt32TreeNodeSize(node.left)
		}
	}
	return rank
}
...
func (m *CharInt32) SelectKey(i int) (uint16, bool) {
	node := m.selectNode(i)
	if node == nil {
		return 0, false
	}
...
func (m *CharInt32) SelectEntry(i int) (uint16, int32, bool) {
	node := m.selectNode(i)
	if node == nil {
		return 0, 0, false
	}
...
// O(log n).
func (m *CharInt32) selectNode(i int) *charInt32TreeNode {
	if i < 0 {
		return nil
	}
	node := m.root
	for node != nil {
		left := charInt32TreeNodeSize(node.left)
		if i < left {
			node = node.left
		} else if i == left {
			return node
...
func (m *CharInt32) findNode(key uint16) *charInt32TreeNode {
	node := m.root
	for node != nil {
		if key < node.key {
			node = node.left
...
func (m *CharInt32) minNode(node *charInt32TreeNode) *charInt32TreeNode {
	for node.left != nil {
		node = node.left
	}
	return node
...
func (m *CharInt32) maxNode(node *charInt32TreeNode) *charInt32TreeNode {
	for node.right != nil {
		node = node.right
	}
	return node
...
func (m *CharInt32) rotateLeft(x *charInt32TreeNode) {
	y := x.right
	x.right = y.left
	if y.left != nil {
		y.left.parent = x
	...
	charInt32TreeNodeFixSize(x)
	charInt32TreeNodeFixSize(y)
}

func (m *CharInt32) rotateRight(x *charInt32TreeNode) {
	y := x.left
	x.left = y.right
	if y.right != nil {
		y.right.parent = x
...
func (m *CharInt32) incSizeToRoot(n *charInt32TreeNode) {
...
func (m *CharInt32) fixSizeToRoot(n *charInt32TreeNode) {
	for ; n != nil; n = n.parent {
		charInt32TreeNodeFixSize(n)
	}
}

func (m *CharInt32) fixAfterInsert(z *charInt32TreeNode) {
...
func (m *CharInt32) deleteNode(z *charInt32TreeNode) {
	...
	var fixSizeFrom *charInt32TreeNode
...
// NewCharInt32FromSorted builds a CharInt32 from presorted, ascending keys
...
func NewCharInt32FromSorted(keys []uint16, values []int32, policy pump.DuplicatePolicy) (*CharInt32, error) {
	if len(keys) != len(values) {
		panic("mapdb: NewCharInt32FromSorted: len(keys) != len(values)")
	}
	dk, dv, err := dedupCharInt32Sorted(keys, values, policy)
	if err != nil {
		return nil, err
	}
	m := NewCharInt32()
	m.root = m.buildcharInt32(dk, dv, 0, len(dk)-1, 0, pump.RedBlackRedLevel(len(dk)), nil)
	m.size = len(dk)
	return m, nil
}

// dedupCharInt32Sorted validates ascending order and applies the duplicate
// policy, returning compacted key/value slices. Equal adjacent keys are the only
// duplicates possible in sorted input.
func dedupCharInt32Sorted(keys []uint16, values []int32, policy pump.DuplicatePolicy) ([]uint16, []int32, error) {
	if len(keys) == 0 {
	...
	for i := 1; i < len(keys); i++ {
		cmp := cmpCharInt32(keys[i], keys[i-1])
		if cmp < 0 {
...
// cmpCharInt32 is the three-way ordering used by the bulk-load validator for
// integer/char keys (float keys use the IEEE total-order helper instead).
func cmpCharInt32(a, b uint16) int {
	switch {
	case a < b:
		return -1
	case a > b:
...
// buildcharInt32 recursively builds a perfectly balanced subtree from the
...
func (m *CharInt32) buildcharInt32(keys []uint16, values []int32, lo, hi, level, redLevel int, parent *charInt32TreeNode) *charInt32TreeNode {
	if lo > hi {
		return nil
	}
	mid := (lo + hi) / 2
	node := &charInt32TreeNode{key: keys[mid], value: values[mid], parent: parent, color: charInt32TreeNodeBlack}
	if level == redLevel {
		node.color = charInt32TreeNodeRed
	}
	node.left = m.buildcharInt32(keys, values, lo, mid-1, level+1, redLevel, node)
	node.right = m.buildcharInt32(keys, values, mid+1, hi, level+1, redLevel, node)
	...
	node.size = 1 + charInt32TreeNodeSize(node.left) + charInt32TreeNodeSize(node.right)
	return node
}

// CharInt32Sink is a streaming builder for a CharInt32: callers Put
// ascending entries one at a time, then Build the finished map. It is a thin
// wrapper over NewCharInt32FromSorted (entries are buffered, then built in one
...
// NewCharInt32Sink creates a streaming sink with the given duplicate policy.
func NewCharInt32Sink(policy pump.DuplicatePolicy) *CharInt32Sink {
	return &CharInt32Sink{policy: policy}
}
...
func (s *CharInt32Sink) Put(key uint16, value int32) {
	if s.done {
		panic("mapdb: Put on a finished CharInt32Sink")
	}
...
func (s *CharInt32Sink) Build() (*CharInt32, error) {
	if s.done {
		panic("mapdb: Build on a finished CharInt32Sink")
	}
	s.done = true
	return NewCharInt32FromSorted(s.keys, s.values, s.policy)
}

func (m *CharInt32) fixAfterDelete(x *charInt32TreeNode) {
