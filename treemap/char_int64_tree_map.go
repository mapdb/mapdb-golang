...
// charInt64TreeNodeSize returns the subtree size of a node link (0 if nil).
func charInt64TreeNodeSize(n *charInt64TreeNode) int {
	if n == nil {
		return 0
	}
	return n.size
...
// charInt64TreeNodeFixSize recomputes a node's cached subtree size from its
// children. Called after any rotation or child relink so the augmentation stays
// consistent.
func charInt64TreeNodeFixSize(n *charInt64TreeNode) {
	n.size = 1 + charInt64TreeNodeSize(n.left) + charInt64TreeNodeSize(n.right)
}
...
func (m *CharInt64) Put(key uint16, value int64) (int64, bool) {
	if m.root == nil {
		m.root = &charInt64TreeNode{key: key, value: value, color: charInt64TreeNodeBlack, size: 1}
		m.size++
		return 0, false
	}
	node := m.root
	for {
		if key < node.key {
			if node.left == nil {
				node.left = &charInt64TreeNode{key: key, value: value, parent: node, color: charInt64TreeNodeRed, size: 1}
			...
			if node.right == nil {
				node.right = &charInt64TreeNode{key: key, value: value, parent: node, color: charInt64TreeNodeRed, size: 1}
				m.incSizeToRoot(node)
...
// reproduces All in ascending key order — so a *CharInt64 satisfies
// par.Segmenter2[uint16, int64] and feeds par.From2 with ORDERED
...
func (m *CharInt64) Segments2(n int) []iter.Seq2[uint16, int64] {
	ranges := segment.SplitRanges(m.size, n)
	segs := make([]iter.Seq2[uint16, int64], len(ranges))
	for i := range ranges {
		lo, hi := ranges[i][0], ranges[i][1] // per-iteration copies for this view's closure
		segs[i] = func(yield func(uint16, int64) bool) {
			// In-order walk yielding pairs whose 0-based key rank is in [lo, hi).
			// base is the rank of the first element in node's subtree.
			var walk func(node *charInt64TreeNode, base int) bool
			walk = func(node *charInt64TreeNode, base int) bool {
				if node == nil {
					return true
				}
				rank := base + charInt64TreeNodeSize(node.left)
...
// query; never mutates.
func (m *CharInt64) Rank(key uint16) int {
	rank := 0
	node := m.root
	for node != nil {
		if key < node.key {
			...
			rank += 1 + charInt64TreeNodeSize(node.left)
			node = node.right
		} else {
			// key == node.key: exactly the left subtree is strictly less.
			return rank + charInt64TreeNodeSize(node.left)
		}
	}
	return rank
}
...
func (m *CharInt64) SelectKey(i int) (uint16, bool) {
	node := m.selectNode(i)
	if node == nil {
		return 0, false
	}
...
func (m *CharInt64) SelectEntry(i int) (uint16, int64, bool) {
	node := m.selectNode(i)
	if node == nil {
		return 0, 0, false
	}
...
// O(log n).
func (m *CharInt64) selectNode(i int) *charInt64TreeNode {
	if i < 0 {
		return nil
	}
	node := m.root
	for node != nil {
		left := charInt64TreeNodeSize(node.left)
		if i < left {
			node = node.left
		} else if i == left {
			return node
...
func (m *CharInt64) rotateLeft(x *charInt64TreeNode) {
	...
	charInt64TreeNodeFixSize(x)
	charInt64TreeNodeFixSize(y)
}
...
func (m *CharInt64) incSizeToRoot(n *charInt64TreeNode) {
...
func (m *CharInt64) fixSizeToRoot(n *charInt64TreeNode) {
	for ; n != nil; n = n.parent {
		charInt64TreeNodeFixSize(n)
	}
}
...
func (m *CharInt64) deleteNode(z *charInt64TreeNode) {
	...
	var fixSizeFrom *charInt64TreeNode
