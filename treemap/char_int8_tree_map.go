...
// reproduces All in ascending key order — so a *CharInt8 satisfies
// par.Segmenter2[uint16, int8] and feeds par.From2 with ORDERED
...
func (m *CharInt8) Segments2(n int) []iter.Seq2[uint16, int8] {
	ranges := segment.SplitRanges(m.size, n)
	segs := make([]iter.Seq2[uint16, int8], len(ranges))
	for i := range ranges {
		lo, hi := ranges[i][0], ranges[i][1] // per-iteration copies for this view's closure
		segs[i] = func(yield func(uint16, int8) bool) {
			// In-order walk yielding pairs whose 0-based key rank is in [lo, hi).
			// base is the rank of the first element in node's subtree.
			var walk func(node *charInt8TreeNode, base int) bool
			walk = func(node *charInt8TreeNode, base int) bool {
				if node == nil {
					return true
				}
				rank := base + charInt8TreeNodeSize(node.left)
