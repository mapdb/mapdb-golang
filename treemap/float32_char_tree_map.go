...
// reproduces All in ascending key order — so a *Float32Char satisfies
// par.Segmenter2[float32, uint16] and feeds par.From2 with ORDERED
...
func (m *Float32Char) Segments2(n int) []iter.Seq2[float32, uint16] {
	ranges := segment.SplitRanges(m.size, n)
	segs := make([]iter.Seq2[float32, uint16], len(ranges))
	for i := range ranges {
		lo, hi := ranges[i][0], ranges[i][1] // per-iteration copies for this view's closure
		segs[i] = func(yield func(float32, uint16) bool) {
			// In-order walk yielding pairs whose 0-based key rank is in [lo, hi).
			// base is the rank of the first element in node's subtree.
			var walk func(node *float32CharTreeNode, base int) bool
			walk = func(node *float32CharTreeNode, base int) bool {
				if node == nil {
					return true
				}
				rank := base + float32CharTreeNodeSize(node.left)
