...
// reproduces All in ascending key order — so a *Float32Float64 satisfies
// par.Segmenter2[float32, float64] and feeds par.From2 with ORDERED
...
func (m *Float32Float64) Segments2(n int) []iter.Seq2[float32, float64] {
	ranges := segment.SplitRanges(m.size, n)
	segs := make([]iter.Seq2[float32, float64], len(ranges))
	for i := range ranges {
		lo, hi := ranges[i][0], ranges[i][1] // per-iteration copies for this view's closure
		segs[i] = func(yield func(float32, float64) bool) {
			// In-order walk yielding pairs whose 0-based key rank is in [lo, hi).
			// base is the rank of the first element in node's subtree.
			var walk func(node *float32Float64TreeNode, base int) bool
			walk = func(node *float32Float64TreeNode, base int) bool {
				if node == nil {
					return true
				}
				rank := base + float32Float64TreeNodeSize(node.left)
...
func (m *Float32Float64) buildfloat32Float64(keys []float32, values []float64, lo, hi, level, redLevel int, parent *float32Float64TreeNode) *float32Float64TreeNode {
	...
	node.size = 1 + float32Float64TreeNodeSize(node.left) + float32Float64TreeNodeSize(node.right)
	return node
}
