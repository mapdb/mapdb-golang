...
// reproduces All in ascending key order — so a *Float32Int16 satisfies
// par.Segmenter2[float32, int16] and feeds par.From2 with ORDERED
...
func (m *Float32Int16) Segments2(n int) []iter.Seq2[float32, int16] {
	ranges := segment.SplitRanges(m.size, n)
	segs := make([]iter.Seq2[float32, int16], len(ranges))
	for i := range ranges {
		lo, hi := ranges[i][0], ranges[i][1] // per-iteration copies for this view's closure
		segs[i] = func(yield func(float32, int16) bool) {
			// In-order walk yielding pairs whose 0-based key rank is in [lo, hi).
			// base is the rank of the first element in node's subtree.
			var walk func(node *float32Int16TreeNode, base int) bool
			walk = func(node *float32Int16TreeNode, base int) bool {
				if node == nil {
					return true
				}
				rank := base + float32Int16TreeNodeSize(node.left)
...
func (m *Float32Int16) buildfloat32Int16(keys []float32, values []int16, lo, hi, level, redLevel int, parent *float32Int16TreeNode) *float32Int16TreeNode {
	...
	node.size = 1 + float32Int16TreeNodeSize(node.left) + float32Int16TreeNodeSize(node.right)
	return node
}
