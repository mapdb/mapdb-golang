...
// reproduces All in ascending key order — so a *Float32Int32 satisfies
// par.Segmenter2[float32, int32] and feeds par.From2 with ORDERED
...
func (m *Float32Int32) Segments2(n int) []iter.Seq2[float32, int32] {
	ranges := segment.SplitRanges(m.size, n)
	segs := make([]iter.Seq2[float32, int32], len(ranges))
	for i := range ranges {
		lo, hi := ranges[i][0], ranges[i][1] // per-iteration copies for this view's closure
		segs[i] = func(yield func(float32, int32) bool) {
			// In-order walk yielding pairs whose 0-based key rank is in [lo, hi).
			// base is the rank of the first element in node's subtree.
			var walk func(node *float32Int32TreeNode, base int) bool
			walk = func(node *float32Int32TreeNode, base int) bool {
				if node == nil {
					return true
				}
				rank := base + float32Int32TreeNodeSize(node.left)
...
func (m *Float32Int32) buildfloat32Int32(keys []float32, values []int32, lo, hi, level, redLevel int, parent *float32Int32TreeNode) *float32Int32TreeNode {
	...
	node.size = 1 + float32Int32TreeNodeSize(node.left) + float32Int32TreeNodeSize(node.right)
	return node
}
