...
// float32Int64TreeNodeSize returns the subtree size of a node link (0 if nil).
func float32Int64TreeNodeSize(n *float32Int64TreeNode) int {
	if n == nil {
		return 0
	}
	return n.size
...
// float32Int64TreeNodeFixSize recomputes a node's cached subtree size from its
// children. Called after any rotation or child relink so the augmentation stays
// consistent.
func float32Int64TreeNodeFixSize(n *float32Int64TreeNode) {
	n.size = 1 + float32Int64TreeNodeSize(n.left) + float32Int64TreeNodeSize(n.right)
}

// Float32Int64 is a sorted map with float32 keys and int64 values, backed by a red-black tree.
...
// NewFloat32Int64 creates a new empty sorted map.
...
func (m *Float32Int64) Put(key float32, value int64) (int64, bool) {
	if m.root == nil {
		m.root = &float32Int64TreeNode{key: key, value: value, color: float32Int64TreeNodeBlack, size: 1}
		m.size++
		return 0, false
	}
	node := m.root
	for {
		if cmpFloat32(key, node.key) < 0 {
			if node.left == nil {
				node.left = &float32Int64TreeNode{key: key, value: value, parent: node, color: float32Int64TreeNodeRed, size: 1}
			...
			if node.right == nil {
				node.right = &float32Int64TreeNode{key: key, value: value, parent: node, color: float32Int64TreeNodeRed, size: 1}
				m.incSizeToRoot(node)
...
func (m *Float32Int64) Min() (float32, int64, bool) {
	if m.root == nil {
		return 0, 0, false
	}
...
func (m *Float32Int64) Max() (float32, int64, bool) {
	if m.root == nil {
		return 0, 0, false
	}
...
func (m *Float32Int64) Floor(key float32) (float32, int64, bool) {
...
func (m *Float32Int64) Ceiling(key float32) (float32, int64, bool) {
	var result *float32Int64TreeNode
	node := m.root
	for node != nil {
		if cmpFloat32(key, node.key) == 0 {
...
func (m *Float32Int64) RangeKeys(fromKey, toKey float32) iter.Seq2[float32, int64] {
	return func(yield func(float32, int64) bool) {
		for k, v := range m.All() {
...
func (m *Float32Int64) Higher(key float32) (float32, int64, bool) {
	var result *float32Int64TreeNode
	node := m.root
	for node != nil {
		if cmpFloat32(key, node.key) < 0 {
...
func (m *Float32Int64) Lower(key float32) (float32, int64, bool) {
	var result *float32Int64TreeNode
	node := m.root
	for node != nil {
		if cmpFloat32(key, node.key) > 0 {
...
func (m *Float32Int64) HeadMap(toKey float32) iter.Seq2[float32, int64] {
	return func(yield func(float32, int64) bool) {
		for k, v := range m.All() {
			if cmpFloat32(k, toKey) >= 0 {
				return
...
func (m *Float32Int64) TailMap(fromKey float32) iter.Seq2[float32, int64] {
	return func(yield func(float32, int64) bool) {
		for k, v := range m.All() {
			if cmpFloat32(k, fromKey) < 0 {
				continue
...
func (m *Float32Int64) SubMap(fromKey, toKey float32) iter.Seq2[float32, int64] {
	return m.RangeKeys(fromKey, toKey)
}

// FirstEntry is an alias of Min — the smallest key and its value, or zero/false.
func (m *Float32Int64) FirstEntry() (float32, int64, bool) { return m.Min() }
...
func (m *Float32Int64) PollLastEntry() (float32, int64, bool) {
	k, v, ok := m.Max()
	if !ok {
		return 0, 0, false
	}
...
func (m *Float32Int64) DescendingMap() iter.Seq2[float32, int64] {
	return func(yield func(float32, int64) bool) {
...
func (m *Float32Int64) DescendingKeys() iter.Seq[float32] {
	return func(yield func(float32) bool) {
...
// query; never mutates.
func (m *Float32Int64) Rank(key float32) int {
	rank := 0
	node := m.root
	for node != nil {
		if cmpFloat32(key, node.key) < 0 {
			...
			rank += 1 + float32Int64TreeNodeSize(node.left)
			node = node.right
		} else {
			// key == node.key: exactly the left subtree is strictly less.
			return rank + float32Int64TreeNodeSize(node.left)
		}
	}
	return rank
}
...
func (m *Float32Int64) SelectKey(i int) (float32, bool) {
	node := m.selectNode(i)
	if node == nil {
		return 0, false
	}
...
func (m *Float32Int64) SelectEntry(i int) (float32, int64, bool) {
	node := m.selectNode(i)
	if node == nil {
		return 0, 0, false
	}
...
// O(log n).
func (m *Float32Int64) selectNode(i int) *float32Int64TreeNode {
	if i < 0 {
		return nil
	}
	node := m.root
	for node != nil {
		left := float32Int64TreeNodeSize(node.left)
		if i < left {
			node = node.left
		} else if i == left {
			return node
...
func (m *Float32Int64) findNode(key float32) *float32Int64TreeNode {
	node := m.root
	for node != nil {
		if cmpFloat32(key, node.key) < 0 {
			node = node.left
...
func (m *Float32Int64) minNode(node *float32Int64TreeNode) *float32Int64TreeNode {
	for node.left != nil {
		node = node.left
	}
	return node
...
func (m *Float32Int64) maxNode(node *float32Int64TreeNode) *float32Int64TreeNode {
	for node.right != nil {
		node = node.right
	}
	return node
...
func (m *Float32Int64) rotateLeft(x *float32Int64TreeNode) {
	y := x.right
	x.right = y.left
	if y.left != nil {
		y.left.parent = x
	...
	float32Int64TreeNodeFixSize(x)
	float32Int64TreeNodeFixSize(y)
}

func (m *Float32Int64) rotateRight(x *float32Int64TreeNode) {
	y := x.left
	x.left = y.right
	if y.right != nil {
		y.right.parent = x
...
func (m *Float32Int64) incSizeToRoot(n *float32Int64TreeNode) {
...
func (m *Float32Int64) fixSizeToRoot(n *float32Int64TreeNode) {
	for ; n != nil; n = n.parent {
		float32Int64TreeNodeFixSize(n)
	}
}

func (m *Float32Int64) fixAfterInsert(z *float32Int64TreeNode) {
...
func (m *Float32Int64) deleteNode(z *float32Int64TreeNode) {
	...
	var fixSizeFrom *float32Int64TreeNode
...
// NewFloat32Int64FromSorted builds a Float32Int64 from presorted, ascending keys
...
func NewFloat32Int64FromSorted(keys []float32, values []int64, policy pump.DuplicatePolicy) (*Float32Int64, error) {
	if len(keys) != len(values) {
		panic("mapdb: NewFloat32Int64FromSorted: len(keys) != len(values)")
	}
	dk, dv, err := dedupFloat32Int64Sorted(keys, values, policy)
	if err != nil {
		return nil, err
	}
	m := NewFloat32Int64()
	m.root = m.buildfloat32Int64(dk, dv, 0, len(dk)-1, 0, pump.RedBlackRedLevel(len(dk)), nil)
	m.size = len(dk)
	return m, nil
}

// dedupFloat32Int64Sorted validates ascending order and applies the duplicate
// policy, returning compacted key/value slices. Equal adjacent keys are the only
// duplicates possible in sorted input.
func dedupFloat32Int64Sorted(keys []float32, values []int64, policy pump.DuplicatePolicy) ([]float32, []int64, error) {
	if len(keys) == 0 {
...
// buildfloat32Int64 recursively builds a perfectly balanced subtree from the
...
func (m *Float32Int64) buildfloat32Int64(keys []float32, values []int64, lo, hi, level, redLevel int, parent *float32Int64TreeNode) *float32Int64TreeNode {
	if lo > hi {
		return nil
	}
	mid := (lo + hi) / 2
	node := &float32Int64TreeNode{key: keys[mid], value: values[mid], parent: parent, color: float32Int64TreeNodeBlack}
	if level == redLevel {
		node.color = float32Int64TreeNodeRed
	}
	...
	float32Int64TreeNodeFixSize(node)
	return node
}

// Float32Int64Sink is a streaming builder for a Float32Int64: callers Put
// ascending entries one at a time, then Build the finished map. It is a thin
// wrapper over NewFloat32Int64FromSorted (entries are buffered, then built in one
...
// NewFloat32Int64Sink creates a streaming sink with the given duplicate policy.
func NewFloat32Int64Sink(policy pump.DuplicatePolicy) *Float32Int64Sink {
	return &Float32Int64Sink{policy: policy}
}
...
func (s *Float32Int64Sink) Put(key float32, value int64) {
	if s.done {
		panic("mapdb: Put on a finished Float32Int64Sink")
	}
...
func (s *Float32Int64Sink) Build() (*Float32Int64, error) {
	if s.done {
		panic("mapdb: Build on a finished Float32Int64Sink")
	}
	s.done = true
	return NewFloat32Int64FromSorted(s.keys, s.values, s.policy)
}

func (m *Float32Int64) fixAfterDelete(x *float32Int64TreeNode) {
