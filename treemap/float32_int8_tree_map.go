...
// float32Int8TreeNodeSize returns the subtree size of a node link (0 if nil).
func float32Int8TreeNodeSize(n *float32Int8TreeNode) int {
	if n == nil {
		return 0
	}
	return n.size
...
// float32Int8TreeNodeFixSize recomputes a node's cached subtree size from its
// children. Called after any rotation or child relink so the augmentation stays
// consistent.
func float32Int8TreeNodeFixSize(n *float32Int8TreeNode) {
	n.size = 1 + float32Int8TreeNodeSize(n.left) + float32Int8TreeNodeSize(n.right)
}
...
func (m *Float32Int8) Put(key float32, value int8) (int8, bool) {
	if m.root == nil {
		m.root = &float32Int8TreeNode{key: key, value: value, color: float32Int8TreeNodeBlack, size: 1}
		m.size++
		return 0, false
	}
	node := m.root
	for {
		if cmpFloat32(key, node.key) < 0 {
			if node.left == nil {
				node.left = &float32Int8TreeNode{key: key, value: value, parent: node, color: float32Int8TreeNodeRed, size: 1}
			...
			if node.right == nil {
				node.right = &float32Int8TreeNode{key: key, value: value, parent: node, color: float32Int8TreeNodeRed, size: 1}
				m.incSizeToRoot(node)
...
// reproduces All in ascending key order — so a *Float32Int8 satisfies
// par.Segmenter2[float32, int8] and feeds par.From2 with ORDERED
...
func (m *Float32Int8) Segments2(n int) []iter.Seq2[float32, int8] {
	ranges := segment.SplitRanges(m.size, n)
	segs := make([]iter.Seq2[float32, int8], len(ranges))
	for i := range ranges {
		lo, hi := ranges[i][0], ranges[i][1] // per-iteration copies for this view's closure
		segs[i] = func(yield func(float32, int8) bool) {
			// In-order walk yielding pairs whose 0-based key rank is in [lo, hi).
			// base is the rank of the first element in node's subtree.
			var walk func(node *float32Int8TreeNode, base int) bool
			walk = func(node *float32Int8TreeNode, base int) bool {
				if node == nil {
					return true
				}
				rank := base + float32Int8TreeNodeSize(node.left)
...
// query; never mutates.
func (m *Float32Int8) Rank(key float32) int {
	rank := 0
	node := m.root
	for node != nil {
		if cmpFloat32(key, node.key) < 0 {
			...
			rank += 1 + float32Int8TreeNodeSize(node.left)
			node = node.right
		} else {
			// key == node.key: exactly the left subtree is strictly less.
			return rank + float32Int8TreeNodeSize(node.left)
		}
	}
	return rank
}
...
func (m *Float32Int8) SelectKey(i int) (float32, bool) {
	node := m.selectNode(i)
	if node == nil {
		return 0, false
	}
...
func (m *Float32Int8) SelectEntry(i int) (float32, int8, bool) {
	node := m.selectNode(i)
	if node == nil {
		return 0, 0, false
	}
...
// O(log n).
func (m *Float32Int8) selectNode(i int) *float32Int8TreeNode {
	if i < 0 {
		return nil
	}
	node := m.root
	for node != nil {
		left := float32Int8TreeNodeSize(node.left)
		if i < left {
			node = node.left
		} else if i == left {
			return node
...
func (m *Float32Int8) rotateLeft(x *float32Int8TreeNode) {
	...
	float32Int8TreeNodeFixSize(x)
	float32Int8TreeNodeFixSize(y)
}
...
func (m *Float32Int8) incSizeToRoot(n *float32Int8TreeNode) {
...
func (m *Float32Int8) fixSizeToRoot(n *float32Int8TreeNode) {
	for ; n != nil; n = n.parent {
		float32Int8TreeNodeFixSize(n)
	}
}
...
func (m *Float32Int8) deleteNode(z *float32Int8TreeNode) {
	...
	var fixSizeFrom *float32Int8TreeNode
...
func (m *Float32Int8) buildfloat32Int8(keys []float32, values []int8, lo, hi, level, redLevel int, parent *float32Int8TreeNode) *float32Int8TreeNode {
	...
	node.size = 1 + float32Int8TreeNodeSize(node.left) + float32Int8TreeNodeSize(node.right)
	return node
}
