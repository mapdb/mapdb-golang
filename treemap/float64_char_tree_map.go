...
// reproduces All in ascending key order — so a *Float64Char satisfies
// par.Segmenter2[float64, uint16] and feeds par.From2 with ORDERED
...
func (m *Float64Char) Segments2(n int) []iter.Seq2[float64, uint16] {
	ranges := segment.SplitRanges(m.size, n)
	segs := make([]iter.Seq2[float64, uint16], len(ranges))
	for i := range ranges {
		lo, hi := ranges[i][0], ranges[i][1] // per-iteration copies for this view's closure
		segs[i] = func(yield func(float64, uint16) bool) {
			// In-order walk yielding pairs whose 0-based key rank is in [lo, hi).
			// base is the rank of the first element in node's subtree.
			var walk func(node *float64CharTreeNode, base int) bool
			walk = func(node *float64CharTreeNode, base int) bool {
				if node == nil {
					return true
				}
				rank := base + float64CharTreeNodeSize(node.left)
...
func (m *Float64Char) buildfloat64Char(keys []float64, values []uint16, lo, hi, level, redLevel int, parent *float64CharTreeNode) *float64CharTreeNode {
	...
	node.size = 1 + float64CharTreeNodeSize(node.left) + float64CharTreeNodeSize(node.right)
	return node
}
