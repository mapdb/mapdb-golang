...
// float64Float32TreeNodeSize returns the subtree size of a node link (0 if nil).
func float64Float32TreeNodeSize(n *float64Float32TreeNode) int {
	if n == nil {
		return 0
	}
	return n.size
...
// float64Float32TreeNodeFixSize recomputes a node's cached subtree size from its
// children. Called after any rotation or child relink so the augmentation stays
// consistent.
func float64Float32TreeNodeFixSize(n *float64Float32TreeNode) {
	n.size = 1 + float64Float32TreeNodeSize(n.left) + float64Float32TreeNodeSize(n.right)
}

// Float64Float32 is a sorted map with float64 keys and float32 values, backed by a red-black tree.
...
// NewFloat64Float32 creates a new empty sorted map.
...
func (m *Float64Float32) Put(key float64, value float32) (float32, bool) {
	if m.root == nil {
		m.root = &float64Float32TreeNode{key: key, value: value, color: float64Float32TreeNodeBlack, size: 1}
		m.size++
		return 0.0, false
	}
	node := m.root
	for {
		if cmpFloat64(key, node.key) < 0 {
			if node.left == nil {
				node.left = &float64Float32TreeNode{key: key, value: value, parent: node, color: float64Float32TreeNodeRed, size: 1}
			...
			if node.right == nil {
				node.right = &float64Float32TreeNode{key: key, value: value, parent: node, color: float64Float32TreeNodeRed, size: 1}
				m.incSizeToRoot(node)
...
func (m *Float64Float32) Min() (float64, float32, bool) {
	if m.root == nil {
		return 0, 0.0, false
	}
...
func (m *Float64Float32) Max() (float64, float32, bool) {
	if m.root == nil {
		return 0, 0.0, false
	}
...
func (m *Float64Float32) Floor(key float64) (float64, float32, bool) {
...
func (m *Float64Float32) Ceiling(key float64) (float64, float32, bool) {
	var result *float64Float32TreeNode
	node := m.root
	for node != nil {
		if cmpFloat64(key, node.key) == 0 {
...
// reproduces All in ascending key order — so a *Float64Float32 satisfies
// par.Segmenter2[float64, float32] and feeds par.From2 with ORDERED
...
func (m *Float64Float32) Segments2(n int) []iter.Seq2[float64, float32] {
	ranges := segment.SplitRanges(m.size, n)
	segs := make([]iter.Seq2[float64, float32], len(ranges))
	for i := range ranges {
		lo, hi := ranges[i][0], ranges[i][1] // per-iteration copies for this view's closure
		segs[i] = func(yield func(float64, float32) bool) {
			// In-order walk yielding pairs whose 0-based key rank is in [lo, hi).
			// base is the rank of the first element in node's subtree.
			var walk func(node *float64Float32TreeNode, base int) bool
			walk = func(node *float64Float32TreeNode, base int) bool {
				if node == nil {
					return true
				}
				rank := base + float64Float32TreeNodeSize(node.left)
...
func (m *Float64Float32) Higher(key float64) (float64, float32, bool) {
	var result *float64Float32TreeNode
	node := m.root
	for node != nil {
		if cmpFloat64(key, node.key) < 0 {
...
func (m *Float64Float32) Lower(key float64) (float64, float32, bool) {
	var result *float64Float32TreeNode
	node := m.root
	for node != nil {
		if cmpFloat64(key, node.key) > 0 {
...
func (m *Float64Float32) PollLastEntry() (float64, float32, bool) {
	k, v, ok := m.Max()
	if !ok {
		return 0, 0.0, false
	}
...
func (m *Float64Float32) DescendingMap() iter.Seq2[float64, float32] {
	return func(yield func(float64, float32) bool) {
...
func (m *Float64Float32) DescendingKeys() iter.Seq[float64] {
	return func(yield func(float64) bool) {
...
func (m *Float64Float32) Detect(predicate func(float64, float32) bool) (float64, float32, bool) {
	for k, v := range m.All() {
		if predicate(k, v) {
			return k, v, true
		}
...
func (m *Float64Float32) Count(predicate func(float64, float32) bool) int {
	c := 0
	for k, v := range m.All() {
		if predicate(k, v) {
			c++
...
// query; never mutates.
func (m *Float64Float32) Rank(key float64) int {
	rank := 0
	node := m.root
	for node != nil {
		if cmpFloat64(key, node.key) < 0 {
			...
			rank += 1 + float64Float32TreeNodeSize(node.left)
			node = node.right
		} else {
			// key == node.key: exactly the left subtree is strictly less.
			return rank + float64Float32TreeNodeSize(node.left)
		}
	}
	return rank
}
...
func (m *Float64Float32) SelectKey(i int) (float64, bool) {
	node := m.selectNode(i)
	if node == nil {
		return 0, false
	}
...
func (m *Float64Float32) SelectEntry(i int) (float64, float32, bool) {
	node := m.selectNode(i)
	if node == nil {
		return 0, 0.0, false
	}
...
// O(log n).
func (m *Float64Float32) selectNode(i int) *float64Float32TreeNode {
	if i < 0 {
		return nil
	}
	node := m.root
	for node != nil {
		left := float64Float32TreeNodeSize(node.left)
		if i < left {
			node = node.left
		} else if i == left {
			return node
...
func (m *Float64Float32) findNode(key float64) *float64Float32TreeNode {
	node := m.root
	for node != nil {
		if cmpFloat64(key, node.key) < 0 {
			node = node.left
...
func (m *Float64Float32) minNode(node *float64Float32TreeNode) *float64Float32TreeNode {
	for node.left != nil {
		node = node.left
	}
	return node
...
func (m *Float64Float32) maxNode(node *float64Float32TreeNode) *float64Float32TreeNode {
	for node.right != nil {
		node = node.right
	}
	return node
...
func (m *Float64Float32) rotateLeft(x *float64Float32TreeNode) {
	y := x.right
	x.right = y.left
	if y.left != nil {
		y.left.parent = x
	...
	float64Float32TreeNodeFixSize(x)
	float64Float32TreeNodeFixSize(y)
}

func (m *Float64Float32) rotateRight(x *float64Float32TreeNode) {
	y := x.left
	x.left = y.right
	if y.right != nil {
		y.right.parent = x
...
func (m *Float64Float32) incSizeToRoot(n *float64Float32TreeNode) {
...
func (m *Float64Float32) fixSizeToRoot(n *float64Float32TreeNode) {
	for ; n != nil; n = n.parent {
		float64Float32TreeNodeFixSize(n)
	}
}

func (m *Float64Float32) fixAfterInsert(z *float64Float32TreeNode) {
...
func (m *Float64Float32) deleteNode(z *float64Float32TreeNode) {
	...
	var fixSizeFrom *float64Float32TreeNode
...
// NewFloat64Float32FromSorted builds a Float64Float32 from presorted, ascending keys
...
func NewFloat64Float32FromSorted(keys []float64, values []float32, policy pump.DuplicatePolicy) (*Float64Float32, error) {
	if len(keys) != len(values) {
		panic("mapdb: NewFloat64Float32FromSorted: len(keys) != len(values)")
	}
	dk, dv, err := dedupFloat64Float32Sorted(keys, values, policy)
	if err != nil {
		return nil, err
	}
	m := NewFloat64Float32()
	m.root = m.buildfloat64Float32(dk, dv, 0, len(dk)-1, 0, pump.RedBlackRedLevel(len(dk)), nil)
	m.size = len(dk)
	return m, nil
}

// dedupFloat64Float32Sorted validates ascending order and applies the duplicate
// policy, returning compacted key/value slices. Equal adjacent keys are the only
// duplicates possible in sorted input.
func dedupFloat64Float32Sorted(keys []float64, values []float32, policy pump.DuplicatePolicy) ([]float64, []float32, error) {
	if len(keys) == 0 {
...
// buildfloat64Float32 recursively builds a perfectly balanced subtree from the
...
func (m *Float64Float32) buildfloat64Float32(keys []float64, values []float32, lo, hi, level, redLevel int, parent *float64Float32TreeNode) *float64Float32TreeNode {
	if lo > hi {
		return nil
	}
	mid := (lo + hi) / 2
	node := &float64Float32TreeNode{key: keys[mid], value: values[mid], parent: parent, color: float64Float32TreeNodeBlack}
	if level == redLevel {
		node.color = float64Float32TreeNodeRed
	}
	node.left = m.buildfloat64Float32(keys, values, lo, mid-1, level+1, redLevel, node)
	node.right = m.buildfloat64Float32(keys, values, mid+1, hi, level+1, redLevel, node)
	...
	node.size = 1 + float64Float32TreeNodeSize(node.left) + float64Float32TreeNodeSize(node.right)
	return node
}

// Float64Float32Sink is a streaming builder for a Float64Float32: callers Put
// ascending entries one at a time, then Build the finished map. It is a thin
// wrapper over NewFloat64Float32FromSorted (entries are buffered, then built in one
...
type Float64Float32Sink struct {
	keys   []float64
	values []float32
...
// NewFloat64Float32Sink creates a streaming sink with the given duplicate policy.
func NewFloat64Float32Sink(policy pump.DuplicatePolicy) *Float64Float32Sink {
	return &Float64Float32Sink{policy: policy}
}
...
func (s *Float64Float32Sink) Put(key float64, value float32) {
	if s.done {
		panic("mapdb: Put on a finished Float64Float32Sink")
	}
...
func (s *Float64Float32Sink) Build() (*Float64Float32, error) {
	if s.done {
		panic("mapdb: Build on a finished Float64Float32Sink")
	}
	s.done = true
	return NewFloat64Float32FromSorted(s.keys, s.values, s.policy)
}

func (m *Float64Float32) fixAfterDelete(x *float64Float32TreeNode) {
