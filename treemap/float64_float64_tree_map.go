...
// float64Float64TreeNodeSize returns the subtree size of a node link (0 if nil).
func float64Float64TreeNodeSize(n *float64Float64TreeNode) int {
	if n == nil {
		return 0
	}
	return n.size
...
// float64Float64TreeNodeFixSize recomputes a node's cached subtree size from its
// children. Called after any rotation or child relink so the augmentation stays
// consistent.
func float64Float64TreeNodeFixSize(n *float64Float64TreeNode) {
	n.size = 1 + float64Float64TreeNodeSize(n.left) + float64Float64TreeNodeSize(n.right)
}

// Float64Float64 is a sorted map with float64 keys and float64 values, backed by a red-black tree.
// Keys are maintained in ascending order.
type Float64Float64 struct {
	root *float64Float64TreeNode
	size int
}

// NewFloat64Float64 creates a new empty sorted map.
func NewFloat64Float64() *Float64Float64 {
	return &Float64Float64{}
}

// Put inserts or updates a key-value pair. Returns the previous value and true if the key existed.
func (m *Float64Float64) Put(key float64, value float64) (float64, bool) {
	if m.root == nil {
		m.root = &float64Float64TreeNode{key: key, value: value, color: float64Float64TreeNodeBlack, size: 1}
		m.size++
		return 0.0, false
	}
	node := m.root
	for {
		if cmpFloat64(key, node.key) < 0 {
			if node.left == nil {
				node.left = &float64Float64TreeNode{key: key, value: value, parent: node, color: float64Float64TreeNodeRed, size: 1}
			...
			if node.right == nil {
				node.right = &float64Float64TreeNode{key: key, value: value, parent: node, color: float64Float64TreeNodeRed, size: 1}
				m.incSizeToRoot(node)
...
func (m *Float64Float64) Get(key float64) (float64, bool) {
	node := m.findNode(key)
	if node == nil {
		return 0.0, false
	}
...
func (m *Float64Float64) GetOrDefault(key float64, defaultValue float64) float64 {
...
func (m *Float64Float64) Len() int {
	return m.size
}

// Clear removes all entries.
func (m *Float64Float64) Clear() {
	m.root = nil
	m.size = 0
}

// Min returns the smallest key and its value, or zero values and false if empty.
func (m *Float64Float64) Min() (float64, float64, bool) {
	if m.root == nil {
		return 0, 0.0, false
	}
...
func (m *Float64Float64) Max() (float64, float64, bool) {
	if m.root == nil {
		return 0, 0.0, false
	}
...
func (m *Float64Float64) Floor(key float64) (float64, float64, bool) {
...
func (m *Float64Float64) Ceiling(key float64) (float64, float64, bool) {
	var result *float64Float64TreeNode
	node := m.root
	for node != nil {
		if cmpFloat64(key, node.key) == 0 {
...
func (m *Float64Float64) All() iter.Seq2[float64, float64] {
...
// reproduces All in ascending key order — so a *Float64Float64 satisfies
// par.Segmenter2[float64, float64] and feeds par.From2 with ORDERED
...
func (m *Float64Float64) Segments2(n int) []iter.Seq2[float64, float64] {
	ranges := segment.SplitRanges(m.size, n)
	segs := make([]iter.Seq2[float64, float64], len(ranges))
	for i := range ranges {
		lo, hi := ranges[i][0], ranges[i][1] // per-iteration copies for this view's closure
		segs[i] = func(yield func(float64, float64) bool) {
			// In-order walk yielding pairs whose 0-based key rank is in [lo, hi).
			// base is the rank of the first element in node's subtree.
			var walk func(node *float64Float64TreeNode, base int) bool
			walk = func(node *float64Float64TreeNode, base int) bool {
				if node == nil {
					return true
				}
				rank := base + float64Float64TreeNodeSize(node.left)
...
func (m *Float64Float64) Keys() iter.Seq[float64] {
...
func (m *Float64Float64) Values() iter.Seq[float64] {
	return func(yield func(float64) bool) {
		for _, v := range m.All() {
			if !yield(v) {
				return
...
func (m *Float64Float64) RangeKeys(fromKey, toKey float64) iter.Seq2[float64, float64] {
	return func(yield func(float64, float64) bool) {
		for k, v := range m.All() {
...
func (m *Float64Float64) Higher(key float64) (float64, float64, bool) {
	var result *float64Float64TreeNode
	node := m.root
	for node != nil {
		if cmpFloat64(key, node.key) < 0 {
...
func (m *Float64Float64) Lower(key float64) (float64, float64, bool) {
	var result *float64Float64TreeNode
	node := m.root
	for node != nil {
		if cmpFloat64(key, node.key) > 0 {
...
func (m *Float64Float64) HeadMap(toKey float64) iter.Seq2[float64, float64] {
	return func(yield func(float64, float64) bool) {
		for k, v := range m.All() {
			if cmpFloat64(k, toKey) >= 0 {
				return
...
func (m *Float64Float64) TailMap(fromKey float64) iter.Seq2[float64, float64] {
	return func(yield func(float64, float64) bool) {
		for k, v := range m.All() {
			if cmpFloat64(k, fromKey) < 0 {
				continue
...
func (m *Float64Float64) PollLastEntry() (float64, float64, bool) {
	k, v, ok := m.Max()
	if !ok {
		return 0, 0.0, false
	}
...
func (m *Float64Float64) DescendingMap() iter.Seq2[float64, float64] {
	return func(yield func(float64, float64) bool) {
...
func (m *Float64Float64) DescendingKeys() iter.Seq[float64] {
	return func(yield func(float64) bool) {
...
func (m *Float64Float64) ForEach(f func(float64, float64)) {
	for k, v := range m.All() {
		f(k, v)
	}
}
...
func (m *Float64Float64) Select(predicate func(float64, float64) bool) *Float64Float64 {
	result := NewFloat64Float64()
	for k, v := range m.All() {
		if predicate(k, v) {
			result.Put(k, v)
		}
...
func (m *Float64Float64) Reject(predicate func(float64, float64) bool) *Float64Float64 {
	result := NewFloat64Float64()
	for k, v := range m.All() {
		if !predicate(k, v) {
			result.Put(k, v)
...
func (m *Float64Float64) Detect(predicate func(float64, float64) bool) (float64, float64, bool) {
	for k, v := range m.All() {
		if predicate(k, v) {
			return k, v, true
		}
...
func (m *Float64Float64) AnySatisfy(predicate func(float64, float64) bool) bool {
	for k, v := range m.All() {
		if predicate(k, v) {
			return true
		}
...
func (m *Float64Float64) AllSatisfy(predicate func(float64, float64) bool) bool {
	for k, v := range m.All() {
		if !predicate(k, v) {
			return false
		}
...
func (m *Float64Float64) NoneSatisfy(predicate func(float64, float64) bool) bool {
	for k, v := range m.All() {
		if predicate(k, v) {
			return false
		}
...
func (m *Float64Float64) Count(predicate func(float64, float64) bool) int {
	c := 0
	for k, v := range m.All() {
		if predicate(k, v) {
			c++
...
// query; never mutates.
func (m *Float64Float64) Rank(key float64) int {
	rank := 0
	node := m.root
	for node != nil {
		if cmpFloat64(key, node.key) < 0 {
			...
			rank += 1 + float64Float64TreeNodeSize(node.left)
			node = node.right
		} else {
			// key == node.key: exactly the left subtree is strictly less.
			return rank + float64Float64TreeNodeSize(node.left)
		}
	}
	return rank
}
...
func (m *Float64Float64) SelectKey(i int) (float64, bool) {
	node := m.selectNode(i)
	if node == nil {
		return 0, false
	}
...
func (m *Float64Float64) SelectEntry(i int) (float64, float64, bool) {
	node := m.selectNode(i)
	if node == nil {
		return 0, 0.0, false
	}
...
// O(log n).
func (m *Float64Float64) selectNode(i int) *float64Float64TreeNode {
	if i < 0 {
		return nil
	}
	node := m.root
	for node != nil {
		left := float64Float64TreeNodeSize(node.left)
		if i < left {
			node = node.left
		} else if i == left {
			return node
...
func (m *Float64Float64) String() string {
	if m.size == 0 {
		return "{}"
	}
...
func (m *Float64Float64) findNode(key float64) *float64Float64TreeNode {
	node := m.root
	for node != nil {
		if cmpFloat64(key, node.key) < 0 {
			node = node.left
...
func (m *Float64Float64) minNode(node *float64Float64TreeNode) *float64Float64TreeNode {
	for node.left != nil {
		node = node.left
	}
	return node
...
func (m *Float64Float64) maxNode(node *float64Float64TreeNode) *float64Float64TreeNode {
	for node.right != nil {
		node = node.right
	}
	return node
...
func (m *Float64Float64) rotateLeft(x *float64Float64TreeNode) {
	y := x.right
	x.right = y.left
	if y.left != nil {
		y.left.parent = x
	...
	float64Float64TreeNodeFixSize(x)
	float64Float64TreeNodeFixSize(y)
}

func (m *Float64Float64) rotateRight(x *float64Float64TreeNode) {
	y := x.left
	x.left = y.right
	if y.right != nil {
		y.right.parent = x
...
func (m *Float64Float64) incSizeToRoot(n *float64Float64TreeNode) {
...
func (m *Float64Float64) fixSizeToRoot(n *float64Float64TreeNode) {
	for ; n != nil; n = n.parent {
		float64Float64TreeNodeFixSize(n)
	}
}

func (m *Float64Float64) fixAfterInsert(z *float64Float64TreeNode) {
...
func (m *Float64Float64) deleteNode(z *float64Float64TreeNode) {
	...
	var fixSizeFrom *float64Float64TreeNode
...
// NewFloat64Float64FromSorted builds a Float64Float64 from presorted, ascending keys
...
func NewFloat64Float64FromSorted(keys []float64, values []float64, policy pump.DuplicatePolicy) (*Float64Float64, error) {
	if len(keys) != len(values) {
		panic("mapdb: NewFloat64Float64FromSorted: len(keys) != len(values)")
	}
	dk, dv, err := dedupFloat64Float64Sorted(keys, values, policy)
	if err != nil {
		return nil, err
	}
	m := NewFloat64Float64()
	m.root = m.buildfloat64Float64(dk, dv, 0, len(dk)-1, 0, pump.RedBlackRedLevel(len(dk)), nil)
	m.size = len(dk)
	return m, nil
}

// dedupFloat64Float64Sorted validates ascending order and applies the duplicate
// policy, returning compacted key/value slices. Equal adjacent keys are the only
// duplicates possible in sorted input.
func dedupFloat64Float64Sorted(keys []float64, values []float64, policy pump.DuplicatePolicy) ([]float64, []float64, error) {
	if len(keys) == 0 {
...
// buildfloat64Float64 recursively builds a perfectly balanced subtree from the
...
func (m *Float64Float64) buildfloat64Float64(keys []float64, values []float64, lo, hi, level, redLevel int, parent *float64Float64TreeNode) *float64Float64TreeNode {
	if lo > hi {
		return nil
	}
	mid := (lo + hi) / 2
	node := &float64Float64TreeNode{key: keys[mid], value: values[mid], parent: parent, color: float64Float64TreeNodeBlack}
	if level == redLevel {
		node.color = float64Float64TreeNodeRed
	}
	node.left = m.buildfloat64Float64(keys, values, lo, mid-1, level+1, redLevel, node)
	node.right = m.buildfloat64Float64(keys, values, mid+1, hi, level+1, redLevel, node)
	...
	node.size = 1 + float64Float64TreeNodeSize(node.left) + float64Float64TreeNodeSize(node.right)
	return node
}

// Float64Float64Sink is a streaming builder for a Float64Float64: callers Put
// ascending entries one at a time, then Build the finished map. It is a thin
// wrapper over NewFloat64Float64FromSorted (entries are buffered, then built in one
...
type Float64Float64Sink struct {
	keys   []float64
	values []float64
...
// NewFloat64Float64Sink creates a streaming sink with the given duplicate policy.
func NewFloat64Float64Sink(policy pump.DuplicatePolicy) *Float64Float64Sink {
	return &Float64Float64Sink{policy: policy}
}
...
func (s *Float64Float64Sink) Put(key float64, value float64) {
	if s.done {
		panic("mapdb: Put on a finished Float64Float64Sink")
	}
...
func (s *Float64Float64Sink) Build() (*Float64Float64, error) {
	if s.done {
		panic("mapdb: Build on a finished Float64Float64Sink")
	}
	s.done = true
	return NewFloat64Float64FromSorted(s.keys, s.values, s.policy)
}

func (m *Float64Float64) fixAfterDelete(x *float64Float64TreeNode) {
