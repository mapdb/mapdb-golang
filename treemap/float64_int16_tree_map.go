...
// float64Int16TreeNodeSize returns the subtree size of a node link (0 if nil).
func float64Int16TreeNodeSize(n *float64Int16TreeNode) int {
	if n == nil {
		return 0
	}
	return n.size
...
// float64Int16TreeNodeFixSize recomputes a node's cached subtree size from its
// children. Called after any rotation or child relink so the augmentation stays
// consistent.
func float64Int16TreeNodeFixSize(n *float64Int16TreeNode) {
	n.size = 1 + float64Int16TreeNodeSize(n.left) + float64Int16TreeNodeSize(n.right)
}
...
func (m *Float64Int16) Put(key float64, value int16) (int16, bool) {
	if m.root == nil {
		m.root = &float64Int16TreeNode{key: key, value: value, color: float64Int16TreeNodeBlack, size: 1}
		m.size++
		return 0, false
	}
	node := m.root
	for {
		if cmpFloat64(key, node.key) < 0 {
			if node.left == nil {
				node.left = &float64Int16TreeNode{key: key, value: value, parent: node, color: float64Int16TreeNodeRed, size: 1}
			...
			if node.right == nil {
				node.right = &float64Int16TreeNode{key: key, value: value, parent: node, color: float64Int16TreeNodeRed, size: 1}
				m.incSizeToRoot(node)
...
// reproduces All in ascending key order — so a *Float64Int16 satisfies
// par.Segmenter2[float64, int16] and feeds par.From2 with ORDERED
...
func (m *Float64Int16) Segments2(n int) []iter.Seq2[float64, int16] {
	ranges := segment.SplitRanges(m.size, n)
	segs := make([]iter.Seq2[float64, int16], len(ranges))
	for i := range ranges {
		lo, hi := ranges[i][0], ranges[i][1] // per-iteration copies for this view's closure
		segs[i] = func(yield func(float64, int16) bool) {
			// In-order walk yielding pairs whose 0-based key rank is in [lo, hi).
			// base is the rank of the first element in node's subtree.
			var walk func(node *float64Int16TreeNode, base int) bool
			walk = func(node *float64Int16TreeNode, base int) bool {
				if node == nil {
					return true
				}
				rank := base + float64Int16TreeNodeSize(node.left)
...
// query; never mutates.
func (m *Float64Int16) Rank(key float64) int {
	rank := 0
	node := m.root
	for node != nil {
		if cmpFloat64(key, node.key) < 0 {
			...
			rank += 1 + float64Int16TreeNodeSize(node.left)
			node = node.right
		} else {
			// key == node.key: exactly the left subtree is strictly less.
			return rank + float64Int16TreeNodeSize(node.left)
		}
	}
	return rank
}
...
func (m *Float64Int16) SelectKey(i int) (float64, bool) {
	node := m.selectNode(i)
	if node == nil {
		return 0, false
	}
...
func (m *Float64Int16) SelectEntry(i int) (float64, int16, bool) {
	node := m.selectNode(i)
	if node == nil {
		return 0, 0, false
	}
...
// O(log n).
func (m *Float64Int16) selectNode(i int) *float64Int16TreeNode {
	if i < 0 {
		return nil
	}
	node := m.root
	for node != nil {
		left := float64Int16TreeNodeSize(node.left)
		if i < left {
			node = node.left
		} else if i == left {
			return node
...
func (m *Float64Int16) rotateLeft(x *float64Int16TreeNode) {
	...
	float64Int16TreeNodeFixSize(x)
	float64Int16TreeNodeFixSize(y)
}
...
func (m *Float64Int16) incSizeToRoot(n *float64Int16TreeNode) {
...
func (m *Float64Int16) fixSizeToRoot(n *float64Int16TreeNode) {
	for ; n != nil; n = n.parent {
		float64Int16TreeNodeFixSize(n)
	}
}
...
func (m *Float64Int16) deleteNode(z *float64Int16TreeNode) {
	...
	var fixSizeFrom *float64Int16TreeNode
...
func (m *Float64Int16) buildfloat64Int16(keys []float64, values []int16, lo, hi, level, redLevel int, parent *float64Int16TreeNode) *float64Int16TreeNode {
	...
	node.size = 1 + float64Int16TreeNodeSize(node.left) + float64Int16TreeNodeSize(node.right)
	return node
}
