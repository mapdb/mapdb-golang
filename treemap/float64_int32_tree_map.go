...
// float64Int32TreeNodeSize returns the subtree size of a node link (0 if nil).
func float64Int32TreeNodeSize(n *float64Int32TreeNode) int {
	if n == nil {
		return 0
	}
	return n.size
...
// float64Int32TreeNodeFixSize recomputes a node's cached subtree size from its
// children. Called after any rotation or child relink so the augmentation stays
// consistent.
func float64Int32TreeNodeFixSize(n *float64Int32TreeNode) {
	n.size = 1 + float64Int32TreeNodeSize(n.left) + float64Int32TreeNodeSize(n.right)
}
...
func (m *Float64Int32) Put(key float64, value int32) (int32, bool) {
	if m.root == nil {
		m.root = &float64Int32TreeNode{key: key, value: value, color: float64Int32TreeNodeBlack, size: 1}
		m.size++
		return 0, false
	}
	node := m.root
	for {
		if cmpFloat64(key, node.key) < 0 {
			if node.left == nil {
				node.left = &float64Int32TreeNode{key: key, value: value, parent: node, color: float64Int32TreeNodeRed, size: 1}
			...
			if node.right == nil {
				node.right = &float64Int32TreeNode{key: key, value: value, parent: node, color: float64Int32TreeNodeRed, size: 1}
				m.incSizeToRoot(node)
...
// reproduces All in ascending key order — so a *Float64Int32 satisfies
// par.Segmenter2[float64, int32] and feeds par.From2 with ORDERED
...
func (m *Float64Int32) Segments2(n int) []iter.Seq2[float64, int32] {
	ranges := segment.SplitRanges(m.size, n)
	segs := make([]iter.Seq2[float64, int32], len(ranges))
	for i := range ranges {
		lo, hi := ranges[i][0], ranges[i][1] // per-iteration copies for this view's closure
		segs[i] = func(yield func(float64, int32) bool) {
			// In-order walk yielding pairs whose 0-based key rank is in [lo, hi).
			// base is the rank of the first element in node's subtree.
			var walk func(node *float64Int32TreeNode, base int) bool
			walk = func(node *float64Int32TreeNode, base int) bool {
				if node == nil {
					return true
				}
				rank := base + float64Int32TreeNodeSize(node.left)
...
// query; never mutates.
func (m *Float64Int32) Rank(key float64) int {
	rank := 0
	node := m.root
	for node != nil {
		if cmpFloat64(key, node.key) < 0 {
			...
			rank += 1 + float64Int32TreeNodeSize(node.left)
			node = node.right
		} else {
			// key == node.key: exactly the left subtree is strictly less.
			return rank + float64Int32TreeNodeSize(node.left)
		}
	}
	return rank
}
...
func (m *Float64Int32) SelectKey(i int) (float64, bool) {
	node := m.selectNode(i)
	if node == nil {
		return 0, false
	}
...
func (m *Float64Int32) SelectEntry(i int) (float64, int32, bool) {
	node := m.selectNode(i)
	if node == nil {
		return 0, 0, false
	}
...
// O(log n).
func (m *Float64Int32) selectNode(i int) *float64Int32TreeNode {
	if i < 0 {
		return nil
	}
	node := m.root
	for node != nil {
		left := float64Int32TreeNodeSize(node.left)
		if i < left {
			node = node.left
		} else if i == left {
			return node
...
func (m *Float64Int32) rotateLeft(x *float64Int32TreeNode) {
	...
	float64Int32TreeNodeFixSize(x)
	float64Int32TreeNodeFixSize(y)
}
...
func (m *Float64Int32) incSizeToRoot(n *float64Int32TreeNode) {
...
func (m *Float64Int32) fixSizeToRoot(n *float64Int32TreeNode) {
	for ; n != nil; n = n.parent {
		float64Int32TreeNodeFixSize(n)
	}
}
...
func (m *Float64Int32) deleteNode(z *float64Int32TreeNode) {
	...
	var fixSizeFrom *float64Int32TreeNode
...
// NewFloat64Int32FromSorted builds a Float64Int32 from presorted, ascending keys
...
func NewFloat64Int32FromSorted(keys []float64, values []int32, policy pump.DuplicatePolicy) (*Float64Int32, error) {
	if len(keys) != len(values) {
		panic("mapdb: NewFloat64Int32FromSorted: len(keys) != len(values)")
	}
	dk, dv, err := dedupFloat64Int32Sorted(keys, values, policy)
	if err != nil {
		return nil, err
	}
	m := NewFloat64Int32()
	m.root = m.buildfloat64Int32(dk, dv, 0, len(dk)-1, 0, pump.RedBlackRedLevel(len(dk)), nil)
	m.size = len(dk)
	return m, nil
}

// dedupFloat64Int32Sorted validates ascending order and applies the duplicate
// policy, returning compacted key/value slices. Equal adjacent keys are the only
// duplicates possible in sorted input.
func dedupFloat64Int32Sorted(keys []float64, values []int32, policy pump.DuplicatePolicy) ([]float64, []int32, error) {
	if len(keys) == 0 {
...
// buildfloat64Int32 recursively builds a perfectly balanced subtree from the
...
func (m *Float64Int32) buildfloat64Int32(keys []float64, values []int32, lo, hi, level, redLevel int, parent *float64Int32TreeNode) *float64Int32TreeNode {
	if lo > hi {
		return nil
	}
	mid := (lo + hi) / 2
	node := &float64Int32TreeNode{key: keys[mid], value: values[mid], parent: parent, color: float64Int32TreeNodeBlack}
	if level == redLevel {
		node.color = float64Int32TreeNodeRed
	}
	node.left = m.buildfloat64Int32(keys, values, lo, mid-1, level+1, redLevel, node)
	node.right = m.buildfloat64Int32(keys, values, mid+1, hi, level+1, redLevel, node)
	...
	node.size = 1 + float64Int32TreeNodeSize(node.left) + float64Int32TreeNodeSize(node.right)
	return node
}

// Float64Int32Sink is a streaming builder for a Float64Int32: callers Put
// ascending entries one at a time, then Build the finished map. It is a thin
// wrapper over NewFloat64Int32FromSorted (entries are buffered, then built in one
...
// NewFloat64Int32Sink creates a streaming sink with the given duplicate policy.
func NewFloat64Int32Sink(policy pump.DuplicatePolicy) *Float64Int32Sink {
	return &Float64Int32Sink{policy: policy}
}
...
func (s *Float64Int32Sink) Put(key float64, value int32) {
	if s.done {
		panic("mapdb: Put on a finished Float64Int32Sink")
	}
...
func (s *Float64Int32Sink) Build() (*Float64Int32, error) {
	if s.done {
		panic("mapdb: Build on a finished Float64Int32Sink")
	}
	s.done = true
	return NewFloat64Int32FromSorted(s.keys, s.values, s.policy)
}
