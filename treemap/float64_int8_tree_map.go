...
// reproduces All in ascending key order — so a *Float64Int8 satisfies
// par.Segmenter2[float64, int8] and feeds par.From2 with ORDERED
...
func (m *Float64Int8) Segments2(n int) []iter.Seq2[float64, int8] {
	ranges := segment.SplitRanges(m.size, n)
	segs := make([]iter.Seq2[float64, int8], len(ranges))
	for i := range ranges {
		lo, hi := ranges[i][0], ranges[i][1] // per-iteration copies for this view's closure
		segs[i] = func(yield func(float64, int8) bool) {
			// In-order walk yielding pairs whose 0-based key rank is in [lo, hi).
			// base is the rank of the first element in node's subtree.
			var walk func(node *float64Int8TreeNode, base int) bool
			walk = func(node *float64Int8TreeNode, base int) bool {
				if node == nil {
					return true
				}
				rank := base + float64Int8TreeNodeSize(node.left)
