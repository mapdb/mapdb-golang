...
// reproduces All in ascending key order — so a *Int16Char satisfies
// par.Segmenter2[int16, uint16] and feeds par.From2 with ORDERED
...
func (m *Int16Char) Segments2(n int) []iter.Seq2[int16, uint16] {
	ranges := segment.SplitRanges(m.size, n)
	segs := make([]iter.Seq2[int16, uint16], len(ranges))
	for i := range ranges {
		lo, hi := ranges[i][0], ranges[i][1] // per-iteration copies for this view's closure
		segs[i] = func(yield func(int16, uint16) bool) {
			// In-order walk yielding pairs whose 0-based key rank is in [lo, hi).
			// base is the rank of the first element in node's subtree.
			var walk func(node *int16CharTreeNode, base int) bool
			walk = func(node *int16CharTreeNode, base int) bool {
				if node == nil {
					return true
				}
				rank := base + int16CharTreeNodeSize(node.left)
...
func (m *Int16Char) buildint16Char(keys []int16, values []uint16, lo, hi, level, redLevel int, parent *int16CharTreeNode) *int16CharTreeNode {
	...
	node.size = 1 + int16CharTreeNodeSize(node.left) + int16CharTreeNodeSize(node.right)
	return node
}
