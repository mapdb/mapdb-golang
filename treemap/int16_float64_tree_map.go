...
// int16Float64TreeNodeSize returns the subtree size of a node link (0 if nil).
func int16Float64TreeNodeSize(n *int16Float64TreeNode) int {
	if n == nil {
		return 0
	}
	return n.size
...
// int16Float64TreeNodeFixSize recomputes a node's cached subtree size from its
// children. Called after any rotation or child relink so the augmentation stays
// consistent.
func int16Float64TreeNodeFixSize(n *int16Float64TreeNode) {
	n.size = 1 + int16Float64TreeNodeSize(n.left) + int16Float64TreeNodeSize(n.right)
}

// Int16Float64 is a sorted map with int16 keys and float64 values, backed by a red-black tree.
...
// NewInt16Float64 creates a new empty sorted map.
...
func (m *Int16Float64) Put(key int16, value float64) (float64, bool) {
	if m.root == nil {
		m.root = &int16Float64TreeNode{key: key, value: value, color: int16Float64TreeNodeBlack, size: 1}
		m.size++
		return 0.0, false
	}
	node := m.root
	for {
		if key < node.key {
			if node.left == nil {
				node.left = &int16Float64TreeNode{key: key, value: value, parent: node, color: int16Float64TreeNodeRed, size: 1}
			...
			if node.right == nil {
				node.right = &int16Float64TreeNode{key: key, value: value, parent: node, color: int16Float64TreeNodeRed, size: 1}
				m.incSizeToRoot(node)
...
func (m *Int16Float64) Min() (int16, float64, bool) {
	if m.root == nil {
		return 0, 0.0, false
	}
...
func (m *Int16Float64) Max() (int16, float64, bool) {
	if m.root == nil {
		return 0, 0.0, false
	}
...
func (m *Int16Float64) Floor(key int16) (int16, float64, bool) {
...
func (m *Int16Float64) Ceiling(key int16) (int16, float64, bool) {
	var result *int16Float64TreeNode
	node := m.root
	for node != nil {
		if key == node.key {
...
// reproduces All in ascending key order — so a *Int16Float64 satisfies
// par.Segmenter2[int16, float64] and feeds par.From2 with ORDERED
...
func (m *Int16Float64) Segments2(n int) []iter.Seq2[int16, float64] {
	ranges := segment.SplitRanges(m.size, n)
	segs := make([]iter.Seq2[int16, float64], len(ranges))
	for i := range ranges {
		lo, hi := ranges[i][0], ranges[i][1] // per-iteration copies for this view's closure
		segs[i] = func(yield func(int16, float64) bool) {
			// In-order walk yielding pairs whose 0-based key rank is in [lo, hi).
			// base is the rank of the first element in node's subtree.
			var walk func(node *int16Float64TreeNode, base int) bool
			walk = func(node *int16Float64TreeNode, base int) bool {
				if node == nil {
					return true
				}
				rank := base + int16Float64TreeNodeSize(node.left)
...
func (m *Int16Float64) RangeKeys(fromKey, toKey int16) iter.Seq2[int16, float64] {
	return func(yield func(int16, float64) bool) {
		for k, v := range m.All() {
			if k < fromKey {
				continue
...
func (m *Int16Float64) Higher(key int16) (int16, float64, bool) {
	var result *int16Float64TreeNode
	node := m.root
	for node != nil {
		if key < node.key {
...
func (m *Int16Float64) Lower(key int16) (int16, float64, bool) {
	var result *int16Float64TreeNode
	node := m.root
	for node != nil {
		if key > node.key {
...
func (m *Int16Float64) HeadMap(toKey int16) iter.Seq2[int16, float64] {
	return func(yield func(int16, float64) bool) {
		for k, v := range m.All() {
			if k >= toKey {
				return
...
func (m *Int16Float64) TailMap(fromKey int16) iter.Seq2[int16, float64] {
	return func(yield func(int16, float64) bool) {
		for k, v := range m.All() {
			if k < fromKey {
				continue
...
func (m *Int16Float64) PollLastEntry() (int16, float64, bool) {
	k, v, ok := m.Max()
	if !ok {
		return 0, 0.0, false
	}
...
func (m *Int16Float64) DescendingMap() iter.Seq2[int16, float64] {
	return func(yield func(int16, float64) bool) {
...
func (m *Int16Float64) DescendingKeys() iter.Seq[int16] {
	return func(yield func(int16) bool) {
...
// query; never mutates.
func (m *Int16Float64) Rank(key int16) int {
	rank := 0
	node := m.root
	for node != nil {
		if key < node.key {
			...
			rank += 1 + int16Float64TreeNodeSize(node.left)
			node = node.right
		} else {
			// key == node.key: exactly the left subtree is strictly less.
			return rank + int16Float64TreeNodeSize(node.left)
		}
	}
	return rank
}
...
func (m *Int16Float64) SelectKey(i int) (int16, bool) {
	node := m.selectNode(i)
	if node == nil {
		return 0, false
	}
...
func (m *Int16Float64) SelectEntry(i int) (int16, float64, bool) {
	node := m.selectNode(i)
	if node == nil {
		return 0, 0.0, false
	}
...
// O(log n).
func (m *Int16Float64) selectNode(i int) *int16Float64TreeNode {
	if i < 0 {
		return nil
	}
	node := m.root
	for node != nil {
		left := int16Float64TreeNodeSize(node.left)
		if i < left {
			node = node.left
		} else if i == left {
			return node
...
func (m *Int16Float64) findNode(key int16) *int16Float64TreeNode {
	node := m.root
	for node != nil {
		if key < node.key {
			node = node.left
...
func (m *Int16Float64) minNode(node *int16Float64TreeNode) *int16Float64TreeNode {
	for node.left != nil {
		node = node.left
	}
	return node
...
func (m *Int16Float64) maxNode(node *int16Float64TreeNode) *int16Float64TreeNode {
	for node.right != nil {
		node = node.right
	}
	return node
...
func (m *Int16Float64) rotateLeft(x *int16Float64TreeNode) {
	y := x.right
	x.right = y.left
	if y.left != nil {
		y.left.parent = x
	...
	int16Float64TreeNodeFixSize(x)
	int16Float64TreeNodeFixSize(y)
}

func (m *Int16Float64) rotateRight(x *int16Float64TreeNode) {
	y := x.left
	x.left = y.right
	if y.right != nil {
		y.right.parent = x
...
func (m *Int16Float64) incSizeToRoot(n *int16Float64TreeNode) {
...
func (m *Int16Float64) fixSizeToRoot(n *int16Float64TreeNode) {
	for ; n != nil; n = n.parent {
		int16Float64TreeNodeFixSize(n)
	}
}

func (m *Int16Float64) fixAfterInsert(z *int16Float64TreeNode) {
...
func (m *Int16Float64) deleteNode(z *int16Float64TreeNode) {
	...
	var fixSizeFrom *int16Float64TreeNode
...
// NewInt16Float64FromSorted builds a Int16Float64 from presorted, ascending keys
...
func NewInt16Float64FromSorted(keys []int16, values []float64, policy pump.DuplicatePolicy) (*Int16Float64, error) {
	if len(keys) != len(values) {
		panic("mapdb: NewInt16Float64FromSorted: len(keys) != len(values)")
	}
	dk, dv, err := dedupInt16Float64Sorted(keys, values, policy)
	if err != nil {
		return nil, err
	}
	m := NewInt16Float64()
	m.root = m.buildint16Float64(dk, dv, 0, len(dk)-1, 0, pump.RedBlackRedLevel(len(dk)), nil)
	m.size = len(dk)
	return m, nil
}

// dedupInt16Float64Sorted validates ascending order and applies the duplicate
// policy, returning compacted key/value slices. Equal adjacent keys are the only
// duplicates possible in sorted input.
func dedupInt16Float64Sorted(keys []int16, values []float64, policy pump.DuplicatePolicy) ([]int16, []float64, error) {
	if len(keys) == 0 {
	...
	for i := 1; i < len(keys); i++ {
		cmp := cmpInt16Float64(keys[i], keys[i-1])
		if cmp < 0 {
...
// cmpInt16Float64 is the three-way ordering used by the bulk-load validator for
// integer/char keys (float keys use the IEEE total-order helper instead).
func cmpInt16Float64(a, b int16) int {
	switch {
	case a < b:
		return -1
	case a > b:
...
// buildint16Float64 recursively builds a perfectly balanced subtree from the
...
func (m *Int16Float64) buildint16Float64(keys []int16, values []float64, lo, hi, level, redLevel int, parent *int16Float64TreeNode) *int16Float64TreeNode {
	if lo > hi {
		return nil
	}
	mid := (lo + hi) / 2
	node := &int16Float64TreeNode{key: keys[mid], value: values[mid], parent: parent, color: int16Float64TreeNodeBlack}
	if level == redLevel {
		node.color = int16Float64TreeNodeRed
	}
	node.left = m.buildint16Float64(keys, values, lo, mid-1, level+1, redLevel, node)
	node.right = m.buildint16Float64(keys, values, mid+1, hi, level+1, redLevel, node)
	...
	node.size = 1 + int16Float64TreeNodeSize(node.left) + int16Float64TreeNodeSize(node.right)
	return node
}

// Int16Float64Sink is a streaming builder for a Int16Float64: callers Put
// ascending entries one at a time, then Build the finished map. It is a thin
// wrapper over NewInt16Float64FromSorted (entries are buffered, then built in one
...
// NewInt16Float64Sink creates a streaming sink with the given duplicate policy.
func NewInt16Float64Sink(policy pump.DuplicatePolicy) *Int16Float64Sink {
	return &Int16Float64Sink{policy: policy}
}
...
func (s *Int16Float64Sink) Put(key int16, value float64) {
	if s.done {
		panic("mapdb: Put on a finished Int16Float64Sink")
	}
...
func (s *Int16Float64Sink) Build() (*Int16Float64, error) {
	if s.done {
		panic("mapdb: Build on a finished Int16Float64Sink")
	}
	s.done = true
	return NewInt16Float64FromSorted(s.keys, s.values, s.policy)
}

func (m *Int16Float64) fixAfterDelete(x *int16Float64TreeNode) {
