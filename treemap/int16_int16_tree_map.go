...
// int16Int16TreeNodeSize returns the subtree size of a node link (0 if nil).
func int16Int16TreeNodeSize(n *int16Int16TreeNode) int {
	if n == nil {
		return 0
	}
	return n.size
...
// int16Int16TreeNodeFixSize recomputes a node's cached subtree size from its
// children. Called after any rotation or child relink so the augmentation stays
// consistent.
func int16Int16TreeNodeFixSize(n *int16Int16TreeNode) {
	n.size = 1 + int16Int16TreeNodeSize(n.left) + int16Int16TreeNodeSize(n.right)
}
...
func (m *Int16Int16) Put(key int16, value int16) (int16, bool) {
	if m.root == nil {
		m.root = &int16Int16TreeNode{key: key, value: value, color: int16Int16TreeNodeBlack, size: 1}
		m.size++
		return 0, false
	}
	node := m.root
	for {
		if key < node.key {
			if node.left == nil {
				node.left = &int16Int16TreeNode{key: key, value: value, parent: node, color: int16Int16TreeNodeRed, size: 1}
			...
			if node.right == nil {
				node.right = &int16Int16TreeNode{key: key, value: value, parent: node, color: int16Int16TreeNodeRed, size: 1}
				m.incSizeToRoot(node)
...
// reproduces All in ascending key order — so a *Int16Int16 satisfies
// par.Segmenter2[int16, int16] and feeds par.From2 with ORDERED
...
func (m *Int16Int16) Segments2(n int) []iter.Seq2[int16, int16] {
	ranges := segment.SplitRanges(m.size, n)
	segs := make([]iter.Seq2[int16, int16], len(ranges))
	for i := range ranges {
		lo, hi := ranges[i][0], ranges[i][1] // per-iteration copies for this view's closure
		segs[i] = func(yield func(int16, int16) bool) {
			// In-order walk yielding pairs whose 0-based key rank is in [lo, hi).
			// base is the rank of the first element in node's subtree.
			var walk func(node *int16Int16TreeNode, base int) bool
			walk = func(node *int16Int16TreeNode, base int) bool {
				if node == nil {
					return true
				}
				rank := base + int16Int16TreeNodeSize(node.left)
...
// query; never mutates.
func (m *Int16Int16) Rank(key int16) int {
	rank := 0
	node := m.root
	for node != nil {
		if key < node.key {
			...
			rank += 1 + int16Int16TreeNodeSize(node.left)
			node = node.right
		} else {
			// key == node.key: exactly the left subtree is strictly less.
			return rank + int16Int16TreeNodeSize(node.left)
		}
	}
	return rank
}
...
func (m *Int16Int16) SelectKey(i int) (int16, bool) {
	node := m.selectNode(i)
	if node == nil {
		return 0, false
	}
...
func (m *Int16Int16) SelectEntry(i int) (int16, int16, bool) {
	node := m.selectNode(i)
	if node == nil {
		return 0, 0, false
	}
...
// O(log n).
func (m *Int16Int16) selectNode(i int) *int16Int16TreeNode {
	if i < 0 {
		return nil
	}
	node := m.root
	for node != nil {
		left := int16Int16TreeNodeSize(node.left)
		if i < left {
			node = node.left
		} else if i == left {
			return node
...
func (m *Int16Int16) rotateLeft(x *int16Int16TreeNode) {
	...
	int16Int16TreeNodeFixSize(x)
	int16Int16TreeNodeFixSize(y)
}
...
func (m *Int16Int16) incSizeToRoot(n *int16Int16TreeNode) {
...
func (m *Int16Int16) fixSizeToRoot(n *int16Int16TreeNode) {
	for ; n != nil; n = n.parent {
		int16Int16TreeNodeFixSize(n)
	}
}
...
func (m *Int16Int16) deleteNode(z *int16Int16TreeNode) {
	...
	var fixSizeFrom *int16Int16TreeNode
...
func (m *Int16Int16) buildint16Int16(keys []int16, values []int16, lo, hi, level, redLevel int, parent *int16Int16TreeNode) *int16Int16TreeNode {
	...
	node.size = 1 + int16Int16TreeNodeSize(node.left) + int16Int16TreeNodeSize(node.right)
	return node
}
