...
// int16Int32TreeNodeSize returns the subtree size of a node link (0 if nil).
func int16Int32TreeNodeSize(n *int16Int32TreeNode) int {
	if n == nil {
		return 0
	}
	return n.size
...
// int16Int32TreeNodeFixSize recomputes a node's cached subtree size from its
// children. Called after any rotation or child relink so the augmentation stays
// consistent.
func int16Int32TreeNodeFixSize(n *int16Int32TreeNode) {
	n.size = 1 + int16Int32TreeNodeSize(n.left) + int16Int32TreeNodeSize(n.right)
}
...
func (m *Int16Int32) Put(key int16, value int32) (int32, bool) {
	if m.root == nil {
		m.root = &int16Int32TreeNode{key: key, value: value, color: int16Int32TreeNodeBlack, size: 1}
		m.size++
		return 0, false
	}
	node := m.root
	for {
		if key < node.key {
			if node.left == nil {
				node.left = &int16Int32TreeNode{key: key, value: value, parent: node, color: int16Int32TreeNodeRed, size: 1}
			...
			if node.right == nil {
				node.right = &int16Int32TreeNode{key: key, value: value, parent: node, color: int16Int32TreeNodeRed, size: 1}
				m.incSizeToRoot(node)
...
// reproduces All in ascending key order — so a *Int16Int32 satisfies
// par.Segmenter2[int16, int32] and feeds par.From2 with ORDERED
...
func (m *Int16Int32) Segments2(n int) []iter.Seq2[int16, int32] {
	ranges := segment.SplitRanges(m.size, n)
	segs := make([]iter.Seq2[int16, int32], len(ranges))
	for i := range ranges {
		lo, hi := ranges[i][0], ranges[i][1] // per-iteration copies for this view's closure
		segs[i] = func(yield func(int16, int32) bool) {
			// In-order walk yielding pairs whose 0-based key rank is in [lo, hi).
			// base is the rank of the first element in node's subtree.
			var walk func(node *int16Int32TreeNode, base int) bool
			walk = func(node *int16Int32TreeNode, base int) bool {
				if node == nil {
					return true
				}
				rank := base + int16Int32TreeNodeSize(node.left)
...
// query; never mutates.
func (m *Int16Int32) Rank(key int16) int {
	rank := 0
	node := m.root
	for node != nil {
		if key < node.key {
			...
			rank += 1 + int16Int32TreeNodeSize(node.left)
			node = node.right
		} else {
			// key == node.key: exactly the left subtree is strictly less.
			return rank + int16Int32TreeNodeSize(node.left)
		}
	}
	return rank
}
...
func (m *Int16Int32) SelectKey(i int) (int16, bool) {
	node := m.selectNode(i)
	if node == nil {
		return 0, false
	}
...
func (m *Int16Int32) SelectEntry(i int) (int16, int32, bool) {
	node := m.selectNode(i)
	if node == nil {
		return 0, 0, false
	}
...
// O(log n).
func (m *Int16Int32) selectNode(i int) *int16Int32TreeNode {
	if i < 0 {
		return nil
	}
	node := m.root
	for node != nil {
		left := int16Int32TreeNodeSize(node.left)
		if i < left {
			node = node.left
		} else if i == left {
			return node
...
func (m *Int16Int32) rotateLeft(x *int16Int32TreeNode) {
	...
	int16Int32TreeNodeFixSize(x)
	int16Int32TreeNodeFixSize(y)
}
...
func (m *Int16Int32) incSizeToRoot(n *int16Int32TreeNode) {
...
func (m *Int16Int32) fixSizeToRoot(n *int16Int32TreeNode) {
	for ; n != nil; n = n.parent {
		int16Int32TreeNodeFixSize(n)
	}
}
...
func (m *Int16Int32) deleteNode(z *int16Int32TreeNode) {
	...
	var fixSizeFrom *int16Int32TreeNode
...
// NewInt16Int32FromSorted builds a Int16Int32 from presorted, ascending keys
...
func NewInt16Int32FromSorted(keys []int16, values []int32, policy pump.DuplicatePolicy) (*Int16Int32, error) {
	if len(keys) != len(values) {
		panic("mapdb: NewInt16Int32FromSorted: len(keys) != len(values)")
	}
	dk, dv, err := dedupInt16Int32Sorted(keys, values, policy)
	if err != nil {
		return nil, err
	}
	m := NewInt16Int32()
	m.root = m.buildint16Int32(dk, dv, 0, len(dk)-1, 0, pump.RedBlackRedLevel(len(dk)), nil)
	m.size = len(dk)
	return m, nil
}

// dedupInt16Int32Sorted validates ascending order and applies the duplicate
// policy, returning compacted key/value slices. Equal adjacent keys are the only
// duplicates possible in sorted input.
func dedupInt16Int32Sorted(keys []int16, values []int32, policy pump.DuplicatePolicy) ([]int16, []int32, error) {
	if len(keys) == 0 {
	...
	for i := 1; i < len(keys); i++ {
		cmp := cmpInt16Int32(keys[i], keys[i-1])
		if cmp < 0 {
...
// cmpInt16Int32 is the three-way ordering used by the bulk-load validator for
// integer/char keys (float keys use the IEEE total-order helper instead).
func cmpInt16Int32(a, b int16) int {
	switch {
	case a < b:
		return -1
	case a > b:
...
// buildint16Int32 recursively builds a perfectly balanced subtree from the
...
func (m *Int16Int32) buildint16Int32(keys []int16, values []int32, lo, hi, level, redLevel int, parent *int16Int32TreeNode) *int16Int32TreeNode {
	if lo > hi {
		return nil
	}
	mid := (lo + hi) / 2
	node := &int16Int32TreeNode{key: keys[mid], value: values[mid], parent: parent, color: int16Int32TreeNodeBlack}
	if level == redLevel {
		node.color = int16Int32TreeNodeRed
	}
	node.left = m.buildint16Int32(keys, values, lo, mid-1, level+1, redLevel, node)
	node.right = m.buildint16Int32(keys, values, mid+1, hi, level+1, redLevel, node)
	...
	node.size = 1 + int16Int32TreeNodeSize(node.left) + int16Int32TreeNodeSize(node.right)
	return node
}

// Int16Int32Sink is a streaming builder for a Int16Int32: callers Put
// ascending entries one at a time, then Build the finished map. It is a thin
// wrapper over NewInt16Int32FromSorted (entries are buffered, then built in one
...
// NewInt16Int32Sink creates a streaming sink with the given duplicate policy.
func NewInt16Int32Sink(policy pump.DuplicatePolicy) *Int16Int32Sink {
	return &Int16Int32Sink{policy: policy}
}
...
func (s *Int16Int32Sink) Put(key int16, value int32) {
	if s.done {
		panic("mapdb: Put on a finished Int16Int32Sink")
	}
...
func (s *Int16Int32Sink) Build() (*Int16Int32, error) {
	if s.done {
		panic("mapdb: Build on a finished Int16Int32Sink")
	}
	s.done = true
	return NewInt16Int32FromSorted(s.keys, s.values, s.policy)
}
