...
// int16Int64TreeNodeSize returns the subtree size of a node link (0 if nil).
func int16Int64TreeNodeSize(n *int16Int64TreeNode) int {
	if n == nil {
		return 0
	}
	return n.size
...
// int16Int64TreeNodeFixSize recomputes a node's cached subtree size from its
// children. Called after any rotation or child relink so the augmentation stays
// consistent.
func int16Int64TreeNodeFixSize(n *int16Int64TreeNode) {
	n.size = 1 + int16Int64TreeNodeSize(n.left) + int16Int64TreeNodeSize(n.right)
}
...
func (m *Int16Int64) Put(key int16, value int64) (int64, bool) {
	if m.root == nil {
		m.root = &int16Int64TreeNode{key: key, value: value, color: int16Int64TreeNodeBlack, size: 1}
		m.size++
		return 0, false
	}
	node := m.root
	for {
		if key < node.key {
			if node.left == nil {
				node.left = &int16Int64TreeNode{key: key, value: value, parent: node, color: int16Int64TreeNodeRed, size: 1}
			...
			if node.right == nil {
				node.right = &int16Int64TreeNode{key: key, value: value, parent: node, color: int16Int64TreeNodeRed, size: 1}
				m.incSizeToRoot(node)
...
// reproduces All in ascending key order — so a *Int16Int64 satisfies
// par.Segmenter2[int16, int64] and feeds par.From2 with ORDERED
...
func (m *Int16Int64) Segments2(n int) []iter.Seq2[int16, int64] {
	ranges := segment.SplitRanges(m.size, n)
	segs := make([]iter.Seq2[int16, int64], len(ranges))
	for i := range ranges {
		lo, hi := ranges[i][0], ranges[i][1] // per-iteration copies for this view's closure
		segs[i] = func(yield func(int16, int64) bool) {
			// In-order walk yielding pairs whose 0-based key rank is in [lo, hi).
			// base is the rank of the first element in node's subtree.
			var walk func(node *int16Int64TreeNode, base int) bool
			walk = func(node *int16Int64TreeNode, base int) bool {
				if node == nil {
					return true
				}
				rank := base + int16Int64TreeNodeSize(node.left)
...
// query; never mutates.
func (m *Int16Int64) Rank(key int16) int {
	rank := 0
	node := m.root
	for node != nil {
		if key < node.key {
			...
			rank += 1 + int16Int64TreeNodeSize(node.left)
			node = node.right
		} else {
			// key == node.key: exactly the left subtree is strictly less.
			return rank + int16Int64TreeNodeSize(node.left)
		}
	}
	return rank
}
...
func (m *Int16Int64) SelectKey(i int) (int16, bool) {
	node := m.selectNode(i)
	if node == nil {
		return 0, false
	}
...
func (m *Int16Int64) SelectEntry(i int) (int16, int64, bool) {
	node := m.selectNode(i)
	if node == nil {
		return 0, 0, false
	}
...
// O(log n).
func (m *Int16Int64) selectNode(i int) *int16Int64TreeNode {
	if i < 0 {
		return nil
	}
	node := m.root
	for node != nil {
		left := int16Int64TreeNodeSize(node.left)
		if i < left {
			node = node.left
		} else if i == left {
			return node
...
func (m *Int16Int64) rotateLeft(x *int16Int64TreeNode) {
	...
	int16Int64TreeNodeFixSize(x)
	int16Int64TreeNodeFixSize(y)
}
...
func (m *Int16Int64) incSizeToRoot(n *int16Int64TreeNode) {
...
func (m *Int16Int64) fixSizeToRoot(n *int16Int64TreeNode) {
	for ; n != nil; n = n.parent {
		int16Int64TreeNodeFixSize(n)
	}
}
...
func (m *Int16Int64) deleteNode(z *int16Int64TreeNode) {
	...
	var fixSizeFrom *int16Int64TreeNode
...
func (m *Int16Int64) buildint16Int64(keys []int16, values []int64, lo, hi, level, redLevel int, parent *int16Int64TreeNode) *int16Int64TreeNode {
	...
	node.size = 1 + int16Int64TreeNodeSize(node.left) + int16Int64TreeNodeSize(node.right)
	return node
}
