...
// reproduces All in ascending key order — so a *Int16Int8 satisfies
// par.Segmenter2[int16, int8] and feeds par.From2 with ORDERED
...
func (m *Int16Int8) Segments2(n int) []iter.Seq2[int16, int8] {
	ranges := segment.SplitRanges(m.size, n)
	segs := make([]iter.Seq2[int16, int8], len(ranges))
	for i := range ranges {
		lo, hi := ranges[i][0], ranges[i][1] // per-iteration copies for this view's closure
		segs[i] = func(yield func(int16, int8) bool) {
			// In-order walk yielding pairs whose 0-based key rank is in [lo, hi).
			// base is the rank of the first element in node's subtree.
			var walk func(node *int16Int8TreeNode, base int) bool
			walk = func(node *int16Int8TreeNode, base int) bool {
				if node == nil {
					return true
				}
				rank := base + int16Int8TreeNodeSize(node.left)
...
func (m *Int16Int8) buildint16Int8(keys []int16, values []int8, lo, hi, level, redLevel int, parent *int16Int8TreeNode) *int16Int8TreeNode {
	...
	node.size = 1 + int16Int8TreeNodeSize(node.left) + int16Int8TreeNodeSize(node.right)
	return node
}
