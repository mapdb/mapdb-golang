...
// int32CharTreeNodeSize returns the subtree size of a node link (0 if nil).
func int32CharTreeNodeSize(n *int32CharTreeNode) int {
	if n == nil {
		return 0
	}
	return n.size
...
// int32CharTreeNodeFixSize recomputes a node's cached subtree size from its
// children. Called after any rotation or child relink so the augmentation stays
// consistent.
func int32CharTreeNodeFixSize(n *int32CharTreeNode) {
	n.size = 1 + int32CharTreeNodeSize(n.left) + int32CharTreeNodeSize(n.right)
}
...
func (m *Int32Char) Put(key int32, value uint16) (uint16, bool) {
	if m.root == nil {
		m.root = &int32CharTreeNode{key: key, value: value, color: int32CharTreeNodeBlack, size: 1}
		m.size++
		return 0, false
	}
	node := m.root
	for {
		if key < node.key {
			if node.left == nil {
				node.left = &int32CharTreeNode{key: key, value: value, parent: node, color: int32CharTreeNodeRed, size: 1}
			...
			if node.right == nil {
				node.right = &int32CharTreeNode{key: key, value: value, parent: node, color: int32CharTreeNodeRed, size: 1}
				m.incSizeToRoot(node)
...
// reproduces All in ascending key order — so a *Int32Char satisfies
// par.Segmenter2[int32, uint16] and feeds par.From2 with ORDERED
...
func (m *Int32Char) Segments2(n int) []iter.Seq2[int32, uint16] {
	ranges := segment.SplitRanges(m.size, n)
	segs := make([]iter.Seq2[int32, uint16], len(ranges))
	for i := range ranges {
		lo, hi := ranges[i][0], ranges[i][1] // per-iteration copies for this view's closure
		segs[i] = func(yield func(int32, uint16) bool) {
			// In-order walk yielding pairs whose 0-based key rank is in [lo, hi).
			// base is the rank of the first element in node's subtree.
			var walk func(node *int32CharTreeNode, base int) bool
			walk = func(node *int32CharTreeNode, base int) bool {
				if node == nil {
					return true
				}
				rank := base + int32CharTreeNodeSize(node.left)
...
// query; never mutates.
func (m *Int32Char) Rank(key int32) int {
	rank := 0
	node := m.root
	for node != nil {
		if key < node.key {
			...
			rank += 1 + int32CharTreeNodeSize(node.left)
			node = node.right
		} else {
			// key == node.key: exactly the left subtree is strictly less.
			return rank + int32CharTreeNodeSize(node.left)
		}
	}
	return rank
}
...
func (m *Int32Char) SelectKey(i int) (int32, bool) {
	node := m.selectNode(i)
	if node == nil {
		return 0, false
	}
...
func (m *Int32Char) SelectEntry(i int) (int32, uint16, bool) {
	node := m.selectNode(i)
	if node == nil {
		return 0, 0, false
	}
...
// O(log n).
func (m *Int32Char) selectNode(i int) *int32CharTreeNode {
	if i < 0 {
		return nil
	}
	node := m.root
	for node != nil {
		left := int32CharTreeNodeSize(node.left)
		if i < left {
			node = node.left
		} else if i == left {
			return node
...
func (m *Int32Char) rotateLeft(x *int32CharTreeNode) {
	...
	int32CharTreeNodeFixSize(x)
	int32CharTreeNodeFixSize(y)
}
...
func (m *Int32Char) incSizeToRoot(n *int32CharTreeNode) {
...
func (m *Int32Char) fixSizeToRoot(n *int32CharTreeNode) {
	for ; n != nil; n = n.parent {
		int32CharTreeNodeFixSize(n)
	}
}
...
func (m *Int32Char) deleteNode(z *int32CharTreeNode) {
	...
	var fixSizeFrom *int32CharTreeNode
...
func (m *Int32Char) buildint32Char(keys []int32, values []uint16, lo, hi, level, redLevel int, parent *int32CharTreeNode) *int32CharTreeNode {
	...
	node.size = 1 + int32CharTreeNodeSize(node.left) + int32CharTreeNodeSize(node.right)
	return node
}
