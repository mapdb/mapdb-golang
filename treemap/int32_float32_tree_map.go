...
// int32Float32TreeNodeSize returns the subtree size of a node link (0 if nil).
func int32Float32TreeNodeSize(n *int32Float32TreeNode) int {
	if n == nil {
		return 0
	}
	return n.size
...
// int32Float32TreeNodeFixSize recomputes a node's cached subtree size from its
// children. Called after any rotation or child relink so the augmentation stays
// consistent.
func int32Float32TreeNodeFixSize(n *int32Float32TreeNode) {
	n.size = 1 + int32Float32TreeNodeSize(n.left) + int32Float32TreeNodeSize(n.right)
}
...
func (m *Int32Float32) Put(key int32, value float32) (float32, bool) {
	if m.root == nil {
		m.root = &int32Float32TreeNode{key: key, value: value, color: int32Float32TreeNodeBlack, size: 1}
		m.size++
		return 0.0, false
	}
	node := m.root
	for {
		if key < node.key {
			if node.left == nil {
				node.left = &int32Float32TreeNode{key: key, value: value, parent: node, color: int32Float32TreeNodeRed, size: 1}
			...
			if node.right == nil {
				node.right = &int32Float32TreeNode{key: key, value: value, parent: node, color: int32Float32TreeNodeRed, size: 1}
				m.incSizeToRoot(node)
...
// reproduces All in ascending key order — so a *Int32Float32 satisfies
// par.Segmenter2[int32, float32] and feeds par.From2 with ORDERED
...
func (m *Int32Float32) Segments2(n int) []iter.Seq2[int32, float32] {
	ranges := segment.SplitRanges(m.size, n)
	segs := make([]iter.Seq2[int32, float32], len(ranges))
	for i := range ranges {
		lo, hi := ranges[i][0], ranges[i][1] // per-iteration copies for this view's closure
		segs[i] = func(yield func(int32, float32) bool) {
			// In-order walk yielding pairs whose 0-based key rank is in [lo, hi).
			// base is the rank of the first element in node's subtree.
			var walk func(node *int32Float32TreeNode, base int) bool
			walk = func(node *int32Float32TreeNode, base int) bool {
				if node == nil {
					return true
				}
				rank := base + int32Float32TreeNodeSize(node.left)
...
// query; never mutates.
func (m *Int32Float32) Rank(key int32) int {
	rank := 0
	node := m.root
	for node != nil {
		if key < node.key {
			...
			rank += 1 + int32Float32TreeNodeSize(node.left)
			node = node.right
		} else {
			// key == node.key: exactly the left subtree is strictly less.
			return rank + int32Float32TreeNodeSize(node.left)
		}
	}
	return rank
}
...
func (m *Int32Float32) SelectKey(i int) (int32, bool) {
	node := m.selectNode(i)
	if node == nil {
		return 0, false
	}
...
func (m *Int32Float32) SelectEntry(i int) (int32, float32, bool) {
	node := m.selectNode(i)
	if node == nil {
		return 0, 0.0, false
	}
...
// O(log n).
func (m *Int32Float32) selectNode(i int) *int32Float32TreeNode {
	if i < 0 {
		return nil
	}
	node := m.root
	for node != nil {
		left := int32Float32TreeNodeSize(node.left)
		if i < left {
			node = node.left
		} else if i == left {
			return node
...
func (m *Int32Float32) rotateLeft(x *int32Float32TreeNode) {
	...
	int32Float32TreeNodeFixSize(x)
	int32Float32TreeNodeFixSize(y)
}
...
func (m *Int32Float32) incSizeToRoot(n *int32Float32TreeNode) {
...
func (m *Int32Float32) fixSizeToRoot(n *int32Float32TreeNode) {
	for ; n != nil; n = n.parent {
		int32Float32TreeNodeFixSize(n)
	}
}
...
func (m *Int32Float32) deleteNode(z *int32Float32TreeNode) {
	...
	var fixSizeFrom *int32Float32TreeNode
...
func (m *Int32Float32) buildint32Float32(keys []int32, values []float32, lo, hi, level, redLevel int, parent *int32Float32TreeNode) *int32Float32TreeNode {
	...
	node.size = 1 + int32Float32TreeNodeSize(node.left) + int32Float32TreeNodeSize(node.right)
	return node
}
