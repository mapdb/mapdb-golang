...
// int32Float64TreeNodeSize returns the subtree size of a node link (0 if nil).
func int32Float64TreeNodeSize(n *int32Float64TreeNode) int {
	if n == nil {
		return 0
	}
	return n.size
...
// int32Float64TreeNodeFixSize recomputes a node's cached subtree size from its
// children. Called after any rotation or child relink so the augmentation stays
// consistent.
func int32Float64TreeNodeFixSize(n *int32Float64TreeNode) {
	n.size = 1 + int32Float64TreeNodeSize(n.left) + int32Float64TreeNodeSize(n.right)
}

// Int32Float64 is a sorted map with int32 keys and float64 values, backed by a red-black tree.
...
// NewInt32Float64 creates a new empty sorted map.
...
func (m *Int32Float64) Put(key int32, value float64) (float64, bool) {
	if m.root == nil {
		m.root = &int32Float64TreeNode{key: key, value: value, color: int32Float64TreeNodeBlack, size: 1}
		m.size++
		return 0.0, false
	}
	node := m.root
	for {
		if key < node.key {
			if node.left == nil {
				node.left = &int32Float64TreeNode{key: key, value: value, parent: node, color: int32Float64TreeNodeRed, size: 1}
			...
			if node.right == nil {
				node.right = &int32Float64TreeNode{key: key, value: value, parent: node, color: int32Float64TreeNodeRed, size: 1}
				m.incSizeToRoot(node)
...
func (m *Int32Float64) Min() (int32, float64, bool) {
	if m.root == nil {
		return 0, 0.0, false
	}
...
func (m *Int32Float64) Max() (int32, float64, bool) {
	if m.root == nil {
		return 0, 0.0, false
	}
...
func (m *Int32Float64) Floor(key int32) (int32, float64, bool) {
...
func (m *Int32Float64) Ceiling(key int32) (int32, float64, bool) {
	var result *int32Float64TreeNode
	node := m.root
	for node != nil {
		if key == node.key {
...
// reproduces All in ascending key order — so a *Int32Float64 satisfies
// par.Segmenter2[int32, float64] and feeds par.From2 with ORDERED
...
func (m *Int32Float64) Segments2(n int) []iter.Seq2[int32, float64] {
	ranges := segment.SplitRanges(m.size, n)
	segs := make([]iter.Seq2[int32, float64], len(ranges))
	for i := range ranges {
		lo, hi := ranges[i][0], ranges[i][1] // per-iteration copies for this view's closure
		segs[i] = func(yield func(int32, float64) bool) {
			// In-order walk yielding pairs whose 0-based key rank is in [lo, hi).
			// base is the rank of the first element in node's subtree.
			var walk func(node *int32Float64TreeNode, base int) bool
			walk = func(node *int32Float64TreeNode, base int) bool {
				if node == nil {
					return true
				}
				rank := base + int32Float64TreeNodeSize(node.left)
...
func (m *Int32Float64) RangeKeys(fromKey, toKey int32) iter.Seq2[int32, float64] {
	return func(yield func(int32, float64) bool) {
		for k, v := range m.All() {
			if k < fromKey {
				continue
...
func (m *Int32Float64) Higher(key int32) (int32, float64, bool) {
	var result *int32Float64TreeNode
	node := m.root
	for node != nil {
		if key < node.key {
...
func (m *Int32Float64) Lower(key int32) (int32, float64, bool) {
	var result *int32Float64TreeNode
	node := m.root
	for node != nil {
		if key > node.key {
...
func (m *Int32Float64) HeadMap(toKey int32) iter.Seq2[int32, float64] {
	return func(yield func(int32, float64) bool) {
		for k, v := range m.All() {
			if k >= toKey {
				return
...
func (m *Int32Float64) TailMap(fromKey int32) iter.Seq2[int32, float64] {
	return func(yield func(int32, float64) bool) {
		for k, v := range m.All() {
			if k < fromKey {
				continue
...
func (m *Int32Float64) PollLastEntry() (int32, float64, bool) {
	k, v, ok := m.Max()
	if !ok {
		return 0, 0.0, false
	}
...
func (m *Int32Float64) DescendingMap() iter.Seq2[int32, float64] {
	return func(yield func(int32, float64) bool) {
...
func (m *Int32Float64) DescendingKeys() iter.Seq[int32] {
	return func(yield func(int32) bool) {
...
// query; never mutates.
func (m *Int32Float64) Rank(key int32) int {
	rank := 0
	node := m.root
	for node != nil {
		if key < node.key {
			...
			rank += 1 + int32Float64TreeNodeSize(node.left)
			node = node.right
		} else {
			// key == node.key: exactly the left subtree is strictly less.
			return rank + int32Float64TreeNodeSize(node.left)
		}
	}
	return rank
}
...
func (m *Int32Float64) SelectKey(i int) (int32, bool) {
	node := m.selectNode(i)
	if node == nil {
		return 0, false
	}
...
func (m *Int32Float64) SelectEntry(i int) (int32, float64, bool) {
	node := m.selectNode(i)
	if node == nil {
		return 0, 0.0, false
	}
...
// O(log n).
func (m *Int32Float64) selectNode(i int) *int32Float64TreeNode {
	if i < 0 {
		return nil
	}
	node := m.root
	for node != nil {
		left := int32Float64TreeNodeSize(node.left)
		if i < left {
			node = node.left
		} else if i == left {
			return node
...
func (m *Int32Float64) findNode(key int32) *int32Float64TreeNode {
	node := m.root
	for node != nil {
		if key < node.key {
			node = node.left
...
func (m *Int32Float64) minNode(node *int32Float64TreeNode) *int32Float64TreeNode {
	for node.left != nil {
		node = node.left
	}
	return node
...
func (m *Int32Float64) maxNode(node *int32Float64TreeNode) *int32Float64TreeNode {
	for node.right != nil {
		node = node.right
	}
	return node
...
func (m *Int32Float64) rotateLeft(x *int32Float64TreeNode) {
	y := x.right
	x.right = y.left
	if y.left != nil {
		y.left.parent = x
	...
	int32Float64TreeNodeFixSize(x)
	int32Float64TreeNodeFixSize(y)
}

func (m *Int32Float64) rotateRight(x *int32Float64TreeNode) {
	y := x.left
	x.left = y.right
	if y.right != nil {
		y.right.parent = x
...
func (m *Int32Float64) incSizeToRoot(n *int32Float64TreeNode) {
...
func (m *Int32Float64) fixSizeToRoot(n *int32Float64TreeNode) {
	for ; n != nil; n = n.parent {
		int32Float64TreeNodeFixSize(n)
	}
}

func (m *Int32Float64) fixAfterInsert(z *int32Float64TreeNode) {
...
func (m *Int32Float64) deleteNode(z *int32Float64TreeNode) {
	...
	var fixSizeFrom *int32Float64TreeNode
...
// NewInt32Float64FromSorted builds a Int32Float64 from presorted, ascending keys
...
func NewInt32Float64FromSorted(keys []int32, values []float64, policy pump.DuplicatePolicy) (*Int32Float64, error) {
	if len(keys) != len(values) {
		panic("mapdb: NewInt32Float64FromSorted: len(keys) != len(values)")
	}
	dk, dv, err := dedupInt32Float64Sorted(keys, values, policy)
	if err != nil {
		return nil, err
	}
	m := NewInt32Float64()
	m.root = m.buildint32Float64(dk, dv, 0, len(dk)-1, 0, pump.RedBlackRedLevel(len(dk)), nil)
	m.size = len(dk)
	return m, nil
}

// dedupInt32Float64Sorted validates ascending order and applies the duplicate
// policy, returning compacted key/value slices. Equal adjacent keys are the only
// duplicates possible in sorted input.
func dedupInt32Float64Sorted(keys []int32, values []float64, policy pump.DuplicatePolicy) ([]int32, []float64, error) {
	if len(keys) == 0 {
	...
	for i := 1; i < len(keys); i++ {
		cmp := cmpInt32Float64(keys[i], keys[i-1])
		if cmp < 0 {
...
// cmpInt32Float64 is the three-way ordering used by the bulk-load validator for
// integer/char keys (float keys use the IEEE total-order helper instead).
func cmpInt32Float64(a, b int32) int {
	switch {
	case a < b:
		return -1
	case a > b:
...
// buildint32Float64 recursively builds a perfectly balanced subtree from the
...
func (m *Int32Float64) buildint32Float64(keys []int32, values []float64, lo, hi, level, redLevel int, parent *int32Float64TreeNode) *int32Float64TreeNode {
	if lo > hi {
		return nil
	}
	mid := (lo + hi) / 2
	node := &int32Float64TreeNode{key: keys[mid], value: values[mid], parent: parent, color: int32Float64TreeNodeBlack}
	if level == redLevel {
		node.color = int32Float64TreeNodeRed
	}
	node.left = m.buildint32Float64(keys, values, lo, mid-1, level+1, redLevel, node)
	node.right = m.buildint32Float64(keys, values, mid+1, hi, level+1, redLevel, node)
	...
	node.size = 1 + int32Float64TreeNodeSize(node.left) + int32Float64TreeNodeSize(node.right)
	return node
}

// Int32Float64Sink is a streaming builder for a Int32Float64: callers Put
// ascending entries one at a time, then Build the finished map. It is a thin
// wrapper over NewInt32Float64FromSorted (entries are buffered, then built in one
...
// NewInt32Float64Sink creates a streaming sink with the given duplicate policy.
func NewInt32Float64Sink(policy pump.DuplicatePolicy) *Int32Float64Sink {
	return &Int32Float64Sink{policy: policy}
}
...
func (s *Int32Float64Sink) Put(key int32, value float64) {
	if s.done {
		panic("mapdb: Put on a finished Int32Float64Sink")
	}
...
func (s *Int32Float64Sink) Build() (*Int32Float64, error) {
	if s.done {
		panic("mapdb: Build on a finished Int32Float64Sink")
	}
	s.done = true
	return NewInt32Float64FromSorted(s.keys, s.values, s.policy)
}

func (m *Int32Float64) fixAfterDelete(x *int32Float64TreeNode) {
