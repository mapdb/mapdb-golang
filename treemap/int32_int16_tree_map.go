...
// reproduces All in ascending key order — so a *Int32Int16 satisfies
// par.Segmenter2[int32, int16] and feeds par.From2 with ORDERED
...
func (m *Int32Int16) Segments2(n int) []iter.Seq2[int32, int16] {
	ranges := segment.SplitRanges(m.size, n)
	segs := make([]iter.Seq2[int32, int16], len(ranges))
	for i := range ranges {
		lo, hi := ranges[i][0], ranges[i][1] // per-iteration copies for this view's closure
		segs[i] = func(yield func(int32, int16) bool) {
			// In-order walk yielding pairs whose 0-based key rank is in [lo, hi).
			// base is the rank of the first element in node's subtree.
			var walk func(node *int32Int16TreeNode, base int) bool
			walk = func(node *int32Int16TreeNode, base int) bool {
				if node == nil {
					return true
				}
				rank := base + int32Int16TreeNodeSize(node.left)
...
func (m *Int32Int16) buildint32Int16(keys []int32, values []int16, lo, hi, level, redLevel int, parent *int32Int16TreeNode) *int32Int16TreeNode {
	...
	node.size = 1 + int32Int16TreeNodeSize(node.left) + int32Int16TreeNodeSize(node.right)
	return node
}
