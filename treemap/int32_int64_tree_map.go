...
// int32Int64TreeNodeSize returns the subtree size of a node link (0 if nil).
func int32Int64TreeNodeSize(n *int32Int64TreeNode) int {
	if n == nil {
		return 0
	}
	return n.size
...
// int32Int64TreeNodeFixSize recomputes a node's cached subtree size from its
// children. Called after any rotation or child relink so the augmentation stays
// consistent.
func int32Int64TreeNodeFixSize(n *int32Int64TreeNode) {
	n.size = 1 + int32Int64TreeNodeSize(n.left) + int32Int64TreeNodeSize(n.right)
}
...
func (m *Int32Int64) Put(key int32, value int64) (int64, bool) {
	if m.root == nil {
		m.root = &int32Int64TreeNode{key: key, value: value, color: int32Int64TreeNodeBlack, size: 1}
		m.size++
		return 0, false
	}
	node := m.root
	for {
		if key < node.key {
			if node.left == nil {
				node.left = &int32Int64TreeNode{key: key, value: value, parent: node, color: int32Int64TreeNodeRed, size: 1}
			...
			if node.right == nil {
				node.right = &int32Int64TreeNode{key: key, value: value, parent: node, color: int32Int64TreeNodeRed, size: 1}
				m.incSizeToRoot(node)
...
// reproduces All in ascending key order — so a *Int32Int64 satisfies
// par.Segmenter2[int32, int64] and feeds par.From2 with ORDERED
...
func (m *Int32Int64) Segments2(n int) []iter.Seq2[int32, int64] {
	ranges := segment.SplitRanges(m.size, n)
	segs := make([]iter.Seq2[int32, int64], len(ranges))
	for i := range ranges {
		lo, hi := ranges[i][0], ranges[i][1] // per-iteration copies for this view's closure
		segs[i] = func(yield func(int32, int64) bool) {
			// In-order walk yielding pairs whose 0-based key rank is in [lo, hi).
			// base is the rank of the first element in node's subtree.
			var walk func(node *int32Int64TreeNode, base int) bool
			walk = func(node *int32Int64TreeNode, base int) bool {
				if node == nil {
					return true
				}
				rank := base + int32Int64TreeNodeSize(node.left)
...
// query; never mutates.
func (m *Int32Int64) Rank(key int32) int {
	rank := 0
	node := m.root
	for node != nil {
		if key < node.key {
			...
			rank += 1 + int32Int64TreeNodeSize(node.left)
			node = node.right
		} else {
			// key == node.key: exactly the left subtree is strictly less.
			return rank + int32Int64TreeNodeSize(node.left)
		}
	}
	return rank
}
...
func (m *Int32Int64) SelectKey(i int) (int32, bool) {
	node := m.selectNode(i)
	if node == nil {
		return 0, false
	}
...
func (m *Int32Int64) SelectEntry(i int) (int32, int64, bool) {
	node := m.selectNode(i)
	if node == nil {
		return 0, 0, false
	}
...
// O(log n).
func (m *Int32Int64) selectNode(i int) *int32Int64TreeNode {
	if i < 0 {
		return nil
	}
	node := m.root
	for node != nil {
		left := int32Int64TreeNodeSize(node.left)
		if i < left {
			node = node.left
		} else if i == left {
			return node
...
func (m *Int32Int64) rotateLeft(x *int32Int64TreeNode) {
	...
	int32Int64TreeNodeFixSize(x)
	int32Int64TreeNodeFixSize(y)
}
...
func (m *Int32Int64) incSizeToRoot(n *int32Int64TreeNode) {
...
func (m *Int32Int64) fixSizeToRoot(n *int32Int64TreeNode) {
	for ; n != nil; n = n.parent {
		int32Int64TreeNodeFixSize(n)
	}
}
...
func (m *Int32Int64) deleteNode(z *int32Int64TreeNode) {
	...
	var fixSizeFrom *int32Int64TreeNode
...
// NewInt32Int64FromSorted builds a Int32Int64 from presorted, ascending keys
...
func NewInt32Int64FromSorted(keys []int32, values []int64, policy pump.DuplicatePolicy) (*Int32Int64, error) {
	if len(keys) != len(values) {
		panic("mapdb: NewInt32Int64FromSorted: len(keys) != len(values)")
	}
	dk, dv, err := dedupInt32Int64Sorted(keys, values, policy)
	if err != nil {
		return nil, err
	}
	m := NewInt32Int64()
	m.root = m.buildint32Int64(dk, dv, 0, len(dk)-1, 0, pump.RedBlackRedLevel(len(dk)), nil)
	m.size = len(dk)
	return m, nil
}

// dedupInt32Int64Sorted validates ascending order and applies the duplicate
// policy, returning compacted key/value slices. Equal adjacent keys are the only
// duplicates possible in sorted input.
func dedupInt32Int64Sorted(keys []int32, values []int64, policy pump.DuplicatePolicy) ([]int32, []int64, error) {
	if len(keys) == 0 {
	...
	for i := 1; i < len(keys); i++ {
		cmp := cmpInt32Int64(keys[i], keys[i-1])
		if cmp < 0 {
...
// cmpInt32Int64 is the three-way ordering used by the bulk-load validator for
// integer/char keys (float keys use the IEEE total-order helper instead).
func cmpInt32Int64(a, b int32) int {
	switch {
	case a < b:
		return -1
	case a > b:
...
// buildint32Int64 recursively builds a perfectly balanced subtree from the
...
func (m *Int32Int64) buildint32Int64(keys []int32, values []int64, lo, hi, level, redLevel int, parent *int32Int64TreeNode) *int32Int64TreeNode {
	if lo > hi {
		return nil
	}
	mid := (lo + hi) / 2
	node := &int32Int64TreeNode{key: keys[mid], value: values[mid], parent: parent, color: int32Int64TreeNodeBlack}
	if level == redLevel {
		node.color = int32Int64TreeNodeRed
	}
	node.left = m.buildint32Int64(keys, values, lo, mid-1, level+1, redLevel, node)
	node.right = m.buildint32Int64(keys, values, mid+1, hi, level+1, redLevel, node)
	...
	node.size = 1 + int32Int64TreeNodeSize(node.left) + int32Int64TreeNodeSize(node.right)
	return node
}

// Int32Int64Sink is a streaming builder for a Int32Int64: callers Put
// ascending entries one at a time, then Build the finished map. It is a thin
// wrapper over NewInt32Int64FromSorted (entries are buffered, then built in one
...
// NewInt32Int64Sink creates a streaming sink with the given duplicate policy.
func NewInt32Int64Sink(policy pump.DuplicatePolicy) *Int32Int64Sink {
	return &Int32Int64Sink{policy: policy}
}
...
func (s *Int32Int64Sink) Put(key int32, value int64) {
	if s.done {
		panic("mapdb: Put on a finished Int32Int64Sink")
	}
...
func (s *Int32Int64Sink) Build() (*Int32Int64, error) {
	if s.done {
		panic("mapdb: Build on a finished Int32Int64Sink")
	}
	s.done = true
	return NewInt32Int64FromSorted(s.keys, s.values, s.policy)
}
