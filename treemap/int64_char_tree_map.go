...
// reproduces All in ascending key order — so a *Int64Char satisfies
// par.Segmenter2[int64, uint16] and feeds par.From2 with ORDERED
...
func (m *Int64Char) Segments2(n int) []iter.Seq2[int64, uint16] {
	ranges := segment.SplitRanges(m.size, n)
	segs := make([]iter.Seq2[int64, uint16], len(ranges))
	for i := range ranges {
		lo, hi := ranges[i][0], ranges[i][1] // per-iteration copies for this view's closure
		segs[i] = func(yield func(int64, uint16) bool) {
			// In-order walk yielding pairs whose 0-based key rank is in [lo, hi).
			// base is the rank of the first element in node's subtree.
			var walk func(node *int64CharTreeNode, base int) bool
			walk = func(node *int64CharTreeNode, base int) bool {
				if node == nil {
					return true
				}
				rank := base + int64CharTreeNodeSize(node.left)
...
func (m *Int64Char) buildint64Char(keys []int64, values []uint16, lo, hi, level, redLevel int, parent *int64CharTreeNode) *int64CharTreeNode {
	...
	node.size = 1 + int64CharTreeNodeSize(node.left) + int64CharTreeNodeSize(node.right)
	return node
}
