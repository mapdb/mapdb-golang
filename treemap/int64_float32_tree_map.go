...
// reproduces All in ascending key order — so a *Int64Float32 satisfies
// par.Segmenter2[int64, float32] and feeds par.From2 with ORDERED
...
func (m *Int64Float32) Segments2(n int) []iter.Seq2[int64, float32] {
	ranges := segment.SplitRanges(m.size, n)
	segs := make([]iter.Seq2[int64, float32], len(ranges))
	for i := range ranges {
		lo, hi := ranges[i][0], ranges[i][1] // per-iteration copies for this view's closure
		segs[i] = func(yield func(int64, float32) bool) {
			// In-order walk yielding pairs whose 0-based key rank is in [lo, hi).
			// base is the rank of the first element in node's subtree.
			var walk func(node *int64Float32TreeNode, base int) bool
			walk = func(node *int64Float32TreeNode, base int) bool {
				if node == nil {
					return true
				}
				rank := base + int64Float32TreeNodeSize(node.left)
...
func (m *Int64Float32) buildint64Float32(keys []int64, values []float32, lo, hi, level, redLevel int, parent *int64Float32TreeNode) *int64Float32TreeNode {
	...
	node.size = 1 + int64Float32TreeNodeSize(node.left) + int64Float32TreeNodeSize(node.right)
	return node
}
