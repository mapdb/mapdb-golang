...
// int64Int16TreeNodeSize returns the subtree size of a node link (0 if nil).
func int64Int16TreeNodeSize(n *int64Int16TreeNode) int {
	if n == nil {
		return 0
	}
	return n.size
...
// int64Int16TreeNodeFixSize recomputes a node's cached subtree size from its
// children. Called after any rotation or child relink so the augmentation stays
// consistent.
func int64Int16TreeNodeFixSize(n *int64Int16TreeNode) {
	n.size = 1 + int64Int16TreeNodeSize(n.left) + int64Int16TreeNodeSize(n.right)
}

// Int64Int16 is a sorted map with int64 keys and int16 values, backed by a red-black tree.
...
// NewInt64Int16 creates a new empty sorted map.
...
func (m *Int64Int16) Put(key int64, value int16) (int16, bool) {
	if m.root == nil {
		m.root = &int64Int16TreeNode{key: key, value: value, color: int64Int16TreeNodeBlack, size: 1}
		m.size++
		return 0, false
	}
	node := m.root
	for {
		if key < node.key {
			if node.left == nil {
				node.left = &int64Int16TreeNode{key: key, value: value, parent: node, color: int64Int16TreeNodeRed, size: 1}
			...
			if node.right == nil {
				node.right = &int64Int16TreeNode{key: key, value: value, parent: node, color: int64Int16TreeNodeRed, size: 1}
				m.incSizeToRoot(node)
...
func (m *Int64Int16) Min() (int64, int16, bool) {
	if m.root == nil {
		return 0, 0, false
	}
...
func (m *Int64Int16) Max() (int64, int16, bool) {
	if m.root == nil {
		return 0, 0, false
	}
...
func (m *Int64Int16) Floor(key int64) (int64, int16, bool) {
...
func (m *Int64Int16) Ceiling(key int64) (int64, int16, bool) {
	var result *int64Int16TreeNode
	node := m.root
	for node != nil {
		if key == node.key {
...
// reproduces All in ascending key order — so a *Int64Int16 satisfies
// par.Segmenter2[int64, int16] and feeds par.From2 with ORDERED
...
func (m *Int64Int16) Segments2(n int) []iter.Seq2[int64, int16] {
	ranges := segment.SplitRanges(m.size, n)
	segs := make([]iter.Seq2[int64, int16], len(ranges))
	for i := range ranges {
		lo, hi := ranges[i][0], ranges[i][1] // per-iteration copies for this view's closure
		segs[i] = func(yield func(int64, int16) bool) {
			// In-order walk yielding pairs whose 0-based key rank is in [lo, hi).
			// base is the rank of the first element in node's subtree.
			var walk func(node *int64Int16TreeNode, base int) bool
			walk = func(node *int64Int16TreeNode, base int) bool {
				if node == nil {
					return true
				}
				rank := base + int64Int16TreeNodeSize(node.left)
...
func (m *Int64Int16) RangeKeys(fromKey, toKey int64) iter.Seq2[int64, int16] {
	return func(yield func(int64, int16) bool) {
		for k, v := range m.All() {
			if k < fromKey {
				continue
...
func (m *Int64Int16) Higher(key int64) (int64, int16, bool) {
	var result *int64Int16TreeNode
	node := m.root
	for node != nil {
		if key < node.key {
...
func (m *Int64Int16) Lower(key int64) (int64, int16, bool) {
	var result *int64Int16TreeNode
	node := m.root
	for node != nil {
		if key > node.key {
...
func (m *Int64Int16) HeadMap(toKey int64) iter.Seq2[int64, int16] {
	return func(yield func(int64, int16) bool) {
		for k, v := range m.All() {
			if k >= toKey {
				return
...
func (m *Int64Int16) TailMap(fromKey int64) iter.Seq2[int64, int16] {
	return func(yield func(int64, int16) bool) {
		for k, v := range m.All() {
			if k < fromKey {
				continue
...
func (m *Int64Int16) PollLastEntry() (int64, int16, bool) {
	k, v, ok := m.Max()
	if !ok {
		return 0, 0, false
	}
...
func (m *Int64Int16) DescendingMap() iter.Seq2[int64, int16] {
	return func(yield func(int64, int16) bool) {
...
func (m *Int64Int16) DescendingKeys() iter.Seq[int64] {
	return func(yield func(int64) bool) {
...
// query; never mutates.
func (m *Int64Int16) Rank(key int64) int {
	rank := 0
	node := m.root
	for node != nil {
		if key < node.key {
			...
			rank += 1 + int64Int16TreeNodeSize(node.left)
			node = node.right
		} else {
			// key == node.key: exactly the left subtree is strictly less.
			return rank + int64Int16TreeNodeSize(node.left)
		}
	}
	return rank
}
...
func (m *Int64Int16) SelectKey(i int) (int64, bool) {
	node := m.selectNode(i)
	if node == nil {
		return 0, false
	}
...
func (m *Int64Int16) SelectEntry(i int) (int64, int16, bool) {
	node := m.selectNode(i)
	if node == nil {
		return 0, 0, false
	}
...
// O(log n).
func (m *Int64Int16) selectNode(i int) *int64Int16TreeNode {
	if i < 0 {
		return nil
	}
	node := m.root
	for node != nil {
		left := int64Int16TreeNodeSize(node.left)
		if i < left {
			node = node.left
		} else if i == left {
			return node
...
func (m *Int64Int16) findNode(key int64) *int64Int16TreeNode {
	node := m.root
	for node != nil {
		if key < node.key {
			node = node.left
...
func (m *Int64Int16) minNode(node *int64Int16TreeNode) *int64Int16TreeNode {
	for node.left != nil {
		node = node.left
	}
	return node
...
func (m *Int64Int16) maxNode(node *int64Int16TreeNode) *int64Int16TreeNode {
	for node.right != nil {
		node = node.right
	}
	return node
...
func (m *Int64Int16) rotateLeft(x *int64Int16TreeNode) {
	y := x.right
	x.right = y.left
	if y.left != nil {
		y.left.parent = x
	...
	int64Int16TreeNodeFixSize(x)
	int64Int16TreeNodeFixSize(y)
}

func (m *Int64Int16) rotateRight(x *int64Int16TreeNode) {
	y := x.left
	x.left = y.right
	if y.right != nil {
		y.right.parent = x
...
func (m *Int64Int16) incSizeToRoot(n *int64Int16TreeNode) {
...
func (m *Int64Int16) fixSizeToRoot(n *int64Int16TreeNode) {
	for ; n != nil; n = n.parent {
		int64Int16TreeNodeFixSize(n)
	}
}

func (m *Int64Int16) fixAfterInsert(z *int64Int16TreeNode) {
...
func (m *Int64Int16) deleteNode(z *int64Int16TreeNode) {
	...
	var fixSizeFrom *int64Int16TreeNode
...
// NewInt64Int16FromSorted builds a Int64Int16 from presorted, ascending keys
...
func NewInt64Int16FromSorted(keys []int64, values []int16, policy pump.DuplicatePolicy) (*Int64Int16, error) {
	if len(keys) != len(values) {
		panic("mapdb: NewInt64Int16FromSorted: len(keys) != len(values)")
	}
	dk, dv, err := dedupInt64Int16Sorted(keys, values, policy)
	if err != nil {
		return nil, err
	}
	m := NewInt64Int16()
	m.root = m.buildint64Int16(dk, dv, 0, len(dk)-1, 0, pump.RedBlackRedLevel(len(dk)), nil)
	m.size = len(dk)
	return m, nil
}

// dedupInt64Int16Sorted validates ascending order and applies the duplicate
// policy, returning compacted key/value slices. Equal adjacent keys are the only
// duplicates possible in sorted input.
func dedupInt64Int16Sorted(keys []int64, values []int16, policy pump.DuplicatePolicy) ([]int64, []int16, error) {
	if len(keys) == 0 {
	...
	for i := 1; i < len(keys); i++ {
		cmp := cmpInt64Int16(keys[i], keys[i-1])
		if cmp < 0 {
...
// cmpInt64Int16 is the three-way ordering used by the bulk-load validator for
// integer/char keys (float keys use the IEEE total-order helper instead).
func cmpInt64Int16(a, b int64) int {
	switch {
	case a < b:
		return -1
	case a > b:
...
// buildint64Int16 recursively builds a perfectly balanced subtree from the
...
func (m *Int64Int16) buildint64Int16(keys []int64, values []int16, lo, hi, level, redLevel int, parent *int64Int16TreeNode) *int64Int16TreeNode {
	if lo > hi {
		return nil
	}
	mid := (lo + hi) / 2
	node := &int64Int16TreeNode{key: keys[mid], value: values[mid], parent: parent, color: int64Int16TreeNodeBlack}
	if level == redLevel {
		node.color = int64Int16TreeNodeRed
	}
	node.left = m.buildint64Int16(keys, values, lo, mid-1, level+1, redLevel, node)
	node.right = m.buildint64Int16(keys, values, mid+1, hi, level+1, redLevel, node)
	...
	node.size = 1 + int64Int16TreeNodeSize(node.left) + int64Int16TreeNodeSize(node.right)
	return node
}

// Int64Int16Sink is a streaming builder for a Int64Int16: callers Put
// ascending entries one at a time, then Build the finished map. It is a thin
// wrapper over NewInt64Int16FromSorted (entries are buffered, then built in one
...
// NewInt64Int16Sink creates a streaming sink with the given duplicate policy.
func NewInt64Int16Sink(policy pump.DuplicatePolicy) *Int64Int16Sink {
	return &Int64Int16Sink{policy: policy}
}
...
func (s *Int64Int16Sink) Put(key int64, value int16) {
	if s.done {
		panic("mapdb: Put on a finished Int64Int16Sink")
	}
...
func (s *Int64Int16Sink) Build() (*Int64Int16, error) {
	if s.done {
		panic("mapdb: Build on a finished Int64Int16Sink")
	}
	s.done = true
	return NewInt64Int16FromSorted(s.keys, s.values, s.policy)
}

func (m *Int64Int16) fixAfterDelete(x *int64Int16TreeNode) {
