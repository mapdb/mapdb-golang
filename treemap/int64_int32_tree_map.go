...
// int64Int32TreeNodeSize returns the subtree size of a node link (0 if nil).
func int64Int32TreeNodeSize(n *int64Int32TreeNode) int {
	if n == nil {
		return 0
	}
	return n.size
...
// int64Int32TreeNodeFixSize recomputes a node's cached subtree size from its
// children. Called after any rotation or child relink so the augmentation stays
// consistent.
func int64Int32TreeNodeFixSize(n *int64Int32TreeNode) {
	n.size = 1 + int64Int32TreeNodeSize(n.left) + int64Int32TreeNodeSize(n.right)
}

// Int64Int32 is a sorted map with int64 keys and int32 values, backed by a red-black tree.
...
// NewInt64Int32 creates a new empty sorted map.
...
func (m *Int64Int32) Put(key int64, value int32) (int32, bool) {
	if m.root == nil {
		m.root = &int64Int32TreeNode{key: key, value: value, color: int64Int32TreeNodeBlack, size: 1}
		m.size++
		return 0, false
	}
	node := m.root
	for {
		if key < node.key {
			if node.left == nil {
				node.left = &int64Int32TreeNode{key: key, value: value, parent: node, color: int64Int32TreeNodeRed, size: 1}
			...
			if node.right == nil {
				node.right = &int64Int32TreeNode{key: key, value: value, parent: node, color: int64Int32TreeNodeRed, size: 1}
				m.incSizeToRoot(node)
...
func (m *Int64Int32) Min() (int64, int32, bool) {
	if m.root == nil {
		return 0, 0, false
	}
...
func (m *Int64Int32) Max() (int64, int32, bool) {
	if m.root == nil {
		return 0, 0, false
	}
...
func (m *Int64Int32) Floor(key int64) (int64, int32, bool) {
...
func (m *Int64Int32) Ceiling(key int64) (int64, int32, bool) {
	var result *int64Int32TreeNode
	node := m.root
	for node != nil {
		if key == node.key {
...
// reproduces All in ascending key order — so a *Int64Int32 satisfies
// par.Segmenter2[int64, int32] and feeds par.From2 with ORDERED
...
func (m *Int64Int32) Segments2(n int) []iter.Seq2[int64, int32] {
	ranges := segment.SplitRanges(m.size, n)
	segs := make([]iter.Seq2[int64, int32], len(ranges))
	for i := range ranges {
		lo, hi := ranges[i][0], ranges[i][1] // per-iteration copies for this view's closure
		segs[i] = func(yield func(int64, int32) bool) {
			// In-order walk yielding pairs whose 0-based key rank is in [lo, hi).
			// base is the rank of the first element in node's subtree.
			var walk func(node *int64Int32TreeNode, base int) bool
			walk = func(node *int64Int32TreeNode, base int) bool {
				if node == nil {
					return true
				}
				rank := base + int64Int32TreeNodeSize(node.left)
...
func (m *Int64Int32) RangeKeys(fromKey, toKey int64) iter.Seq2[int64, int32] {
	return func(yield func(int64, int32) bool) {
		for k, v := range m.All() {
			if k < fromKey {
				continue
...
func (m *Int64Int32) Higher(key int64) (int64, int32, bool) {
	var result *int64Int32TreeNode
	node := m.root
	for node != nil {
		if key < node.key {
...
func (m *Int64Int32) Lower(key int64) (int64, int32, bool) {
	var result *int64Int32TreeNode
	node := m.root
	for node != nil {
		if key > node.key {
...
func (m *Int64Int32) HeadMap(toKey int64) iter.Seq2[int64, int32] {
	return func(yield func(int64, int32) bool) {
		for k, v := range m.All() {
			if k >= toKey {
				return
...
func (m *Int64Int32) TailMap(fromKey int64) iter.Seq2[int64, int32] {
	return func(yield func(int64, int32) bool) {
		for k, v := range m.All() {
			if k < fromKey {
				continue
...
func (m *Int64Int32) PollLastEntry() (int64, int32, bool) {
	k, v, ok := m.Max()
	if !ok {
		return 0, 0, false
	}
...
func (m *Int64Int32) DescendingMap() iter.Seq2[int64, int32] {
	return func(yield func(int64, int32) bool) {
...
func (m *Int64Int32) DescendingKeys() iter.Seq[int64] {
	return func(yield func(int64) bool) {
...
func (m *Int64Int32) Detect(predicate func(int64, int32) bool) (int64, int32, bool) {
	for k, v := range m.All() {
		if predicate(k, v) {
			return k, v, true
		}
...
func (m *Int64Int32) Count(predicate func(int64, int32) bool) int {
	c := 0
	for k, v := range m.All() {
		if predicate(k, v) {
			c++
...
// query; never mutates.
func (m *Int64Int32) Rank(key int64) int {
	rank := 0
	node := m.root
	for node != nil {
		if key < node.key {
			...
			rank += 1 + int64Int32TreeNodeSize(node.left)
			node = node.right
		} else {
			// key == node.key: exactly the left subtree is strictly less.
			return rank + int64Int32TreeNodeSize(node.left)
		}
	}
	return rank
}
...
func (m *Int64Int32) SelectKey(i int) (int64, bool) {
	node := m.selectNode(i)
	if node == nil {
		return 0, false
	}
...
func (m *Int64Int32) SelectEntry(i int) (int64, int32, bool) {
	node := m.selectNode(i)
	if node == nil {
		return 0, 0, false
	}
...
// O(log n).
func (m *Int64Int32) selectNode(i int) *int64Int32TreeNode {
	if i < 0 {
		return nil
	}
	node := m.root
	for node != nil {
		left := int64Int32TreeNodeSize(node.left)
		if i < left {
			node = node.left
		} else if i == left {
			return node
...
func (m *Int64Int32) findNode(key int64) *int64Int32TreeNode {
	node := m.root
	for node != nil {
		if key < node.key {
			node = node.left
...
func (m *Int64Int32) minNode(node *int64Int32TreeNode) *int64Int32TreeNode {
	for node.left != nil {
		node = node.left
	}
	return node
...
func (m *Int64Int32) maxNode(node *int64Int32TreeNode) *int64Int32TreeNode {
	for node.right != nil {
		node = node.right
	}
	return node
...
func (m *Int64Int32) rotateLeft(x *int64Int32TreeNode) {
	y := x.right
	x.right = y.left
	if y.left != nil {
		y.left.parent = x
	...
	int64Int32TreeNodeFixSize(x)
	int64Int32TreeNodeFixSize(y)
}

func (m *Int64Int32) rotateRight(x *int64Int32TreeNode) {
	y := x.left
	x.left = y.right
	if y.right != nil {
		y.right.parent = x
...
func (m *Int64Int32) incSizeToRoot(n *int64Int32TreeNode) {
...
func (m *Int64Int32) fixSizeToRoot(n *int64Int32TreeNode) {
	for ; n != nil; n = n.parent {
		int64Int32TreeNodeFixSize(n)
	}
}

func (m *Int64Int32) fixAfterInsert(z *int64Int32TreeNode) {
...
func (m *Int64Int32) deleteNode(z *int64Int32TreeNode) {
	...
	var fixSizeFrom *int64Int32TreeNode
...
// NewInt64Int32FromSorted builds a Int64Int32 from presorted, ascending keys
...
func NewInt64Int32FromSorted(keys []int64, values []int32, policy pump.DuplicatePolicy) (*Int64Int32, error) {
	if len(keys) != len(values) {
		panic("mapdb: NewInt64Int32FromSorted: len(keys) != len(values)")
	}
	dk, dv, err := dedupInt64Int32Sorted(keys, values, policy)
	if err != nil {
		return nil, err
	}
	m := NewInt64Int32()
	m.root = m.buildint64Int32(dk, dv, 0, len(dk)-1, 0, pump.RedBlackRedLevel(len(dk)), nil)
	m.size = len(dk)
	return m, nil
}

// dedupInt64Int32Sorted validates ascending order and applies the duplicate
// policy, returning compacted key/value slices. Equal adjacent keys are the only
// duplicates possible in sorted input.
func dedupInt64Int32Sorted(keys []int64, values []int32, policy pump.DuplicatePolicy) ([]int64, []int32, error) {
	if len(keys) == 0 {
	...
	for i := 1; i < len(keys); i++ {
		cmp := cmpInt64Int32(keys[i], keys[i-1])
		if cmp < 0 {
...
// cmpInt64Int32 is the three-way ordering used by the bulk-load validator for
// integer/char keys (float keys use the IEEE total-order helper instead).
func cmpInt64Int32(a, b int64) int {
	switch {
	case a < b:
		return -1
	case a > b:
...
// buildint64Int32 recursively builds a perfectly balanced subtree from the
...
func (m *Int64Int32) buildint64Int32(keys []int64, values []int32, lo, hi, level, redLevel int, parent *int64Int32TreeNode) *int64Int32TreeNode {
	if lo > hi {
		return nil
	}
	mid := (lo + hi) / 2
	node := &int64Int32TreeNode{key: keys[mid], value: values[mid], parent: parent, color: int64Int32TreeNodeBlack}
	if level == redLevel {
		node.color = int64Int32TreeNodeRed
	}
	node.left = m.buildint64Int32(keys, values, lo, mid-1, level+1, redLevel, node)
	node.right = m.buildint64Int32(keys, values, mid+1, hi, level+1, redLevel, node)
	...
	node.size = 1 + int64Int32TreeNodeSize(node.left) + int64Int32TreeNodeSize(node.right)
	return node
}

// Int64Int32Sink is a streaming builder for a Int64Int32: callers Put
// ascending entries one at a time, then Build the finished map. It is a thin
// wrapper over NewInt64Int32FromSorted (entries are buffered, then built in one
...
// NewInt64Int32Sink creates a streaming sink with the given duplicate policy.
func NewInt64Int32Sink(policy pump.DuplicatePolicy) *Int64Int32Sink {
	return &Int64Int32Sink{policy: policy}
}
...
func (s *Int64Int32Sink) Put(key int64, value int32) {
	if s.done {
		panic("mapdb: Put on a finished Int64Int32Sink")
	}
...
func (s *Int64Int32Sink) Build() (*Int64Int32, error) {
	if s.done {
		panic("mapdb: Build on a finished Int64Int32Sink")
	}
	s.done = true
	return NewInt64Int32FromSorted(s.keys, s.values, s.policy)
}

func (m *Int64Int32) fixAfterDelete(x *int64Int32TreeNode) {
