...
// int64Int64TreeNodeSize returns the subtree size of a node link (0 if nil).
func int64Int64TreeNodeSize(n *int64Int64TreeNode) int {
	if n == nil {
		return 0
	}
	return n.size
...
// int64Int64TreeNodeFixSize recomputes a node's cached subtree size from its
// children. Called after any rotation or child relink so the augmentation stays
// consistent.
func int64Int64TreeNodeFixSize(n *int64Int64TreeNode) {
	n.size = 1 + int64Int64TreeNodeSize(n.left) + int64Int64TreeNodeSize(n.right)
}
...
func (m *Int64Int64) Put(key int64, value int64) (int64, bool) {
	if m.root == nil {
		m.root = &int64Int64TreeNode{key: key, value: value, color: int64Int64TreeNodeBlack, size: 1}
		m.size++
		return 0, false
	}
	node := m.root
	for {
		if key < node.key {
			if node.left == nil {
				node.left = &int64Int64TreeNode{key: key, value: value, parent: node, color: int64Int64TreeNodeRed, size: 1}
			...
			if node.right == nil {
				node.right = &int64Int64TreeNode{key: key, value: value, parent: node, color: int64Int64TreeNodeRed, size: 1}
				m.incSizeToRoot(node)
...
// reproduces All in ascending key order — so a *Int64Int64 satisfies
// par.Segmenter2[int64, int64] and feeds par.From2 with ORDERED
...
func (m *Int64Int64) Segments2(n int) []iter.Seq2[int64, int64] {
	ranges := segment.SplitRanges(m.size, n)
	segs := make([]iter.Seq2[int64, int64], len(ranges))
	for i := range ranges {
		lo, hi := ranges[i][0], ranges[i][1] // per-iteration copies for this view's closure
		segs[i] = func(yield func(int64, int64) bool) {
			// In-order walk yielding pairs whose 0-based key rank is in [lo, hi).
			// base is the rank of the first element in node's subtree.
			var walk func(node *int64Int64TreeNode, base int) bool
			walk = func(node *int64Int64TreeNode, base int) bool {
				if node == nil {
					return true
				}
				rank := base + int64Int64TreeNodeSize(node.left)
...
// query; never mutates.
func (m *Int64Int64) Rank(key int64) int {
	rank := 0
	node := m.root
	for node != nil {
		if key < node.key {
			...
			rank += 1 + int64Int64TreeNodeSize(node.left)
			node = node.right
		} else {
			// key == node.key: exactly the left subtree is strictly less.
			return rank + int64Int64TreeNodeSize(node.left)
		}
	}
	return rank
}
...
func (m *Int64Int64) SelectKey(i int) (int64, bool) {
	node := m.selectNode(i)
	if node == nil {
		return 0, false
	}
...
func (m *Int64Int64) SelectEntry(i int) (int64, int64, bool) {
	node := m.selectNode(i)
	if node == nil {
		return 0, 0, false
	}
...
// O(log n).
func (m *Int64Int64) selectNode(i int) *int64Int64TreeNode {
	if i < 0 {
		return nil
	}
	node := m.root
	for node != nil {
		left := int64Int64TreeNodeSize(node.left)
		if i < left {
			node = node.left
		} else if i == left {
			return node
...
func (m *Int64Int64) rotateLeft(x *int64Int64TreeNode) {
	...
	int64Int64TreeNodeFixSize(x)
	int64Int64TreeNodeFixSize(y)
}
...
func (m *Int64Int64) incSizeToRoot(n *int64Int64TreeNode) {
...
func (m *Int64Int64) fixSizeToRoot(n *int64Int64TreeNode) {
	for ; n != nil; n = n.parent {
		int64Int64TreeNodeFixSize(n)
	}
}
...
func (m *Int64Int64) deleteNode(z *int64Int64TreeNode) {
	...
	var fixSizeFrom *int64Int64TreeNode
...
func (m *Int64Int64) buildint64Int64(keys []int64, values []int64, lo, hi, level, redLevel int, parent *int64Int64TreeNode) *int64Int64TreeNode {
	...
	node.size = 1 + int64Int64TreeNodeSize(node.left) + int64Int64TreeNodeSize(node.right)
	return node
}
