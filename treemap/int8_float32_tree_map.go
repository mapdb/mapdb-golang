...
// int8Float32TreeNodeSize returns the subtree size of a node link (0 if nil).
func int8Float32TreeNodeSize(n *int8Float32TreeNode) int {
	if n == nil {
		return 0
	}
	return n.size
...
// int8Float32TreeNodeFixSize recomputes a node's cached subtree size from its
// children. Called after any rotation or child relink so the augmentation stays
// consistent.
func int8Float32TreeNodeFixSize(n *int8Float32TreeNode) {
	n.size = 1 + int8Float32TreeNodeSize(n.left) + int8Float32TreeNodeSize(n.right)
}

// Int8Float32 is a sorted map with int8 keys and float32 values, backed by a red-black tree.
...
// NewInt8Float32 creates a new empty sorted map.
...
func (m *Int8Float32) Put(key int8, value float32) (float32, bool) {
	if m.root == nil {
		m.root = &int8Float32TreeNode{key: key, value: value, color: int8Float32TreeNodeBlack, size: 1}
		m.size++
		return 0.0, false
	}
	node := m.root
	for {
		if key < node.key {
			if node.left == nil {
				node.left = &int8Float32TreeNode{key: key, value: value, parent: node, color: int8Float32TreeNodeRed, size: 1}
			...
			if node.right == nil {
				node.right = &int8Float32TreeNode{key: key, value: value, parent: node, color: int8Float32TreeNodeRed, size: 1}
				m.incSizeToRoot(node)
...
func (m *Int8Float32) Min() (int8, float32, bool) {
	if m.root == nil {
		return 0, 0.0, false
	}
...
func (m *Int8Float32) Max() (int8, float32, bool) {
	if m.root == nil {
		return 0, 0.0, false
	}
...
func (m *Int8Float32) Floor(key int8) (int8, float32, bool) {
...
func (m *Int8Float32) Ceiling(key int8) (int8, float32, bool) {
	var result *int8Float32TreeNode
	node := m.root
	for node != nil {
		if key == node.key {
...
// reproduces All in ascending key order — so a *Int8Float32 satisfies
// par.Segmenter2[int8, float32] and feeds par.From2 with ORDERED
...
func (m *Int8Float32) Segments2(n int) []iter.Seq2[int8, float32] {
	ranges := segment.SplitRanges(m.size, n)
	segs := make([]iter.Seq2[int8, float32], len(ranges))
	for i := range ranges {
		lo, hi := ranges[i][0], ranges[i][1] // per-iteration copies for this view's closure
		segs[i] = func(yield func(int8, float32) bool) {
			// In-order walk yielding pairs whose 0-based key rank is in [lo, hi).
			// base is the rank of the first element in node's subtree.
			var walk func(node *int8Float32TreeNode, base int) bool
			walk = func(node *int8Float32TreeNode, base int) bool {
				if node == nil {
					return true
				}
				rank := base + int8Float32TreeNodeSize(node.left)
...
func (m *Int8Float32) RangeKeys(fromKey, toKey int8) iter.Seq2[int8, float32] {
	return func(yield func(int8, float32) bool) {
		for k, v := range m.All() {
			if k < fromKey {
				continue
...
func (m *Int8Float32) Higher(key int8) (int8, float32, bool) {
	var result *int8Float32TreeNode
	node := m.root
	for node != nil {
		if key < node.key {
...
func (m *Int8Float32) Lower(key int8) (int8, float32, bool) {
	var result *int8Float32TreeNode
	node := m.root
	for node != nil {
		if key > node.key {
...
func (m *Int8Float32) HeadMap(toKey int8) iter.Seq2[int8, float32] {
	return func(yield func(int8, float32) bool) {
		for k, v := range m.All() {
			if k >= toKey {
				return
...
func (m *Int8Float32) TailMap(fromKey int8) iter.Seq2[int8, float32] {
	return func(yield func(int8, float32) bool) {
		for k, v := range m.All() {
			if k < fromKey {
				continue
...
func (m *Int8Float32) PollLastEntry() (int8, float32, bool) {
	k, v, ok := m.Max()
	if !ok {
		return 0, 0.0, false
	}
...
func (m *Int8Float32) DescendingMap() iter.Seq2[int8, float32] {
	return func(yield func(int8, float32) bool) {
...
func (m *Int8Float32) DescendingKeys() iter.Seq[int8] {
	return func(yield func(int8) bool) {
...
// query; never mutates.
func (m *Int8Float32) Rank(key int8) int {
	rank := 0
	node := m.root
	for node != nil {
		if key < node.key {
			...
			rank += 1 + int8Float32TreeNodeSize(node.left)
			node = node.right
		} else {
			// key == node.key: exactly the left subtree is strictly less.
			return rank + int8Float32TreeNodeSize(node.left)
		}
	}
	return rank
}
...
func (m *Int8Float32) SelectKey(i int) (int8, bool) {
	node := m.selectNode(i)
	if node == nil {
		return 0, false
	}
...
func (m *Int8Float32) SelectEntry(i int) (int8, float32, bool) {
	node := m.selectNode(i)
	if node == nil {
		return 0, 0.0, false
	}
...
// O(log n).
func (m *Int8Float32) selectNode(i int) *int8Float32TreeNode {
	if i < 0 {
		return nil
	}
	node := m.root
	for node != nil {
		left := int8Float32TreeNodeSize(node.left)
		if i < left {
			node = node.left
		} else if i == left {
			return node
...
func (m *Int8Float32) findNode(key int8) *int8Float32TreeNode {
	node := m.root
	for node != nil {
		if key < node.key {
			node = node.left
...
func (m *Int8Float32) minNode(node *int8Float32TreeNode) *int8Float32TreeNode {
	for node.left != nil {
		node = node.left
	}
	return node
...
func (m *Int8Float32) maxNode(node *int8Float32TreeNode) *int8Float32TreeNode {
	for node.right != nil {
		node = node.right
	}
	return node
...
func (m *Int8Float32) rotateLeft(x *int8Float32TreeNode) {
	y := x.right
	x.right = y.left
	if y.left != nil {
		y.left.parent = x
	...
	int8Float32TreeNodeFixSize(x)
	int8Float32TreeNodeFixSize(y)
}

func (m *Int8Float32) rotateRight(x *int8Float32TreeNode) {
	y := x.left
	x.left = y.right
	if y.right != nil {
		y.right.parent = x
...
func (m *Int8Float32) incSizeToRoot(n *int8Float32TreeNode) {
...
func (m *Int8Float32) fixSizeToRoot(n *int8Float32TreeNode) {
	for ; n != nil; n = n.parent {
		int8Float32TreeNodeFixSize(n)
	}
}

func (m *Int8Float32) fixAfterInsert(z *int8Float32TreeNode) {
...
func (m *Int8Float32) deleteNode(z *int8Float32TreeNode) {
	...
	var fixSizeFrom *int8Float32TreeNode
...
// NewInt8Float32FromSorted builds a Int8Float32 from presorted, ascending keys
...
func NewInt8Float32FromSorted(keys []int8, values []float32, policy pump.DuplicatePolicy) (*Int8Float32, error) {
	if len(keys) != len(values) {
		panic("mapdb: NewInt8Float32FromSorted: len(keys) != len(values)")
	}
	dk, dv, err := dedupInt8Float32Sorted(keys, values, policy)
	if err != nil {
		return nil, err
	}
	m := NewInt8Float32()
	m.root = m.buildint8Float32(dk, dv, 0, len(dk)-1, 0, pump.RedBlackRedLevel(len(dk)), nil)
	m.size = len(dk)
	return m, nil
}

// dedupInt8Float32Sorted validates ascending order and applies the duplicate
// policy, returning compacted key/value slices. Equal adjacent keys are the only
// duplicates possible in sorted input.
func dedupInt8Float32Sorted(keys []int8, values []float32, policy pump.DuplicatePolicy) ([]int8, []float32, error) {
	if len(keys) == 0 {
	...
	for i := 1; i < len(keys); i++ {
		cmp := cmpInt8Float32(keys[i], keys[i-1])
		if cmp < 0 {
...
// cmpInt8Float32 is the three-way ordering used by the bulk-load validator for
// integer/char keys (float keys use the IEEE total-order helper instead).
func cmpInt8Float32(a, b int8) int {
	switch {
	case a < b:
		return -1
	case a > b:
...
// buildint8Float32 recursively builds a perfectly balanced subtree from the
...
func (m *Int8Float32) buildint8Float32(keys []int8, values []float32, lo, hi, level, redLevel int, parent *int8Float32TreeNode) *int8Float32TreeNode {
	if lo > hi {
		return nil
	}
	mid := (lo + hi) / 2
	node := &int8Float32TreeNode{key: keys[mid], value: values[mid], parent: parent, color: int8Float32TreeNodeBlack}
	if level == redLevel {
		node.color = int8Float32TreeNodeRed
	}
	node.left = m.buildint8Float32(keys, values, lo, mid-1, level+1, redLevel, node)
	node.right = m.buildint8Float32(keys, values, mid+1, hi, level+1, redLevel, node)
	...
	node.size = 1 + int8Float32TreeNodeSize(node.left) + int8Float32TreeNodeSize(node.right)
	return node
}

// Int8Float32Sink is a streaming builder for a Int8Float32: callers Put
// ascending entries one at a time, then Build the finished map. It is a thin
// wrapper over NewInt8Float32FromSorted (entries are buffered, then built in one
...
// NewInt8Float32Sink creates a streaming sink with the given duplicate policy.
func NewInt8Float32Sink(policy pump.DuplicatePolicy) *Int8Float32Sink {
	return &Int8Float32Sink{policy: policy}
}
...
func (s *Int8Float32Sink) Put(key int8, value float32) {
	if s.done {
		panic("mapdb: Put on a finished Int8Float32Sink")
	}
...
func (s *Int8Float32Sink) Build() (*Int8Float32, error) {
	if s.done {
		panic("mapdb: Build on a finished Int8Float32Sink")
	}
	s.done = true
	return NewInt8Float32FromSorted(s.keys, s.values, s.policy)
}

func (m *Int8Float32) fixAfterDelete(x *int8Float32TreeNode) {
