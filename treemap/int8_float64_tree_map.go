...
// int8Float64TreeNodeSize returns the subtree size of a node link (0 if nil).
func int8Float64TreeNodeSize(n *int8Float64TreeNode) int {
	if n == nil {
		return 0
	}
	return n.size
...
// int8Float64TreeNodeFixSize recomputes a node's cached subtree size from its
// children. Called after any rotation or child relink so the augmentation stays
// consistent.
func int8Float64TreeNodeFixSize(n *int8Float64TreeNode) {
	n.size = 1 + int8Float64TreeNodeSize(n.left) + int8Float64TreeNodeSize(n.right)
}
...
func (m *Int8Float64) Put(key int8, value float64) (float64, bool) {
	if m.root == nil {
		m.root = &int8Float64TreeNode{key: key, value: value, color: int8Float64TreeNodeBlack, size: 1}
		m.size++
		return 0.0, false
	}
	node := m.root
	for {
		if key < node.key {
			if node.left == nil {
				node.left = &int8Float64TreeNode{key: key, value: value, parent: node, color: int8Float64TreeNodeRed, size: 1}
			...
			if node.right == nil {
				node.right = &int8Float64TreeNode{key: key, value: value, parent: node, color: int8Float64TreeNodeRed, size: 1}
				m.incSizeToRoot(node)
...
// reproduces All in ascending key order — so a *Int8Float64 satisfies
// par.Segmenter2[int8, float64] and feeds par.From2 with ORDERED
...
func (m *Int8Float64) Segments2(n int) []iter.Seq2[int8, float64] {
	ranges := segment.SplitRanges(m.size, n)
	segs := make([]iter.Seq2[int8, float64], len(ranges))
	for i := range ranges {
		lo, hi := ranges[i][0], ranges[i][1] // per-iteration copies for this view's closure
		segs[i] = func(yield func(int8, float64) bool) {
			// In-order walk yielding pairs whose 0-based key rank is in [lo, hi).
			// base is the rank of the first element in node's subtree.
			var walk func(node *int8Float64TreeNode, base int) bool
			walk = func(node *int8Float64TreeNode, base int) bool {
				if node == nil {
					return true
				}
				rank := base + int8Float64TreeNodeSize(node.left)
...
// query; never mutates.
func (m *Int8Float64) Rank(key int8) int {
	rank := 0
	node := m.root
	for node != nil {
		if key < node.key {
			...
			rank += 1 + int8Float64TreeNodeSize(node.left)
			node = node.right
		} else {
			// key == node.key: exactly the left subtree is strictly less.
			return rank + int8Float64TreeNodeSize(node.left)
		}
	}
	return rank
}
...
func (m *Int8Float64) SelectKey(i int) (int8, bool) {
	node := m.selectNode(i)
	if node == nil {
		return 0, false
	}
...
func (m *Int8Float64) SelectEntry(i int) (int8, float64, bool) {
	node := m.selectNode(i)
	if node == nil {
		return 0, 0.0, false
	}
...
// O(log n).
func (m *Int8Float64) selectNode(i int) *int8Float64TreeNode {
	if i < 0 {
		return nil
	}
	node := m.root
	for node != nil {
		left := int8Float64TreeNodeSize(node.left)
		if i < left {
			node = node.left
		} else if i == left {
			return node
...
func (m *Int8Float64) rotateLeft(x *int8Float64TreeNode) {
	...
	int8Float64TreeNodeFixSize(x)
	int8Float64TreeNodeFixSize(y)
}
...
func (m *Int8Float64) incSizeToRoot(n *int8Float64TreeNode) {
...
func (m *Int8Float64) fixSizeToRoot(n *int8Float64TreeNode) {
	for ; n != nil; n = n.parent {
		int8Float64TreeNodeFixSize(n)
	}
}
...
func (m *Int8Float64) deleteNode(z *int8Float64TreeNode) {
	...
	var fixSizeFrom *int8Float64TreeNode
...
func (m *Int8Float64) buildint8Float64(keys []int8, values []float64, lo, hi, level, redLevel int, parent *int8Float64TreeNode) *int8Float64TreeNode {
	...
	node.size = 1 + int8Float64TreeNodeSize(node.left) + int8Float64TreeNodeSize(node.right)
	return node
}
