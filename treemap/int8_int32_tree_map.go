...
// reproduces All in ascending key order — so a *Int8Int32 satisfies
// par.Segmenter2[int8, int32] and feeds par.From2 with ORDERED
...
func (m *Int8Int32) Segments2(n int) []iter.Seq2[int8, int32] {
	ranges := segment.SplitRanges(m.size, n)
	segs := make([]iter.Seq2[int8, int32], len(ranges))
	for i := range ranges {
		lo, hi := ranges[i][0], ranges[i][1] // per-iteration copies for this view's closure
		segs[i] = func(yield func(int8, int32) bool) {
			// In-order walk yielding pairs whose 0-based key rank is in [lo, hi).
			// base is the rank of the first element in node's subtree.
			var walk func(node *int8Int32TreeNode, base int) bool
			walk = func(node *int8Int32TreeNode, base int) bool {
				if node == nil {
					return true
				}
				rank := base + int8Int32TreeNodeSize(node.left)
