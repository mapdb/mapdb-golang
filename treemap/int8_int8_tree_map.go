...
// int8Int8TreeNodeSize returns the subtree size of a node link (0 if nil).
func int8Int8TreeNodeSize(n *int8Int8TreeNode) int {
	if n == nil {
		return 0
	}
	return n.size
...
// int8Int8TreeNodeFixSize recomputes a node's cached subtree size from its
// children. Called after any rotation or child relink so the augmentation stays
// consistent.
func int8Int8TreeNodeFixSize(n *int8Int8TreeNode) {
	n.size = 1 + int8Int8TreeNodeSize(n.left) + int8Int8TreeNodeSize(n.right)
}
...
func (m *Int8Int8) Put(key int8, value int8) (int8, bool) {
	if m.root == nil {
		m.root = &int8Int8TreeNode{key: key, value: value, color: int8Int8TreeNodeBlack, size: 1}
		m.size++
		return 0, false
	}
	node := m.root
	for {
		if key < node.key {
			if node.left == nil {
				node.left = &int8Int8TreeNode{key: key, value: value, parent: node, color: int8Int8TreeNodeRed, size: 1}
			...
			if node.right == nil {
				node.right = &int8Int8TreeNode{key: key, value: value, parent: node, color: int8Int8TreeNodeRed, size: 1}
				m.incSizeToRoot(node)
...
// reproduces All in ascending key order — so a *Int8Int8 satisfies
// par.Segmenter2[int8, int8] and feeds par.From2 with ORDERED
...
func (m *Int8Int8) Segments2(n int) []iter.Seq2[int8, int8] {
	ranges := segment.SplitRanges(m.size, n)
	segs := make([]iter.Seq2[int8, int8], len(ranges))
	for i := range ranges {
		lo, hi := ranges[i][0], ranges[i][1] // per-iteration copies for this view's closure
		segs[i] = func(yield func(int8, int8) bool) {
			// In-order walk yielding pairs whose 0-based key rank is in [lo, hi).
			// base is the rank of the first element in node's subtree.
			var walk func(node *int8Int8TreeNode, base int) bool
			walk = func(node *int8Int8TreeNode, base int) bool {
				if node == nil {
					return true
				}
				rank := base + int8Int8TreeNodeSize(node.left)
...
// query; never mutates.
func (m *Int8Int8) Rank(key int8) int {
	rank := 0
	node := m.root
	for node != nil {
		if key < node.key {
			...
			rank += 1 + int8Int8TreeNodeSize(node.left)
			node = node.right
		} else {
			// key == node.key: exactly the left subtree is strictly less.
			return rank + int8Int8TreeNodeSize(node.left)
		}
	}
	return rank
}
...
func (m *Int8Int8) SelectKey(i int) (int8, bool) {
	node := m.selectNode(i)
	if node == nil {
		return 0, false
	}
...
func (m *Int8Int8) SelectEntry(i int) (int8, int8, bool) {
	node := m.selectNode(i)
	if node == nil {
		return 0, 0, false
	}
...
// O(log n).
func (m *Int8Int8) selectNode(i int) *int8Int8TreeNode {
	if i < 0 {
		return nil
	}
	node := m.root
	for node != nil {
		left := int8Int8TreeNodeSize(node.left)
		if i < left {
			node = node.left
		} else if i == left {
			return node
...
func (m *Int8Int8) rotateLeft(x *int8Int8TreeNode) {
	...
	int8Int8TreeNodeFixSize(x)
	int8Int8TreeNodeFixSize(y)
}
...
func (m *Int8Int8) incSizeToRoot(n *int8Int8TreeNode) {
...
func (m *Int8Int8) fixSizeToRoot(n *int8Int8TreeNode) {
	for ; n != nil; n = n.parent {
		int8Int8TreeNodeFixSize(n)
	}
}
...
func (m *Int8Int8) deleteNode(z *int8Int8TreeNode) {
	...
	var fixSizeFrom *int8Int8TreeNode
