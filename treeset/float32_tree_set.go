...
// reproduces All in ascending order — so a *Float32 satisfies
// par.Segmenter[float32] and feeds par.From with ORDERED segments, each
...
func (s *Float32) Segments(n int) []iter.Seq[float32] {
	...
	segs := make([]iter.Seq[float32], len(ranges))
	for i := range ranges {
		lo, hi := ranges[i][0], ranges[i][1] // per-iteration copies for this view's closure
		segs[i] = func(yield func(float32) bool) {
			...
			var walk func(node *float32Node, base int) bool
			walk = func(node *float32Node, base int) bool {
				if node == nil {
					return true
				}
				rank := base + float32NodeSize(node.left)
...
func (s *Float32) buildfloat32(values []float32, lo, hi, level, redLevel int, parent *float32Node) *float32Node {
	...
	node.size = 1 + float32NodeSize(node.left) + float32NodeSize(node.right)
	return node
}
