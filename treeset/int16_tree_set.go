...
// reproduces All in ascending order — so a *Int16 satisfies
// par.Segmenter[int16] and feeds par.From with ORDERED segments, each
...
func (s *Int16) Segments(n int) []iter.Seq[int16] {
	ranges := segment.SplitRanges(s.size, n)
	segs := make([]iter.Seq[int16], len(ranges))
	for i := range ranges {
		lo, hi := ranges[i][0], ranges[i][1] // per-iteration copies for this view's closure
		segs[i] = func(yield func(int16) bool) {
			...
			var walk func(node *int16Node, base int) bool
			walk = func(node *int16Node, base int) bool {
				if node == nil {
					return true
				}
				rank := base + int16NodeSize(node.left)
