...
// int32NodeSize returns the subtree size of a node link (0 if nil).
func int32NodeSize(n *int32Node) int {
	if n == nil {
		return 0
	}
	return n.size
...
// int32NodeFixSize recomputes a node's cached subtree size from its
// children. Called after any rotation or child relink so the augmentation stays
// consistent.
func int32NodeFixSize(n *int32Node) {
	n.size = 1 + int32NodeSize(n.left) + int32NodeSize(n.right)
}
...
func (s *Int32) Add(value int32) bool {
	if s.root == nil {
		s.root = &int32Node{key: value, color: int32NodeBlack, size: 1}
		s.size++
		return true
	}
	node := s.root
	for {
		if value < node.key {
			if node.left == nil {
				node.left = &int32Node{key: value, parent: node, color: int32NodeRed, size: 1}
			...
			if node.right == nil {
				node.right = &int32Node{key: value, parent: node, color: int32NodeRed, size: 1}
				s.incSizeToRoot(node)
...
// reproduces All in ascending order — so a *Int32 satisfies
// par.Segmenter[int32] and feeds par.From with ORDERED segments, each
...
func (s *Int32) Segments(n int) []iter.Seq[int32] {
	...
	for i := range ranges {
		...
		segs[i] = func(yield func(int32) bool) {
			...
			var walk func(node *int32Node, base int) bool
			walk = func(node *int32Node, base int) bool {
				if node == nil {
					return true
				}
				rank := base + int32NodeSize(node.left)
...
func (s *Int32) SelectWhere(predicate func(int32) bool) *Int32 {
	result := NewInt32()
	for v := range s.All() {
		if predicate(v) {
			result.Add(v)
...
func (s *Int32) Rank(value int32) int {
	rank := 0
	node := s.root
	for node != nil {
		if value < node.key {
			...
			rank += 1 + int32NodeSize(node.left)
			node = node.right
		} else {
			return rank + int32NodeSize(node.left)
		}
	}
	return rank
}
...
func (s *Int32) Select(i int) (int32, bool) {
	if i < 0 {
		return 0, false
	}
	node := s.root
	for node != nil {
		left := int32NodeSize(node.left)
		if i < left {
			node = node.left
		} else if i == left {
			return node.key, true
...
func (s *Int32) buildint32(values []int32, lo, hi, level, redLevel int, parent *int32Node) *int32Node {
	...
	node.size = 1 + int32NodeSize(node.left) + int32NodeSize(node.right)
	return node
}
...
func (s *Int32) incSizeToRoot(n *int32Node) {
...
func (s *Int32) fixSizeToRoot(n *int32Node) {
	for ; n != nil; n = n.parent {
		int32NodeFixSize(n)
	}
}
