...
// reproduces All in ascending order — so a *Int64 satisfies
// par.Segmenter[int64] and feeds par.From with ORDERED segments, each
...
func (s *Int64) Segments(n int) []iter.Seq[int64] {
	...
	for i := range ranges {
		...
		segs[i] = func(yield func(int64) bool) {
			...
			var walk func(node *int64Node, base int) bool
			walk = func(node *int64Node, base int) bool {
				if node == nil {
					return true
				}
				rank := base + int64NodeSize(node.left)
...
func (s *Int64) buildint64(values []int64, lo, hi, level, redLevel int, parent *int64Node) *int64Node {
	...
	node.size = 1 + int64NodeSize(node.left) + int64NodeSize(node.right)
	return node
}
